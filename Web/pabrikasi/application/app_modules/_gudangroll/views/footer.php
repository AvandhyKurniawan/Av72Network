...
<!--===============================================General Function (Start) ===============================================-->
		<script type="text/javascript">
          ...
          ajax:{
			...
			function modalTambahDataAwal(param1){
				...
				$("#btnSaveTambahDataAwal").attr("onclick","saveDataAwal('"+param1+"')");
				$("#cmbUkuran2").select2({
          placeholder : "Pilih Roll ("+param1+")",
					...
					$.ajax({
						...
						success : function(response){
							if(response.length > 0){
								$("#txtJumlahStok").val(response[0].berat);
								$("#txtJumlahBobin").val(response[0].bobin);
								$("#txtJumlahPayung").val(response[0].payung);
								$("#txtJumlahPayungKuning").val(response[0].payung_kuning);
								$("#txtTglTransaksi").val(response[0].tgl_transaksi);
								$("#btnTambahDataAwal").attr("onclick","editDataAwalGudangRoll('"+param1+"','"+response[0].id+"','"+response[0].kd_gd_roll+"')")
								...
								$("#txtJumlahBobin").val("");
								$("#txtJumlahPayung").val("");
								$("#txtJumlahPayungKuning").val("");
								$("#txtTglTransaksi").val("");
								$("#btnTambahDataAwal").attr("onclick","saveAddDataAwalGudangRoll('"+param1+"')")
				...
				$.ajax({
					...
					success : function(response){
						$.each(response, function(AvIndex, AvValue){
							...
							// if(AvValue.keterangan_history == "OPERATOR POTONG"){
							// 	$(".hiddenRow").css("display","table-row");
							// }else{
							// 	$(".hiddenRow").css("display","none");
							// 	$(".hiddenRow input[type='text']").val("");
							// }
						});
					},
					error : function(response){
          ...
          ajax:{
			...
			function saveAddDataAwalGudangRoll(param){
				...
				var payungKuning1 = $("#txtJumlahPayungKuning").val().replace(/,/g, "");

				if(kdGdRoll1=="" || jnsPermintaan1=="" || tglTransaksi1=="" ||
					 berat1=="" || bobin1=="" || payung1=="" || payungKuning1==""
				 ){
					 $("#modal-notif").addClass("modal-warning");
					 $("#modalNotifContent").text("Semua Kolom Berwarna Kuning Tidak Boleh Kosong!");
					 $("#modal-notif").modal("show");
			...
			function saveDataAwal(param){
				if(confirm("Apakah Anda Yakin Sudah Tidak Ada Yang Salah?")){
					$.ajax({
						type : "POST",
						url : "<?php echo base_url('_gudangroll/main/saveDataAwal'); ?>",
						dataText : "TEXT",
						data : {
							jenis : param
						},
						success : function(response){
							if($.trim(response) === "Berhasil"){
								$("#modal-notif").addClass("modal-info");
								$("#modalNotifContent").text("Transaksi Berhasil Di Simpan");
								$("#modal-notif").modal("show");
								setTimeout(function(){
								 $("#modal-notif").modal("hide");
								 $("#modal-notif").removeClass("modal-info");
								...
								$("#modal-notif").addClass("modal-danger");
								$("#modalNotifContent").text("Transaksi Gagal Di Simpan");
								$("#modal-notif").modal("show");
								setTimeout(function(){
								 $("#modal-notif").modal("hide");
								 $("#modal-notif").removeClass("modal-danger");
			...
			function perbaruiStokMaster(){
				...
				if(arrValue.length > 0){
					if(confirm("Apakah Anda Yakin Ingin Perbarui Stok Master?")){
						$.ajax({
							type : "POST",
							url : "<?php echo base_url('_gudangroll/main/perbaruiStokMaster'); ?>",
							dataType : "TEXT",
							data : {
								arrValue : arrValue
							},
			...
			function editDataAwalGudangRoll(param, param2, param3){
				var tglTransaksi = $("#txtTglTransaksi").val();
				var berat = $("#txtJumlahStok").val().replace(/,/gi,"");
				var bobin = $("#txtJumlahBobin").val().replace(/,/gi,"");
				var payung = $("#txtJumlahPayung").val().replace(/,/gi,"");
				var payungKuning = $("#txtJumlahPayungKuning").val().replace(/,/gi,"");

				if(tglTransaksi==""||berat==""||bobin==""||payung==""||payungKuning==""){
					$("#modal-notif").addClass("modal-warning");
					$("#modalNotifContent").text("Semua Kolom Berwarna Kuning Tidak Boleh Kosong!");
					$("#modal-notif").modal("show");
					setTimeout(function(){
					...
					$.ajax({
						type : "POST",
						url : "<?php echo base_url('_gudangroll/main/editDataAwalGudangRoll'); ?>",
						dataType : "TEXT",
						data : {
							id 					 : param2,
							kdGdRoll		 : param3,
        ...
        $.ajax({
          ...
          error : function(response){
			...
			function searchCetakDataKartuStok(){
				...
				if(tglAwal==""||tglAkhir==""||jenis==""||kdGdRoll==""){
					...
					var arrTanggalAwal = tglAwal.split("-");
					var arrTanggalAkhir = tglAkhir.split("-");

					var tglAwalModify = arrTanggalAwal[2]+" "+arrBulan[parseFloat(arrTanggalAwal[1]) -1]+" "+arrTanggalAwal[0];
					var tglAkhirModify = arrTanggalAkhir[2]+" "+arrBulan[parseFloat(arrTanggalAkhir[1]) -1]+" "+arrTanggalAkhir[0];

					var periode = tglAwalModify+" - "+tglAkhirModify;

					$("#tdNamaBarang").text(arrDataText[1]+" "+arrDataText[2]+" "+arrDataText[3]);
					$("#tdStatusBarang").text(arrDataText[4]+" "+arrDataText[5]);
					$("#tdPeriode").text(periode);

					tableListCetakDataKartuStok(tglAwal,tglAkhir,kdGdRoll);
					$("#btnPrint").attr("href","<?php echo base_url('_gudangroll/main/printKartuStok/'); ?>"+tglAwal+"/"+tglAkhir+"/"+kdGdRoll);
					$("#btnExportExcel").attr("href","<?php echo base_url('_gudangroll/main/exportKartuStokToSpreadsheet/'); ?>"+tglAwal+"/"+tglAkhir+"/"+kdGdRoll);
					$("#btnExportPdf").attr("href","<?php echo base_url('_gudangroll/main/exportKartuStokToPdf/'); ?>"+tglAwal+"/"+tglAkhir+"/"+kdGdRoll);
					$("#modalCariDataKartuStok").modal("hide");
				}
			}
          ...
          "fnRowCallback":function(nRow,aData,iDisplayIndex,iDisplayIndexFull){
						...
						$("td:eq(5)",nRow).text(parseFloat(aData["stok"]).toLocaleString());
						$("td:eq(6)",nRow).text(parseFloat(aData["bobin"]).toLocaleString());
						$("td:eq(7)",nRow).text(parseFloat(aData["payung"]).toLocaleString());
						$("td:eq(8)",nRow).text(parseFloat(aData["payung_kuning"]).toLocaleString());
          ...
          "fnRowCallback":function(nRow,aData,iDisplayIndex,iDisplayIndexFull){
						...
						if(aData["jumlah_lembar"].indexOf("#") != -1){
							...
							for (var i = 0; i < arrJumlahLembar.length; i++) {
								arrJumlahLembar[i] = parseFloat(arrJumlahLembar[i]).toLocaleString();
							}
							var jumlahLembar = arrJumlahLembar.join("#");
							$("td:eq(6)",nRow).html(jumlahLembar.replace(/#/g,"<br>"));
						}else{
							$("td:eq(6)",nRow).html(parseFloat(aData["jumlah_lembar"]).toLocaleString());
						}

						if(aData["keterangan"].toUpperCase().indexOf("TUMPUK") != -1){
							$("td:eq(8)",nRow).text(parseFloat(aData["berat_pengambilan"]).toLocaleString()+" / "+parseFloat(aData["berat_pengambilan_tumpuk"]).toLocaleString());
							$("td:eq(9)",nRow).text(parseFloat(aData["payung_pengambilan"]).toLocaleString()+" / "+parseFloat(aData["payung_pengambilan_tumpuk"]).toLocaleString());
							$("td:eq(10)",nRow).text(parseFloat(aData["payung_kuning_pengambilan"]).toLocaleString()+" / "+parseFloat(aData["payung_kuning_pengambilan_tumpuk"]).toLocaleString());
							$("td:eq(11)",nRow).text(parseFloat(aData["bobin_pengambilan"]).toLocaleString()+" / "+parseFloat(aData["bobin_pengambilan_tumpuk"]).toLocaleString());
							$("td:eq(12)",nRow).text(parseFloat(aData["berat_sisa"]).toLocaleString()+" / "+parseFloat(aData["berat_sisa_tumpuk"]).toLocaleString());
							$("td:eq(13)",nRow).text(parseFloat(aData["payung_sisa"]).toLocaleString()+" / "+parseFloat(aData["payung_sisa_tumpuk"]).toLocaleString());
							$("td:eq(14)",nRow).text(parseFloat(aData["payung_kuning_sisa"]).toLocaleString()+" / "+parseFloat(aData["payung_kuning_sisa_tumpuk"]).toLocaleString());
							$("td:eq(15)",nRow).text(parseFloat(aData["bobin_sisa"]).toLocaleString()+" / "+parseFloat(aData["bobin_sisa_tumpuk"]).toLocaleString());
						}else{
          ...
          "fnRowCallback":function(nRow,aData,iDisplayIndex,iDisplayIndexFull){
			...
			function datatablesHistoryGudangRoll(param1, param2, param3){
				...
				$("#tableHistoryGudangRoll").dataTable({
					...
					"fnRowCallback" : function(AvRow, AvData, AvDisplayIndex, AvDisplayIndexFull){
						$("td:eq(1)",AvRow).text(parseFloat(AvData["berat"]).toLocaleString());
						$("td:eq(2)",AvRow).text(parseFloat(AvData["payung"]).toLocaleString());
						$("td:eq(3)",AvRow).text(parseFloat(AvData["payung_kuning"]).toLocaleString());
						$("td:eq(4)",AvRow).text(parseFloat(AvData["bobin"]).toLocaleString());
			...
			function datatablesApprovePermintaan(param){
				...
				$("#tableApprovePermintaanPOTONG").dataTable({
					...
					"fnRowCallback" : function(AvRow, AvData, AvDisplayIndex, AvDisplayIndexFull){
						$("td:eq(3)",AvRow).text(parseFloat(AvData["berat"]).toLocaleString());
						$("td:eq(4)",AvRow).text(parseFloat(AvData["bobin"]).toLocaleString());
						$("td:eq(5)",AvRow).text(parseFloat(AvData["payung"]).toLocaleString());
						$("td:eq(6)",AvRow).text(parseFloat(AvData["payung_kuning"]).toLocaleString());
			...
			function datatablesApproveHasilExtruder(){
				...
				$("#tableApproveHasilExtruder").dataTable({
					...
					"columns" : [
						{"data":"tgl_transaksi", "name":"TGR.tgl_transaksi"},
						{"data":"hasil_ukuran", "name":"GR.ukuran"},
						{"data":"panjang", "name":"GR.ukuran"},
						{"data":"warna_plastik", "name":"GR.warna_plastik"},
						{"data":"berat", "name":"TGR.berat"},
						{"data":"bobin", "name":"TGR.bobin"},
						{"data":"payung", "name":"TGR.payung"},
					...
					"fnRowCallback" : function(AvRow, AvData, AvDisplayIndex, AvDisplayIndexFull){
						...
						$("td:eq(4)",AvRow).text(parseFloat(AvData["berat"]).toLocaleString());
						$("td:eq(5)",AvRow).text(parseFloat(AvData["bobin"]).toLocaleString());
						$("td:eq(6)",AvRow).text(parseFloat(AvData["payung"]).toLocaleString());
						$("td:eq(7)",AvRow).text(parseFloat(AvData["payung_kuning"]).toLocaleString());
						$("td:eq(10)",AvRow).html("<button class='btn btn-md btn-flat btn-warning')>Ubah</button>"+
			...
			function tableListDataKartuStok(param1, param2, param3){
				$.ajax({
					...
					success : function(response){
						var kdGdRoll = "";
						var saldoAwal = 0;
						var saldoAwalBobin = 0;
						var saldoAwalPayung = 0;
						var saldoAwalPayungKuning = 0;
						var totalMasukBeratPerPeriodeBagian = 0;
						var totalMasukBobinPerPeriodeBagian = 0;
						var totalMasukPayungPerPeriodeBagian = 0;
						var totalMasukPayungKuningPerPeriodeBagian = 0;
						var totalMasukBeratPerPeriode = 0;
						var totalMasukBobinPerPeriode = 0;
						var totalMasukPayungPerPeriode = 0;
						var totalMasukPayungKuningPerPeriode = 0;
						var totalKeluarBeratPerPeriode = 0;
						var totalKeluarBobinPerPeriode = 0;
						var totalKeluarPayungPerPeriode = 0;
						var totalKeluarPayungKuningPerPeriode = 0;
            ...
            var periode = arrTglAkhir[0]+"-"+arrTglAkhir[1];
						...
						$.each(response.dataStokMaster, function(AvIndex, AvValue){
							var saldoAkhir = 0;
							var saldoAkhirBobin = 0;
							var saldoAkhirPayung = 0;
							var saldoAkhirPayungKuning = 0;
							...
							$.each(response.saldoAwal, function(AvIndex2, AvValue2){
								if(AvValue2.kd_gd_roll == kdGdRoll){
									...
									saldoAwalBobin = AvValue2.saldoAwalBobin;
									saldoAwalPayung = AvValue2.saldoAwalPayung;
									saldoAwalPayungKuning = AvValue2.saldoAwalPayungKuning;

									saldoAkhir += parseFloat(saldoAwal);
									saldoAkhirBobin += parseFloat(AvValue2.saldoAwalBobin);
									saldoAkhirPayung += parseFloat(AvValue2.saldoAwalPayung);
									saldoAkhirPayungKuning += parseFloat(AvValue2.saldoAwalPayungKuning);
									return false;
								}else{
									saldoAwal = 0;
								}
							...
							$.each(response.totalPerPeriode, function(AvIndex2, AvValue2){
								if(AvValue2.kd_gd_roll == kdGdRoll){
									...
									totalMasukBobinPerPeriode = AvValue2.totalMasukBobinPerPeriode
									totalMasukPayungPerPeriode = AvValue2.totalMasukPayungPerPeriode
									totalMasukPayungKuningPerPeriode = AvValue2.totalMasukPayungKuningPerPeriode

									totalKeluarBeratPerPeriode = AvValue2.totalKeluarBeratPerPeriode;
									totalKeluarBobinPerPeriode = AvValue2.totalKeluarBobinPerPeriode
									totalKeluarPayungPerPeriode = AvValue2.totalKeluarPayungPerPeriode
									totalKeluarPayungKuningPerPeriode = AvValue2.totalKeluarPayungKuningPerPeriode
									saldoAkhir += parseFloat(totalMasukBeratPerPeriode);
									saldoAkhir -= parseFloat(totalKeluarBeratPerPeriode);

									saldoAkhirBobin += parseFloat(AvValue2.totalMasukBobinPerPeriode);
									saldoAkhirPayung += parseFloat(AvValue2.totalMasukPayungPerPeriode);
									saldoAkhirPayungKuning += parseFloat(AvValue2.totalMasukPayungKuningPerPeriode);

									saldoAkhirBobin -= parseFloat(AvValue2.totalKeluarBobinPerPeriode);
									saldoAkhirPayung -= parseFloat(AvValue2.totalKeluarPayungPerPeriode);
									saldoAkhirPayungKuning -= parseFloat(AvValue2.totalKeluarPayungKuningPerPeriode);
									return false;
								}else{
									totalMasukBeratPerPeriode = 0;
									totalKeluarBeratPerPeriode = 0;
									totalMasukBobinPerPeriode = 0;
									totalMasukPayungPerPeriode = 0;
									totalMasukPayungKuningPerPeriode = 0;
									totalKeluarBobinPerPeriode = 0;
									totalKeluarPayungPerPeriode = 0;
									totalKeluarPayungKuningPerPeriode = 0;
								}
							});
							...
							$.each(response.totalPerPeriodeBagian, function(AvIndex2, AvValue2){
								if(AvValue2.kd_gd_roll == kdGdRoll){
									...
									totalMasukBobinPerPeriodeBagian = AvValue2.totalMasukBobinPerPeriodeBagian;
									totalMasukPayungPerPeriodeBagian = AvValue2.totalMasukPayungPerPeriodeBagian;
									totalMasukPayungKuningPerPeriodeBagian = AvValue2.totalMasukPayungKuningPerPeriodeBagian;
									saldoAkhir += parseFloat(totalMasukBeratPerPeriodeBagian);
									saldoAkhirBobin += parseFloat(AvValue2.totalMasukBobinPerPeriodeBagian);
									saldoAkhirPayung += parseFloat(AvValue2.totalMasukPayungPerPeriodeBagian);
									saldoAkhirPayungKuning += parseFloat(AvValue2.totalMasukPayungKuningPerPeriodeBagian);
									return false;
								}else{
									totalMasukBeratPerPeriodeBagian=0;
									totalMasukBobinPerPeriodeBagian=0;
									totalMasukPayungPerPeriodeBagian=0;
									totalMasukPayungKuningPerPeriodeBagian=0;
								}
							});
							var checkboxValue = kdGdRoll+"|"+saldoAkhir.toFixed(2)+"|"+saldoAkhirBobin+"|"+saldoAkhirPayung+"|"+saldoAkhirPayungKuning;
              ...
              $.each(response.dataCheklist, function(AvIndex2, AvValue2){
                if(AvValue2.kd_gd_roll == kdGdRoll){
                  if(AvValue2.ceklis == "FALSE"){
                    if(group == "gudang roll"){
                      attr = "";
                    }else{
                      attr = "onclick=checkedAndUnchecked(this,'TRUE','"+AvValue2.id_ceklis_gr+"','"+periode+"')";
                    }
                    ...
                    if(group == "gudang roll"){
                      ...
                      attr = "onclick=checkedAndUnchecked(this,'FALSE','"+AvValue2.id_ceklis_gr+"','"+periode+"')";
                    }
							...
							if(
								...
								$("#tableDataKartuStok > tbody:last-child").append(
									"<tr>"+
										...
										"<td>"+ parseFloat(AvValue.bobin).toLocaleString() +"</td>"+
										"<td>"+ parseFloat(AvValue.payung).toLocaleString() +"</td>"+
										"<td>"+ parseFloat(AvValue.payung_kuning).toLocaleString() +"</td>"+
										"<td>"+ parseFloat(saldoAwal).toLocaleString() +"</td>"+
										"<td>"+ parseFloat(saldoAwalBobin).toLocaleString() +"</td>"+
										"<td>"+ parseFloat(saldoAwalPayung).toLocaleString() +"</td>"+
										"<td>"+ parseFloat(saldoAwalPayungKuning).toLocaleString() +"</td>"+
										"<td>"+ parseFloat(totalMasukBeratPerPeriodeBagian).toLocaleString() +"</td>"+
										"<td>"+ parseFloat(totalMasukBobinPerPeriodeBagian).toLocaleString() +"</td>"+
										"<td>"+ parseFloat(totalMasukPayungPerPeriodeBagian).toLocaleString() +"</td>"+
										"<td>"+ parseFloat(totalMasukPayungKuningPerPeriodeBagian).toLocaleString() +"</td>"+
										"<td>"+ parseFloat(totalMasukBeratPerPeriode).toLocaleString() +"</td>"+
										"<td>"+ parseFloat(totalMasukBobinPerPeriode).toLocaleString() +"</td>"+
										"<td>"+ parseFloat(totalMasukPayungPerPeriode).toLocaleString() +"</td>"+
										"<td>"+ parseFloat(totalMasukPayungKuningPerPeriode).toLocaleString() +"</td>"+
										"<td>"+ parseFloat(totalKeluarBeratPerPeriode).toLocaleString() +"</td>"+
										"<td>"+ parseFloat(totalKeluarBobinPerPeriode).toLocaleString() +"</td>"+
										"<td>"+ parseFloat(totalKeluarPayungPerPeriode).toLocaleString() +"</td>"+
										"<td>"+ parseFloat(totalKeluarPayungKuningPerPeriode).toLocaleString() +"</td>"+
										"<td>"+ parseFloat(saldoAkhir).toLocaleString() +"</td>"+
										"<td>"+ parseFloat(saldoAkhirBobin).toLocaleString() +"</td>"+
										"<td>"+ parseFloat(saldoAkhirPayung).toLocaleString() +"</td>"+
										"<td>"+ parseFloat(saldoAkhirPayungKuning).toLocaleString() +"</td>"+
										"<td>"+
											"<input type='checkbox' name='ceklis' value='"+checkboxValue+"'> <br>"+
											"<button class='btn btn-sm btn-default btn-flat' "+attr+" id='"+AvValue.kd_gd_roll+"'>"+icon+"</button>"+
										"</td>"+
									"</tr>"
								);
							}
			...
			function tableListDataKartuStokSort(param1, param2, param3){
	            ...
	            var periode = arrTglAkhir[0]+"-"+arrTglAkhir[1];
							$.each(response.saldoAkhir, function(AvIndex, AvValue){
								var kdGdRoll = AvValue.kd_gd_roll;
								var attr = "onclick=checkedAndUnchecked(this,'TRUE','','"+periode+"')";
	              var icon = "<i class='fa fa-square-o'></i>";
	                ...
	                if(AvValue2.kd_gd_roll == kdGdRoll){
	                  if(AvValue2.ceklis == "FALSE"){
	                    ...
	                    icon = "<i class='fa fa-check-square-o'></i>";
								...
								var checkboxValue = AvValue.kd_gd_roll+"|"+parseFloat(AvValue.saldoAkhir).toFixed(2)+"|"+AvValue.saldoAkhirBobin+"|"+AvValue.saldoAkhirPayung+"|"+AvValue.saldoAkhirPayungKuning;
								if(
									parseFloat(AvValue.saldoAwalBerat) > 0 ||
									parseFloat(AvValue.saldoAwalBobin) > 0 ||
									parseFloat(AvValue.saldoAwalPayung) > 0 ||
									parseFloat(AvValue.saldoAwalPayungKuning) > 0 ||
									parseFloat(AvValue.totalMasukBeratPerPeriodeBagian) > 0 ||
									parseFloat(AvValue.totalMasukBobinPerPeriodeBagian) > 0 ||
									parseFloat(AvValue.totalMasukPayungPerPeriodeBagian) > 0 ||
									parseFloat(AvValue.totalMasukPayungKuningPerPeriodeBagian) > 0 ||
									parseFloat(AvValue.totalMasukBeratPerPeriode) > 0 ||
									parseFloat(AvValue.totalMasukBobinPerPeriode) > 0 ||
									parseFloat(AvValue.totalMasukPayungPerPeriode) > 0 ||
									parseFloat(AvValue.totalMasukPayungKuningPerPeriode) > 0 ||
									parseFloat(AvValue.totalKeluarBeratPerPeriode) > 0 ||
									parseFloat(AvValue.totalKeluarBobinPerPeriode) > 0 ||
									parseFloat(AvValue.totalKeluarPayungPerPeriode) > 0 ||
									parseFloat(AvValue.totalKeluarPayungKuningPerPeriode) > 0
								){
									$("#tableDataKartuStok > tbody:last-child").append(
									"<tr>"+
										"<td>"+ ++AvIndex +"</td>"+
										...
										"<td>"+ parseFloat(AvValue.saldoAwalBobin).toLocaleString() +"</td>"+
										"<td>"+ parseFloat(AvValue.saldoAwalPayung).toLocaleString() +"</td>"+
										"<td>"+ parseFloat(AvValue.saldoAwalPayungKuning).toLocaleString() +"</td>"+
										"<td>"+ parseFloat(AvValue.totalMasukBeratPerPeriodeBagian).toLocaleString() +"</td>"+
										"<td>"+ parseFloat(AvValue.totalMasukBobinPerPeriodeBagian).toLocaleString() +"</td>"+
										"<td>"+ parseFloat(AvValue.totalMasukPayungPerPeriodeBagian).toLocaleString() +"</td>"+
										"<td>"+ parseFloat(AvValue.totalMasukPayungKuningPerPeriodeBagian).toLocaleString() +"</td>"+
										"<td>"+ parseFloat(AvValue.totalMasukBeratPerPeriode).toLocaleString() +"</td>"+
										"<td>"+ parseFloat(AvValue.totalMasukBobinPerPeriode).toLocaleString() +"</td>"+
										"<td>"+ parseFloat(AvValue.totalMasukPayungPerPeriode).toLocaleString() +"</td>"+
										"<td>"+ parseFloat(AvValue.totalMasukPayungKuningPerPeriode).toLocaleString() +"</td>"+
										"<td>"+ parseFloat(AvValue.totalKeluarBeratPerPeriode).toLocaleString() +"</td>"+
										"<td>"+ parseFloat(AvValue.totalKeluarBobinPerPeriode).toLocaleString() +"</td>"+
										"<td>"+ parseFloat(AvValue.totalKeluarPayungPerPeriode).toLocaleString() +"</td>"+
										"<td>"+ parseFloat(AvValue.totalKeluarPayungKuningPerPeriode).toLocaleString() +"</td>"+
										"<td>"+ parseFloat(AvValue.saldoAkhir).toLocaleString() +"</td>"+
										"<td>"+ parseFloat(AvValue.saldoAkhirBobin).toLocaleString() +"</td>"+
										"<td>"+ parseFloat(AvValue.saldoAkhirPayung).toLocaleString() +"</td>"+
										"<td>"+ parseFloat(AvValue.saldoAkhirPayungKuning).toLocaleString() +"</td>"+
										"<td>"+
											"<input type='checkbox' name='ceklis' value='"+checkboxValue+"'> <br>"+
											"<button class='btn btn-sm btn-default btn-flat' "+attr+" id='"+AvValue.kd_gd_roll+"'>"+icon+"</button>"+
										"</td>"+
			...
			function tableListCetakDataKartuStok(param1, param2, param3){
				var arrBulan = ["Januari","Februari","Maret","April","Mei","Juni","Juli",
												"Agustus","September","Oktober","November","Desember"];
				var arrTglAwal = param1.split("-");
				var arrTglAkhir = param2.split("-");

				var awalBulan = arrBulan[parseFloat(arrTglAwal[1])-1];
				var akhirBulan = arrBulan[parseFloat(arrTglAkhir[1])-1];
				$.ajax({
					type : "POST",
					...
					success : function(response){
						...
						var totalJumlahSisaBerat = 0;
						var totalJumlahSisaBobin = 0;
						var totalJumlahSisaPayung = 0;
						var totalJumlahSisaPayungKuning = 0;

						var totalJumlahSubSaldoBerat = 0;
						var totalJumlahSubSaldoBobin = 0;
						var totalJumlahSubSaldoPayung = 0;
						var totalJumlahSubSaldoPayungKuning = 0;

						var totalJumlahSaldoBerat = 0;
						var totalJumlahSaldoBobin = 0;
						var totalJumlahSaldoPayung = 0;
						var totalJumlahSaldoPayungKuning = 0;
						...
						$("#tableDataKartuStok > tbody > tr").empty();
						$.each(response.saldoAwal, function(AvIndex, AvValue){
							saldoAwalBerat = parseFloat(AvValue.saldoAwalBerat);
							saldoAwalBobin = parseFloat(AvValue.saldoAwalBobin);
							saldoAwalPayung = parseFloat(AvValue.saldoAwalPayung);
							saldoAwalPayungKuning = parseFloat(AvValue.saldoAwalPayungKuning);

							$("#tableDataKartuStok > tbody:last-child").append(
								"<tr style='background-color: rgba(255, 123, 0, 0.8);'>"+
									"<td>"+""+"</td>"+
									"<td>Saldo Awal Bulan "+akhirBulan+"</td>"+
									"<td>"+""+"</td>"+
									"<td>"+""+"</td>"+
									"<td>"+""+"</td>"+
									"<td>"+""+"</td>"+
						...
						totalJumlahSubSaldoBerat += saldoAwalBerat;
						totalJumlahSubSaldoBobin += saldoAwalBobin;
						totalJumlahSubSaldoPayung += saldoAwalPayung;
						totalJumlahSubSaldoPayungKuning += saldoAwalPayungKuning;

						$.each(response.dataTransaksi, function(AvIndex, AvValue){
							...
							totalJumlahKeluarBerat += (parseFloat(AvValue.beratKeluar) + parseFloat(AvValue.beratOperatorCetak));
							totalJumlahKeluarBobin += (parseFloat(AvValue.bobinKeluar) + parseFloat(AvValue.bobinOperatorCetak));
							totalJumlahKeluarPayung += (parseFloat(AvValue.payungKeluar) + parseFloat(AvValue.payungOperatorCetak));
							totalJumlahKeluarPayungKuning += (parseFloat(AvValue.payungKuningKeluar) + parseFloat(AvValue.payungKuningOperatorCetak));

							totalJumlahSisaBerat += parseFloat(AvValue.beratKembalian);
							totalJumlahSisaBobin += parseFloat(AvValue.bobinKembalian);
							totalJumlahSisaPayung += parseFloat(AvValue.payungKembalian);
							totalJumlahSisaPayungKuning += parseFloat(AvValue.payungKuningKembalian);

							totalJumlahSubSaldoBerat += ((parseFloat(AvValue.beratMasuk) +
																					 parseFloat(AvValue.beratKembalian)) -
																				 	(parseFloat(AvValue.beratKeluar) + parseFloat(AvValue.beratOperatorCetak)))

							totalJumlahSubSaldoBobin += ((parseFloat(AvValue.bobinMasuk) +
																					 parseFloat(AvValue.bobinKembalian)) -
																				 	(parseFloat(AvValue.bobinKeluar) + parseFloat(AvValue.bobinOperatorCetak)))

							totalJumlahSubSaldoPayung += ((parseFloat(AvValue.payungMasuk) +
																					 	parseFloat(AvValue.payungKembalian)) -
																						(parseFloat(AvValue.payungKeluar) + parseFloat(AvValue.payungOperatorCetak)))

							totalJumlahSubSaldoPayungKuning += ((parseFloat(AvValue.payungKuningMasuk) +
																					 				parseFloat(AvValue.payungKuningKembalian)) -
																									(parseFloat(AvValue.payungKuningKeluar) + parseFloat(AvValue.payungKuningOperatorCetak)))

							totalJumlahSaldoBerat += totalJumlahSubSaldoBerat;
							totalJumlahSaldoBobin += totalJumlahSubSaldoBobin;
							totalJumlahSaldoPayung += totalJumlahSubSaldoPayung;
							totalJumlahSaldoPayungKuning += totalJumlahSubSaldoPayungKuning;

							$("#tableDataKartuStok > tbody:last-child").append(
								"<tr>"+
									"<td>"+AvValue.tgl_transaksi+"</td>"+
									"<td>Transaksi Bulan "+akhirBulan+"</td>"+
									"<td>"+(parseFloat(AvValue.beratMasuk) != 0 ? parseFloat(AvValue.beratMasuk).toLocaleString() : "")+"</td>"+
									"<td>"+(parseFloat(AvValue.bobinMasuk) != 0 ? parseFloat(AvValue.bobinMasuk) : "")+"</td>"+
									"<td>"+(parseFloat(AvValue.payungMasuk) != 0 ? parseFloat(AvValue.payungMasuk) : "")+"</td>"+
									"<td>"+(parseFloat(AvValue.payungKuningMasuk) != 0 ? parseFloat(AvValue.payungKuningMasuk) : "")+"</td>"+
									"<td>"+(parseFloat(AvValue.beratKeluar) != 0 ? parseFloat(AvValue.beratKeluar).toLocaleString() : "")+"</td>"+
									"<td>"+(parseFloat(AvValue.bobinKeluar) != 0 ? parseFloat(AvValue.bobinKeluar) : "")+"</td>"+
									"<td>"+(parseFloat(AvValue.payungKeluar) != 0 ? parseFloat(AvValue.payungKeluar) : "")+"</td>"+
									"<td>"+(parseFloat(AvValue.payungKuningKeluar) != 0 ? parseFloat(AvValue.payungKuningKeluar) : "")+"</td>"+
									"<td>"+(parseFloat(AvValue.beratKembalian) != 0 ? parseFloat(AvValue.beratKembalian).toLocaleString() : "")+"</td>"+
									"<td>"+(parseFloat(AvValue.bobinKembalian) != 0 ? parseFloat(AvValue.bobinKembalian) : "")+"</td>"+
									"<td>"+(parseFloat(AvValue.payungKembalian) != 0 ? parseFloat(AvValue.payungKembalian) : "")+"</td>"+
									"<td>"+(parseFloat(AvValue.payungKuningKembalian) != 0 ? parseFloat(AvValue.payungKuningKembalian) : "")+"</td>"+
									"<td>"+totalJumlahSubSaldoBerat.toLocaleString()+"</td>"+
									"<td>"+totalJumlahSubSaldoBobin.toLocaleString()+"</td>"+
									"<td>"+totalJumlahSubSaldoPayung.toLocaleString()+"</td>"+
									"<td>"+totalJumlahSubSaldoPayungKuning.toLocaleString()+"</td>"+
								"</tr>"
							);

							if(AvValue.beratOperatorCetak != 0){
								$("#tableDataKartuStok > tbody:last-child").append(
									"<tr style='background-color: rgba(255, 0, 0, 0.5);'>"+
										...
										"<td></td>"+
										"<td></td>"+
										"<td>"+(parseFloat(AvValue.beratOperatorCetak) != 0 ? parseFloat(AvValue.beratOperatorCetak).toLocaleString() : "")+"</td>"+
										"<td>"+(parseFloat(AvValue.bobinOperatorCetak) != 0 ? parseFloat(AvValue.bobinOperatorCetak) : "")+"</td>"+
										"<td>"+(parseFloat(AvValue.payungOperatorCetak) != 0 ? parseFloat(AvValue.payungOperatorCetak) : "")+"</td>"+
										"<td>"+(parseFloat(AvValue.payungKuningOperatorCetak) != 0 ? parseFloat(AvValue.payungKuningOperatorCetak) : "")+"</td>"+
										"<td></td>"+
										"<td></td>"+
										"<td></td>"+
										"<td></td>"+
							...
							if(AvValue.beratSelisihTambah != 0){
								$("#tableDataKartuStok > tbody:last-child").append(
									"<tr>"+
										"<td>"+AvValue.tgl_transaksi+"</td>"+
										"<td>MASUK KE GUDANG (SELISIH TAMBAH)</td>"+
										"<td>"+(parseFloat(AvValue.beratSelisihTambah) != 0 ? parseFloat(AvValue.beratSelisihTambah).toLocaleString() : "")+"</td>"+
										"<td>"+(parseFloat(AvValue.bobinSelisihTambah) != 0 ? parseFloat(AvValue.bobinSelisihTambah) : "")+"</td>"+
										"<td>"+(parseFloat(AvValue.payungSelisihTambah) != 0 ? parseFloat(AvValue.payungSelisihTambah) : "")+"</td>"+
										"<td>"+(parseFloat(AvValue.payungKuningSelisihTambah) != 0 ? parseFloat(AvValue.payungKuningSelisihTambah) : "")+"</td>"+
										"<td></td>"+
										"<td></td>"+
										"<td></td>"+
										"<td></td>"+
							...
							if(AvValue.beratSelisihKurang != 0){
								$("#tableDataKartuStok > tbody:last-child").append(
									"<tr>"+
										"<td>"+AvValue.tgl_transaksi+"</td>"+
										...
										"<td></td>"+
										"<td></td>"+
										"<td>"+(parseFloat(AvValue.beratSelisihKurang) != 0 ? parseFloat(AvValue.beratSelisihKurang).toLocaleString() : "")+"</td>"+
										"<td>"+(parseFloat(AvValue.bobinSelisihKurang) != 0 ? parseFloat(AvValue.bobinSelisihKurang) : "")+"</td>"+
										"<td>"+(parseFloat(AvValue.payungSelisihKurang) != 0 ? parseFloat(AvValue.payungSelisihKurang) : "")+"</td>"+
										"<td>"+(parseFloat(AvValue.payungKuningSelisihKurang) != 0 ? parseFloat(AvValue.payungKuningSelisihKurang) : "")+"</td>"+
										"<td></td>"+
										"<td></td>"+
										"<td></td>"+
										"<td></td>"+
						...
						totalJumlahMasukBerat += saldoAwalBerat;
						totalJumlahMasukBobin += saldoAwalBobin;
						totalJumlahMasukPayung += saldoAwalPayung;
						totalJumlahMasukPayungKuning += saldoAwalPayungKuning;
						...
						//
						// $("#totalJumlahSisaBerat").text(totalJumlahSisaBerat.toLocaleString());
						// $("#totalJumlahSisaBobin").text(totalJumlahSisaBobin.toLocaleString());
						// $("#totalJumlahSisaPayung").text(totalJumlahSisaPayung.toLocaleString());
						// $("#totalJumlahSisaPayungKuning").text(totalJumlahSisaPayungKuning.toLocaleString());
						//
						// $("#totalJumlahSaldoBerat").text(totalJumlahSaldoBerat.toLocaleString());
						// $("#totalJumlahSaldoBobin").text(totalJumlahSaldoBobin.toLocaleString());
						// $("#totalJumlahSaldoPayung").text(totalJumlahSaldoPayung.toLocaleString());
						// $("#totalJumlahSaldoPayungKuning").text(totalJumlahSaldoPayungKuning.toLocaleString());
					}
				});
			}
			...
			function checkAll(param){
				$("input:checkbox").prop('checked', param.checked);
				$("#checkAll").attr("onclick","uncheckAll(this)");
			}

			function uncheckAll(param){
				$("input:checkbox").prop('checked', param.checked);
				$("#checkAll").attr("onclick","checkAll(this)");
			}
			...
			function checkedAndUnchecked(param, param2, param3="", param4=""){
				...
				$.ajax({
					type : "POST",
					url : "<?php echo base_url('_gudangroll/main/updateCheckList'); ?>",
					dataType : "TEXT",
					data : {
						kdGdRoll : param.id,
						ceklis : param2,
			...
			function getLaporanHasilExtruderGlobal(){
				...
				if(tglAwal == "" || tglAwal==null){
					...
					$.ajax({
						type : "POST",
						url : "<?php echo base_url('_gudangroll/main/getCekHasilExtruder'); ?>",
						dataType : "JSON",
						data : {
							tglAwal : tglAwal
						},
						success : function(response){
							...
							$("#tableMasukGudang > tbody").empty();
							var totalHasilExtruder = 0;
							var totalMasukGudang = 0;
							$.each(response.HasilExtruder, function(AvIndex, AvValue){
								totalHasilExtruder += parseFloat(AvValue.JumlahSelesai);
								$("#tableHasilExtruder > tbody").append(
									"<tr>"+
										"<td>"+AvValue.shift+"</td>"+
										"<td>"+parseFloat(AvValue.JumlahSelesai).toLocaleString()+"</td>"+
									"</tr>"
								);
							});

							$.each(response.MasukGudang, function(AvIndex, AvValue){
								totalMasukGudang += parseFloat(AvValue.Berat);
								$("#tableMasukGudang > tbody").append(
									"<tr>"+
										"<td>"+AvValue.shift+"</td>"+
										"<td>"+parseFloat(AvValue.Berat).toLocaleString()+"</td>"+
									"</tr>"
								);
							});

							$("#totalHasilExtruder").text(totalHasilExtruder.toLocaleString());
							$("#totalMasukGudang").text(totalMasukGudang.toLocaleString());
