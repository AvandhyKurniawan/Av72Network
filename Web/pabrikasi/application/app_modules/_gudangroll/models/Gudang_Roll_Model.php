...
class Gudang_Roll_Model extends CI_Model{
  ...
  public function insertTransaksiGudangRoll_DataAwal($param){
    $this->db->trans_begin();
    $arrCheck = $this->db->query("SELECT COUNT(id) AS counter
                                  FROM transaksi_gudang_roll
                                  WHERE keterangan_history = 'DATA AWAL'
                                  AND kd_gd_roll = '$param[kd_gd_roll]'
                                  AND deleted='FALSE'")->result_array();
    if($arrCheck[0]["counter"] > 0){
      $this->db->trans_rollback();
      return FALSE;
    }else{
      ...
      $this->db->set("payung_kuning","payung_kuning + ".$param["payung_kuning"], FALSE);
  ...
  public function insertTambahSelisihBarang($data){
    $this->db->trans_begin();
    $YM = date("Y-m",strtotime($data["tgl_transaksi"]));
    $jnsPermintaan = $data["jns_permintaan"];
    $Q = "SELECT COUNT(id) AS counter
          FROM transaksi_gudang_roll
          WHERE DATE_FORMAT(tgl_transaksi, '%Y-%m') = '$YM'
          AND jns_permintaan = '$jnsPermintaan'
          AND status_lock = 'TRUE'
          AND deleted = 'FALSE'";
    $arrCounter = $this->db->query($Q)->result_array();
  ...
  public function selectCheckDataAwal($param){
    $arrData = $this->db->query("SELECT * FROM transaksi_gudang_roll WHERE kd_gd_roll='$param' AND keterangan_history='DATA AWAL'")->result_array();
    return json_encode($arrData);
  }
  ...
  public function selectHasilJob($param){
    $this->datatables->select("THP.kd_hasil_potong,
                               ...
                               RC.warna_plastik,
                               RC.merek,RC.kd_gd_roll,IF(RC.ket_merek IS NULL OR RC.ket_merek='', RC.ket_barang, RC.ket_merek) AS keterangan,
                               ...
                               (TPHP.berat_pengambilan_bagian_tumpuk + TPHP.berat_pengambilan_gudang_tumpuk + TPHP.berat_sisa_semalam_tumpuk) AS berat_pengambilan_tumpuk,
                               (TPHP.payung_pengambilan_bagian_tumpuk + TPHP.payung_pengambilan_gudang_tumpuk + TPHP.payung_sisa_semalam_tumpuk) AS payung_pengambilan_tumpuk,
                               (TPHP.payung_kuning_pengambilan_bagian_tumpuk + TPHP.payung_kuning_pengambilan_gudang_tumpuk + TPHP.payung_kuning_sisa_semalam_tumpuk) AS payung_kuning_pengambilan_tumpuk,
                               (TPHP.bobin_pengambilan_bagian_tumpuk + TPHP.bobin_pengambilan_gudang_tumpuk + TPHP.bobin_sisa_semalam_tumpuk) AS bobin_pengambilan_tumpuk,
                               ...
                               (TPHP.berat_sisa_hari_ini_tumpuk) AS berat_sisa_tumpuk,
                               (TPHP.bobin_sisa_hari_ini_tumpuk) AS bobin_sisa_tumpuk,
                               (TPHP.payung_sisa_hari_ini_tumpuk) AS payung_sisa_tumpuk,
                               (TPHP.payung_kuning_sisa_hari_ini_tumpuk) AS payung_kuning_sisa_tumpuk,
    ...
    $this->db->group_by("THP.kd_hasil_potong");
  ...
  public function selectCariHasilJob($param){
    ...
    $this->db->group_by("THP.kd_hasil_potong");
    $this->db->order_by("RC.tgl_rencana ASC");
    return $this->datatables->generate();
  }
  ...
  public function selectHasilExtruder(){
    $this->datatables->select("TGR.kd_gd_roll, GR.tebal, GR.ukuran AS hasil_ukuran, GR.ukuran AS panjang, TGR.tgl_transaksi,
                               GR.warna_plastik, TGR.berat, TGR.bobin, TGR.payung, TGR.payung_kuning,
                               TGR.shift, GR.merek, TGR.id");
    $this->datatables->from("transaksi_gudang_roll TGR");
    $this->datatables->join("gudang_roll GR","TGR.kd_gd_roll = GR.kd_gd_roll", "INNER");
    $this->datatables->where("TGR.deleted='FALSE' AND
                              TGR.status_transaksi = 'PROGRESS'");
  ...
  public function countHasilExtruder(){
    $data = $this->db->query("SELECT id FROM transaksi_gudang_roll
                              WHERE deleted='FALSE'
  ...
  public function selectDataKartuStok($param){
    ...
    if($param["jnsPermintaan"] == "POLOS"){
      ...
      $arrTotalPerPeriodeBagian = $this->db->query("SELECT SUM(IF(status_history='MASUK',berat,0)) AS totalMasukBeratPerPeriodeBagian,
                                                           ...
                                                           kd_gd_roll
                                                    FROM transaksi_gudang_roll
                                                    WHERE tgl_transaksi BETWEEN '$tglAwalPlus1' AND '$param[tglAkhir]'
                                                    AND deleted='FALSE'
                                                    AND keterangan_history != 'DATA AWAL'
                                                    AND bagian ='CETAK'
                                                    AND status_transaksi='FINISH'
    ...
    $dataCheklist = $this->db->query("SELECT * FROM ceklis_gudang_roll WHERE periode = '$periode'")->result_array();
  ...
  public function selectDataKartuStokSort($param){
    ...
    $arrSaldoAkhir = $this->db->query("SELECT GR.stok, GR.bobin, GR.payung, GR.payung_kuning, GR.kd_gd_roll,CONCAT(GR.ukuran, ' ', GR.merek, ' ', GR.warna_plastik) AS nm_barang, GR.ceklist,
                                              (
                                              ...
                                              (
                                                SUM(IF(TGR.status_history='MASUK' AND TGR.tgl_transaksi <= '$param[tglAwal]',TGR.bobin,0))-
                                                SUM(IF(TGR.status_history='KELUAR' AND TGR.tgl_transaksi <= '$param[tglAwal]',TGR.bobin,0))
                                              )  AS saldoAwalBobin,
                                              (
                                                SUM(IF(TGR.status_history='MASUK' AND TGR.tgl_transaksi <= '$param[tglAwal]',TGR.payung,0))-
                                                SUM(IF(TGR.status_history='KELUAR' AND TGR.tgl_transaksi <= '$param[tglAwal]',TGR.payung,0))
                                              )  AS saldoAwalPayung,
                                              (
                                                SUM(IF(TGR.status_history='MASUK' AND TGR.tgl_transaksi <= '$param[tglAwal]',TGR.payung_kuning,0))-
                                                SUM(IF(TGR.status_history='KELUAR' AND TGR.tgl_transaksi <= '$param[tglAwal]',TGR.payung_kuning,0))
                                              )  AS saldoAwalPayungKuning,
                                              ...
                                              (SUM(IF(TGR.status_history='MASUK' AND
                                                      ...
                                                      TGR.keterangan_history != 'DATA AWAL' AND
                                                      TGR.keterangan_history NOT IN ('HASIL CETAK') AND
                                                      TGR.bagian IN ('POTONG','CETAK','GUDANG ROLL'),TGR.bobin,0))
                                              ) AS totalMasukBobinPerPeriode,
                                              (SUM(IF(TGR.status_history='MASUK' AND
                                                      TGR.tgl_transaksi BETWEEN '$tglAwalPlus1' AND '$param[tglAkhir]' AND
                                                      TGR.keterangan_history != 'DATA AWAL' AND
                                                      TGR.keterangan_history NOT IN ('HASIL CETAK') AND
                                                      TGR.bagian IN ('POTONG','CETAK','GUDANG ROLL'),TGR.payung,0))
                                              ) AS totalMasukPayungPerPeriode,
                                              (SUM(IF(TGR.status_history='MASUK' AND
                                                      TGR.tgl_transaksi BETWEEN '$tglAwalPlus1' AND '$param[tglAkhir]' AND
                                                      TGR.keterangan_history != 'DATA AWAL' AND
                                                      TGR.keterangan_history NOT IN ('HASIL CETAK') AND
                                                      TGR.bagian IN ('POTONG','CETAK','GUDANG ROLL'),TGR.payung_kuning,0))
                                              ) AS totalMasukPayungKuningPerPeriode,
                                              ...
                                              (SUM(IF(TGR.status_history='KELUAR' AND
                                                      ...
                                                      TGR.keterangan_history NOT IN ('HASIL CETAK') AND
                                                      TGR.bagian IN ('POTONG','CETAK','GUDANG ROLL'),TGR.bobin,0))
                                              ) AS totalKeluarBobinPerPeriode,
                                              (SUM(IF(TGR.status_history='KELUAR' AND
                                                      TGR.tgl_transaksi BETWEEN '$tglAwalPlus1' AND '$param[tglAkhir]' AND
                                                      TGR.keterangan_history != 'DATA AWAL' AND
                                                      TGR.keterangan_history NOT IN ('HASIL CETAK') AND
                                                      TGR.bagian IN ('POTONG','CETAK','GUDANG ROLL'),TGR.payung,0))
                                              ) AS totalKeluarPayungPerPeriode,
                                              (SUM(IF(TGR.status_history='KELUAR' AND
                                                      TGR.tgl_transaksi BETWEEN '$tglAwalPlus1' AND '$param[tglAkhir]' AND
                                                      TGR.keterangan_history != 'DATA AWAL' AND
                                                      TGR.keterangan_history NOT IN ('HASIL CETAK') AND
                                                      TGR.bagian IN ('POTONG','CETAK','GUDANG ROLL'),TGR.payung_kuning,0))
                                              ) AS totalKeluarPayungKuningPerPeriode,
                                              ...
                                              (
                                                SUM(IF(TGR.status_history='MASUK' AND
                                                       TGR.tgl_transaksi BETWEEN '$tglAwalPlus1' AND '$param[tglAkhir]' AND
                                                       TGR.keterangan_history != 'DATA AWAL' AND
                                                       TGR.bagian='$bagian',TGR.bobin,0))
                                              ) AS totalMasukBobinPerPeriodeBagian,
                                              (
                                                SUM(IF(TGR.status_history='MASUK' AND
                                                       TGR.tgl_transaksi BETWEEN '$tglAwalPlus1' AND '$param[tglAkhir]' AND
                                                       TGR.keterangan_history != 'DATA AWAL' AND
                                                       TGR.bagian='$bagian',TGR.payung,0))
                                              ) AS totalMasukPayungPerPeriodeBagian,
                                              (
                                                SUM(IF(TGR.status_history='MASUK' AND
                                                       TGR.tgl_transaksi BETWEEN '$tglAwalPlus1' AND '$param[tglAkhir]' AND
                                                       TGR.keterangan_history != 'DATA AWAL' AND
                                                       TGR.bagian='$bagian',TGR.payung_kuning,0))
                                              ) AS totalMasukPayungKuningPerPeriodeBagian,
  ...
  public function selectCetakKartuStok($param){
    $tglAwal = date("Y-m-d", strtotime("+1 Days", strtotime($param["tglAwal"])));
    $arrSaldoAwal = $this->db->query("SELECT (
                                                SUM(IF(status_history='MASUK' AND tgl_transaksi <= '$param[tglAwal]',berat,0))-
                                                SUM(IF(status_history='KELUAR' AND tgl_transaksi <= '$param[tglAwal]',berat,0))
                                             )  AS saldoAwalBerat,
    ...
    $Q = "SELECT SUM(IF(TGR.status_history = 'MASUK' AND
                        TGR.keterangan_history IN ('HASIL EXTRUDER', 'HASIL CETAK'), TGR.berat, 0)) AS beratMasuk,
                 SUM(IF(TGR.status_history = 'KELUAR' AND
                        TGR.keterangan_history NOT IN ('OPERATOR CETAK','MANDOR CETAK (EXTRUDER)','MASUK KE GUDANG (SELISIH KURANG)'), TGR.berat, 0)) AS beratKeluar,
                 SUM(IF(TGR.status_history = 'MASUK' AND
                        TGR.keterangan_history IN ('OPERATOR(SISA MESIN)', 'OPERATOR(SISA SEMALAM)'), TGR.berat, 0)) AS beratKembalian,

                 SUM(IF(TGR.status_history = 'MASUK' AND
                        TGR.keterangan_history IN ('HASIL EXTRUDER', 'HASIL CETAK'), TGR.bobin, 0)) AS bobinMasuk,
                 SUM(IF(TGR.status_history = 'KELUAR' AND
                        TGR.keterangan_history NOT IN ('OPERATOR CETAK','MANDOR CETAK (EXTRUDER)','MASUK KE GUDANG (SELISIH KURANG)'), TGR.bobin, 0)) AS bobinKeluar,
                 SUM(IF(TGR.status_history = 'MASUK' AND
                        TGR.keterangan_history IN ('OPERATOR(SISA MESIN)', 'OPERATOR(SISA SEMALAM)'), TGR.bobin, 0)) AS bobinKembalian,

                 SUM(IF(TGR.status_history = 'MASUK' AND
                        TGR.keterangan_history IN ('HASIL EXTRUDER', 'HASIL CETAK'), TGR.payung, 0)) AS payungMasuk,
                 SUM(IF(TGR.status_history = 'KELUAR' AND
                        TGR.keterangan_history NOT IN ('OPERATOR CETAK','MANDOR CETAK (EXTRUDER)','MASUK KE GUDANG (SELISIH KURANG)'), TGR.payung, 0)) AS payungKeluar,
                 SUM(IF(TGR.status_history = 'MASUK' AND
                        TGR.keterangan_history IN ('OPERATOR(SISA MESIN)', 'OPERATOR(SISA SEMALAM)'), TGR.payung, 0)) AS payungKembalian,

                 SUM(IF(TGR.status_history = 'MASUK' AND
                        TGR.keterangan_history IN ('HASIL EXTRUDER', 'HASIL CETAK'), TGR.payung_kuning, 0)) AS payungKuningMasuk,
                 SUM(IF(TGR.status_history = 'KELUAR' AND
                        TGR.keterangan_history NOT IN ('OPERATOR CETAK','MANDOR CETAK (EXTRUDER)','MASUK KE GUDANG (SELISIH KURANG)'), TGR.payung_kuning, 0)) AS payungKuningKeluar,
                 SUM(IF(TGR.status_history = 'MASUK' AND
                        TGR.keterangan_history IN ('OPERATOR(SISA MESIN)', 'OPERATOR(SISA SEMALAM)'), TGR.payung_kuning, 0)) AS payungKuningKembalian,

                 SUM(IF(TGR.status_history = 'KELUAR' AND
                        TGR.keterangan_history IN ('OPERATOR CETAK','MANDOR CETAK (EXTRUDER)'), TGR.berat, 0)) AS beratOperatorCetak,
                 SUM(IF(TGR.status_history = 'KELUAR' AND
                        TGR.keterangan_history IN ('OPERATOR CETAK','MANDOR CETAK (EXTRUDER)'), TGR.bobin, 0)) AS bobinOperatorCetak,
                 SUM(IF(TGR.status_history = 'KELUAR' AND
                        TGR.keterangan_history IN ('OPERATOR CETAK','MANDOR CETAK (EXTRUDER)'), TGR.payung, 0)) AS payungOperatorCetak,
                 SUM(IF(TGR.status_history = 'KELUAR' AND
                        TGR.keterangan_history IN ('OPERATOR CETAK','MANDOR CETAK (EXTRUDER)'), TGR.payung_kuning, 0)) AS payungKuningOperatorCetak,

                 SUM(IF(TGR.status_history = 'MASUK' AND
                        TGR.keterangan_history IN ('MASUK KE GUDANG (SELISIH TAMBAH)'), TGR.berat, 0)) AS beratSelisihTambah,
                 SUM(IF(TGR.status_history = 'MASUK' AND
                        TGR.keterangan_history IN ('MASUK KE GUDANG (SELISIH TAMBAH)'), TGR.bobin, 0)) AS bobinSelisihTambah,
                 SUM(IF(TGR.status_history = 'MASUK' AND
                        TGR.keterangan_history IN ('MASUK KE GUDANG (SELISIH TAMBAH)'), TGR.payung, 0)) AS payungSelisihTambah,
                 SUM(IF(TGR.status_history = 'MASUK' AND
                        TGR.keterangan_history IN ('MASUK KE GUDANG (SELISIH TAMBAH)'), TGR.payung_kuning, 0)) AS payungKuningSelisihTambah,

                 SUM(IF(TGR.status_history = 'KELUAR' AND
                        TGR.keterangan_history IN ('MASUK KE GUDANG (SELISIH KURANG)'), TGR.berat, 0)) AS beratSelisihKurang,
                 SUM(IF(TGR.status_history = 'KELUAR' AND
                        TGR.keterangan_history IN ('MASUK KE GUDANG (SELISIH KURANG)'), TGR.bobin, 0)) AS bobinSelisihKurang,
                 SUM(IF(TGR.status_history = 'KELUAR' AND
                        TGR.keterangan_history IN ('MASUK KE GUDANG (SELISIH KURANG)'), TGR.payung, 0)) AS payungSelisihKurang,
                 SUM(IF(TGR.status_history = 'KELUAR' AND
                       TGR.keterangan_history IN ('MASUK KE GUDANG (SELISIH KURANG)'), TGR.payung_kuning, 0)) AS payungKuningSelisihKurang,
          ...
          GROUP BY TGR.tgl_transaksi
          ORDER BY TGR.tgl_transaksi ASC";
    $result = $this->db->query($Q)->result_array();
    $finalResult = array("saldoAwal" => $arrSaldoAwal,
                         "dataTransaksi" => $result);
    return json_encode($finalResult);
  }

  public function selectCekHasilExtruder($param){
    $Q = "SELECT shift, SUM(jumlah_selesai) AS JumlahSelesai
          FROM transaksi_hasil_extruder
          WHERE tgl_rencana = '$param'
          AND deleted='FALSE'
          GROUP BY shift
          ORDER BY shift ASC";

    $Q2 = "SELECT shift, SUM(berat) AS Berat
           FROM transaksi_gudang_roll
           WHERE bagian = 'EXTRUDER'
           AND status_history = 'MASUK'
           AND deleted='FALSE'
           AND status_transaksi = 'FINISH'
           AND tgl_transaksi = '$param'
           GROUP BY shift
           ORDER BY shift ASC";
     $result = array("HasilExtruder" => $this->db->query($Q)->result_array(),
                     "MasukGudang"   => $this->db->query($Q2)->result_array());
     return json_encode($result);
  }
  ...
  public function updateStokMasterBatch($param){
    ...
    for ($i=0; $i < count($param); $i++) {
      $this->db->where("kd_gd_roll",$param[$i]["kd_gd_roll"]);
      $this->db->update("gudang_roll",$param[$i]);
    }
    if($this->db->trans_status() === FALSE){
      $this->db->trans_rollback();
      return FALSE;
  ...
  public function updateDeleteTransaksiDataAwal($param){
    $this->db->trans_begin();
    $this->db->where("id", $param["id"]);
    $this->db->update("transaksi_gudang_roll",$param);
    $arrData = $this->db->query("SELECT berat, bobin, payung, payung_kuning, kd_gd_roll WHERE id='$param[id]'")->result_array();
    $this->db->set("stok","stok - ".$arrData[0]["berat"], FALSE);
    $this->db->set("bobin","bobin - ".$arrData[0]["bobin"], FALSE);
    $this->db->set("payung","payung - ".$arrData[0]["payung"], FALSE);
    $this->db->set("payung_kuning","payung_kuning - ".$arrData[0]["payung_kuning"], FALSE);
  ...
  public function updateDataAwalGudangRoll($param){
    $this->db->trans_begin();
    $arrDataLama = $this->db->query("SELECT berat, bobin, payung, payung_kuning
                                     FROM transaksi_gudang_roll
                                     WHERE id = '$param[id]'")->result_array();

    $this->db->set("stok","stok + ".($arrDataLama[0]["berat"] - $param["berat"]), FALSE);
    $this->db->set("bobin","bobin + ".($arrDataLama[0]["bobin"] - $param["bobin"]), FALSE);
    $this->db->set("payung","payung + ".($arrDataLama[0]["payung"] - $param["payung"]), FALSE);
    $this->db->set("payung_kuning","payung_kuning + ".($arrDataLama[0]["payung_kuning"] - $param["payung_kuning"]), FALSE);
    $this->db->where("kd_gd_roll",$param["kd_gd_roll"]);
    $this->db->update("gudang_roll");
  ...
  public function updateCheckListGudangRoll($param){
    $this->db->trans_begin();
    if(empty($param["id_ceklis_gr"])){
      $this->db->insert("ceklis_gudang_roll",$param);
    }else{
      $this->db->set("ceklis",$param["ceklis"]);
      $this->db->set("id_user",$param["id_user"]);
      $this->db->where("id_ceklis_gr", $param["id_ceklis_gr"]);
      $this->db->update("ceklis_gudang_roll");
    }
    if($this->db->trans_status() === FALSE){
      $this->db->trans_rollback();
      return "Gagal";
