...
class Main extends MX_Controller{
    ...
    if($isLogin){
      ...
      if($status==1){
        $listMenu["parentMenu"] = array(
          ...
          array("Content"=>"Cek Hasil Extruder","Link"=>"_gudangroll/main/cek_hasil_extruder_global","Name"=>"Hasil_Extruder_Global","Status"=>"Single","Icon"=>"fa fa-book","Id"=>"M_Hasil_Extruder_Global"),
  ...
  public function cek_hasil_extruder_global(){
    $isLogin = $this->isLogin();
    if($isLogin){
      $data["Data"] = array("Title"=>"CEK HASIL EXTRUDER GLOBAL");
      $data["Link"] = array("Segment1" =>$this->uri->rsegment(1),"Segment2"=>$this->uri->rsegment(2));
      $this->load->view("header");
      $this->load->view("sidebar",$this->checkStatus());
      $this->load->view("frm_cek_hasil_extruder_global",$data);
      $this->load->view("footer");
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
      redirect("_main/main","refresh");
  ...
  public function saveAddDataAwalGudangRoll(){
    ...
    if($isLogin){
      ...
      if(empty($kdGdRoll)||empty($jnsPermintaan)||empty($tglTransaksi)||
         $berat==""||$bobin==""||$payung==""||$payungKuning==""
       ){
         echo "Data Kosong";
       }else{
         $data = array("kd_gd_roll" => $kdGdRoll,
  ...
  public function deleteAndRestoreListDataAwalTemp(){
    ...
    if($isLogin){
      ...
      if(empty($id) || empty($deleted)){
        ...
        $data = array("id" => $id,
                      "deleted" => $deleted);
        $result = $this->Gudang_Roll_Model->updateDeleteTransaksiDataAwal($data);
        if($result){
          echo "Berhasil";
        }else{
          echo "Gagal";
  ...
  public function printKartuStok($param1, $param2, $param3){
    $isLogin = $this->isLogin();
    if($isLogin){
      $tglAwal = $param1;
      $tglAkhir = $param2;
      ...
      $data = array("tglAwal" => $tglAwal,
                    "tglAkhir" => $tglAkhir,
                    "kdGdRoll" => $kdGdRoll);
      $dataKartuStok = json_decode($this->Gudang_Roll_Model->selectCetakKartuStok($data), TRUE);
      $dataDetailBarang = json_decode($this->Gudang_Roll_Model->selectGudangRollDetail($kdGdRoll), TRUE);
      $result = array("dataDetailBarang" => $dataDetailBarang,
                      "dataKartuStok" => $dataKartuStok,
                      "namaFile" => $namaFile);
      $this->load->view("frm_print_kartu_stok", $result);
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
      redirect("_main/main","refresh");
    }
  ...
  public function exportKartuStokToSpreadsheet($param1, $param2, $param3){
    $isLogin = $this->isLogin();
    if($isLogin){
      $tglAwal = $param1;
      $tglAkhir = $param2;
      ...
      $dataKartuStok = json_decode($this->Gudang_Roll_Model->selectCetakKartuStok($data), TRUE);
      $dataDetailBarang = json_decode($this->Gudang_Roll_Model->selectGudangRollDetail($kdGdRoll), TRUE);
      $namaFile = "Kartu_Stok_".
                  $dataDetailBarang[0]["ukuran"]."_".
                  $dataDetailBarang[0]["merek"]."_".
                  $dataDetailBarang[0]["warna_plastik"]."_".
  ...
  public function exportKartuStokToPdf($param1, $param2, $param3){
    $isLogin = $this->isLogin();
    if($isLogin){
      $tglAwal = $param1;
      $tglAkhir = $param2;
      ...
      $page = $this->load->view("frm_print_kartu_stok",$result,true);
      // $this->load->view("frm_print_faktur_produksi",$result);
      $this->load->library('m_pdf');
      $this->mpdf->mPDF("utf-8","A4-L",0,"",5,5,5,5,0,0);#L,R,T,B
  ...
  public function perbaruiStokMaster(){
    ...
    if($isLogin){
      ...
      foreach ($arrValue as $arrTemp) {
        ...
        array_push($arrData, array("kd_gd_roll" => $temp[0],
                                   "stok" => floatval($temp[1]),
                                   "bobin" => floatval($temp[2]),
                                   "payung" => floatval($temp[3]),
                                   "payung_kuning" => floatval($temp[4])));
      }
      $result = $this->Gudang_Roll_Model->updateStokMasterBatch($arrData);
      if($result){
        echo "Berhasil";
      }else{
        echo "Gagal";
  ...
  public function saveDataAwal(){
    $isLogin = $this->isLogin();
    if($isLogin){
      $jnsPermintaan = $this->input->post("jenis");
      $result = $this->Gudang_Roll_Model->updateTransaksiDataAwal($jnsPermintaan);
      if($result){
        echo "Berhasil";
      }else{
        echo "Gagal";
  ...
  public function editDataAwalGudangRoll(){
    $isLogin = $this->isLogin();
    if($isLogin){
      $id = $this->input->post("id");
      $kdGdRoll = $this->input->post("kdGdRoll");
      ...
      $data = array("id"            => $id,
                    ...
                    "payung"        => $payung,
                    "payung_kuning" => $payungKuning);
      $result = $this->Gudang_Roll_Model->updateDataAwalGudangRoll($data);
      if($result){
        echo "Berhasil";
      }else{
        echo "Gagal";
  ...
  public function updateCheckList(){
    ...
    if($isLogin){
      ...
      $idCeklisGr = $this->input->post("idCeklisGr");
      $idUser = $this->session->userdata("fabricationIdUser");
      $periode = $this->input->post("periode");
      $data = array("kd_gd_roll" => $kdGdRoll,
                    "ceklis" => $ceklis,
                    "id_ceklis_gr" => $idCeklisGr,
                    "id_user" => $idUser,
                    "periode" => $periode);
      $result = $this->Gudang_Roll_Model->updateCheckListGudangRoll($data);
      if($result){
        echo "Berhasil";
      }else{
        echo "Gagal";
  ...
  public function getCekHasilExtruder(){
    $isLogin = $this->isLogin();
		if($isLogin){
      $tglAwal = $this->input->post("tglAwal");
      $result = $this->Gudang_Roll_Model->selectCekHasilExtruder($tglAwal);
      echo $result;
		}else{
			echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
			redirect("_main/main","refresh");
