...
class Main extends MX_Controller{
  public function __construct(){
		parent::__construct();
    $this->load->model("Gudang_Kanvas_Model");
    $this->load->model("Barang_Pack_Model");
	}
  ...
  private function isLogin(){
    if(!empty($this->session->userdata("fabricationIdUser"))&&
       ...
       ($this->session->userdata("fabricationGroup")=="gudang kanvas"||
    ...
    if($isLogin){
      ...
      if($status==1){
        $listMenu["parentMenu"] = array(
          ...
          array("Content"=>"Cek Kartu Stok","Link"=>"_gudangkanvas/main/cek_kartu_stok","Name"=>"Cek_Kartu_Stok","Status"=>"Single","Icon"=>"fa fa-book","Id"=>"M_Cek_Kartu_Stok")
        );

        $listMenu["childMenu"] = array(
          array("Content"=>"Data Order Terkirim","Link"=>"_gudangkanvas/main/data_order_terkirim","Parent"=>"Data_Order_Marketing","Id"=>""),
          array("Content"=>"Data Sisa Order Belum Terkirim","Link"=>"_gudangkanvas/main/data_sisa_order_belum_terkirim","Parent"=>"Data_Order_Marketing","Id"=>""),
          array("Content"=>"Pengiriman Baru","Link"=>"_gudangkanvas/main/pengiriman_baru","Parent"=>"Data_Order_Marketing","Id"=>""),

          array("Content"=>"Stok Barang Kanvas","Link"=>"_gudangkanvas/main/stok_barang_kanvas","Parent"=>"Stok_Barang","Id"=>""),
          array("Content"=>"Stok Barang Pack","Link"=>"_gudangkanvas/main/stok_barang_pack","Parent"=>"Stok_Barang","Id"=>""),
          array("Content"=>"Stok Barang Online","Link"=>"_gudangkanvas/main/stok_barang_online","Parent"=>"Stok_Barang","Id"=>""),
  ...
  public function stok_barang_kanvas(){
    $isLogin = $this->isLogin();
    if($isLogin){
      $data["Data"] = array("Title"=>"Stok Barang Kanvas");
      $data["Link"] = array("Segment1" =>$this->uri->rsegment(1),"Segment2"=>$this->uri->rsegment(2));
      $this->load->view("header");
      $this->load->view("sidebar",$this->checkStatus());
      $this->load->view("frm_stok_barang_kanvas",$data);
      $this->load->view("footer");
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
      redirect("_main/main","refresh");
  ...
  public function stok_barang_pack(){
    $isLogin = $this->isLogin();
    if($isLogin){
      $data["Data"] = array("Title"=>"Stok Barang Pack");
      $data["Link"] = array("Segment1" =>$this->uri->rsegment(1),"Segment2"=>$this->uri->rsegment(2));
      $this->load->view("header");
      $this->load->view("sidebar",$this->checkStatus());
      $this->load->view("frm_stok_barang_pack",$data);
      $this->load->view("footer");
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
      redirect("_main/main","refresh");
  ...
  public function stok_barang_online(){
    $isLogin = $this->isLogin();
    if($isLogin){
      $data["Data"] = array("Title"=>"Stok Barang Online");
      $data["Link"] = array("Segment1" =>$this->uri->rsegment(1),"Segment2"=>$this->uri->rsegment(2));
      $this->load->view("header");
      $this->load->view("sidebar",$this->checkStatus());
      $this->load->view("frm_stok_barang_online",$data);
      $this->load->view("footer");
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
      redirect("_main/main","refresh");
  ...
  public function cek_kartu_stok(){
    $isLogin = $this->isLogin();
    if($isLogin){
      if($this->session->userdata("fabricationGroup") == "administrator" ||
         $this->session->userdata("fabricationGroup") == "it_department"){
  ...
  public function getGeneratedGudangHasilCode(){
    ...
    if($isLogin){
      ...
      $result['Code'] = $this->Gudang_Kanvas_Model->generateGudangHasilCode($data);
  ...
  public function getCountTrashGudangHasil(){
    $isLogin = $this->isLogin();
    if($isLogin){
      $result = $this->Gudang_Kanvas_Model->selectCountTrashBarangGudangHasil();
      $result_1 = $this->Gudang_Kanvas_Model->selectCountTrashTransaksiGudangHasil();
  ...
  public function getComboBoxValueHasil($param,$param2=""){
    ...
    if($isLogin){
      ...
      if(empty($keyword)){
        ...
        $result = $this->Gudang_Kanvas_Model->selectComboBoxValueHasil($data);
      }else{
        $data = array("jenis"=>$param,
                      "jns_permintaan"=>$param2,
                      "key"=>$keyword);
        $result = $this->Gudang_Kanvas_Model->selectComboBoxValueHasilSearch($data);
      }
      echo $result;
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
  ...
  public function getComboBoxValueAneka(){
    $isLogin = $this->isLogin();
    if($isLogin){
      $keyword = $this->input->get("q");
      if(empty($keyword)){
        $result = $this->Gudang_Kanvas_Model->selectComboBoxValueAneka();
      }else{
        $result = $this->Gudang_Kanvas_Model->selectComboBoxValueAnekaSearch($keyword);
      }
      echo $result;
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
  ...
  //   $isLogin = $this->isLogin();
  //   if($isLogin){
  //     $result = $this->Gudang_Kanvas_Model->selectComboBoxValueGudangHasilHd();
  //     echo $result;
  //   }else{
  //     echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
  //     redirect("_main/main","refresh");
  ...
  public function getListPengirimanBaruDatatable(){
    ...
    if($isLogin){
      ...
      $result = $this->Gudang_Kanvas_Model->selectListPengirimanBaruDatatable($key);
      echo $result;
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
      redirect("_main/main","refresh");
  ...
  public function getListPesananTerkirim(){
    ...
    if($isLogin){
      $key = $this->input->post("key");
      $result = $this->Gudang_Kanvas_Model->selectListPesananTerkirim($key);
      echo $result;
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
      redirect("_main/main","refresh");
  ...
  public function getListPesananBelumTerkirim(){
    $isLogin = $this->isLogin();
    if($isLogin){
      $result = $this->Gudang_Kanvas_Model->selectListPesananBelumTerkirim();
      echo $result;
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
      redirect("_main/main","refresh");
  ...
  public function getNotePesanan(){
    ...
    if($isLogin){
      ...
      $data = array("noOrder" => $noOrder,
                    "idDP" => $idDp);
      $result = $this->Gudang_Kanvas_Model->selectPesananData($data);
      echo json_encode($result);
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
      redirect("_main/main","refresh");
  ...
  public function getPesananDetailData(){
    ...
    if($isLogin){
      ...
      $result = $this->Gudang_Kanvas_Model->selectPesananDetailData($id);
      echo $result;
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
      redirect("_main/main","refresh");
  ...
  public function addPengirimanBaru(){
    $isLogin = $this->isLogin();
    if($isLogin){
      $kdGdHasil = (empty($this->input->post("kdGdHasil"))) ? NULL : $this->input->post("kdGdHasil");
      $kdGdBahan = (empty($this->input->post("kdGdBahan"))) ? NULL : $this->input->post("kdGdBahan");
      $kdPack = (empty($this->input->post("kdPack"))) ? 'NULL' : $this->input->post("kdPack");
      ...
      if(empty($idUser)||empty($idDp)||
         ...
         $jumlahLembar==""||empty($warna)||empty($statusPengiriman)||empty($jenisJumlah) || $jumlahDikirim==""
           ...
           if(empty($kdGdHasil)&&empty($kdGdBahan) && empty($kdPack)){
             echo "Data Kosong";
           }else{
            //  $data = array("kdGdHasil"        => $kdGdHasil,
            //                "kdGdBahan"        => $kdGdBahan,
            //                "kdPack"           => $kdPack,
            //                "idDp"             => $idDp,
            //                "idUser"           => $idUser,
            ...
            if(empty($kdGdHasil) && empty($kdGdBahan)){
              $data = array("kd_pack"           => $kdPack,
                           ...
                           "jenis_jumlah"      => $jenisJumlah,
                           "keterangan"       => $keterangan,
                           "jumlah_terkirim"    => $jumlahDikirim);
            } else {
            ...
            $result = $this->Gudang_Kanvas_Model->insertAddPengirimanBaru($data);
            if($result){
              echo "Berhasil";
            }else{
              echo "Gagal";
  ...
  public function getListPengirimanBaruTemp(){
    $isLogin = $this->isLogin();
    if($isLogin){
      $result = $this->Gudang_Kanvas_Model->selectListPengirimanBaruTemp();
      echo $result;
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
      redirect("_main/main","refresh");
  ...
  public function deletePengirimanBaruTemp(){
    ...
    if($isLogin){
      ...
      $result = $this->Gudang_Kanvas_Model->updateDetailPengiriman(array("id_detail_pengiriman"=>$idTransaksi,"deleted"=>"TRUE"));
      if($result){
        echo "Berhasil";
      }else{
        echo "Gagal";
  ...
  public function savePengirimanBaru(){
    $isLogin = $this->isLogin();
    if($isLogin){
      $result = json_decode($this->Gudang_Kanvas_Model->selectListPengirimanBaruTemp(),TRUE);
      ...
      if(empty($result)){
        ...
        for ($i=0; $i < count($result); $i++) {
          if(!empty($result[$i]["kd_gd_bahan"])){
            ...
            $result2 = $this->Gudang_Kanvas_Model->insertTransaksiGudangBahan_Pengiriman($data);
            if($result2){
              $jumlahTransaksiSuccess++;
            }
          }elseif(!empty($result[$i]["kd_gd_hasil"])){
            $data = array("no_order"              => $result[$i]["no_order"],
                          ...
                          "bagian"                => "GUDANG KANVAS",
                          "keterangan_history"    => $result[$i]["keterangan"],
                          "keterangan_barang"     => "PENGIRIMAN",
                          "keterangan"            => "Barang Telah Dikirim Ke Bagian Pengiriman Pada : ".date("d-m-Y H:i:s"),
                          "jumlah_terkirim"       => $result[$i]["jumlah_terkirim"]);
            $result2 = $this->Gudang_Kanvas_Model->insertTransaksiGudangHasil_Pengiriman($data);
            if($result2){
              $jumlahTransaksiSuccess++;
            }
          } elseif (!empty($result[$i]["kd_pack"])) {
            $data = array("no_order"              => $result[$i]["no_order"],
                          "kd_pack"           => $result[$i]["kd_pack"],
                          "id_user"               => $result[$i]["id_user"],
                          "id_dp"                 => $result[$i]["id_dp"],
                          "id_detail_pengiriman"  => $result[$i]["id_detail_pengiriman"],
                          "jumlah_berat"          => $result[$i]["jumlah_kg"],
                          ...
                          "keterangan_barang"     => "PENGIRIMAN",
                          "keterangan"            => "Barang Telah Dikirim Ke Bagian Pengiriman Pada : ".date("d-m-Y H:i:s"),
                          "jumlah_kirim"       => $result[$i]["jumlah_terkirim"]);

            $result2 = $this->Gudang_Kanvas_Model->insertTransaksiBarangPack_Pengiriman($data);
            if($result2){
              $jumlahTransaksiSuccess++;
            }
          }
          $data2 = array("id_detail_pengiriman" => $result[$i]["id_detail_pengiriman"],
                         "status"               => "FINISH");
          $result3 = $this->Gudang_Kanvas_Model->updateDetailPengiriman($data2);
          if($result3){
  ...
  public function getListDetailPesananTerkirim(){
    ...
    if($isLogin){
      $idDp = $this->input->post("idDp");
      $result = $this->Gudang_Kanvas_Model->selectListDetailPengiriman($idDp);
      echo $result;
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
      redirect("_main/main","refresh");
  ...
  public function getListGudangHasil(){
    ...
    if($isLogin){
      ...
      $result = $this->Gudang_Kanvas_Model->selectListGudangHasilDatatable($data);
      echo $result;
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
      redirect("_main/main","refresh");
  ...
  public function getListPack(){
    echo $this->Barang_Pack_Model->getDatatable();
  }

  public function getTrashListGudangHasil(){
    $isLogin = $this->isLogin();
    if($isLogin){
      $result = $this->Gudang_Kanvas_Model->selectTrashListGudangHasilDatatable();
      echo $result;
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
      redirect("_main/main","refresh");
  ...
  public function getTrashListTransaksiGudangHasil(){
    $isLogin = $this->isLogin();
    if($isLogin){
      $result = $this->Gudang_Kanvas_Model->selectTrashListTransaksiGudangHasil();
      echo $result;
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
      redirect("_main/main","refresh");
    ...
    if($isLogin){
      ...
      if(empty($kdGdHasil)||empty($idUser)||empty($tglBuat)||empty($warnaPlastik)||$tebal==""||empty($ukuran)||
         ...
         $data = array("kd_gd_hasil"    => $kdGdHasil,
                       ...
                       "sts_brg"        => $stsBarang,
                       "keterangan"     => $keterangan);
        $result = $this->Gudang_Kanvas_Model->insertGudangHasil($data);
        if($result){
          echo "Berhasil";
        }else{
          echo "Gagal";
  ...
  public function deleteBarangGudangHasil(){
    ...
    if($isLogin){
      ...
      $data = array("kd_gd_hasil" => $kdGdHasil,
                    "deleted" => "TRUE");
      $result = $this->Gudang_Kanvas_Model->updateGudangHasil($data);
      if($result){
        echo "Berhasil";
      }else{
        echo "Gagal";
    ...
    if($isLogin){
      ...
      if(empty($tglAwal)||empty($tglAkhir)||empty($kdGdHasil)){
        ...
        $data = array("tglAwal"   => $tglAwal,
                      "tglAkhir"  => $tglAkhir,
                      "kdGdHasil" => $kdGdHasil);
        $result = $this->Gudang_Kanvas_Model->selectListHistoryGudangHasilMasukDatatable($data);
        echo $result;
      }
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
    ...
    if($isLogin){
      ...
      if(empty($tglAwal)||empty($tglAkhir)||empty($kdGdHasil)){
        ...
        $data = array("tglAwal"   => $tglAwal,
                      "tglAkhir"  => $tglAkhir,
                      "kdGdHasil" => $kdGdHasil);
        $result = $this->Gudang_Kanvas_Model->selectListHistoryGudangHasilBufferMasukDatatable($data);
        echo $result;
      }
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
  ...
  public function getListHistoryAnekaMacamMasukDatatable(){
    $isLogin = $this->isLogin();
    if($isLogin){
      $tglAwal = $this->input->post("tglAwal");
      $tglAkhir = $this->input->post("tglAkhir");
      $kdPack = $this->input->post("kdPack");

      if(empty($tglAwal)||empty($tglAkhir)||empty($kdPack)){
        echo "Data Kosong";
      }else{
        $data = array("tglAwal"   => $tglAwal,
                      "tglAkhir"  => $tglAkhir,
                      "kdPack" => $kdPack);
        $result = $this->Gudang_Kanvas_Model->selectListHistoryPackMasukDatatable($data);
        echo $result;
      }
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
    ...
    if($isLogin){
      ...
      if(empty($tglAwal)||empty($tglAkhir)||empty($kdGdHasil)){
        ...
        $data = array("tglAwal"   => $tglAwal,
                      "tglAkhir"  => $tglAkhir,
                      "kdGdHasil" => $kdGdHasil);
        $result = $this->Gudang_Kanvas_Model->selectListHistoryGudangHasilKeluarDatatable($data);
        echo $result;
      }
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
    ...
    if($isLogin){
      ...
      if(empty($tglAwal)||empty($tglAkhir)||empty($kdGdHasil)){
        ...
        $data = array("tglAwal"   => $tglAwal,
                      "tglAkhir"  => $tglAkhir,
                      "kdGdHasil" => $kdGdHasil);
        $result = $this->Gudang_Kanvas_Model->selectListHistoryGudangBufferKeluarDatatable($data);
        echo $result;
      }
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
  ...
  public function getListHistoryAnekaMacamKeluarDatatable(){
    $isLogin = $this->isLogin();
    if($isLogin){
      $tglAwal = $this->input->post("tglAwal");
      $tglAkhir = $this->input->post("tglAkhir");
      ...
      if(empty($tglAwal)||empty($tglAkhir)||empty($kdPack)){
        ...
        $data = array("tglAwal"   => $tglAwal,
                      "tglAkhir"  => $tglAkhir,
                      "kdPack" => $kdPack);
        $result = $this->Gudang_Kanvas_Model->selectListHistoryAnekaMacamKeluarDatatable($data);
        echo $result;
      }
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
    ...
    if($isLogin){
      ...
      if(empty($tglAwal)||empty($tglAkhir)||empty($kdGdHasil)){
        ...
        $data = array("tglAwal"   => $tglAwal,
                      "tglAkhir"  => $tglAkhir,
                      "kdGdHasil" => $kdGdHasil);
        $result = $this->Gudang_Kanvas_Model->selectSaldoAwalBulanGudangHasil($data);
        echo $result;
      }
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
    ...
    if($isLogin){
      ...
      if(empty($tglAwal)||empty($tglAkhir)||empty($kdGdHasil)){
        ...
        $data = array("tglAwal"=>$tglAwal,
                      "tglAkhir"=>$tglAkhir,
                      "kdGdHasil"=>$kdGdHasil);
        $result = $this->Gudang_Kanvas_Model->selectSaldoAwalBulanGudangHasilBuffer($data);
        echo $result;
      }
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
  ...
  public function getSaldoAwalAnekaMacam(){
    $isLogin = $this->isLogin();
    if($isLogin){
      $tglAwal = $this->input->post("tglAwal");
      $tglAkhir = $this->input->post("tglAkhir");
      ...
      if(empty($tglAwal)||empty($tglAkhir)||empty($kdPack)){
        ...
        $data = array("tglAwal"=>$tglAwal,
                      "tglAkhir"=>$tglAkhir,
                      "kdPack"=>$kdPack);
        $result = $this->Gudang_Kanvas_Model->selectSaldoAwalBulanAnekaMacam($data);
        echo $result;
      }
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
  ...
  public function getDetailBarangGudangHasil(){
    ...
    if($isLogin){
      $kdGdHasil = $this->input->post("kdGdHasil");
      $result = $this->Gudang_Kanvas_Model->selectDetailBarangGudangHasil($kdGdHasil);
      echo $result;
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
      redirect("_main/main","refresh");
  ...
  public function getDetailTransaksiGudangHasil(){
    ...
    if($isLogin){
      ...
      $result = $this->Gudang_Kanvas_Model->selectDetailTransaksiGudangHasil($idTransaksi);
      echo $result;
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
      redirect("_main/main","refresh");
  ...
  public function getDetailTransaksiPack(){
    $isLogin = $this->isLogin();
    if($isLogin){
      $idTransaksi = $this->input->post("idTransaksi");
      $result = $this->Gudang_Kanvas_Model->selectDetailTransaksiPack($idTransaksi);
      echo $result;
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
      redirect("_main/main","refresh");
    ...
    if($isLogin){
      ...
      if(empty($idTransaksi)||$berat==""||$lembar==""||
        ...
        if(empty($kdGdHasil2)){
          ...
          $result = $this->Gudang_Kanvas_Model->updateTransaksiGudangHasilAndDetailPengiriman($data);
          if($result){
            $data_1 = array("tglAwal"   => $tglAwal,
                            "tglAkhir"  => $tglAkhir,
                            "kdGdHasil" => $kdGdHasil1);
            $arrSaldo = json_decode($this->Gudang_Kanvas_Model->selectSaldoAwalBulanGudangHasilBuffer($data_1),TRUE);
            ...
            $result_1 = $this->Gudang_Kanvas_Model->updateGudangHasil($dataForUpdateMaster);
            if($result_1){
              echo "Berhasil";
            }else{
          ...
          if($jumlahPengiriman == ""){
            $data = array("id_permintaan_jadi"    => $idTransaksi,
                          "kd_gd_hasil_secondary" => $kdGdHasil2,
                          "ukuran"                => $ukuran,
                          "warna"                 => $warnaPlastik,
                          "merek"                 => $merek,
                          "jns_permintaan"        => $jnsPermintaan,
          ...
          if($result){
            ...
            $arrSaldo_1 = json_decode($this->Gudang_Kanvas_Model->selectSaldoAwalBulanGudangHasilBuffer($data_1),TRUE); #Data Hasil History Barang Pertama
            $arrSaldo_2 = json_decode($this->Gudang_Kanvas_Model->selectSaldoAwalBulanGudangHasilBuffer($data_2),TRUE); #Data Hasil History Barang Kedua
            ...
            $result_1 = $this->Gudang_Kanvas_Model->updateGudangHasil($dataForUpdateMaster_1); #Update Data Master Barang Pertama
            $result_2 = $this->Gudang_Kanvas_Model->updateGudangHasil($dataForUpdateMaster_2); #Update Data Master Barang Kedua
  ...
  public function editHistoryPack(){
    $isLogin = $this->isLogin();
    if($isLogin){
      $idTransaksi = $this->input->post("idTransaksi");
      $kdPack = $this->input->post("kdPack"); #Kode Hasil Jika Tidak Ingin Memindahakan Barang
      $kdPackBaru = $this->input->post("kdPackBaru"); #Kode Hasil Jika Ingin Memindahkan Barang
      $tanggal = $this->input->post("tanggal");
      $berat = $this->input->post("berat");
      ...
      if(empty($idTransaksi)||$berat==""||$pack==""||
         empty($tglAwal)||empty($tglAkhir)
       ){
           echo "Data Kosong";
      }else{
        if(empty($kdPackBaru)){
          if($jumlahPengiriman==""){
            $data = array("id_transaksi"  => $idTransaksi,
                          "jumlah_berat"        => $berat,
                          "jumlah_kirim"       => $pack,
                          "tgl_transaksi"       => $tanggal);
          }else{
          ...
          $result = $this->Gudang_Kanvas_Model->updateTransaksiPackAndDetailPengiriman($data);
          if($result){
            $data_1 = array("tglAwal"   => $tglAwal,
                            "tglAkhir"  => $tglAkhir,
                            "kdPack" => $kdPack);
            $arrSaldo = json_decode($this->Gudang_Kanvas_Model->selectSaldoAwalBulanAnekaMacam($data_1),TRUE);
            $dataForUpdateMaster = array("jumlah_berat"   => str_replace(",","",$arrSaldo["saldoAkhirBerat"]),
                                         "stock"  => str_replace(",","",$arrSaldo["saldoAkhirKirim"]),
                                         "kd_produk"  => $kdPack);
            $result_1 = $this->Gudang_Kanvas_Model->updatePack($dataForUpdateMaster);
            if($result_1){
              echo "Berhasil";
            }else{
              echo "Update Stok Master Gagal";
          ...
          if($result){
            $data_1 = array("tglAwal"=>$tglAwal,
                            "tglAkhir"=>$tglAkhir,
                            "kdPack"=>$kdPack); #Data Untuk Mencari History Barang Pertama

            $data_2 = array("tglAwal"=>$tglAwal,
                            "tglAkhir"=>$tglAkhir,
                            "kdGdHasil"=>$kdPackBaru); #Data Untuk Mencari History Barang Kedua

            $arrSaldo_1 = json_decode($this->Gudang_Kanvas_Model->selectSaldoAwalBulanAnekaMacam($data_1),TRUE); #Data Hasil History Barang Pertama
            $arrSaldo_2 = json_decode($this->Gudang_Kanvas_Model->selectSaldoAwalBulanAnekaMacam($data_2),TRUE); #Data Hasil History Barang Kedua

            $dataForUpdateMaster_1 = array("jumlah_berat"=>str_replace(",","",$arrSaldo_1["saldoAkhirBerat"]),
                                           "stock"=>str_replace(",","",$arrSaldo_1["saldoAkhirKirim"]),
                                           "kd_produk"=>$kdPack);

            $dataForUpdateMaster_2 = array("jumlah_berat"=>str_replace(",","",$arrSaldo_2["saldoAkhirBerat"]),
                                           "stock"=>str_replace(",","",$arrSaldo_2["saldoAkhirKirim"]),
                                           "kd_produk"=>$kdPackBaru);

            $result_1 = $this->Gudang_Kanvas_Model->updatePack($dataForUpdateMaster_1); #Update Data Master Barang Pertama
            $result_2 = $this->Gudang_Kanvas_Model->updatePack($dataForUpdateMaster_2); #Update Data Master Barang Kedua
  ...
  public function deleteAndRestoreTransaksiGudangHasil(){
    ...
    if($isLogin){
      ...
      $data = array("id_permintaan_jadi" => $idDetailTransaksi,
                    "deleted"=>$deleted);
      $result = $this->Gudang_Kanvas_Model->updateTransaksiGudangHasil($data);
      if($result){
        $data_1 = array("tglAwal" => $tglAwal,
                        "tglAkhir" => $tglAkhir,
                        "kdGdHasil" => $kdGdHasil);
        $arrSaldo = json_decode($this->Gudang_Kanvas_Model->selectSaldoAwalBulanGudangHasil($data_1),TRUE);
  ...
  public function deleteAndRestoreTransaksiPack(){
    $isLogin = $this->isLogin();
    if($isLogin){
      $idDetailTransaksi = $this->input->post("idTransaksi");
      $tglAwal = $this->input->post("tglAwal");
      $tglAkhir = $this->input->post("tglAkhir");
      $kd_pack = $this->input->post("kd_pack");
      $deleted = $this->input->post("deleted");

      $data = array("id_transaksi" => $idDetailTransaksi,
                    "deleted"=>$deleted);
      $result = $this->Gudang_Kanvas_Model->updateTransaksiPack($data);
      if($result){
        $data_1 = array("tglAwal" => $tglAwal,
                        "tglAkhir" => $tglAkhir,
                        "kdPack" => $kdGdHasil);
  ...
  public function addPengambilanSablon(){
    ...
    if($isLogin){
      ...
      if(empty($tanggal) || empty($idUser) || $jumlahLembar == "" || $jumlahBerat ==""){
        ...
        $result = $this->Gudang_Kanvas_Model->insertTransaksiPengambilanSablon($data);
        echo $result;
      }
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
  ...
  public function getListPengambilanSablonTemp(){
    $isLogin = $this->isLogin();
    if($isLogin){
      $result = $this->Gudang_Kanvas_Model->selectListPengambilanSablonTemp();
      echo $result;
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
      redirect("_main/main","refresh");
  ...
  public function deleteAndRestorePengambilanSablonTemp(){
    ...
    if($isLogin){
      ...
      $data = array("id_pengambilan_sablon" => $idTransaksi,
                    "deleted" => $deleted);
      $result = $this->Gudang_Kanvas_Model->updateTransaksiPengambilanSablon($data);
      if($result){
        echo "Berhasil";
      }else{
        echo "Gagal";
  ...
  public function savePengambilanSablon(){
    $isLogin = $this->isLogin();
    if($isLogin){
      $arrData = json_decode($this->Gudang_Kanvas_Model->selectListPengambilanSablonTemp(),TRUE);
      ...
      if(empty($arrData)){
        ...
        foreach ($arrData as $data) {
          ...
          $result = $this->Gudang_Kanvas_Model->insertTransaksiGudangHasil_PengambilanSablon($newArrData);
          if($result){
            $jumlahDataSelesai++;
          }
        }
    ...
    if($isLogin){
      ...
      if(empty($idUser) || empty($tanggal) || empty($namaCustomer) || empty($kdGdHasil) ||
        ...
        $data = array("kd_gd_hasil" => $kdGdHasil,
                      ...
                      "keterangan" => $keterangan,
                      "sts_pengiriman" => $statusPengiriman);
        $result = $this->Gudang_Kanvas_Model->insertAddPengirimanGudang($data);
        if($result){
          echo "Berhasil";
        }else{
          echo "Gagal";
  ...
  public function getListPengirimanGudangTemp(){
    ...
    if($isLogin){
      ...
      $result = $this->Gudang_Kanvas_Model->selectListPengirimanGudangTemp($stsPengiriman);
      echo $result;
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
      redirect("_main/main","refresh");
  ...
  public function getDetailPengirimanGudangTemp(){
    ...
    if($isLogin){
      ...
      $result = $this->Gudang_Kanvas_Model->selectDetailPengirimanGudangTemp($idDetailPengiriman);
      echo $result;
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
      redirect("_main/main","refresh");
    ...
    if($isLogin){
      ...
      if(empty($idUser) || empty($tanggal) || empty($namaCustomer) ||
        ...
        $data = array("id_detail_pengiriman" => $idTransaksi,
                      ...
                      "jumlah_lembar" => $jumlahLembar,
                      "keterangan" => $keterangan);
        $result = $this->Gudang_Kanvas_Model->updateDetailPengiriman($data);
        if($result){
          echo "Berhasil";
        }else{
          echo "Gagal";
  ...
  public function savePengirimanGudang(){
    ...
    if($isLogin){
      $stsPengiriman = $this->input->post("stsPengiriman");
      $arrData = json_decode($this->Gudang_Kanvas_Model->selectListPengirimanGudangTemp($stsPengiriman),TRUE);
      ...
      foreach ($arrData as $data) {
        ...
        $result = $this->Gudang_Kanvas_Model->insertTransaksiGudangHasil_PengirimanGudang($newArrData);
        if($result == "Lock"){
          break;
  ...
  public function getComboBoxValuetGudangApal(){
    $isLogin = $this->isLogin();
    if($isLogin){
      $result = $this->Gudang_Kanvas_Model->selectComboBoxValueGudangApal();
      echo $result;
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
      redirect("_main/main","refresh");
  ...
  public function saveAddKirimanApalTemp(){
    ...
    if($isLogin){
      ...
      if(empty($kdGdApal) || empty($idUser) || empty($tanggal) || empty($bagian) ||
        ...
        $result = $this->Gudang_Kanvas_Model->insertTransaksiDetailHistoryApal($data);
        echo $result;
      }
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
  ...
  public function getListKirimanApalTemp(){
    $isLogin = $this->isLogin();
    if($isLogin){
      $result = $this->Gudang_Kanvas_Model->selectListKirimanApalTemp();
      echo $result;
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
      redirect("_main/main","refresh");
    ...
    if($isLogin){
      ...
      if(empty($idTransaksi) || empty($deleted)){
        ...
        $data = array("id" => $idTransaksi,
                      "deleted" => $deleted);
        $result = $this->Gudang_Kanvas_Model->updateTransaksiDetailHistoryApal($data);
        if($result){
          echo "Berhasil";
        }else{
          echo "Gagal";
  ...
  public function saveKirimanApal(){
    $isLogin = $this->isLogin();
    if($isLogin){
      $result = json_decode($this->Gudang_Kanvas_Model->selectListKirimanApalTemp(),TRUE);
      $jumlahData = count($result);
      $jumlahSelesai = 0;
      foreach ($result as $resultValue) {
        $resultStatus = $this->Gudang_Kanvas_Model->updateTransaksiDetailHistoryApal(array("id"=>$resultValue["id"],"sts_transaksi"=>"PROGRESS"));
        if($resultStatus){
          $jumlahSelesai++;
        }else{
          break;
    ...
    if($isLogin){
      ...
      if(empty($kdGdHasil)||empty($idUser)||empty($namaCustomer)||empty($tanggal)||$jumlahBerat==""||$jumlahLembar==""||
         ...
         $data = array("kd_gd_hasil"          => $kdGdHasil,
                       ...
                       "keterangan"           => $keterangan);
          $result = $this->Gudang_Kanvas_Model->insertTransaksiGudangHasil($data);
          echo $result;
       }
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
  ...
  public function getListPengembalianBarang(){
    ...
    if($isLogin){
      $statusBarang = $this->input->post("statusBarang");
      $result = $this->Gudang_Kanvas_Model->selectListPengembalianBarang($statusBarang);
      echo $result;
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
      redirect("_main/main","refresh");
  ...
  public function saveKembalianGudang(){
    ...
    if($isLogin){
      $statusBarang = $this->input->post("statusBarang");
      $result = json_decode($this->Gudang_Kanvas_Model->selectListPengembalianBarang($statusBarang),TRUE);
      $jumlahData = count($result);
      $jumlahTerkirim = 0;
      if($jumlahData > 0){
        for ($i=0; $i < $jumlahData; $i++) {
          $booleanResult = $this->Gudang_Kanvas_Model->updateTransaksiGudangHasil(array("id_permintaan_jadi"=>$result[$i]["id_permintaan_jadi"], "status_transaksi"=>"FINISH"));
          if($booleanResult){
            ...
            $arrSaldo = json_decode($this->Gudang_Kanvas_Model->selectSaldoAwalBulanGudangHasil($data),TRUE);

            $dataForUpdateMaster = array("kd_gd_hasil" => $result[$i]["kd_gd_hasil"],
                                         "stok_berat" => str_replace(",","",$arrSaldo["saldoAkhirBerat"]),
                                         "stok_lembar" => str_replace(",","",$arrSaldo["saldoAkhirLembar"])
                                        );
            $booleanResultMaster = $this->Gudang_Kanvas_Model->updateGudangHasil($dataForUpdateMaster);
  ...
  public function getListPembelianBarangHdTemp(){
    $isLogin = $this->isLogin();
    if($isLogin){
      $result = $this->Gudang_Kanvas_Model->selectListPembelianBarangHd();
      echo $result;
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
      redirect("_main/main","refresh");
  ...
  public function savePembelianHd(){
    $isLogin = $this->isLogin();
    if($isLogin){
      $result = json_decode($this->Gudang_Kanvas_Model->selectListPembelianBarangHd(),TRUE);
      $jumlahData = count($result);
      $jumlahTerkirim = 0;
      if($jumlahData > 0){
        for ($i=0; $i < $jumlahData; $i++) {
  ...
  public function getListHistoryGudangApal(){
    ...
    if($isLogin){
      ...
      $data = array("tglAwal" => $tglAwal,
                    "tglAkhir" => $tglAkhir,
                    "kdGdApal" => $kdGdApal);
      $result = $this->Gudang_Kanvas_Model->selectListHistoryGudangApal($data);
      echo $result;
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
      redirect("_main/main","refresh");
  ...
  public function tambahStockPack(){
    if($this->isLogin()){
      $data = array(
        'kd_pack'=>$this->input->post('kdPack'),
        'id_user'=>$this->session->userdata("fabricationIdUser"),
        'jumlah_kirim'=>$this->input->post('jumlahPack'),
        'jumlah_berat'=>$this->input->post('jumlahBerat'),
        'jumlah_lembar'=>$this->input->post('jumlahLembar'),
        'bagian'=>$this->session->userdata("fabricationGroup"),
        'tgl_transaksi'=>date('Y-m-d'),
        'jns_permintaan'=>'POLOS',
        'sts_barang'=>'ANEKA MACAM',
        'status_history'=>'MASUK',
        'status_transaksi'=>'FINISH',
        'keterangan_history'=>'Penambahan Stock Pack',
        'keterangan_barang'=>'UNTUK ANEKA MACAM',
        'keterangan'=> "Penambahan Stock Pada : ".date("d-m-Y H:i:s")
      );
      if($this->Gudang_Kanvas_Model->updateStockAneka($data)){
  ...
  public function saveAddJadiAnekaMacam(){
    $isLogin = $this->isLogin();
    if($isLogin){
      $aneka = $this->input->post("aneka");
      $kdGdHasil = $this->input->post("kdGdHasil");#
      $idUser = $this->session->userdata("fabricationIdUser");#
      $ukuran = $this->input->post("ukuran");
      $jumlahBerat = $this->input->post("jumlahBerat");#
      ...
      if($aneka == "TRUE"){
        $keteranganHistory = "UNTUK ANEKA MACAM";
        $keteranganBarang = "UNTUK ANEKA MACAM";
      } else {
      ...
      if(empty($kdGdHasil)||empty($idUser)||$jumlahBerat==""||$jumlahLembar==""||empty($bagian)||
        ...
        $data = array("kd_gd_hasil" => $kdGdHasil,
        "id_user" => $idUser,
        "id_transaksi_aneka"=>$insertAneka['insert_id'],
        "ukuran" => $ukuran,
        "jumlah_berat" => $jumlahBerat,
        "jumlah_lembar" => $jumlahLembar,
        "warna" => $warna,
  ...
  public function saveAddKoreksiGudangBuffer(){
    ...
    if($isLogin){
      ...
      if(empty($kdGdHasil)||empty($idUser)||$jumlahBerat==""||$jumlahLembar==""||empty($bagian)||
         ...
         $data = array("kd_gd_hasil" => $kdGdHasil,
                       "kd_gd_hasil_secondary" => $kdGdHasil,
                       "id_user" => $idUser,
                       "ukuran" => $ukuran,
                       "jumlah_berat" => $jumlahBerat,
                       "jumlah_lembar" => $jumlahLembar,
  ...
  public function saveAddKoreksiPack(){
    $isLogin = $this->isLogin();
    if($isLogin){
      $kdPack = $this->input->post("kdPack");#
      $idUser = $this->session->userdata("fabricationIdUser");#
      $jumlahBerat = $this->input->post("jumlahBerat");#
      $jumlahPack = $this->input->post("jumlahPack");#
      $customer = strtoupper($this->session->userdata("fabricationGroup"));
      $bagian = $this->session->userdata("fabricationGroup");#
      $tglTransaksi = $this->input->post("tglTransaksi");#
      $stsBarang = $this->input->post("stsBarang");#
      $stsApprove = "TRUE";#
      $jenisKoreksi = $this->input->post("jenisKoreksi");#
      $jnsPermintaan = $this->input->post('jns_permintaan');
      $statusHistori = ($jenisKoreksi == "PLUS" ? "MASUK" : "KELUAR");#
      if($aneka == "TRUE"){
        $keteranganHistory = "UNTUK ANEKA MACAM";
        $keteranganBarang = "UNTUK ANEKA MACAM";
      ...
      $keterangan = strtoupper($this->input->post("keterangan"));
      if(empty($kdPack)||empty($idUser)||$jumlahBerat==""||$jumlahPack==""||empty($bagian)||
         empty($tglTransaksi)||empty($jnsPermintaan)||empty($stsBarang)||empty($jenisKoreksi)||empty($statusHistori)
       ){
         echo "Data Kosong";
       }else{
         $data = array("kd_pack" => $kdPack,
                       ...
                       "keterangan" => $keterangan);
        $result = $this->Gudang_Kanvas_Model->insertTransaksiAnekaMacam($data);
        echo $result;
       }
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
  ...
  public function getListKoreksiGudangBufferTemp(){
    $isLogin = $this->isLogin();
    if($isLogin){
      $result = $this->Gudang_Kanvas_Model->selectListKoreksiGudangBufferTemp();
      echo $result;
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
      redirect("_main/main","refresh");
  ...
  public function getListKoreksiPackTemp(){
    $isLogin = $this->isLogin();
    if($isLogin){
      $result = $this->Gudang_Kanvas_Model->selectListKoreksiPackTemp();
      echo $result;
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
      redirect("_main/main","refresh");
  ...
  public function editKoreksiPackTemp(){
    $isLogin = $this->isLogin();
    if($isLogin){
      $idTransaksi = $this->input->post("idTransaksi");
      $kdPack = $this->input->post("kdPack");#
      ...
      if(empty($idUser)||$jumlahBerat==""||$jumlahPack==""||empty($bagian)||
         empty($tglTransaksi)||empty($stsBarang)||empty($jenisKoreksi)||empty($statusHistori)
       ){
         echo "Data Kosong";
       }else{
  ...
  public function saveKoreksiGudangBuffer(){
    $isLogin = $this->isLogin();
    if($isLogin){
      $result = json_decode($this->Gudang_Kanvas_Model->selectListKoreksiGudangBufferTemp(),TRUE);
      $jumlahData = count($result);
      $jumlahTerkirim = 0;
      if($jumlahData > 0){
        for ($i=0; $i < count($result); $i++) {
          $booleanResult = $this->Gudang_Kanvas_Model->updateTransaksiGudangHasil(array("id_permintaan_jadi"=>$result[$i]["id_permintaan_jadi"], "status_transaksi"=>"FINISH"));
          $transaksiAneka = $this->Gudang_Kanvas_Model->updateTransaksiAnekaJadi(array("id_transaksi"=>$result[$i]["id_transaksi_aneka"], "status_transaksi"=>"FINISH"));
          ...
          if($booleanResult){
            ...
            $arrSaldo = json_decode($this->Gudang_Kanvas_Model->selectSaldoAwalBulanGudangHasilBuffer($data),TRUE);
  ...
  public function saveKoreksiPack(){
    $isLogin = $this->isLogin();
    if($isLogin){
      $result = json_decode($this->Gudang_Kanvas_Model->selectListKoreksiPackTemp(),TRUE);
      $jumlahData = count($result);
      $jumlahTerkirim = 0;
      if($jumlahData > 0){
        for ($i=0; $i < count($result); $i++) {
          $booleanResult = $this->Gudang_Kanvas_Model->updateTransaksiAnekaJadi(array("id_transaksi"=>$result[$i]["id_transaksi"], "status_transaksi"=>"FINISH"));

          if($booleanResult){
            $data = array("kdPack" => $result[$i]["kd_pack"],
                          "tglAwal" => date("Y-m-t",strtotime("-1 months",strtotime(date("Y-m-d")))),
                          "tglAkhir" => date("Y-m-t",strtotime(date("Y-m-d")))
                         );
            $arrSaldo = json_decode($this->Gudang_Kanvas_Model->selectSaldoAwalBulanAnekaMacam($data),TRUE);

            $dataForUpdateMaster = array("kd_produk" => $result[$i]["kd_gd_hasil"],
                                         "jumlah_berat" => str_replace(",","",$arrSaldo["saldoAkhirBerat"]),
                                         "stock" => str_replace(",","",$arrSaldo["saldoAkhirKirim"])
                                        );
            $booleanResultMaster = $this->Gudang_Kanvas_Model->updatePack($dataForUpdateMaster);
            if($booleanResultMaster){
              $jumlahTerkirim++;
            }else{
              break;
  ...
  public function getListComboBoxCustom(){
    ...
    if($isLogin){
      ...
      if(empty($ukuran)||empty($warna)||empty($jnsBarang)){
        echo "Data Kosong";
      }else{
        $data = array("ukuran"          => $ukuran,
                      ...
                      "jns_brg"         => $jnsBarang,
                      "key"             => $key);
        $result = $this->Gudang_Kanvas_Model->selectGudangHasilCustom($data);
        echo $result;
      }
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
  ...
  public function getListPengembalianBarangGudangBufferKeStandardTemp(){
    $isLogin = $this->isLogin();
    if($isLogin){
      $jnsGudang = $this->input->post("jnsGudang");
      $result = $this->Gudang_Kanvas_Model->selectListPengembalianBarangGudangBufferKeStandardTemp($jnsGudang);
      echo $result;
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
      redirect("_main/main","refresh");
  ...
  public function savePengembalianKeGudangStandard(){
    $isLogin = $this->isLogin();
    if($isLogin){
      $jnsGudang = $this->input->post("stsBarang");
      $arrData = json_decode($this->Gudang_Kanvas_Model->selectListPengembalianBarangGudangBufferKeStandardTemp($jnsGudang),TRUE);
      $jumlahData = count($arrData);
      $jumlahSelesai = 0;
      $stsBarang = $this->input->post("stsBarang");
      for ($i=0; $i < $jumlahData; $i++) {
        $booleanResult = $this->Gudang_Kanvas_Model->updateTransaksiGudangHasil(array("id_permintaan_jadi"=>$arrData[$i]["id_permintaan_jadi"],"status_transaksi"=>"FINISH"));
        if($booleanResult){
          $dataForInsertTransaksiGudangHasilStandard = array("kd_gd_hasil"          => $arrData[$i]["kd_gd_hasil_secondary"],
                                                             ...
                                                             "keterangan"           => "PENGEMBALIAN DARI GUDANG KANVAS");

            $result = $this->Gudang_Kanvas_Model->insertTransaksiGudangHasil($dataForInsertTransaksiGudangHasilStandard);
            if($result == "Berhasil"){
              #Proses Update Data Master
              $dataGudangBuffer = array("kdGdHasil" => $arrData[$i]["kd_gd_hasil"],
                                        "tglAwal" => date("Y-m-t",strtotime("-1 months")),
                                        "tglAkhir" => date("Y-m-t")
                                      );# Data Untuk Menjumlahkan Saldo Akhir Dari Gudang Kanvas
              $dataGudangHasil = array("kdGdHasil" => $arrData[$i]["kd_gd_hasil_secondary"],
                                       "tglAwal" => date("Y-m-t",strtotime("-1 months")),
                                       "tglAkhir" => date("Y-m-t")
                                     );# Data Untuk Menjumlahkan Saldo Akhir Dari Gudang Hasil
              # Proses Penghitungan Saldo Akhir
              $arrSaldoGudangBuffer  = json_decode($this->Gudang_Kanvas_Model->selectSaldoAwalBulanGudangHasilBuffer($dataGudangBuffer),TRUE);
              $arrSaldoGudangHasil  = json_decode($this->Gudang_Kanvas_Model->selectSaldoAwalBulanGudangHasil($dataGudangHasil),TRUE);
              ...
              $dataForUpdateMasterGudangHasil = array("kd_gd_hasil" => $arrData[$i]["kd_gd_hasil_secondary"],
                                                      "stok_berat" => str_replace(",","",$arrSaldoGudangHasil["saldoAkhirBerat"]),
                                                      "stok_lembar" => str_replace(",","",$arrSaldoGudangHasil["saldoAkhirLembar"])
                                                     );
              $booleanResultGudangBuffer = $this->Gudang_Kanvas_Model->updateGudangHasil($dataForUpdateMasterGudangBuffer);
              $booleanResultGudangHasil = $this->Gudang_Kanvas_Model->updateGudangHasil($dataForUpdateMasterGudangHasil);
            }else if($result == "Lock"){
              break;
              echo "Lock";
            }else{
          ...
          if($booleanResultGudangBuffer && $booleanResultGudangHasil){
            $jumlahSelesai++;
          }else{
            break;
  ...
  public function getListPengeluaranGudangHasil(){
    ...
    if($isLogin){
      ...
      echo $this->Gudang_Kanvas_Model->selectListPengeluaranGudangHasil($data);
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
      redirect("_main/main","refresh");
    }
  ...
  public function getListKirimanDariBagian(){
    ...
    if($isLogin){
      ...
      $data = array("bagian" => $bagian,
                    ...
                    "jnsPermintaan" => $jnsPermintaan);
      $result = $this->Gudang_Kanvas_Model->selectListKirimanDariBagian($data);
      echo $result;
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
      redirect("_main/main","refresh");
  ...
  public function saveApproveKirimanDariBagian(){
    ...
    if($isLogin){
      ...
      $data = array("bagian" => $bagian,
                    "statusBarang" => $stsBarang,
                    "jnsPermintaan" => $jnsPermintaan);
      $result = $this->Gudang_Kanvas_Model->updateApproveKirimanDariBagian($data);
      echo $result;
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
      redirect("_main/main","refresh");
  ...
  public function getListKirimanUntukGudangKanvas(){
    $isLogin = $this->isLogin();
    if($isLogin){
      $stsBarang = $this->input->post("stsBarang");
      $data = array("stsBarang" => $stsBarang);
      $result = $this->Gudang_Kanvas_Model->selectListKirimanUntukGudangKanvas($data);
      echo $result;
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
      redirect("_main/main","refresh");
  ...
  public function saveApproveKirimanUntukBufferSablon(){
    ...
    if($isLogin){
      $stsBarang = $this->input->post("stsBarang");
      $data = array("stsBarang" => $stsBarang);
      $result = $this->Gudang_Kanvas_Model->updateApproveKirimanUntukBufferSablon($data);
      echo $result;
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
      redirect("_main/main","refresh");
  ...
  {
    $result = $this->Gudang_Kanvas_Model->getListKembalianHDSablon();
    echo $result;
  }
  ...
  public function approveKembalianHDSablon()
  {
    $isLogin = $this->isLogin();
    if($isLogin){
      $result = $this->Gudang_Kanvas_Model->approveKembalianHDSablon();
      echo $result;
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
      redirect("_main/main","refresh");
  ...
  {
    $stsBarang = $this->input->post("stsBarang");
    $result = $this->Gudang_Kanvas_Model->getListDataBarangRetur($stsBarang);
    echo $result;
  }
  ...
  function approveReturBarang()
  {
    ...
    if($isLogin){
      ...
      $result = $this->Gudang_Kanvas_Model->approveReturBarang($jenis);
      echo $result;
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
      redirect("_main/main","refresh");
  ...
  function countReturBarang()
  {
    ...
    $result = $this->Gudang_Kanvas_Model->countReturBarang($jenis);
    echo $result;
  }
  ...
  function getBahanSablon($jenis)
  {
    ...
    if(empty($key)){
      $result = $result = $this->Gudang_Kanvas_Model->getBahanSablon(str_replace("%20", " ", $jenis));
      echo $result;
    }else{
      $result = $this->Gudang_Kanvas_Model->searchBahanSablon($key,str_replace("%20", " ", $jenis));
      echo $result;
    }
  }
  ...
  function getBahanSablonFull($jenis)
  {
    $key = $this->input->get("q");
    if(empty($key)){
      $result = $result = $this->Gudang_Kanvas_Model->getBahanSablonFull(str_replace("%20", " ", $jenis));
      echo $result;
    }else{
      $result = $this->Gudang_Kanvas_Model->searchBahanSablonFull($key,str_replace("%20", " ", $jenis));
      echo $result;
    }
  }
  ...
  {
    $result = $this->Gudang_Kanvas_Model->genKodeBahanSablon();
    echo $result;
  }
  ...
  function addBahanSablon()
  {
    ...
    if($isLogin){
      ...
      $result = $this->Gudang_Kanvas_Model->addBahanSablon($data);
      echo $result;
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
      redirect("_main/main","refresh");
  ...
  {
    $result = $this->Gudang_Kanvas_Model->getTempDataBahanSablon();
    echo $result;
  }
  ...
  function countListTambahBahanSablon()
  {
    $isLogin = $this->isLogin();
    if($isLogin){
      $result = $this->Gudang_Kanvas_Model->countListTambahBahanSablon();
      echo $result;
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
      redirect("_main/main","refresh");
  ...
  function deleteListTempBahanSablon()
  {
    ...
    if($isLogin){
      ...
      $result = $this->Gudang_Kanvas_Model->deleteListTempBahanSablon($id);
      echo $result;
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
      redirect("_main/main","refresh");
  ...
  function saveBahanSablon()
  {
    $isLogin = $this->isLogin();
    if($isLogin){
      $result = $this->Gudang_Kanvas_Model->saveBahanSablon();
      echo $result;
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
      redirect("_main/main","refresh");
  ...
  {
    $result = $this->Gudang_Kanvas_Model->getListBahanSablon();
    echo $result;
  }

  function countKirimanBahanSablon()
  {
    $result = $this->Gudang_Kanvas_Model->countKirimanBahanSablon();
    echo $result;
  }

  function getDataKirimanBahanSablon()
  {
    $result = $this->Gudang_Kanvas_Model->getDataKirimanBahanSablon();
    echo $result;
  }

  function approveKirimanBahanSablon()
  {
    $result = $this->Gudang_Kanvas_Model->approveKirimanBahanSablon();
    echo $result;
  }
  ...
  function deleteBahanSablon()
  {
    ...
    if($isLogin){
      $id = $this->input->post("id");
      $result = $this->Gudang_Kanvas_Model->deleteBahanSablon($id);
      echo $result;
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
      redirect("_main/main","refresh");
  ...
  function editListBahanSablon()
  {
    ...
    if($isLogin){
      $id = $this->input->post("id");
      $result = $this->Gudang_Kanvas_Model->getDataBahanSablonPerId($id);
      echo $result;
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
      redirect("_main/main","refresh");
  ...
  function updateListBahanSablon()
  {
    ...
    if($isLogin){
      ...
      $result = $this->Gudang_Kanvas_Model->updateListBahanSablon($id,$stok);
      echo $result;
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
      redirect("_main/main","refresh");
  ...
  function getListHistoryBahanSablonKeluar()
  {
    ...
    $result = $this->Gudang_Kanvas_Model->getListHistoryBahanSablonKeluar($data);
    echo $result;
  }
  ...
  function getListHistoryBahanSablonMasuk()
  {
    ...
    $result = $this->Gudang_Kanvas_Model->getListHistoryBahanSablonMasuk($data);
    echo $result;
  }
  ...
  function getDetailHistoryBahanSablon()
  {
    ...
    $result = $this->Gudang_Kanvas_Model->getDetailHistoryBahanSablon($data);
    echo $result;
  }
  ...
  function addPermintaanBahanSablon()
  {
    ...
    if($isLogin){
      ...
      $result = $this->Gudang_Kanvas_Model->addPermintaanBahanSablon($data);
      echo $result;
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
      redirect("_main/main","refresh");
  ...
  function getPermintaanBahanSablon($jenis)
  {
    $key = $this->input->get("q");
    if(empty($key)){
      $result = $result = $this->Gudang_Kanvas_Model->getPermintaanBahanSablon(str_replace("%20", " ", $jenis));
      echo $result;
    }else{
      $result = $this->Gudang_Kanvas_Model->searchPermintaanBahanSablon($key,str_replace("%20", " ", $jenis));
      echo $result;
    }
  }
  ...
  {
    $result = $this->Gudang_Kanvas_Model->getTempDataPermintaanBahanSablon();
    echo $result;
  }
  ...
  function countPermintaanBahanSablonPending()
  {
    $isLogin = $this->isLogin();
    if($isLogin){
      $result = $this->Gudang_Kanvas_Model->countPermintaanBahanSablonPending();
      echo $result;
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
      redirect("_main/main","refresh");
  ...
  function deleteListPermintaanBahanSablon()
  {
    ...
    if($isLogin){
      $id = $this->input->post("id");
      $result = $this->Gudang_Kanvas_Model->deleteListPermintaanBahanSablon($id);
      echo $result;
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
      redirect("_main/main","refresh");
  ...
  function kirimPermintaanBahanSablon()
  {
    $isLogin = $this->isLogin();
    if($isLogin){
      $result = $this->Gudang_Kanvas_Model->kirimPermintaanBahanSablon();
      echo $result;
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
      redirect("_main/main","refresh");
  ...
  function deletedListRetur()
  {
    ...
    if($isLogin){
      $id = $this->input->post("id");
      $result = $this->Gudang_Kanvas_Model->deletedListRetur($id);
      echo $result;
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
      redirect("_main/main","refresh");
  ...
  function getListDetailHasilJadi()
  {
    $tglAwal  = $this->input->post("tglAwal");
    $tglAkhir = $this->input->post("tglAkhir");
    $result = $this->Gudang_Kanvas_Model->getListDetailHasilJadi($tglAwal,$tglAkhir);
    echo $result;
  }
  ...
  function getKirimanPotongPerId()
  {
    ...
    if($isLogin){
      $id = $this->input->post("id");
      $result = $this->Gudang_Kanvas_Model->getKirimanPotongPerId($id);
      echo $result;
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
      redirect("_main/main","refresh");
  ...
  function kirimBalikKiriman()
  {
    ...
    if($isLogin){
      ...
      $result = $this->Gudang_Kanvas_Model->kirimBalikKiriman($id,$note);
      echo $result;
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
      redirect("_main/main","refresh");
  ...
  {
    $result = $this->Gudang_Kanvas_Model->countKirimanBarangKeGudangBuffer();
    echo $result;
  }
  ...
  function countKirimanBarang()
  {
    ...
    if($isLogin){
      ...
      $result = $this->Gudang_Kanvas_Model->countKirimanBarang($data);
      echo $result;
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
      redirect("_main/main","refresh");
  ...
  function countKembalianHDSablon()
  {
    $isLogin = $this->isLogin();
    if($isLogin){
      $result = $this->Gudang_Kanvas_Model->countKembalianHDSablon();
      echo $result;
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
      redirect("_main/main","refresh");
  ...
  public function getDetailDataOrderTerkirim(){
    ...
    if($isLogin){
      $idTransaksi = $this->input->post("idTransaksi");
      $result = $this->Gudang_Kanvas_Model->selectDetailDataOrderTerkirim($idTransaksi);
      echo $result;
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
      redirect("_main/main","refresh");
  ...
  function updateDataOrderTerkirim()
  {
    ...
    if($isLogin){
      ...
      $result = $this->Gudang_Kanvas_Model->updateDataOrderTerkirim($data);
      echo $result;
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
      redirect("_main/main","refresh");
  ...
  function deleteDataOrderTerkirim()
  {
    ...
    if($isLogin){
      $data = array();
      $id = $this->input->post("id");
      $result = $this->Gudang_Kanvas_Model->deleteDataOrderTerkirim($id);
      echo $result;
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
      redirect("_main/main","refresh");
  ...
  function updateStatus()
  {
    ...
    if($isLogin){
      ...
      $result = $this->Gudang_Kanvas_Model->updateStatus($id,$status);
      echo $result;
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
      redirect("_main/main","refresh");
  ...
  function getDataStokMaster()
  {
    ...
    $result = $this->Gudang_Kanvas_Model->selectDataStokMaster($kd_gd_hasil);
    echo $result;
  }
  ...
  public function getListGudangBahanDatatable(){
    ...
    if($isLogin){
      ...
      $result = $this->Gudang_Kanvas_Model->selectListGudangBahanDatatable($param);
      echo $result;
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
      redirect("_main/main","refresh");
  ...
  public function getGeneratedGudangBahanCode(){
    $isLogin = $this->isLogin();
    if($isLogin){
      $result["Code"] = $this->Gudang_Kanvas_Model->generateGudangBahanCode();
      echo json_encode($result);
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
      redirect("_main/main","refresh");
  ...
  public function saveGudangBahan(){
    ...
    if($isLogin){
      ...
      if(empty($kdGdBahan)||empty($idUser)||empty($nmBarang)||$stok == ""||
        ...
        $result = $this->Gudang_Kanvas_Model->insertGudangBahan($data);
        echo $result;
      }
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
  ...
  public function getDetailBarangBahan(){
    ...
    if($isLogin){
      ...
      $result = $this->Gudang_Kanvas_Model->selectDetailBarangBahan($kdGudangBahan);
      echo $result;
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
      redirect("_main/main","refresh");
    ...
    if($isLogin){
      ...
      if(empty($kdGdBahan)||empty($idUser)||empty($nmBarang)||
        ...
        $data = array("kd_gd_bahan"=>$kdGdBahan,
                      ...
                      "tgl_masuk"=>$tglMasuk,
                      "status"=>$status);
        $result = $this->Gudang_Kanvas_Model->updateGudangBahan($data);
        if($result){
          echo "Berhasil";
        }else{
          echo "Gagal";
  ...
  public function getGeneratedRequestCode(){
    $isLogin = $this->isLogin();
    if($isLogin){
      $arrData["Code"] = $this->Gudang_Kanvas_Model->generateKodePermintaan();
  ...
  public function getComboBoxValueBahan($param){
    ...
    if($isLogin){
      ...
      if (!empty($query)) {
        ...
        "key"=>$query);
        $result = $this->Gudang_Kanvas_Model->selectComboBoxValueBahanSearch($data);
      }else{
        $data = array("jenis"=>str_replace("_"," ",$param),
        "key"=>"");
        $result = $this->Gudang_Kanvas_Model->selectComboBoxValueBahan($data);
      }
      echo $result;
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
    ...
    if($isLogin){
      ...
      if(count($data) == 0){
        ...
        for($i=0; $i<count($data); $i++){
          $result = $this->Gudang_Kanvas_Model->insertPermintaanBarang($data[$i]);
  ...
  public function getListHistoryGudangBahan(){
    ...
    if($isLogin){
      ...
      if(empty($tglAwal) || empty($tglAkhir) || empty($kdGudangBahan)){
        ...
        $result = $this->Gudang_Kanvas_Model->selectListHistoryGudangBahan($data);
        echo $result;
      }
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
    ...
    if($isLogin){
      ...
      if(empty($tglAwal) || empty($tglAkhir) || empty($kdGudangBahan)){
        ...
        $data = array("tglAwal"=>$tglAwal,
                      "tglAkhir"=>$tglAkhir,
                      "kdGdBahan"=>$kdGudangBahan);
        $result = $this->Gudang_Kanvas_Model->selectSaldoAwalBulanGudangBahan($data);
        echo $result;
      }
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
  ...
  public function getKoreksiTemp(){
    ...
    if($isLogin){
      ...
      $result = $this->Gudang_Kanvas_Model->selectKoreksiGudangBahan($jenis);
      echo json_encode($result);
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
      redirect("_main/main","refresh");
    ...
    if($isLogin){
      ...
      if(empty($tanggal)||empty($kdGdBahan)||
         ...
         $keterangan == "#"){
          ...
          $result = $this->Gudang_Kanvas_Model->insertTransaksiGudangBahan($data);
          echo $result;
         }
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
  ...
  public function getDetailKoreksi(){
    ...
    if($isLogin){
      ...
      $result = $this->Gudang_Kanvas_Model->selectDetailTransaksiGudangBahan($idTransaksi);
      echo $result;
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
      redirect("_main/main","refresh");
    ...
    if($isLogin){
      ...
      if(empty($tanggal)||empty($idUser)||empty($nmBahanBaku)||empty($id)||
         empty($jumlahKoreksi)||empty($jenisKoreksi)||empty($keterangan)
          ...
          $result = $this->Gudang_Kanvas_Model->updateTransaksiGudangBahan($data);
          if($result){
            echo "Berhasil";
          }else{
            echo "Gagal";
    ...
    if($isLogin){
      ...
      if($result){
        $data2 = array("kdGdBahan"=>$kdGdBahan,
                       "tglAwal"=>$tglAwal,
                       "tglAkhir"=>$tglAkhir);
        $saldo = json_decode($this->Gudang_Kanvas_Model->selectSaldoAwalBulanGudangBahan($data2),TRUE);
        $result2 = $this->Gudang_Kanvas_Model->updateGudangBahan(array("kd_gd_bahan"=>$kdGdBahan,"stok"=>str_replace(",","",$saldo["saldoAkhir"])));
        if($result2){
          echo "Berhasil";
        }else{
          echo "Gagal";
  ...
  public function getPengeluaranGudangBahanTemp(){
    ...
    if($isLogin){
      $jenis = str_replace("_"," ",$this->input->post("jenis"));
      $result = $this->Gudang_Kanvas_Model->selectPengeluaranGudangBahan($jenis);
      echo json_encode($result);
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
      redirect("_main/main","refresh");
      ...
      if(empty($result)){
        ...
        foreach($result as $arrResult){
          $data = array("id" => $arrResult["id"],
                        ...
                        "status" => "FINISH");
          $response = $this->Gudang_Kanvas_Model->updateTransaksiGudangBahanAndStok($data);
        }
        if($response){
          echo "Berhasil";
        }else{
  ...
  public function getCountAlert(){
    ...
    if($isLogin){
      ...
      $data = array("bagian"=>$bagian,
                    "jenis"=>$jenis);
      $result = $this->Gudang_Kanvas_Model->selectCountAlert($data);
      echo $result;
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
      redirect("_main/main","refresh");
  ...
  public function getCountAlertPembelianGudangBahan(){
    ...
    if($isLogin){
      $jenis = $this->input->post("jenis");
      $result = $this->Gudang_Kanvas_Model->selectCountAlertPembelianGudangBahan($jenis);
      echo $result;
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
      redirect("_main/main","refresh");
  ...
  function countPermintaanCatSablon(){
    $result = $this->Gudang_Kanvas_Model->countPermintaanCatSablon();
    echo $result;
  }
  ...
  public function getPermintaanBarang(){
    ...
    if($isLogin){
      ...
      $data = array("idUser" => $idUser,
                    "bagian" => $bagian);
      $result = $this->Gudang_Kanvas_Model->selectPermintaanBarang($data);
      echo $result;
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
      redirect("_main/main","refresh");
  ...
  public function printBonPermintaanBarang($param){
    $isLogin = $this->isLogin();
    if($isLogin){
      $data = array("DataPermintaan" => $this->Gudang_Kanvas_Model->selectPermintaanBarang_Print($param),
                    "DataDetailPermintaan" => $this->Gudang_Kanvas_Model->selectDetailPermintaanBaru_Print($param));
  ...
  public function getDetailPermintaanBaru(){
    ...
    if($isLogin){
      ...
      $result = $this->Gudang_Kanvas_Model->selectDetailPermintaanBaru($data);
      echo $result;
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
      redirect("_main/main","refresh");
  ...
  public function terimaBarangFull(){
    ...
    if($isLogin){
      ...
      $arrData = $this->Gudang_Kanvas_Model->selectDataPermintaanUntukInputTransaksiGudangBahan($idDPB);
      ...
      $result = $this->Gudang_Kanvas_Model->updateTerimaBarangFull($Data);
      echo $result;
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
      redirect("_main/main","refresh");
  ...
  public function terimaBarangSetengah(){
    ...
    if($isLogin){
      ...
      $result = $this->Gudang_Kanvas_Model->updateTerimaBarangSetengah($Data);
      echo $result;
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
      redirect("_main/main","refresh");
  ...
  public function selesaiPermintaan(){
    ...
    if($isLogin){
      ...
      $result = $this->Gudang_Kanvas_Model->updateSelesaiPermintaan($data);
      if($result){
        echo "Berhasil";
      }else{
        echo "Gagal";
  ...
  public function deleteAndRestorePermintaanBarang(){
    ...
    if($isLogin){
      ...
      $data = array("kd_permintaan_barang" => $idTransaksi,
                    "deleted" => $deleted);
      $result = $this->Gudang_Kanvas_Model->updateDeleteAndRestorePermintaanBarang($data);
      echo $result;
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
      redirect("_main/main","refresh");
  ...
  public function getTransaksiGudangBahanForApproveDataTable(){
    ...
    if($isLogin){
      ...
      if(empty($bagian)||empty($jenis)){
        ...
        $data = array("bagian"=>$bagian,
                      "jenis"=>$jenis);
        $result = $this->Gudang_Kanvas_Model->selectTransaksiGudangBahanForApproveDataTable($data);
        echo $result;
      }
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
  ...
  public function saveApproveTransaksiGudangBahan(){
    ...
    if($isLogin){
      ...
      if(empty($jenis) || empty($bagian)){
        ...
        $data = array("bagian"=>$bagian,
                      "jenis"=>$jenis);
        $result = $this->Gudang_Kanvas_Model->selectGudangBahanDataForApprove($data);
        if(empty($result)){
          echo "Item Kosong";
        }else{
          for ($i=0; $i < count($result); $i++) {
            $param = array("id"=>$result[$i]["id"],
                           ...
                           "status"=>"FINISH");
            $result2 = $this->Gudang_Kanvas_Model->updateTransaksiGudangBahanAndStok($param);
            if($result2){
              $statusNumber++;
            }
          }
  ...
  public function getListDataAwalPending(){
    ...
    if($isLogin){
      ...
      $data = array("idUser" => $idUser,
                    "jenis" => $jenis);
      $result = $this->Gudang_Kanvas_Model->selectListDataAwalPending($data);
      echo $result;
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
      redirect("_main/main","refresh");
    ...
    if($isLogin){
      ...
      if($kdGdBahan=="" || $jumlahPermintaan==""){
        ...
        $data["TGB"] = array("kd_gd_bahan"          => $kdGdBahan,
                             ...
                             "jenis"                => $jenis);
        $result = $this->Gudang_Kanvas_Model->insertDataAwalPending($data);
        echo $result;
      }
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
  ...
  public function saveDataAwal(){
    ...
    if($isLogin){
      ...
      $param = array("idUser" => $idUser,
                     "jenis" => $jenis);
      $data = json_decode($this->Gudang_Kanvas_Model->selectListDataAwalPending($param),TRUE);
      $data2 = array();
      ...
      $result = $this->Gudang_Kanvas_Model->updateStokDataAwalBaru($data2);
      if($result){
        echo "Berhasil";
      }else{
        echo "Gagal";
  ...
  public function getDetailDataAwal(){
    ...
    if($isLogin){
      $kdGdBahan = $this->input->post("kdGdBahan");
      $result = $this->Gudang_Kanvas_Model->selectDetailDataAwal($kdGdBahan);
      echo $result;
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
      redirect("_main/main","refresh");
  ...
  public function editDataAwal(){
    ...
    if($isLogin){
      ...
      $data = json_decode($this->Gudang_Kanvas_Model->selectDetailDataAwal($kdBarang),TRUE);

      $data2 = array("id" => $idTransaksi,
                     "kd_gd_bahan" => $kdBarang,
                     "jumlah" => $jumlahBaru - $data[0]["jumlah_permintaan"],
                     "jumlahBaru" => $jumlahBaru);
      $result = $this->Gudang_Kanvas_Model->updateStokDataAwalLama($data2);
  ...
  public function getCekKartuStok(){
    ...
    if($isLogin){
      ...
      $data = array("tglAwal" => $tglAwal,
                    "tglAkhir" => $tglAkhir);
      $result = $this->Gudang_Kanvas_Model->selectCekKartuStokKanvas($data);
      echo $result;
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
      redirect("_main/main","refresh");
  ...
  public function getDetailDataAwalGudangHasil(){
    ...
    if($isLogin){
      $kdGdHasil = $this->input->post("kdGdHasil");
      $result = $this->Gudang_Kanvas_Model->selectDetailDataAwalGudangHasil($kdGdHasil);
      echo $result;
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
      redirect("_main/main","refresh");
  ...
  public function getListDataAwalGudangHasilPending(){
    ...
    if($isLogin){
      $stsBarang = $this->input->post("stsBarang");
      $result = $this->Gudang_Kanvas_Model->selectListDataAwalGudangHasilPending($stsBarang);
      echo $result;
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
      redirect("_main/main","refresh");
  ...
  public function addDataAwalGudangHasil(){
    ...
    if($isLogin){
      ...
      $idUser = $this->session->userdata("fabricationIdUser");
      $customer = strtoupper($this->session->userdata("fabricationGroup"));
      $tglTransaksi = "2018-04-30";
      if(empty($kdGdHasil) || $jumlahBerat=="" || $jumlahLembar==""){
        echo "Data Kosong";
      }else{
        $data = array("kd_gd_hasil" => $kdGdHasil,
                      "id_user" => $idUser,
        ...
        $result = $this->Gudang_Kanvas_Model->insertDataAwalGudangHasilPending($data);
        if($result){
          echo "Berhasil";
        }else{
          echo "Gagal";
  ...
  public function saveDataAwalGudangHasil(){
    ...
    if($isLogin){
      $stsBarang = $this->input->post("stsBarang");
      $arrData = array();
      $arrResult = json_decode($this->Gudang_Kanvas_Model->selectListDataAwalGudangHasilPending($stsBarang), TRUE);
      if(count($arrResult) <= 0){
        ...
        $result = $this->Gudang_Kanvas_Model->updateStokDataAwalGudangHasilBaru($arrData);
        if($result){
          echo "Berhasil";
        }else{
          echo "Gagal";
  ...
  public function editDataAwalGudangHasil(){
    ...
    if($isLogin){
      ...
      $arrResult = json_decode($this->Gudang_Kanvas_Model->selectDetailDataAwalGudangHasil($kdBarang), TRUE);
      ...
      $arrData = array("id_permintaan_jadi" => $idTransaksi,
                       ...
                       "idUser" => $idUser);
      $result = $this->Gudang_Kanvas_Model->updateStokDataAwalGudangHasilLama($arrData);
      if($result){
        echo "Berhasil";
      }else{
        echo "Gagal";
  ...
  public function getCekKartuStokSort(){
    ...
    if($isLogin){
      ...
      $result = $this->Gudang_Kanvas_Model->selectCekKartuStokSort($data);
      echo $result;
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
      redirect("_main/main","refresh");
  ...
  public function deleteAndRestoreDetailPermintaanBarang(){
    ...
    if($isLogin){
      ...
      $data = array("idTransaksi" => $idTransaksi,
                    ...
                    "deleted" => $deleted);
      $result = $this->Gudang_Kanvas_Model->updateDeleteAndRestoreDetailPermintaanBarang($data);
      echo $result;
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
      redirect("_main/main","refresh");
  ...
  public function updateCheckList(){
    ...
    if($isLogin){
      ...
      $idCeklisGh = $this->input->post("idCeklisGh");
      $idUser = $this->session->userdata("fabricationIdUser");
      $periode = $this->input->post("periode");
      $data = array("kd_gd_hasil" => $kdGdHasil,
                    "ceklis" => $ceklis,
                    "id_ceklis_gh" => $idCeklisGh,
                    "id_user" => $idUser,
                    "periode" => $periode);
      $result = $this->Gudang_Kanvas_Model->updateCheckList($data);
      echo $result;
    }else{
         echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
         redirect("_main/main","refresh");
  ...
  public function perbaruiStokMaster(){
    $isLogin = $this->isLogin();
    if($isLogin){
      $arrValue = $this->input->post("arrValue");
      $arrData = array();
      foreach ($arrValue as $arrTemp) {
        $temp = explode("|",$arrTemp["value"]);
        array_push($arrData, array("kd_gd_hasil" => $temp[0],
                                   "stok_berat" => floatval($temp[1]),
                                   "stok_lembar" => floatval($temp[2])));
      }
      $result = $this->Gudang_Kanvas_Model->updateStokMasterBatch($arrData);
      if($result){
        echo "Berhasil";
      }else{
        echo "Gagal";
  ...
  public function getDetailMasterPack(){
    $kd_produk = $this->input->get('kd_produk');
    if(!empty($kd_produk)){
      if($result = $this->Barang_Pack_Model->get(['kd_produk'=>$kd_produk, 'deleted'=>'FALSE'])){
        echo json_encode($result);
      } else {
        echo json_encode(['Code'=>400, 'Text'=>'gagal saat mengambil data']);
      }
    } else {
      echo json_encode(['Code'=>400, 'Text'=>'Kode Produk diperlukan!']);
    }
  }

  public function getDataAwalPack(){
    $kd_produk = $this->input->get('kd_produk');
    if(!empty($kd_produk)){
      $result = $this->Barang_Pack_Model->getDataAwal($kd_produk);
      echo json_encode(['Code'=>200, 'response'=>['stock'=>$result->jumlah_kirim,'berat'=>$result->jumlah_berat]]);
    } else {
      echo json_encode(['Code'=>400, 'Text'=>'Kode Produk diperlukan!']);
    }
  }

  public function updateStockPack(){
    $jumlahBerat = $this->input->post('jumlah_berat');
    $jumlahKirim = $this->input->post('jumlah_packing');
    $kd_produk = $this->input->post("kdPack");
    $lastStock = $this->Barang_Pack_Model->getDataAwal($kd_produk);
    $newData = array(
      "jumlahBeratBaru" => $jumlahBerat,
      "jumlahKirimBaru" => $jumlahKirim
    );
    $data = array(
      'kd_pack'=>$this->input->post('kdPack'),
      'id_user'=>$this->session->userdata("fabricationIdUser"),
      "jumlah_berat" => $jumlahBerat - $lastStock->jumlah_berat,
      "jumlah_kirim" => $jumlahKirim - $lastStock->jumlah_kirim,
      'bagian'=>$this->session->userdata("fabricationGroup"),
      'tgl_transaksi'=>date('Y-m-d'),
      'jns_permintaan'=>'POLOS',
      'sts_barang'=>'ANEKA MACAM',
      ...
      'keterangan_history'=>'DATA AWAL',
      'keterangan_barang'=>'UNTUK ANEKA MACAM',
      'keterangan'=> "Data Awal Masuk Pada : ".date("d-m-Y H:i:s")
    );
    if(!empty($kd_produk)){
      $idTransaksi = $this->Barang_Pack_Model->getIdTransaksi(['keterangan_history'=>'DATA AWAL']);
      if($this->Barang_Pack_Model->updateStokDataAwalAneka($data, $idTransaksi->id_transaksi, $newData)){
        echo json_encode(['Code'=>200,'Text'=>'Berhasil merubah stock']);
      } else {
        echo json_encode(['Code'=>400,'Text'=>'Terjadi kesalahan saat meng-update stock']);
      }
    } else {
      echo json_encode(['Code'=>400, 'Text'=>'Tidak ada produk']);
    }
  }

  public function getDaftarBarangAnekaPlastik(){
    $isLogin = $this->isLogin();
    if($isLogin){
      $items = $this->Gudang_Kanvas_Model->getItem(['kd_produk'=>$this->input->get('kodeBarang')]);
  ...
  public function getListPengambilanKanvasTemp(){
    $isLogin = $this->isLogin();
    if($isLogin){
      $jnsGudang = $this->input->get("jnsGudang");
      $result = $this->Gudang_Kanvas_Model->selectListPengambilanKanvasTemp($jnsGudang);
      echo $result;
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
      redirect("_main/main","refresh");
  ...
  public function addPengambilanKanvas(){
    $isLogin = $this->isLogin();
    if($isLogin){
      $tanggal = $this->input->post("tanggal");
      $idUser = $this->session->userdata("fabricationIdUser");
      ...
      if(empty($tanggal) || empty($idUser) || $jumlahLembar == "" || $jumlahBerat ==""){
        ...
        $result = $this->Gudang_Kanvas_Model->insertTransaksiPengambilanKanvas($data);
        echo $result;
      }
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
  ...
  public function savePengambilanKanvas(){
    $isLogin = $this->isLogin();
    if($isLogin){
      $jnsGudang = $this->input->post("jnsGudang");
      $arrData = json_decode($this->Gudang_Kanvas_Model->selectListPengambilanKanvasTemp($jnsGudang),TRUE);
      $jumlahDataPengambilan = count($arrData);
      $jumlahDataSelesai = 0;
      if(empty($arrData)){
        echo "Item Kosong";
        ...
        foreach ($arrData as $data) {
          $newArrData = array("kd_gd_hasil"=>$data["kd_gd_hasil"],
                              ...
                              "status_history" => "KELUAR",
                              "status_transaksi" => "FINISH",
                              "keterangan_history" => "PENGAMBILAN GUDANG $jnsGudang ($data[jns_brg])",
                              "keterangan_barang" => $data["keterangan"],
                              "id_pengambilan_kanvas" => $data["id_pengambilan_kanvas"]);
          $result = $this->Gudang_Kanvas_Model->insertTransaksiGudangHasil_PengambilanKanvas($newArrData);
          if($result){
            $jumlahDataSelesai++;
          }
        }
