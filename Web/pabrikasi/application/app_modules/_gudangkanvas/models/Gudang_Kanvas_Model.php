<?php
class Gudang_Kanvas_Model extends CI_Model{
  ...
  public function generateGudangHasilCode($param){
    switch($param["jns_brg"]) {
      ...
      case "SABLON"   : if($param["jns_permintaan"] == "POLOS"){
                        ...
                        break;
      case "KANVAS"  : $prefix = "HSKV";
                        $flags = TRUE;
                        break;
      case "ONLINE"  : $prefix = "HSOL";
                        $flags = TRUE;
                        break;
    ...
    if($flags){
      $maxCode = $this->db->query("SELECT MAX(RIGHT(kd_gd_hasil,4)) AS kode FROM gudang_hasil WHERE SUBSTRING(kd_gd_hasil, 1, 4) = '$prefix'");
  ...
  public function insertGudangHasil($param){
    ...
    if($param["jns_permintaan"]=="CETAK" && $param["jns_brg"]=="SABLON"){
      $this->db->query("INSERT INTO transaksi_gudang_hasil (kd_gd_hasil,kd_gd_hasil_secondary,id_user,ukuran,jumlah_berat,jumlah_lembar,warna,customer,tgl_transaksi,merek,
                                                            jns_permintaan,sts_barang,sts_approve,status_history,keterangan_history,keterangan,status_lock,status_transaksi,bagian)
                        VALUES ('$param[kd_gd_hasil]','$param[kd_gd_hasil]','$param[id_user]','$param[ukuran]','$param[stok_berat]','$param[stok_lembar]','$param[warna]','GUDANG KANVAS','$param[tgl_buat]','$param[merek]','$param[jns_permintaan]',
                                '$param[jns_brg]','TRUE','MASUK','DATA AWAL','$param[keterangan]','TRUE','FINISH','GUDANG KANVAS')");
    }else{
      $this->db->query("INSERT INTO transaksi_gudang_hasil (kd_gd_hasil,id_user,ukuran,jumlah_berat,jumlah_lembar,warna,customer,tgl_transaksi,merek,
                                                            jns_permintaan,sts_barang,sts_approve,status_history,status_transaksi,keterangan_history,keterangan,status_lock,bagian)
                        VALUES ('$param[kd_gd_hasil]','$param[id_user]','$param[ukuran]','$param[stok_berat]','$param[stok_lembar]','$param[warna_plastik]','GUDANG KANVAS','$param[tgl_buat]','$param[merek]','$param[jns_permintaan]',
                                '$param[jns_brg]','TRUE','MASUK','FINISH','DATA AWAL','$param[keterangan]','TRUE','GUDANG KANVAS')");
    }
  ...
  public function insertAddPengirimanBaru($param){
    $this->db->trans_begin();
    // if(empty($param['kdGdHasil']) && empty($parak['kdPack'])){
    ...
    // }elseif (!empty($param['kdPack']) && empty($param['kdGdHasil'])) {
    //   $this->db->query("INSERT INTO transaksi_detail_pengiriman (kd_pack,id_dp,id_user,customer,tgl_pengiriman,jumlah_pesanan,jumlah_kg,jumlah_lembar,warna,keterangan,sts_pengiriman,jenis_jumlah,jumlah_terkirim)
    //                     VALUES ('$param[kdPack]','$param[idDp]','$param[idUser]','$param[customer]','$param[tglPengiriman]','$param[jumlahPesanan]','$param[jumlahKg]','$param[jumlahLembar]','$param[warna]','$param[keterangan]',
    //                             '$param[statusPengiriman]','$param[jenisJumlah]','$param[jumlahDikirim]')");
    // }else{
  ...
  public function insertTransaksiBarangPack_Pengiriman($param){
    $this->db->trans_begin();
    $checkLock = $this->db->query("SELECT COUNT(status_lock) AS count_lock FROM transaksi_produk_aneka_macam
                                   WHERE YEAR(tgl_transaksi)=YEAR('$param[tgl_transaksi]')
                                   AND MONTH(tgl_transaksi)=MONTH('$param[tgl_transaksi]')
                                   AND status_lock='TRUE'
                                   AND sts_barang = '$param[sts_barang]'")->result_array();
    if($checkLock[0]["count_lock"] > 0){
      ...
      $this->db->insert("transaksi_produk_aneka_macam",array_diff_key($param,array_flip(array("jumlah_terkirim","no_order"))));
      $this->db->query("UPDATE produk_aneka_macam
                        SET stock = stock - $param[jumlah_kirim]
                        WHERE kd_produk='$param[kd_pack]'");

      $this->db->query("UPDATE pesanan_detail
                        SET jumlah_kirim = jumlah_kirim + $param[jumlah_kirim]
                        WHERE id_dp = '$param[id_dp]'");
  ...
  public function insertTransaksiGudangHasil($param){
    ...
    $checkLock = $this->db->query("SELECT COUNT(status_lock) AS count_lock FROM transaksi_gudang_hasil
                                   ...
                                   AND MONTH(tgl_transaksi)=MONTH('$param[tgl_transaksi]')
                                   AND status_lock='TRUE'
                                   AND sts_barang = '$param[sts_barang]'
                                   AND keterangan_history NOT IN('DATA AWAL')")->result_array();
    if($checkLock[0]["count_lock"] > 0){
      $this->db->trans_rollback();
      echo "Lock";
    }else{
  ...
  public function insertTransaksiAnekaMacam($param){
    $this->db->trans_begin();
    $checkLock = $this->db->query("SELECT COUNT(status_lock) AS count_lock FROM transaksi_produk_aneka_macam
                                   WHERE YEAR(tgl_transaksi)=YEAR('$param[tgl_transaksi]')
                                   AND MONTH(tgl_transaksi)=MONTH('$param[tgl_transaksi]')
  ...
  public function insertTransaksiPengambilanKanvas($param){
    $this->db->trans_begin();
    $checkLock = $this->db->query("SELECT COUNT(status_lock) AS count_lock FROM transaksi_pengambilan_kanvas
                                   WHERE YEAR(tgl_pengambilan)=YEAR('$param[tgl_pengambilan]')
                                   AND MONTH(tgl_pengambilan)=MONTH('$param[tgl_pengambilan]')
                                   AND status_lock='TRUE'
  ...
  public function selectComboBoxValueAneka(){
    if($result = $this->db->get_where('produk_aneka_macam',[
      'deleted'=>'FALSE',
      'status_produk'=>'FINISH'
    ])){
      return json_encode($result->result_array());
    }
  }
  public function selectComboBoxValueAnekaSearch($param){
      if(strpos($param["key"],"|") === FALSE){
        $Q = "SELECT * FROM produk_aneka_macam WHERE status_produk = 'FINISH'
              AND deleted = 'FALSE'
              AND (kd_produk LIKE '%$param[key]%' OR
                   nama_produk LIKE '%$param[key]%')
        ...
        $Q = "SELECT * FROM produk_aneka_macam WHERE status_produk = 'FINISH'
              AND deleted = 'FALSE'
              AND kd_produk LIKE '%$arrKey[0]%'
              AND nama_produk LIKE '%$arrKey[1]%'
              ORDER BY nama_produk ASC";
      }
  ...
  public function selectGudangHasilCustom($param){
    if(empty($param["jns_permintaan"])){
      ...
      $clause = "AND jns_permintaan = '$param[jns_permintaan]'";
    }
    ...
    if(empty($param["key"])){
      ...
      if(strpos($param["key"],"|") === FALSE){
        ...
        $result = $this->db->query("SELECT * FROM gudang_hasil
                                    ...
                                    AND jns_brg = '$param[jns_brg]'
                                    AND (
                                      ukuran LIKE '%$arrKey[0]%' AND
                                      merek LIKE '%$arrKey[1]%' AND
                                      warna_plastik LIKE '%$arrKey[2]%'
                                    )");
      }
    }
    return json_encode($result->result_array());
  ...
  public function selectListPengirimanBaruDatatable($key){
    $this->datatables->select("IF(C.nm_perusahaan_update IS NULL, C.nm_perusahaan,
                               ...
                               IF(PD.kd_pack IS NULL, PD.merek, AM.nama_produk) as merek,PD.dll,PD.jumlah, PD.jumlah_kirim,PD.keterangan,
                               (PD.jumlah - PD.jumlah_kirim) AS sisa, PD.sm, PD.id_dp, PD.warna_cetak");
    $this->datatables->from("pesanan P");
    if (empty($key)) {
      $this->datatables->where("P.sts_pesanan = 'PROGRESS'
                              ...
                              AND (SUBSTRING(PD.kd_gd_hasil,1,4) = 'HSKV' OR SUBSTRING(PD.kd_pack,1,4) = 'PACK' OR P.kd_order IN ('OL N','OL P','OL'))");
    }else{
      $param = explode(" ", $key);
      $counter = count($param);
      for ($i=0; $i < $counter; $i++) {
      ...
      $this->datatables->where(" $search and P.sts_pesanan = 'PROGRESS'
                                ...
                                AND PD.tgl_kirim_gudang IS NOT NULL
                                AND (PD.jumlah - PD.jumlah_kirim) > 0
                                AND (SUBSTRING(PD.kd_gd_hasil,1,4) = 'HSKV' OR P.kd_order IN ('OL N','OL P','OL'))");
    }
  ...
  public function selectListPesananTerkirim($key){
    ...
    $this->datatables->from("pesanan P");
    if (empty($key)) {
      $this->datatables->where("P.deleted = 'FALSE'
                                AND PD.deleted = 'FALSE'
                                AND PD.tgl_kirim_gudang IS NOT NULL
      ...
      $this->datatables->where(" $search P.deleted = 'FALSE'
                               AND PD.deleted = 'FALSE'
                               AND PD.tgl_kirim_gudang IS NOT NULL
                               AND PD.jumlah_kirim > 0
                               AND (SUBSTRING(PD.kd_gd_hasil,1,4) = 'HSKV' OR P.kd_order IN ('OL N','OL P','OL'))");
  ...
  public function selectListPesananBelumTerkirim(){
    $this->datatables->select("P.no_order,P.tgl_pesan,P.nm_pemesan,P.tgl_estimasi,
                               IF(PD.kd_pack IS NULL, GH.ukuran, '-') as ukuran,GH.warna_plastik,IF(PD.kd_pack IS NULL,GH.merek,AM.nama_produk) as merek,
                               PD.dll,PD.jumlah,
                               GB.nm_barang,IF(PD.kd_pack IS NULL, GB.warna , '-') as warna,
                               (PD.jumlah - PD.jumlah_kirim) AS sisa, PD.sm, PD.id_dp, PD.satuan");
    $this->datatables->from("pesanan P");
    $this->datatables->where("P.sts_pesanan = 'PROGRESS'
                              AND PD.sts_pesanan = 'PROGRESS'
                              AND sts_kirim = 'BELUM TERKIRIM'
                              AND P.deleted = 'FALSE'
                              AND PD.deleted = 'FALSE'
                              AND PD.tgl_kirim_gudang IS NOT NULL
                              AND (PD.jumlah - PD.jumlah_kirim) > 0
  ...
  public function selectPesananDetailData($param){
    $arrData = $this->db->query("SELECT PD.*,IF(PD.kd_pack IS NULL, PD.merek, AM.nama_produk) as merek,
                                        P.tgl_pesan,P.nm_pemesan,IF(PD.kd_pack IS NULL, GH.ukuran, '-') as ukuran,
                                        IF(PD.kd_pack IS NULL, GH.warna_plastik, '-') as warna_plastik,
                                        IF(PD.kd_pack IS NULL, GB.warna, '-') as warna,
                                        (PD.jumlah-PD.jumlah_kirim) AS sisa, IF(PD.kd_pack IS NULL, GH.jns_brg, 'ANEKA MACAM') as jns_brg, GB.jenis
  ...
  public function selectListPengirimanBaruTemp(){
    $arrData = $this->db->query("SELECT TDP.*,
                                        IF(PD.kd_pack IS NULL,GH.ukuran,AM.nama_produk) as ukuran,PD.merek,GH.jns_permintaan,PD.no_order
                                 FROM transaksi_detail_pengiriman TDP
                                 INNER JOIN pesanan_detail PD ON TDP.id_dp = PD.id_dp
                                 LEFT JOIN produk_aneka_macam AM ON TDP.kd_pack = AM.kd_produk
                                 ...
                                 AND PD.deleted='FALSE'
                                 AND (SUBSTRING(TDP.kd_gd_hasil,1,4) = 'HSKV' OR SUBSTRING(TDP.kd_pack,1,4) = 'PACK' OR SUBSTRING(TDP.kd_gd_hasil,1,4) = 'HSOL') ")->result_array();
    return json_encode($arrData);
  }

  public function selectListDetailPengiriman($param){
    $arrData = $this->db->query("SELECT TDP.*,
                                        IF(PD.kd_pack IS NULL, GH.ukuran, AM.nama_produk ) as ukuran ,PD.merek,GH.jns_permintaan, PD.satuan
                                 FROM transaksi_detail_pengiriman TDP
                                 INNER JOIN pesanan_detail PD ON TDP.id_dp = PD.id_dp
                                 LEFT JOIN produk_aneka_macam AM on TDP.kd_pack = AM.kd_produk
                                 LEFT JOIN gudang_hasil GH ON TDP.kd_gd_hasil = GH.kd_gd_hasil
  ...
  public function selectListGudangHasilDatatable($param){
    $this->datatables->select("kd_gd_hasil,kd_accurate,ukuran,
                               ...
                               warna_plastik,merek,jns_permintaan,jns_brg");
  ...
  public function selectListHistoryPackMasukDatatable($param){
    $tglAwalPlus1 = date("Y-m-d",strtotime("+1 days",strtotime($param["tglAwal"])));
    $this->datatables->select("id_transaksi,tgl_transaksi,FORMAT(jumlah_berat,2) AS jumlah_berat, FORMAT(jumlah_lembar,2) as jumlah_lembar, jumlah_kirim,
                              keterangan_history,status_history,status_lock,customer,keterangan_barang, keterangan");
    $this->datatables->from("transaksi_produk_aneka_macam");
    $this->datatables->where("status_history='MASUK' AND tgl_transaksi BETWEEN '$tglAwalPlus1' AND
                              '$param[tglAkhir]' AND
                              deleted='FALSE' AND
                              status_transaksi='FINISH' AND
                              kd_pack=",$param['kdPack']);
    $this->db->order_by('tgl_transaksi','DESC');
    return $this->datatables->generate();
  }
  ...
  public function selectListHistoryGudangHasilBufferMasukDatatable($param){
    ...
    $this->datatables->select("id_permintaan_jadi,tgl_transaksi,FORMAT(jumlah_berat,2) AS jumlah_berat, FORMAT(jumlah_lembar,2) AS jumlah_lembar,
                              keterangan_history,status_history,status_lock,customer,keterangan_barang, keterangan");
    $this->datatables->from("transaksi_gudang_hasil");
    $this->datatables->where("(((status_history = 'KELUAR' OR status_history = 'MASUK') AND kd_gd_hasil_secondary='$param[kdGdHasil]')
                              OR
                              (status_history = 'MASUK' AND kd_gd_hasil = '$param[kdGdHasil]'))
                              AND
                              tgl_transaksi BETWEEN '$tglAwalPlus1' AND '$param[tglAkhir]' AND
                              deleted='FALSE' AND
  ...
  public function selectListHistoryAnekaMacamKeluarDatatable($param){
    $tglAwalPlus1 = date("Y-m-d",strtotime("+1 days",strtotime($param["tglAwal"])));
    $this->datatables->select("id_transaksi,jumlah_kirim, tgl_transaksi,FORMAT(jumlah_berat,2) AS jumlah_berat, FORMAT(jumlah_lembar,2) AS jumlah_lembar,
                              keterangan_history,status_history,status_lock,customer,keterangan_barang, keterangan");
    $this->datatables->from("transaksi_produk_aneka_macam");
    $this->datatables->where("status_history = 'KELUAR' AND tgl_transaksi BETWEEN '$tglAwalPlus1' AND
                              '$param[tglAkhir]' AND
  ...
  public function selectSaldoAwalBulanGudangHasilBuffer($param){
    ...
    $saldoAwal = $this->db->query("SELECT (
                                           (SUM(IF((keterangan_history != 'PENGAMBILAN SABLON (SABLON)'
                                            ...
                                            +
                                            SUM(IF(keterangan_history = 'DATA AWAL' AND kd_gd_hasil='$param[kdGdHasil]',jumlah_berat,0))
                                            )-
                                           SUM(IF((keterangan_history LIKE '%PENGEMBALIAN BARANG%'
                                                  OR keterangan_history = 'PENGAMBILAN SABLON (SABLON)')
                                                  AND kd_gd_hasil_secondary = '$param[kdGdHasil]'
                                                  AND tgl_transaksi <= '$param[tglAwal]',jumlah_berat,0))-
                                           SUM(IF((keterangan_history IN ('PENGEMBALIAN BARANG','PENGIRIMAN BARANG','UNTUK ANEKA MACAM','PENGAMBILAN GUDANG ONLINE (KANVAS)'))
                                ...
                                AS saldo_awal_berat,
                                          (
                                            (SUM(IF((keterangan_history != 'PENGAMBILAN SABLON (SABLON)'
                                                   ...
                                                   +
                                             SUM(IF(keterangan_history = 'DATA AWAL' AND kd_gd_hasil='$param[kdGdHasil]',jumlah_lembar,0))
                                            )-
                                            SUM(IF((keterangan_history LIKE '%PENGEMBALIAN BARANG%'
                                                   OR keterangan_history = 'PENGAMBILAN SABLON (SABLON)')
                                                   AND kd_gd_hasil_secondary = '$param[kdGdHasil]'
    ...
    $totalPerPeriode = $this->db->query("SELECT SUM(IF(keterangan_history NOT IN ('PENGEMBALIAN BARANG','PENGIRIMAN BARANG','UNTUK ANEKA MACAM')
                                                ...
                                                SUM(IF(status_history='KELUAR' AND
                                                       keterangan_history IN ('PENGEMBALIAN BARANG','PENGIRIMAN BARANG','UNTUK ANEKA MACAM','PENGAMBILAN GUDANG ONLINE (KANVAS)') AND
  ...
  public function selectSaldoAwalBulanAnekaMacam($param){
    $tglAwalPlus1 = date("Y-m-d",strtotime("+1 days",strtotime($param["tglAwal"])));
    $saldoAwal = $this->db->query("SELECT (
                                            SUM(IF(tgl_transaksi <= '$param[tglAwal]' AND status_history = 'MASUK', jumlah_berat,0)) -
                                            SUM(IF(tgl_transaksi <= '$param[tglAwal]' AND status_history = 'KELUAR', jumlah_berat,0))
                                          ) AS saldo_awal_berat,
                                          (
                                            SUM(IF(tgl_transaksi <= '$param[tglAwal]' AND status_history = 'MASUK', jumlah_kirim,0)) -
                                            SUM(IF(tgl_transaksi <= '$param[tglAwal]' AND status_history = 'KELUAR', jumlah_kirim,0))
                                          ) AS saldo_awal_kirim
                                FROM transaksi_produk_aneka_macam
                                WHERE kd_pack = '$param[kdPack]'

                                AND tgl_transaksi BETWEEN '2015-01-01' AND '$param[tglAkhir]'
                                AND deleted='FALSE'
                                AND status_transaksi = 'FINISH'")->result_array();

    $totalPerPeriode = $this->db->query("SELECT SUM(IF(status_history = 'MASUK'
                                                      AND kd_pack = '$param[kdPack]',jumlah_berat,0)) AS total_masuk_berat_per_periode,

                                                SUM(IF(status_history= 'KELUAR'
                                                      AND kd_pack = '$param[kdPack]',jumlah_berat,0)) AS total_keluar_berat_per_periode,

                                                SUM(IF(status_history = 'MASUK'
                                                    AND kd_pack = '$param[kdPack]',jumlah_kirim,0)) AS total_masuk_lembar_per_periode,

                                                SUM(IF(status_history='KELUAR'
                                                       AND kd_pack = '$param[kdPack]',jumlah_kirim,0)) AS total_keluar_lembar_per_periode
                                         FROM transaksi_produk_aneka_macam
                                         WHERE kd_pack = '$param[kdPack]'
                                         AND tgl_transaksi BETWEEN '$tglAwalPlus1' AND '$param[tglAkhir]'
                                         AND deleted='FALSE'
    ...
    $stokMaster = $this->db->query("SELECT stock FROM produk_aneka_macam WHERE kd_produk = '$param[kdPack]'")->result_array();
    $result = array("saldoAwalBerat"=>number_format($saldoAwal[0]["saldo_awal_berat"],2,".",","),
                    "saldoAwalKirim"=>number_format($saldoAwal[0]["saldo_awal_kirim"],2,".",","),
                    "saldoAkhirBerat"=>number_format($saldoAwal[0]["saldo_awal_berat"]+$totalPerPeriode[0]["total_masuk_berat_per_periode"]-$totalPerPeriode[0]["total_keluar_berat_per_periode"],2,".",","),
                    "saldoAkhirKirim"=>number_format($saldoAwal[0]["saldo_awal_kirim"]+$totalPerPeriode[0]["total_masuk_lembar_per_periode"]-$totalPerPeriode[0]["total_keluar_lembar_per_periode"],2,".",","),
                    "saldoMasukBerat"=>number_format($totalPerPeriode[0]["total_masuk_berat_per_periode"],2,".",","),
                    "saldoKeluarBerat"=>number_format($totalPerPeriode[0]["total_keluar_berat_per_periode"],2,".",","),
                    "saldoMasukKirim"=>number_format($totalPerPeriode[0]["total_masuk_lembar_per_periode"],2,".",","),
                    "saldoKeluarLembar"=>number_format($totalPerPeriode[0]["total_keluar_lembar_per_periode"],2,".",","),
                    "saldoBerat"=>number_format($stokMaster[0]["stock"],2,".",","),
                    "saldoKirim"=>number_format($stokMaster[0]["stock"],2,".",","),);
    return json_encode($result);
  }
  ...
  public function selectDetailTransaksiPack($param){
    $this->db->select("TPAM.*,TDP.jumlah_terkirim,PD.satuan");
    $this->db->from("transaksi_produk_aneka_macam TPAM");
    $this->db->where("TPAM.id_transaksi",$param);
    $this->db->join("transaksi_detail_pengiriman TDP","TPAM.id_detail_pengiriman = TDP.id_detail_pengiriman","LEFT");
    $this->db->join("pesanan_detail PD","TPAM.id_dp = PD.id_dp","LEFT");
    $result = $this->db->get()->result_array();
    return json_encode($result);
  }
  ...
  public function selectListKoreksiGudangBufferTemp(){
    $arrData = $this->db->query("SELECT id_permintaan_jadi,kd_gd_hasil,id_transaksi_aneka, tgl_transaksi,ukuran,jumlah_berat,jumlah_lembar,warna,merek,keterangan_history
                                 FROM transaksi_gudang_hasil
                                 WHERE status_transaksi='PENDING'
                                 AND (SUBSTRING(keterangan_history,1,14) = 'KOREKSI BARANG' OR keterangan_history = 'UNTUK ANEKA MACAM')
                                 AND deleted = 'FALSE'")->result_array();
    return json_encode($arrData);
  }
  public function selectListKoreksiPackTemp(){
    $arrData = $this->db->query("SELECT id_transaksi,kd_pack, tgl_transaksi,jumlah_berat,jumlah_lembar,jumlah_kirim,keterangan_history
                                 FROM transaksi_produk_aneka_macam
                                 WHERE status_transaksi='PENDING'
                                 AND (SUBSTRING(keterangan_history,1,14) = 'KOREKSI BARANG' OR keterangan_history = 'UNTUK ANEKA MACAM')
                                 AND deleted = 'FALSE'")->result_array();
  ...
  public function selectListPengembalianBarangGudangBufferKeStandardTemp($param){
    $arrData = $this->db->query("SELECT *
                                  FROM transaksi_gudang_hasil
                                  ...
                                  AND status_transaksi = 'PENDING'
                                  AND keterangan_history = 'PENGEMBALIAN BARANG'
                                  AND keterangan = 'PENGEMBALIAN KE GUDANG $param'
                                  AND deleted = 'FALSE'")->result_array();
    return json_encode($arrData);
  }
  ...
  public function selectListKirimanUntukGudangKanvas($param){
    ...
    $this->datatables->where("kd_gd_hasil_secondary IS NOT NULL AND
                              customer IN ('GUDANG KANVAS','GUDANG ONLINE') AND
                              bagian = 'GUDANG HASIL' AND
                              sts_barang = '$param[stsBarang]' AND
                              sts_approve = 'FALSE' AND
                              status_history = 'KELUAR' AND
                              status_transaksi = 'PROGRESS' AND
                              keterangan_history IN ('PENGAMBILAN GUDANG KANVAS ($param[stsBarang])',
                                                     'PENGAMBILAN GUDANG ONLINE ($param[stsBarang])') AND
                              deleted = 'FALSE'");
    $this->db->order_by("tgl_transaksi","DESC");
    return $this->datatables->generate();
  }
  ...
  public function countKirimanBarangKeGudangBuffer(){
    $data = $this->db->query("SELECT id_permintaan_jadi FROM transaksi_gudang_hasil
                              ...
                              status_history = 'KELUAR' AND
                              status_transaksi = 'PROGRESS' AND
                              keterangan_history IN ('PENGAMBILAN GUDANG KANVAS (STANDARD)',
                                                     'PENGAMBILAN GUDANG KANVAS (CAMPUR)',
                                                     'PENGAMBILAN GUDANG KANVAS (SABLON)',
                                                     'PENGAMBILAN GUDANG ONLINE (STANDARD)',
                                                     'PENGAMBILAN GUDANG ONLINE (CAMPUR)',
                                                     'PENGAMBILAN GUDANG ONLINE (SABLON)') AND
                              deleted = 'FALSE')");
  ...
  public function selectDataStokMaster($kd_gd_hasil){
    $data = $this->db->query("SELECT stok_berat, FORMAT(stok_lembar,2) AS stok_lembar
                              FROM gudang_hasil
  ...
  public function selectCekKartuStokKanvas($param){
    $QBarangMaster = "SELECT CONCAT(ukuran,' ',merek,' ',warna_plastik) AS nm_barang,
                             ...
                             ceklist
                      FROM gudang_hasil
                      WHERE jns_brg='KANVAS'
                      AND deleted='FALSE'
                      ORDER BY FIELD(merek,'Klip','KP','MP','Zippin','HD','CB',merek), CAST(SUBSTRING_INDEX(LCASE(ukuran),'x',1) AS UNSIGNED) ASC, CAST(SUBSTRING_INDEX(LCASE(ukuran),'x',-1) AS UNSIGNED) ASC, warna_plastik DESC";
    ...
    foreach ($arrBarangMaster as $value) {
      $saldoAwal = $this->db->query("SELECT (
                                             ...
                                             SUM(IF((keterangan_history LIKE '%PENGEMBALIAN BARANG%'
                                                    ...
                                                    AND kd_gd_hasil_secondary = '$value[kd_gd_hasil]'
                                                    AND tgl_transaksi <= '$param[tglAwal]',jumlah_berat,0))-
                                             SUM(IF((keterangan_history IN ('PENGEMBALIAN BARANG','PENGIRIMAN BARANG'))
                                                    AND kd_gd_hasil = '$value[kd_gd_hasil]'
                                                    AND tgl_transaksi <= '$param[tglAwal]',jumlah_berat,0))
                                            )
                                  AS saldo_awal_berat,
                                            (
                                  ...
                                  AS saldo_awal_lembar,
                                            IF(kd_gd_hasil NOT LIKE '%HSKV%',kd_gd_hasil_secondary, kd_gd_hasil) AS kd_gd_hasil
                                     FROM transaksi_gudang_hasil
      ...
      $totalPerPeriode = $this->db->query("SELECT SUM(IF(keterangan_history NOT IN ('PENGEMBALIAN BARANG','PENGIRIMAN BARANG')
                                                         AND kd_gd_hasil_secondary='$value[kd_gd_hasil]',jumlah_berat,0)) AS total_masuk_berat_per_periode,

                                                  SUM(IF(status_history='KELUAR' AND
                                                         keterangan_history IN ('PENGEMBALIAN BARANG','PENGIRIMAN BARANG') AND
                                                         kd_gd_hasil = '$value[kd_gd_hasil]',jumlah_berat,0)) AS total_keluar_berat_per_periode,

                                                  SUM(IF(keterangan_history NOT IN ('PENGEMBALIAN BARANG','PENGIRIMAN BARANG')
                                                      AND kd_gd_hasil_secondary='$value[kd_gd_hasil]',jumlah_lembar,0)) AS total_masuk_lembar_per_periode,

                                                  SUM(IF(status_history='KELUAR' AND
                                                         keterangan_history IN ('PENGEMBALIAN BARANG','PENGIRIMAN BARANG') AND
                                                         kd_gd_hasil = '$value[kd_gd_hasil]',jumlah_lembar,0)) AS total_keluar_lembar_per_periode,
                                                  IF(kd_gd_hasil NOT LIKE '%HSKV%',kd_gd_hasil_secondary, kd_gd_hasil) AS kd_gd_hasil
                                           FROM transaksi_gudang_hasil
                                           WHERE (kd_gd_hasil_secondary = '$value[kd_gd_hasil]' OR kd_gd_hasil = '$value[kd_gd_hasil]')
                                           AND tgl_transaksi BETWEEN '$tglAwalPlus1' AND '$param[tglAkhir]'
  ...
  public function selectListPengambilanKanvasTemp($param){
    $result = $this->db->query("SELECT TPK.*,GH.ukuran,GH.warna_plastik,GH.merek,GH.jns_permintaan,GH.jns_brg
                                ...
                                AND SUBSTRING(TPK.kd_gd_hasil_secondary,1,4) = 'HSOL'
                                AND SUBSTRING(TPK.kd_gd_hasil,1,4) = 'HSKV'
  ...
  public function updatePack($param){
    $this->db->trans_begin();
    $this->db->where("kd_produk",$param["kd_produk"]);
    $this->db->update("produk_aneka_macam",$param);
    if($this->db->trans_status() === FALSE){
      $this->db->trans_rollback();
      return FALSE;
    }else{
  ...
  public function updateTransaksiPackAndDetailPengiriman($param){
    $this->db->trans_begin();
    #Check Pengiriman Atau Bukan
    $QCheckPengiriman = $this->db->query("SELECT id_detail_pengiriman FROM transaksi_produk_aneka_macam WHERE id_transaksi='$param[id_transaksi]'")->result_array();
    #Jika Pengiriman Maka Update Data Jumlah Berat Dan Jumlah Lembar Pada Tabel Transaksi_Detail_Pengiriman
    if(!empty($QCheckPengiriman[0]["id_detail_pengiriman"]) ||
       $QCheckPengiriman[0]["id_detail_pengiriman"] != ""   ||
       $QCheckPengiriman[0]["id_detail_pengiriman"] != NULL ||
       $QCheckPengiriman[0]["id_detail_pengiriman"] != "NULL"
        ...
        if(array_key_exists("kd_pack",$param)){
          $idDetailPengiriman = $QCheckPengiriman[0]["id_detail_pengiriman"];
          if(array_key_exists("jumlah_terkirim",$param)){
            $this->db->query("UPDATE transaksi_detail_pengiriman SET kd_pack='$param[kd_gd_hasil]', jumlah_kg='$param[jumlah_berat]', jumlah_lembar='$param[jumlah_lembar]', jumlah_terkirim='$param[jumlah_terkirim]' WHERE id_detail_pengiriman='$idDetailPengiriman'");
          }else{
            $this->db->query("UPDATE transaksi_detail_pengiriman SET kd_pack='$param[kd_gd_hasil]', jumlah_kg='$param[jumlah_berat]', jumlah_lembar='$param[jumlah_lembar]' WHERE id_detail_pengiriman='$idDetailPengiriman'");
          }
        }else{
          $idDetailPengiriman = $QCheckPengiriman[0]["id_detail_pengiriman"];
          if(array_key_exists("jumlah_terkirim",$param)){
    ...
    $this->db->update("transaksi_produk_aneka_macam",array_diff_key($param,array_flip(array("jumlah_terkirim"))));
    if($this->db->trans_status()===FALSE){
      $this->db->trans_rollback();
      return FALSE;
    }else{
  ...
  public function updateTransaksiPack($param){
    $this->db->trans_begin();
    $this->db->where("id_transaksi",$param["id_transaksi"]);
    $this->db->update("transaksi_produk_aneka_macam",$param);
    if($this->db->trans_status()==FALSE){
      $this->db->trans_rollback();
      return FALSE;
    }else{
  ...
  public function updateStockAneka($data){
    $this->db->trans_begin();
    $this->db->insert("transaksi_produk_aneka_macam",$data);
    $this->db->query("UPDATE produk_aneka_macam
                      SET stock = stock + $data[jumlah_kirim]
                      WHERE kd_produk='$data[kd_pack]'");
    if($this->db->trans_status() == FALSE){
      $this->db->trans_rollback();
      return FALSE;
    } else {
  ...
  public function updateTransaksiAnekaJadi($param){
    $this->db->trans_begin();
    $this->db->where("id_transaksi",$param["id_transaksi"]);
    $this->db->update("transaksi_produk_aneka_macam",$param);
    if($this->db->trans_status()==FALSE){
  ...
  public function updateApproveKirimanUntukBufferSablon($param){
    ...
    $periodeLock = $this->db->query("SELECT DATE_FORMAT(tgl_transaksi,'%Y-%m') AS periode
                                     ...
                                     AND customer IN ('GUDANG KANVAS','GUDANG ONLINE')
                                     AND bagian = 'GUDANG HASIL'
                                     AND sts_barang = '$param[stsBarang]'
                                     AND sts_approve = 'FALSE'
                                     AND status_history = 'KELUAR'
                                     AND status_transaksi = 'PROGRESS'
                                     AND keterangan_history IN ('PENGAMBILAN GUDANG KANVAS ($param[stsBarang])',
                                                                'PENGAMBILAN GUDANG ONLINE ($param[stsBarang])')
                                     AND deleted = 'FALSE'
                                     GROUP BY DATE_FORMAT(tgl_transaksi,'%Y-%m')")->result_array();
    foreach ($periodeLock as $value) {
      $checkLock = $this->db->query("SELECT COUNT(id_permintaan_jadi) AS counter
