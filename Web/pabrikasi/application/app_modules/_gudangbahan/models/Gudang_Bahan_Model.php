...
class Gudang_Bahan_Model extends CI_Model{
  ...
  public function insertPermintaanBarang($param, $param2){
    ...
    if($param2 == "MINYAK"){
      ...
      switch (strtoupper($resultMinyak[0]["nm_barang"])) {
  ...
  public function selectListGudangBahanDatatable($param){
    $this->datatables->select("kd_gd_bahan,kd_accurate,nm_barang,stok,satuan,warna");
  ...
  public function selectKoreksiGudangBahan($param){
    $result = $this->db->query("SELECT TGB.tgl_permintaan,
                                ...
                                AND (SUBSTRING(TGB.keterangan_history,1,7) = 'KOREKSI' OR TGB.keterangan_history IN ('PENGIRIMAN KE BREBES(MINUS)'))");
    return $result->result_array();
  }
  ...
  public function selectKoreksiSparePartTemp(){
    $result = $this->db->query("SELECT TDSP.tgl_transaksi,
                                ...
                                AND TDSP.deleted = 'FALSE'
                                AND SP.deleted = 'FALSE'
                                AND (SUBSTRING(TDSP.keterangan_history,1,7) = 'KOREKSI' OR
                                     TDSP.keterangan_history LIKE '%KEMBALIAN%' OR
                                     TDSP.keterangan_history LIKE '%MASUK%')");
    return $result->result_array();
  }
  ...
  public function selectDataPermintaanUntukInputTransaksiGudangBahan($param){
    $Q = "SELECT TDPB.kd_gd_bahan, (TDPB.jumlah_permintaan - TDPB.jumlah_terima) AS jumlah_permintaan,TPB.bagian,
                 UCASE(GB.nm_barang) AS nm_barang, GB.jenis
          FROM transaksi_detail_permintaan_barang TDPB
          INNER JOIN transaksi_permintaan_barang TPB ON TDPB.kd_permintaan_barang = TPB.kd_permintaan_barang
          INNER JOIN gudang_bahan GB ON TDPB.kd_gd_bahan = GB.kd_gd_bahan
  ...
  public function selectCekKartuStokBahan($param){
    $having = "";
    $flags = "";
  ...
  public function selectCekKartuStokSparePart($param){
    $having = "";
    $flags = "";
    if(!empty($param["havingStokMaster"])){
      $flags .= "1";
    ...
    if(!empty($flags)){
      ...
      for ($i=0; $i <count($arrFlags) ; $i++) {
        switch ($arrFlags[$i]) {
          case '1': $having .= " SP.stok ".$param["havingStokMaster"];break;
          case '2': $having .= " saldoAwal ".$param["havingStokAwal"];break;
          case '3': $having .= " saldoAkhir ".$param["havingStokAkhir"];break;
    ...
    $saldoAwal = $this->db->query("SELECT (
                                  ...
                                  (
                                    SUM(IF(TDSP.sts_history='MASUK' AND TDSP.tgl_transaksi BETWEEN '$tglAwalPlus1' AND '$param[tglAkhir]', TDSP.jumlah, 0))
                                  ) AS beratMasukPerPeriode,

                                  (
                                    SUM(IF(TDSP.sts_history='KELUAR' AND TDSP.tgl_transaksi BETWEEN '$tglAwalPlus1' AND '$param[tglAkhir]', TDSP.jumlah, 0))
                                  ) AS beratKeluarPerPeriode,
                                  ...
                                  CONCAT(SP.nm_spare_part, ' ', SP.ukuran) AS nm_barang, SP.kd_spare_part, SP.stok
                                  FROM transaksi_detail_spare_part TDSP
                                  INNER JOIN spare_part SP ON TDSP.kd_spare_part = SP.kd_spare_part
                                  ...
                                  AND SP.deleted='FALSE'
                                  GROUP BY SP.kd_spare_part $having")->result_array();

    $dataCheklist = $this->db->query("SELECT * FROM ceklis_spare_part WHERE periode = '$periode'")->result_array();
  ...
  public function selectCekKartuStokApal($param){
    $having = "";
    $flags = "";
    if(!empty($param["havingStokMaster"])){
      $flags .= "1";
    ...
    if(!empty($flags)){
      ...
      for ($i=0; $i <count($arrFlags) ; $i++) {
        switch ($arrFlags[$i]) {
          case '1': $having .= " GA.stok ".$param["havingStokMaster"];break;
          case '2': $having .= " saldoAwal ".$param["havingStokAwal"];break;
          case '3': $having .= " saldoAkhir ".$param["havingStokAkhir"];break;
    ...
    $periode = date("Y-m",strtotime($param["tglAkhir"]));
    $saldoAwal = $this->db->query("SELECT (
                                           SUM(IF(TDHA.status_history='MASUK' AND TDHA.tgl_transaksi <= '$param[tglAwal]',TDHA.jumlah_apal,0))-
                                           SUM(IF(TDHA.status_history='KELUAR' AND TDHA.tgl_transaksi <= '$param[tglAwal]',TDHA.jumlah_apal,0))
                                          )
                                  AS saldoAwal,

                                  (
                                    SUM(IF(TDHA.status_history='MASUK' AND TDHA.tgl_transaksi BETWEEN '$tglAwalPlus1' AND '$param[tglAkhir]', TDHA.jumlah_apal, 0))
                                  ) AS beratMasukPerPeriode,

                                  (
                                    SUM(IF(TDHA.status_history='KELUAR' AND TDHA.tgl_transaksi BETWEEN '$tglAwalPlus1' AND '$param[tglAkhir]', TDHA.jumlah_apal, 0))
                                  ) AS beratKeluarPerPeriode,
                                  ...
                                  IF(GA.sub_jenis IS NULL OR GA.sub_jenis='', GA.jenis, GA.sub_jenis) AS nm_barang, GA.kd_gd_apal, GA.stok
                                  FROM transaksi_detail_history_apal TDHA
                                  INNER JOIN gudang_apal GA ON TDHA.kd_gd_apal = GA.kd_gd_apal
                                  WHERE TDHA.tgl_transaksi BETWEEN '2015-01-01' AND '$param[tglAkhir]'
                                  AND TDHA.deleted='FALSE'
                                  AND TDHA.sts_transaksi = 'FINISH'
                                  AND GA.deleted='FALSE'
                                  GROUP BY GA.kd_gd_apal $having")->result_array();

    $dataCheklist = $this->db->query("SELECT * FROM ceklis_gudang_apal WHERE periode = '$periode'")->result_array();
  ...
  public function updateCheckListBahan($param){
    $this->db->trans_begin();
    if(array_key_exists("id_ceklis_gb",$param)){
    ...
    }else if(array_key_exists("id_ceklis_sp",$param)){
      if(empty($param["id_ceklis_sp"])){
        $this->db->insert("ceklis_spare_part",$param);
      }else{
        $this->db->set("ceklis",$param["ceklis"]);
        $this->db->set("id_user",$param["id_user"]);
        $this->db->where("id_ceklis_sp", $param["id_ceklis_sp"]);
        $this->db->update("ceklis_spare_part");
      }
    }else{
      if(empty($param["id_ceklis_ga"])){
        $this->db->insert("ceklis_gudang_apal",$param);
      }else{
        $this->db->set("ceklis",$param["ceklis"]);
        $this->db->set("id_user",$param["id_user"]);
        $this->db->where("id_ceklis_ga", $param["id_ceklis_ga"]);
        $this->db->update("ceklis_gudang_apal");
      }
    }
    if($this->db->trans_status() === FALSE){
      $this->db->trans_rollback();
  ...
  public function updateStokMasterBatch($param){
    $this->db->trans_begin();
    for ($i=0; $i < count($param); $i++) {
      $arrColumn = array_keys($param[$i]);
      $this->db->where($arrColumn[0],$param[$i][$arrColumn[0]]);
      if($arrColumn[0] == "kd_spare_part"){
        $table = "spare_part";
      }else if($arrColumn[0] == "kd_gd_apal"){
        $table = "gudang_apal";
      }else{
        $table = "gudang_bahan";
      }
      $this->db->update($table,$param[$i]);
    }
    if($this->db->trans_status() === FALSE){
      $this->db->trans_rollback();
      return FALSE;
