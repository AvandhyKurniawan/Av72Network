...
class Main extends MX_Controller{
    ...
    if($isLogin){
      ...
      if($status==1){
        $listMenu["parentMenu"] = array(
          ...
          array("Content"=>"Cek Kartu Stok Bahan","Link"=>"_gudangbahan/main/cek_kartu_stok_bahan","Name"=>"Cek_Kartu_Stok_Bahan","Status"=>"Single","Icon"=>"fa fa-book","Id"=>"M_Cek_Kartu_Stok_Bahan"),
        );
        return $listMenu;
      }else if($status==2) {
        $listMenu["parentMenu"] = array();
  ...
  public function cek_kartu_stok_bahan(){
    $isLogin = $this->isLogin();
    if($isLogin){
      if($this->session->userdata("fabricationGroup") == "administrator" ||
         $this->session->userdata("fabricationGroup") == "it_department"){
           $data["Data"] = array("Title"=>"Cek Kartu Stok Bahan");
           $data["Link"] = array("Segment1" =>$this->uri->rsegment(1),"Segment2"=>$this->uri->rsegment(2));
           $this->load->view("header");
           $this->load->view("sidebar",$this->checkStatus());
           $this->load->view("frm_cek_kartu_bahan",$data);
           $this->load->view("footer");
      }else{
  ...
  public function terimaBarangSetengah(){
    ...
    if($isLogin){
      ...
      $arrData = $this->Gudang_Bahan_Model->selectDataPermintaanUntukInputTransaksiGudangBahan($idDPB);
      if($arrData[0]["jenis"] == "MINYAK"){
        $arrBeratMinyak = array(160, 15, 170);
        if(in_array($jumlahTerima, $arrBeratMinyak)){
          $beratJumlah = $jumlahTerima;
        }else{
          switch (strtoupper($arrData["nm_barang"])) {
            case 'SMT'                  : $beratJumlah = floatval($jumlahTerima) * 160;
                                          break;
            case 'IPA'                  : $beratJumlah = floatval($jumlahTerima) * 160;
                                          break;
            case 'REDUSER-SABLON-FR001' : $beratJumlah = floatval($jumlahTerima) * 15;
                                          break;

            default: $beratJumlah = floatval($jumlahTerima) * 170;
              break;
          }
        }
      }else{
  ...
  public function getCekKartuStokBahan(){
    $isLogin = $this->isLogin();
    if($isLogin){
      $tglAwal = $this->input->post("tglAwal");
      $tglAkhir = $this->input->post("tglAkhir");
      ...
      if($jenis == "SPARE PART"){
        $result = $this->Gudang_Bahan_Model->selectCekKartuStokSparePart($data);
      }else if($jenis == "APAL"){
        $result = $this->Gudang_Bahan_Model->selectCekKartuStokApal($data);
      }else{
        $result = $this->Gudang_Bahan_Model->selectCekKartuStokBahan($data);
      }
      echo $result;
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
  ...
  public function updateCheckListBahan(){
    ...
    if($isLogin){
      ...
      if(substr($kdGdBahan,0,3) == "SPR"){
        $data = array("kd_spare_part" => $kdGdBahan,
                      "ceklis" => $ceklis,
                      "id_ceklis_sp" => $idCeklisGb,
                      "id_user" => $idUser,
                      "periode" => $periode);
      }else if(substr($kdGdBahan,0,3) == "APL"){
        $data = array("kd_gd_apal" => $kdGdBahan,
                      "ceklis" => $ceklis,
                      "id_ceklis_ga" => $idCeklisGb,
                      "id_user" => $idUser,
                      "periode" => $periode);
      }else{
        $data = array("kd_gd_bahan" => $kdGdBahan,
      ...
      $result = $this->Gudang_Bahan_Model->updateCheckListBahan($data);
      echo $result;
    }else{
         echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
         redirect("_main/main","refresh");
  ...
  public function perbaruiStokMaster(){
    ...
    if($isLogin){
      ...
      foreach ($arrValue as $arrTemp) {
        ...
        if($temp[2] == "SPARE PART"){
          array_push($arrData, array("kd_spare_part" => $temp[0],
                                     "stok" => floatval($temp[1])));
        }else if($temp[2] == "APAL"){
          array_push($arrData, array("kd_gd_apal" => $temp[0],
                                     "stok" => floatval($temp[1])));
        }else{
          array_push($arrData, array("kd_gd_bahan" => $temp[0],
                                     "stok" => floatval($temp[1])));
        }
      }

      $result = $this->Gudang_Bahan_Model->updateStokMasterBatch($arrData);
      if($result){
        echo "Berhasil";
      }else{
        echo "Gagal";
