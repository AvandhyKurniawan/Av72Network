...
<!--===============================================General Function (Start) ===============================================-->
      ...
      function modalTambahApalBaru(){
        resetFormTambahApal();
        var warna = ['MERAH','BIRU','KUNING','HIJAU','COKLAT','ORANGE','PUTIH','PUTIHSUSU','UNGU','SILVER','HITAM',"PINK"];
      ...
      function saveKoreksiSparePartTemp(){
        ...
        var jumlah1 = $("#txtJumlahKoreksi").val().replace(/,/g,"");
        var jenisKoreksi1 = $("#cmbJenisKoreksi").val();
        var keteranganTemp = $("#cmbKeterangan").val();
        if(keteranganTemp == "CUSTOM"){
          var keterangan1 = "KOREKSI ( "+$("#txtKeterangan").val()+" )";
        }else{
          var keterangan1 = $("#cmbKeterangan").val();
        }
      ...
      function searchDataKartuStokBahan(){
        var tglAwal = $("#txtTglAwal").val();
        var tglAkhir = $("#txtTglAkhir").val();
        var jenis = $("#cmbJenis").val();

        tableCekKartuStokBahan(tglAwal,tglAkhir,jenis);
        $("#btnSortir").attr("onclick","searchDataKartuStokBahan('"+tglAwal+"','"+tglAkhir+"','"+jenis+"')");
      ...
      function datatableListBahanBaku(){
        ...
        $("#tableDataMasterBahanBaku").dataTable({
          ...
          "fnRowCallback":function(nRow,aData,iDisplayIndex,iDisplayIndexFull){
            ...
            $("td:eq(2)",nRow).text((parseFloat(aData["stok"])).toLocaleString()+" "+aData["satuan"]);
      ...
      function tableListPermintaanBahanSablon(jenis){
        ...
        function tableCekKartuStokBahan(param, param2, param3){
          ...
          $.ajax({
            type : "POST",
            url : "<?php echo base_url('_gudangbahan/main/getCekKartuStokBahan'); ?>",
            dataType : "JSON",
            data : {
              tglAwal : param,
              tglAkhir : param2,
            ...
            success : function(response){
              ...
              $.each(response.saldoAwal, function(AvIndex, AvValue){
                if(param3 == "SPARE PART"){
                  var kdGdBahan = AvValue.kd_spare_part;
                }else if(param3 == "APAL"){
                  var kdGdBahan = AvValue.kd_gd_apal;
                }else{
                ...
                $.each(response.dataCheklist, function(AvIndex2, AvValue2){
                  if(param3 == "SPARE PART"){
                    var kode = AvValue2.kd_spare_part;
                    var idChecklist = AvValue2.id_ceklis_sp;
                  }else if(param3 == "APAL"){
                    var kode = AvValue2.kd_gd_apal;
                    var idChecklist = AvValue2.id_ceklis_ga;
                  }else{
                    var kode = AvValue2.kd_gd_bahan;
                    var idChecklist = AvValue2.id_ceklis_gb;
                  }
                  if(kode == kdGdBahan){
                    if(AvValue2.ceklis == "FALSE"){
                      if(group == "gudang hasil"){
                        ...
                        attr = "onclick=checkedAndUncheckedBahan(this,'TRUE','"+idChecklist+"','"+periode+"')";
                      }
                      ...
                      if(group == "gudang hasil"){
                        ...
                        attr = "onclick=checkedAndUncheckedBahan(this,'FALSE','"+idChecklist+"','"+periode+"')";
                      }
                ...
                if(parseFloat(AvValue.beratMasukPerPeriode) > 0 ||
                 ...
                 $("#tableDataKartuStok > tbody:last-child").append(
                   "<tr>"+
                     ...
                     "<td>"+
                      "<input type='checkbox' name='ceklis' value='"+kdGdBahan+"|"+parseFloat(AvValue.saldoAkhir)+"|"+param3+"'> "+
                      "<button class='btn btn-sm btn-default btn-flat' "+attr+" id='"+kdGdBahan+"'>"+icon+"</button>"+
                     "</td>"+
                   "</tr>"
                 );
                }
      ...
      function checkedAndUncheckedBahan(param, param2, param3="", param4=""){
        ...
        $.ajax({
          type : "POST",
          url : "<?php echo base_url('_gudangbahan/main/updateCheckListBahan'); ?>",
          dataType : "TEXT",
          data : {
            kdGdBahan : param.id,
            ceklis : param2,
      ...
      function perbaruiStokMaster(){
        ...
        if(arrValue.length > 0){
          if(confirm("Apakah Anda Yakin Ingin Perbarui Stok Master?")){
            $.ajax({
              type : "POST",
              url : "<?php echo base_url('_gudangbahan/main/perbaruiStokMaster'); ?>",
              dataType : "TEXT",
              data : {
                arrValue : arrValue
              },
      ...
      function closeCustomField(src, dst){
        $(src).hide();
        $(dst).show();
      }
      </script>
