...
class Main extends MX_Controller{
    ...
    if($isLogin){
      ...
      if($status==1){
        ...
        $listMenu["childMenu"] = array(
          ...
          array("Content"=>"Barang Kanvas","Link"=>"_accounting/main/barang_kanvas","Status"=>"Child","Parent"=>"M_Gudang_Hasil","Name"=>"MI_Barang_Kanvas","Id"=>"MI_Barang_Kanvas"),
          array("Content"=>"Barang Online","Link"=>"_accounting/main/barang_online","Status"=>"Child","Parent"=>"M_Gudang_Hasil","Name"=>"MI_Barang_Online","Id"=>"MI_Barang_Online"),
  ...
  public function barang_kanvas(){
    $isLogin = $this->isLogin();
    if($isLogin){
      $data["Data"] = array("Title"=>"Cari History Barang Kanvas");
      $data["Link"] = array("Segment1" =>$this->uri->rsegment(1),"Segment2"=>$this->uri->rsegment(2));
      $this->load->view("header");
      $this->load->view("sidebar",$this->checkStatus());
      $this->load->view("frm_barang_kanvas",$data);
      $this->load->view("footer");
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
      redirect("_main/main","refresh");
  ...
  public function barang_online(){
    $isLogin = $this->isLogin();
    if($isLogin){
      $data["Data"] = array("Title"=>"Cari History Barang Online");
      $data["Link"] = array("Segment1" =>$this->uri->rsegment(1),"Segment2"=>$this->uri->rsegment(2));
      $this->load->view("header");
      $this->load->view("sidebar",$this->checkStatus());
      $this->load->view("frm_barang_online",$data);
      $this->load->view("footer");
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
      redirect("_main/main","refresh");
  ...
  public function getBarangKanvas(){
    $key = $this->input->get("q");
      if(empty($key)){
        $result = $result = $this->Accounting_Model->getBarangKanvas("KANVAS");
        echo $result;
      }else{
        $result = $this->Accounting_Model->searchBarangKanvas($key,"KANVAS");
        echo $result;
      }
  }

  public function getBarangOnline(){
    $key = $this->input->get("q");
      if(empty($key)){
        $result = $result = $this->Accounting_Model->getBarangKanvas("ONLINE");
        echo $result;
      }else{
        $result = $this->Accounting_Model->searchBarangKanvas($key,"ONLINE");
        echo $result;
      }
  }
  ...
  public function getListDataBarangKanvasMasuk(){
    $data = array();
    $data["kdGdHasil"]= $this->input->post("kode");
    $data["tglAwal"]  = $this->input->post("tglAwal");
    $data["tglAkhir"] = $this->input->post("tglAkhir");
    $result = $this->Accounting_Model->getListDataBarangKanvasMasuk($data);
    echo $result;
  }

  public function getListDataBarangKanvasKeluar(){
    $data = array();
    $data["kdGdHasil"]     = $this->input->post("kode");
    $data["tglAwal"]  = $this->input->post("tglAwal");
    $data["tglAkhir"] = $this->input->post("tglAkhir");
    $result = $this->Accounting_Model->getListDataBarangKanvasKeluar($data);
    echo $result;
  }
  ...
  public function saldoAwalKanvas(){
    $data = array();
    $data["kdGdHasil"]     = $this->input->post("kd_gd_hasil");
    $data["tglAwal"]  = $this->input->post("tglAwal");
    $data["tglAkhir"] = $this->input->post("tglAkhir");
    $result = $this->Accounting_Model->saldoAwalKanvas($data);
    echo $result;
  }
