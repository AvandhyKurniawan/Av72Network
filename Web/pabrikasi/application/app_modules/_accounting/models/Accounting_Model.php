...
class Accounting_Model extends CI_Model{
  ...
  public function selectListHistoryJobPotong($param){
    ...
    $Q = "SELECT THP.tgl_rencana,
                 GROUP_CONCAT(RC.customer SEPARATOR '<br>') AS customer,
                 RC.ukuran, RC.warna_plastik, RC.merek,
                 GROUP_CONCAT(TSHP.jumlah_lembar SEPARATOR '<br>') AS jumlah_lembar,
  ...
  public function selectListHistoryJobSablon($param){
    $Q = "SELECT THS.tanggal, RS.customer, RS.merek, RS.ukuran, RS.warna_plastik,
                 RS.warna_sablon, THS.hasil_lembar, THS.hasil_berat, THS.berat_pengambilan,
                 THS.lembar_pengambilan, THS.berat_kembalian, THS.lembar_kembalian
  ...
  public function getBarangKanvas($param){
    $data = $this->db->query("SELECT kd_gd_hasil, ukuran, merek, warna_plastik
                              FROM gudang_hasil
                              WHERE jns_brg = '$param'
  ...
  public function searchBarangKanvas($key,$param)
  {
    $data = $this->db->query("SELECT kd_gd_hasil, ukuran, merek, warna_plastik
                              FROM gudang_hasil
                              WHERE jns_brg = '$param'
                              AND concat(kd_gd_hasil,' ',ukuran,' ',merek,' ',warna_plastik)
                              REGEXP '$key'
                              AND deleted = 'FALSE' ")->result_array();
    return json_encode($data);
  }
  ...
  public function getListDataBarangSablonMasuk($data)
  {
    ...
    $this->datatables->where("(deleted = 'FALSE'
                               AND tgl_transaksi BETWEEN '$data[tglAwal]' AND '$data[tglAkhir]'
                               AND kd_gd_hasil_secondary = '$data[kode]'
                               AND keterangan_history = 'PENGAMBILAN SABLON (STANDARD)'
                               AND status_transaksi='FINISH')
                              OR (deleted = 'FALSE'
                                  AND tgl_transaksi BETWEEN '$data[tglAwal]' AND '$data[tglAkhir]'
                                  AND kd_gd_hasil_secondary = '$data[kode]'
  ...
  public function getListDataBarangKanvasMasuk($param){
    $tglAwalPlus1 = date("Y-m-d",strtotime("+1 days",strtotime($param["tglAwal"])));
    $this->datatables->select("kd_gd_hasil, tgl_transaksi, ukuran, merek, warna, jumlah_berat, jumlah_lembar, keterangan_history");
    $this->datatables->from("transaksi_gudang_hasil");
  ...
  public function getListDataBarangSablonKeluar($data)
  {
    ...
    $this->datatables->where("(deleted = 'FALSE'
                              ...
                              OR (deleted = 'FALSE'
                                  ...
                                  AND kd_gd_hasil = '$data[kode]'
                                  AND keterangan_history = 'PENGEMBALIAN BARANG'
                                  AND keterangan_barang = 'PENGEMBALIAN PENGEMBALIAN'
                                  AND keterangan ='PENGEMBALIAN KE GUDANG STANDARD'
                                  AND status_transaksi='FINISH')");
    return $this->datatables->generate();
  }

  public function getListDataBarangKanvasKeluar($param){
    $tglAwalPlus1 = date("Y-m-d",strtotime("+1 days",strtotime($param["tglAwal"])));
    $this->datatables->select("kd_gd_hasil, tgl_transaksi, ukuran, merek, warna, jumlah_berat, jumlah_lembar, keterangan_history");
    $this->datatables->from("transaksi_gudang_hasil");
  ...
  public function saldoAwalKanvas($param){
    $tglAwalPlus1 = date("Y-m-d",strtotime("+1 days",strtotime($param["tglAwal"])));
    ...
    $result = array("saldoAwalBerat"=>($saldoAwal[0]["saldo_awal_berat"]),
                    "saldoAwalLembar"=>($saldoAwal[0]["saldo_awal_lembar"]),
                    "saldoAkhirBerat"=>($saldoAwal[0]["saldo_awal_berat"]+$totalPerPeriode[0]["total_masuk_berat_per_periode"]-$totalPerPeriode[0]["total_keluar_berat_per_periode"]),
                    "saldoAkhirLembar"=>($saldoAwal[0]["saldo_awal_lembar"]+$totalPerPeriode[0]["total_masuk_lembar_per_periode"]-$totalPerPeriode[0]["total_keluar_lembar_per_periode"]),
                    "saldoMasukBerat"=>($totalPerPeriode[0]["total_masuk_berat_per_periode"]),
                    "saldoKeluarBerat"=>($totalPerPeriode[0]["total_keluar_berat_per_periode"]),
                    "saldoMasukLembar"=>($totalPerPeriode[0]["total_masuk_lembar_per_periode"]),
                    "saldoKeluarLembar"=>($totalPerPeriode[0]["total_keluar_lembar_per_periode"]),
                    "saldoBerat"=>($stokMaster[0]["stok_berat"]),
                    "saldoLembar"=>($stokMaster[0]["stok_lembar"]),);
    return json_encode($result);
  }
