...
class Ppic_Model extends CI_Model{
  ...
  public function selectPesananDetails($param){
    $Q = "SELECT DP.id_dp, DP.jumlah, DP.satuan, DP.kd_pack, DP.harga, DP.merek, DP.mata_uang, GH.ukuran, DP.warna_cetak, DP.sm, DP.dll,
                 DP.tgl_kirim_gudang, DP.sts_pesanan, AM.nama_produk, GH.jns_brg
  ...
  public function selectComboBoxValueHasil($param){
    if($param['jns_brg'] == "ALL"){
      $arrData = $this->db->query("SELECT kd_gd_hasil,ukuran,merek,warna_plastik,jns_permintaan,sts_brg,jns_brg
                                   FROM gudang_hasil WHERE jns_brg NOT IN ('SABLON', 'KANVAS','ONLINE')
                                   AND deleted='FALSE'
  ...
  public function selectComboBoxValueHasilSearch($param){
    if($param["jns_brg"] == "ALL"){
      if(strpos($param["key"],"|") === FALSE){
        $arrData = $this->db->query("SELECT kd_gd_hasil,ukuran,merek,warna_plastik,jns_permintaan,sts_brg,jns_brg
                                     FROM gudang_hasil
                                     WHERE jns_brg NOT IN ('SABLON', 'KANVAS','ONLINE')
                                     AND deleted='FALSE'
                                     AND merek NOT IN ('Kantong','kantong','Kantong (Tidak Dipakai)','kantong (Tidak Dipakai)')
                                     AND
  ...
  public function selectCountPesananGlobalBaru(){
    $now = date("Y-m-d");
    $arrData = $this->db->query("SELECT COUNT(no_order) AS jumlah_pesanan
                                 FROM pesanan
                                 WHERE sts_pesanan = 'OPEN'
                                 AND (jumlah_approve >= 2
                                 AND deleted='FALSE'
                                 OR approve_cabang = 'TRUE'
                                 AND deleted='FALSE')");
    return $arrData->result_array();
  }
  ...
  public function selectFakturPesananDetailProduksi($param){
    $arrFakturPesananDetail = $this->db->query("SELECT FORMAT(PD.jumlah,1) AS jumlah,PD.satuan,
                                                       ...
                                                       IF(PD.kd_gd_hasil IS NULL, '', GH.jns_brg) AS jns_brg
  ...
  public function selectOrderMarketing($param){
    $this->datatables->select("P.no_order,P.kd_order,P.tgl_pesan,P.foto_1,P.foto_2,P.proof, P.ket_proof,
                               PD.jumlah ,PD.satuan, PD.merek, IF(PD.kd_pack IS NULL, GH.merek, AM.nama_produk) as nm_barang,
                               GH.ukuran,GH.warna_plastik,PD.warna_cetak,PD.sm,PD.dll,P.note,
                               IF(PD.tgl_kirim_gudang IS NULL OR
                                  PD.sts_pesanan='OPEN' OR
                                  P.sts_pesanan='OPEN','FALSE','TRUE') AS kirim_gudang,P.sts_print,
                                IF(PD.kd_pack IS NULL,GH.jns_brg, 'ANEKA MACAM') as jns_brg,
                               IF(C.nm_perusahaan_update IS NULL, C.nm_perusahaan, C.nm_perusahaan_update) AS nm_perusahaan");
    ...
    $this->db->order_by("FIELD(kirim_gudang, 'FALSE','TRUE'), P.tgl_pesan DESC,P.no_order ASC");
    return $this->datatables->generate();
  }
  ...
  public function selectOrderMarketingTerkirim(){
    $this->datatables->select("P.tgl_pesan,P.nm_pemesan,P.no_order,P.kd_order,CONCAT(PD.jumlah,' ',PD.satuan) AS jumlah,PD.sts_pesanan,
                               ...
                               GH.ukuran,GH.warna_plastik,PD.id_dp,PD.merek,PD.dll,PD.sts_pesanan,PD.warna_cetak,
  ...
  public function selectOrderDetail($param){
    $this->datatables->select("P.no_order,P.kd_order,P.tgl_pesan,P.nm_pemesan,
                               PD.jumlah,PD.satuan,IF(PD.kd_pack IS NULL, PD.merek, AM.nama_produk) as merek,
                               IF(PD.kd_pack IS NULL, GH.ukuran, '-') AS ukuran,GH.warna_plastik,PD.warna_cetak,PD.sm,PD.dll,P.note,
                               IF(PD.tgl_kirim_gudang IS NULL OR
                                  PD.sts_pesanan='OPEN' OR
                                  P.sts_pesanan='OPEN','FALSE','TRUE') AS kirim_gudang,P.sts_print,
                               IF(C.nm_perusahaan_update IS NULL, C.nm_perusahaan, C.nm_perusahaan_update) AS nm_perusahaan");
  ...
  public function selectOrderCabang($param){
    $this->datatables->select("P.no_order,P.no_po,P.tgl_pesan,P.nm_pemesan,
                               PD.jumlah ,PD.satuan,PD.merek,
                               GH.ukuran,GH.warna_plastik,PD.warna_cetak,PD.sm,PD.dll,P.note,
                               IF(PD.tgl_kirim_gudang IS NULL OR
                                  PD.sts_pesanan='OPEN' OR
                                  P.sts_pesanan='OPEN','FALSE','TRUE') AS kirim_gudang,P.sts_print,
                               C.nm_perusahaan,PD.keterangan,IF(PD.kd_pack IS NULL,GH.jns_brg, 'VARIASI') as jns_brg");
    $this->datatables->from("pesanan_detail PD");
    $this->datatables->where("P.deleted='FALSE' AND
                              PD.deleted='FALSE' AND
                              P.sts_pesanan != 'TERKIRIM' AND
  ...
  public function selectOrderCabangTerkirim(){
    $this->datatables->select("P.tgl_pesan,P.nm_pemesan,P.no_order,P.no_po,CONCAT(PD.jumlah,' ',PD.satuan) AS jumlah,
                               CONCAT(PD.jumlah_kirim,' ',PD.satuan) AS jumlah_kirim,CONCAT(PD.jumlah - PD.jumlah_kirim,' ',PD.satuan) AS sisa,PD.jumlah_kirim as jml_kirim,
                               GH.ukuran,GH.warna_plastik,PD.id_dp,PD.merek,PD.dll,PD.sts_pesanan,C.nm_perusahaan,PD.sts_pesanan,PD.warna_cetak");
    $this->datatables->from("pesanan_detail PD");
  ...
  public function selectDetailBarangHasil($param){
    $result = $this->db->query("SELECT merek,warna_plastik,jns_permintaan,ukuran,jns_brg,sts_brg,keterangan
  ...
  public function selectListSpk($param){
    ...
    $this->datatables->where("RP.deleted='FALSE' AND
                              RP.tgl_rencana='$dateNow' AND
                              RP.bagian=",$param);
    $this->db->order_by("LENGTH(RP.kd_ppic) DESC, RP.kd_ppic DESC");
    return $this->datatables->generate();
  }
  ...
  public function selectListSpkPeriode($param){
    $this->datatables->select("RP.kd_ppic,
                               ...
                               RP.satuan,
                               RP.prioritas,
                               ROUND(RP.jumlah_permintaan) AS jumlah_permintaan,
                               ROUND(RP.sisa) AS sisa,
                               RP.strip,
                               RP.keterangan,
                               RP.sts_pengerjaan,
                               RP.permintaan_mesin,
    ...
    $this->datatables->from("rencana_ppic RP");
    $this->datatables->join("gudang_hasil GH","RP.kd_gd_hasil = GH.kd_gd_hasil","LEFT");
    $this->datatables->where("RP.tgl_rencana BETWEEN '$param[tgl_awal]' AND '$param[tgl_akhir]' AND RP.deleted='FALSE' AND RP.bagian=",$param["bagian"]);
    $this->db->order_by("LENGTH(RP.kd_ppic) DESC, RP.kd_ppic DESC");
    return $this->datatables->generate();
  }
  ...
  public function selectListHistorySpk($param){
    ...
    $this->datatables->where("deleted='FALSE' AND DATE_FORMAT(tgl_rencana,'%Y-%m')='$date' AND bagian=",$param);
  ...
  public function selectLaporanHasilExtruderGlobal($param){
    ...
    $Q = "SELECT TDJE.tgl_rencana,
                 SUM(TDJE.jumlah_apal) AS totalApalGlobal
          ...
          AND TDJE.deleted='FALSE'
          GROUP BY TDJE.tgl_rencana
          ORDER BY TDJE.tgl_rencana ASC";

    $Q2 = "SELECT SUM(jumlah_selesai) AS totalGlobal,
                  tgl_rencana
           FROM transaksi_hasil_extruder
           WHERE tgl_rencana
           BETWEEN '$param[tglAwal]' AND '$param[tglAkhir]'
           AND deleted = 'FALSE'
           GROUP BY tgl_rencana
           ORDER BY tgl_rencana ASC";

    $result = $this->db->query($Q)->result_array();
    $result2 = $this->db->query($Q2)->result_array();

    foreach ($result as $arrResult) {
      foreach ($result2 as $arrResult2) {
        if($arrResult["tgl_rencana"] == $arrResult2["tgl_rencana"]){
          array_push($arrFinalResult,array("tgl_rencana" => $arrResult["tgl_rencana"],
                                           "totalGlobal" => $arrResult2["totalGlobal"],
                                           "totalApalGlobal" => $arrResult["totalApalGlobal"]));
        }
      }
    }
    return json_encode($arrFinalResult);
  }
  ...
  public function updateKirimKeGudang($param){
    $this->db->trans_begin();
    $param2 = array_diff_key($param,array_flip(array("tgl_kirim_gudang")));

    $this->db->where("no_order",$param["no_order"]);
    $this->db->update("pesanan_detail",$param);

    $this->db->where("no_order",$param["no_order"]);
    $this->db->update("pesanan",$param2);
