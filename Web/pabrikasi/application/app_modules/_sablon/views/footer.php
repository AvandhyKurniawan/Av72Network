	<footer class="main-footer">
      ...
      <script type="text/javascript">
        ...
        $("#tableDataRencanaPIC").dataTable({
          ...
          "fnRowCallback" : function(nRow,aData,iDisplayIndex,iDisplayIndexFull){
            ...
            $("td:eq(8)",nRow).text(parseInt(aData["jumlah_permintaan"]).toLocaleString());
            if (aData["prioritas"]=="URGENT") {
              $("td:eq(10)",nRow).html("<b style='color:red;'>"+aData["prioritas"]+"</b>");
            }
            if (!aData['satuan_kilo']) {
              $("td:eq(9)",nRow).text(parseInt(aData["sisa"]).toLocaleString());
            ...
            success : function(response){
              ...
              $('.date').datepicker({
                  ...
                  todayHighlight : true
							...
							$("#cmbGudangBuffer").select2({
	              placeholder : "Pilih Bahan Plastik Polos",
	              dropdownParent : $("#ModalRencana"),
	              width : "100%",
	              cache:true,
	              allowClear:true,
	              ajax:{
	                url : "<?php echo base_url(); ?>_sablon/main/getGudangBuffer/"+ukuran.replace(/\//gi,"_")+"/"+warna_plastik,
	                dataType : "JSON",
	                delay : 0,
	                processResults : function(data){
	                  return{
	                    results : $.map(data, function(item){
	                      return{
													text:item.kd_gd_hasil+" - "+item.ukuran+" - "+item.tebal+" - "+item.merek+" - "+
															 item.warna_plastik+" - "+item.jns_permintaan+" - "+item.sts_brg,
		                      id:item.kd_gd_hasil
	                      }
	                    })
	                  };
      ...
      <script type="text/javascript">
        function addRencana(){
          ...
          var satuan_kilo   = $("#satuan_kilo").val();
					var kd_gd_buffer 	= $("#cmbGudangBuffer").val();
          if (!kd_sablon || !nm_operator || !tgl_rencana || !jml_rencana) {
            ...
            $.ajax({
              ...
              data : {
								kd_ppic 				: kd_ppic,
								kd_sablon 			: kd_sablon,
								kd_gd_hasil 		: kd_gd_hasil,
								customer 				: customer,
								merek 					: merek,
								tgl_rencana 		: tgl_rencana,
								ukuran 					: ukuran,
								warna_plastik 	: warna_plastik,
								warna_sablon 		: warna_sablon,
								nm_operator 		: nm_operator,
								jml_permintaan 	: jml_permintaan,
								jml_rencana 		: jml_rencana,
								satuan_kilo			: satuan_kilo,
								kd_gd_buffer		: kd_gd_buffer
							},
              success:function(response){
                if(jQuery.trim(response) === "Berhasil"){
          ...
          if (r == true) {
            $.ajax({
              ...
              success:function(response){
				...
				function hapusRencana(param){
					if(confirm("Apakah Anda Yakin Ingin Menghapus Data Ini?")){
						$.ajax({
							type : "POST",
							url : "<?= base_url('_sablon/main/hapusRencanaMandor'); ?>",
							dataType : "TEXT",
							data : {
								kdSablon : param,
								deleted : "TRUE"
                  ...
                  setTimeout(function(){
                    $("#modal-notif").removeClass("modal-info");
                    $("#modalNotifContent").text("");
										$(".active a[aria-controls='tableDataRencanaSablon']").click();
                  },3000);
                }else if (jQuery.trim(response) === 'Gagal') {
                  $("#modal-notif").modal("show")
                  $("#modalNotifContent").html("<div style='text-align: center;'><b>List Rencana Gagal Dihapus</b></div>");
      ...
      <script type="text/javascript">
        ...
        $("#tableDataRencanaSablon").dataTable({
          ...
          "fnRowCallback" : function(nRow,aData,iDisplayIndex,iDisplayIndexFull){
            $("td:eq(0)",nRow).text(++iDisplayIndex);
            $("td:eq(7)",nRow).text(parseFloat(aData["jml_rencana"]).toLocaleString());
            $("td:eq(8)",nRow).text(parseFloat(aData["jml_sisa"]).toLocaleString());
						if(parseFloat(aData["jml_sisa"]) != parseFloat(aData["jml_rencana"])){
							...
							$("td:eq(9)",nRow).html("<button class='btn btn-sm btn-flat btn-primary' onclick=inputHasil('"+aData['kd_sablon']+"') title='Input Hasil'>Input Hasil</button>" +
																			"<button class='btn btn-sm btn-flat btn-danger' onclick=hapusRencana('"+aData['kd_sablon']+"') title='Hapus Rencana'>Hapus Rencana</button>");
						}
            if (aData["prioritas"]=="URGENT") {
    ...
    <script type="text/javascript">
      function saveHasilSablon(){
        ...
        var sisa        				= jml_sisa - hasil_lembar;
				var berat_pengambilan 	= $("#jumlahBeratPengambilan").val();
				var lembar_pengambilan 	= $("#jumlahLembarPengambilan").val();
				var berat_kembalian 		= $("#jumlahBeratPengembalian").val();
				var lembar_kembalian 		= $("#jumlahLembarPengembalian").val();
        ...
        $.ajax({
          ...
          data : {
						...
						w_plastik 					: w_plastik,
						w_sablon 						: w_sablon,
						hasil_lembar 				: hasil_lembar,
						hasil_berat 				: hasil_berat,
						...
						sisa_cat 						: sisa_cat,
						minyak 							: minyak,
						jum_minyak 					: jum_minyak,
						sisa_minyak 				: sisa_minyak,
						sisa 								: sisa,
						totHasil 						: totHasil,
						satuan_cat 					: satuan_cat,
						satuan_sisaCat 			: satuan_sisaCat,
						satuan_minyak 			: satuan_minyak,
						satuan_sisaMinyak 	: satuan_sisaMinyak,
						berat_pengambilan		: berat_pengambilan,
						lembar_pengambilan	: lembar_pengambilan,
						berat_kembalian			: berat_kembalian,
						lembar_kembalian		: lembar_kembalian
					},
          success: function(response){
            if (jQuery.trim(response) == "Success") {
              datatableRencanaSablon();
              ...
              setTimeout(function(){
                ...
                $("#modalNotifContent").text("");
							...
							$("#modal-notif").modal("show")
              $("#modalNotifContent").html("<div style='text-align: center;'><b>Bulan Tesebut Sudah Dikunci</b></div>");
              $("#modal-notif").addClass("modal-danger");
              setTimeout(function(){
                $("#modal-notif").modal("hide");
              },2000);
      ...
      function datatableHasilSablonPerTgl(tgl1,tgl2){
        ...
        $("#tableDataHasilSablonPerTgl").dataTable({
          ...
          "fnRowCallback" : function(nRow,aData,iDisplayIndex,iDisplayIndexFull){
            ...
            $("td:eq(0)",nRow).text(++iDisplayIndex);
						$("td:eq(7)",nRow).text(parseFloat(aData["hasil_lembar"]).toLocaleString());
						$("td:eq(8)",nRow).text(parseFloat(aData["hasil_berat"]).toLocaleString());
            if (aData['status_bon'] == "FALSE") {
              $("td:eq(10)",nRow).html("<button class='btn btn-sm btn-flat btn-primary' onclick=detailHasilSablon('"+aData['kd_hasil_sablon']+"') title='Detail Hasil'><i class='fa fa-navicon '> Detail</i></button>" +
																			 "<button class='btn btn-sm btn-flat btn-danger' onclick=deleteHasilSablon('"+aData['kd_hasil_sablon']+"','"+aData['kd_sablon']+"') title='Hapus Hasil'><i class='fa fa-trash'> Hapus</i></button>" +
																		 	 "<button class='btn btn-sm btn-flat btn-warning' onclick=modalUbahHasilSablon('"+aData['kd_hasil_sablon']+"','"+aData['kd_sablon']+"') title='Ubah Hasil'><i class='fa fa-pencil'> Ubah</i></button>");
            }else{
        ...
        function edit_detailBahanSablon(id)
        {
          $.ajax({
            ...
            success: function(response)
            {
              ...
              $("#id_penggunaan_sablon").val(response[0].id_penggunaan_sablon);
              $("#kd_hasil_sablon").val(response[0].kd_hasil_sablon);
              $("#jum_pemakaian").val(response[0].jumlah_pengambilan);
              $("#sisa_pemakaian").val(response[0].sisa_pengambilan);
        ...
        function cariRencanaPIC()
        {
          ...
          if (!tgl_awal || !tgl_akhir) {
            ...
            $("#modal_cariRencanaPIC").modal("hide");
				...
				function modalUbahHasilSablon(param, param2){
					$("#cmbJenisBahan").val("").change();

					$("#cmbJenisBahan").on("change",function(){
						var jenisBarang = $("#cmbJenisBahan").val();
						if(jenisBarang=="MINYAK"){
							var url = "<?php echo base_url(); ?>_sablon/main/getMinyak";
						}else if(jenisBarang=="CAT MURNI"){
							var url = "<?php echo base_url(); ?>_sablon/main/getCatMurni";
						}else{
							var url="";
						}

						if(url != ""){
							$("#cmbNamaBahan").select2({
								placeholder : "Pilih "+jenisBarang,
								dropdownParent : $("#modalEditHasilSablon"),
								width : "100%",
								cache:true,
								allowClear:true,
								ajax:{
					...
					$("#btnTambahPenggunaanBahan").html("<i class='fa fa-plus'></i> Tambah")
																				.removeClass("btn-warning")
																				.addClass("btn-primary")
																				.attr("onclick","tambahBahanBaru('"+param+"','"+param2+"')");
					$("#btnBatalTambahBahan").attr("onclick","modalUbahHasilSablon('"+param+"','"+param2+"')");
					$("#txtJumlahBahan").val("0");
					$("#txtSisaBahan").val("0");
					$.ajax({
						type : "POST",
						url : "<?= base_url('_sablon/main/getDataHasilSablonEdit'); ?>",
						dataType : "JSON",
						data : {
							idTransaksi : param,
							kdSablon : param2
						...
						success : function(response){
							$("#tableDetailRencana > tbody").empty();
							$("#tableJenisBahan > tbody").empty();

							$.each(response.DetailRencana, function(AvIndex, AvValue){
								$("#tableDetailRencana > tbody:last-child").append(
									"<tr>"+
										"<td>"+ ++AvIndex +"</td>"+
										...
										"<td>"+parseFloat(AvValue.jml_rencana).toLocaleString()+"</td>"+
										"<td>"+parseFloat(AvValue.satuan_kilo).toLocaleString()+"</td>"+
									"</tr>"
								);
							});

							$("#kd_hasil").val(response.TransaksiHasilSablon[0].kd_hasil_sablon);
							$("#hasil_lembar").val(parseFloat(response.TransaksiHasilSablon[0].hasil_lembar));
							$("#hasil_berat").val(parseFloat(response.TransaksiHasilSablon[0].hasil_berat));
							$("#jenis_barang").val(response.TransaksiHasilSablon[0].jns_brg);
							$("#tgl_pengerjaan").val(response.TransaksiHasilSablon[0].tanggal);
							$("#keterangan").val(response.TransaksiHasilSablon[0].keterangan);
							$("#jumlahBeratPengambilan").val(parseFloat(response.TransaksiHasilSablon[0].berat_pengambilan));
							$("#jumlahLembarPengambilan").val(parseFloat(response.TransaksiHasilSablon[0].lembar_pengambilan));
							$("#jumlahBeratPengembalian").val(parseFloat(response.TransaksiHasilSablon[0].berat_kembalian));
							$("#jumlahLembarPengembalian").val(parseFloat(response.TransaksiHasilSablon[0].lembar_kembalian));

							$.each(response.TransaksiPenggunaanBahanSablon, function(AvIndex, AvValue){
								$("#tableJenisBahan tbody:last-child").append(
									"<tr>"+
										"<td>"+ ++AvIndex+"</td>"+
										"<td>"+AvValue.jenis+"</td>"+
										...
										"<td>"+
											"<button class='btn btn-sm btn-flat btn-warning' onclick=modalUbahPemakaianBahan('"+AvValue.id_penggunaan_sablon+"')><i class='fa fa-pencil'></i> Ubah</button>"+
											"<button class='btn btn-sm btn-flat btn-danger' onclick=hapusPemakaianBahan('"+AvValue.id_penggunaan_sablon+"')><i class='fa fa-trash'></i> Hapus</button>"+
										"</td>"+
									"</tr>"
								);
							});

							$("#modalEditHasilSablon").modal({
								backdrop : "static"
							})
						},
				...
				function tambahBahanBaru(param, param2){
					var kdHasilSablon = param;
					var kdGdBahan = $("#cmbNamaBahan").val();
					var satuan = $("#cmbSatuan").val();
					if(satuan.toUpperCase() == "ONS"){
						var jumlahBahan = parseFloat($("#txtJumlahBahan").val()) * 0.1;
						var sisaBahan = parseFloat($("#txtSisaBahan").val()) * 0.1;
					}else{
						var jumlahBahan = parseFloat($("#txtJumlahBahan").val());
						var sisaBahan = parseFloat($("#txtSisaBahan").val());
					}

					if(kdHasilSablon=="" || kdGdBahan=="" || satuan=="" ||
						 jumlahBahan < 0 || sisaBahan < 0){
					 $("#modal-notif").modal("show")
					 $("#modalNotifContent").html("<div style='text-align: center;'><b>Semua Kolom Tidak Boleh Kosong!</b></div>");
					 $("#modal-notif").addClass("modal-warning");
					 setTimeout(function(){
					 	$("#modal-notif").modal("hide");
					 },2000);
					 setTimeout(function(){
						...
						$.ajax({
							type : "POST",
							url : "<?= base_url('_sablon/main/tambahBahanBaru'); ?>",
							dataType : "TEXT",
							data : {
								kdHasilSablon : kdHasilSablon,
								kdGdBahan 		: kdGdBahan,
							...
							success : function(response){
								if($.trim(response) === "Berhasil"){
									$("#modal-notif").modal("show")
			            $("#modalNotifContent").html("<div style='text-align: center;'><b>Bahan Berhasil Ditambahkan</b></div>");
			            $("#modal-notif").addClass("modal-info");
			            setTimeout(function(){
			              $("#modal-notif").modal("hide");
										modalUbahHasilSablon(param, param2);
			            },2000);
			            setTimeout(function(){
			              $("#modal-notif").removeClass("modal-info");
			              $("#modalNotifContent").text("");
									...
									$("#modal-notif").modal("show")
			            $("#modalNotifContent").html("<div style='text-align: center;'><b>Bahan Gagal Ditambahkan</b></div>");
			            $("#modal-notif").addClass("modal-danger");
			            setTimeout(function(){
			              $("#modal-notif").modal("hide");
			            },2000);
				...
				function ubahHasilSablon(){
					var kdHasil = $("#kd_hasil").val();
					var hasilLembar = $("#hasil_lembar").val();
					var hasilBerat = $("#hasil_berat").val();
					var jenisBarang = $("#jenis_barang").val();
					var tglPengerjaan = $("#tgl_pengerjaan").val();
					var keterangan = $("#keterangan").val();
					var jumlahBeratPengambilan = $("#jumlahBeratPengambilan").val();
					var jumlahLembarPengambilan = $("#jumlahLembarPengambilan").val();
					var jumlahBeratPengembalian = $("#jumlahBeratPengembalian").val();
					var jumlahLembarPengembalian = $("#jumlahLembarPengembalian").val();

					if(confirm("Apakah anda yakin sudah tidak ada yang salah?")){
						$.ajax({
							type : "POST",
							url : "<?= base_url("_sablon/main/ubahHasilSablon"); ?>",
							dataType : "TEXT",
							data : {
								kdHasil 									: kdHasil,
								hasilLembar 							: hasilLembar,
								hasilBerat 								: hasilBerat,
								jenisBarang 							: jenisBarang,
								tglPengerjaan 						: tglPengerjaan,
								keterangan 								: keterangan,
								jumlahBeratPengambilan 		: jumlahBeratPengambilan,
								jumlahLembarPengambilan 	: jumlahLembarPengambilan,
								jumlahBeratPengembalian 	: jumlahBeratPengembalian,
								jumlahLembarPengembalian 	: jumlahLembarPengembalian
							},
							success : function(response){
								if($.trim(response) === "Berhasil"){
									$("#modal-notif").modal("show")
			            $("#modalNotifContent").html("<div style='text-align: center;'><b>Bahan Berhasil Diubah</b></div>");
			            $("#modal-notif").addClass("modal-info");
			            setTimeout(function(){
			              $("#modal-notif").modal("hide");
										$("#modalEditHasilSablon").modal("hide");
										$(".active a[aria-controls='tableDataHasilSablonPerTgl']").click();
			            },2000);
			            setTimeout(function(){
			              $("#modal-notif").removeClass("modal-info");
			              $("#modalNotifContent").text("");
									...
									$("#modal-notif").modal("show")
			            $("#modalNotifContent").html("<div style='text-align: center;'><b>Bahan Gagal Diubah</b></div>");
			            $("#modal-notif").addClass("modal-danger");
			            setTimeout(function(){
			              $("#modal-notif").modal("hide");
			            },2000);
				...
				function modalUbahPemakaianBahan(param){
					$.ajax({
						type : "POST",
						url : "<?= base_url('_sablon/main/getDetailPemakaianBahan'); ?>",
						dataType : "JSON",
						data : {
							idTransaksi : param
						},
						success : function(response){
							$.each(response, function(AvIndex, AvValue){
								$("#cmbJenisBahan").val(AvValue.jenis).change();
								$("#cmbNamaBahan").val(AvValue.kd_gd_bahan);
								$("#txtJumlahBahan").val(AvValue.jumlah_pengambilan);
								$("#txtSisaBahan").val(AvValue.sisa_pengambilan);
								$("#btnTambahPenggunaanBahan").html("<i class='fa fa-pencil'></i> Ubah")
																							.removeClass("btn-primary")
																							.addClass("btn-warning")
																							.attr("onclick","ubahPemakaianBahan('"+AvValue.id_penggunaan_sablon+"')");
							});
						},
						error : function(response){
							$("#modal-notif").addClass("modal-danger");
				...
				function ubahPemakaianBahan(param){
					var kdGdBahan = $("#cmbNamaBahan").val();
					var satuan = $("#cmbSatuan").val();
					if(satuan.toUpperCase() == "ONS"){
						var jumlahBahan = parseFloat($("#txtJumlahBahan").val()) * 0.1;
					...
					if(kdGdBahan=="" || satuan=="" ||
					 ...
					 setTimeout(function(){
						...
						$.ajax({
							type : "POST",
							url : "<?= base_url('_sablon/main/ubahPemakaianBahan'); ?>",
							dataType : "TEXT",
							data : {
								idTransaksi 	: param,
								kdGdBahan 		: kdGdBahan,
				...
				function hapusPemakaianBahan(param){
					if(confirm("Apakah anda yakin ingin menghapus data tersebut?")){
						$.ajax({
							type : "POST",
							url : "<?= base_url('_sablon/main/deletePenggunaanBahan'); ?>",
							dataType : "TEXT",
							data : {
								idTransaksi : param
							},
							success : function(response){
								if($.trim(response) === "Berhasil"){
									$("#modal-notif").modal("show")
			            $("#modalNotifContent").html("<div style='text-align: center;'><b>Bahan Berhasil Dihapus</b></div>");
			            $("#modal-notif").addClass("modal-info");
			            setTimeout(function(){
			              $("#modal-notif").modal("hide");
										$("#btnBatalTambahBahan").click();
			            ...
			            setTimeout(function(){
			              ...
			              $("#modalNotifContent").text("");
									...
									$("#modal-notif").modal("show")
			            $("#modalNotifContent").html("<div style='text-align: center;'><b>Bahan Gagal Dihapus</b></div>");
			            $("#modal-notif").addClass("modal-danger");
			            setTimeout(function(){
			              $("#modal-notif").modal("hide");
			            },2000);
