...
class Main extends MX_Controller{
 	...
 	public function checkStatus(){
		...
		$content= "	<div class='table-responsive'>
			        ...
			        <div class='form-group col-md-6'>
			        	...
			        	<input type='number' class='form-control' placeholder='Jumlah Rencana' name='jml_rencana' id='jml_rencana' required>
							...
							<div class='form-group col-md-6'>
								<label style='text-align:left;'>Pilih Bahan</label>
								<select class='form-control' id='cmbGudangBuffer'></select>
							</div>
  	...
  	function addRencana()
  	{
	    ...
	    if($isLogin){
			...
			$data['kd_gd_buffer']	= $this->input->post("kd_gd_buffer");
  	...
  	function inputHasil()
  	{
  		...
  		$kd_hasilSablon = $this->Sablon_Model->codeHasilSablon();
			...
			$data = array("kd_gd_hasil" => $arrResult[0]["kd_gd_hasil"],
										"kd_gd_buffer" => $arrResult[0]["kd_gd_buffer"],
										"tgl_rencana" => $arrResult[0]["tgl_rencana"]);
			$arrData = $this->Sablon_Model->selectPengambilanGudangBuffer($data);
			    ...
			    <div class='box box-success'>
		            ...
		            <div class='box-body'>
				        ...
				        <div class='form-group col-md-4'>
				        	...
				        	<input type='number' class='form-control' value='".$arrData[0]["jumlah_berat"]."' placeholder='Jumlah Berat Pengambilan Pastik Polos' name='jumlahBeratPengambilan' id='jumlahBeratPengambilan' required>
				        </div>
								<div class='form-group col-md-4'>
				        	<label>Jumlah Lembar Pengambilan</label>
				        	<input type='number' class='form-control' value='".$arrData[0]["jumlah_lembar"]."' placeholder='Jumlah Lembar Pengambilan Pastik Polos' name='jumlahLembarPengambilan' id='jumlahLembarPengambilan' required>
				        </div>
  	...
  	function saveHasilSablon()
  	{
		...
		$hasil["berat_pengambilan"]	=	$this->input->post("berat_pengambilan");
		$hasil["lembar_pengambilan"]	=	$this->input->post("lembar_pengambilan");
		$hasil["berat_kembalian"]	=	$this->input->post("berat_kembalian");
		$hasil["lembar_kembalian"]	=	$this->input->post("lembar_kembalian");
	...
	public function getGudangBuffer($param, $param2){
		$isLogin = $this->isLogin();
    if($isLogin){
			$key = $this->input->get("q");
			$data = array("ukuran" => str_replace("_","/",$param),
										"warna_plastik" => str_replace("%20"," ",$param2),
										"key" => $key);
			$result = $this->Sablon_Model->selectGudangBuffer($data);
			echo $result;
    }else{
         echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
         redirect("_main/main","refresh");
	...
	public function hapusRencanaMandor(){
		$isLogin = $this->isLogin();
    if($isLogin){
			$kdSablon = $this->input->post("kdSablon");
			$deleted = $this->input->post('deleted');
			$data = array("kdSablon" => $kdSablon,
										"deleted" => $deleted);
			$result = $this->Sablon_Model->updateDeleteAndRestoreRencanaMandor($data);
			echo $result;
		}else{
			echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
			redirect("_main/main","refresh");
	...
	public function getDataHasilSablonEdit(){
		$isLogin = $this->isLogin();
    if($isLogin){
			$idTransaksi = $this->input->post("idTransaksi");
			$kdSablon = $this->input->post("kdSablon");

			$data = array("kdHasilSablon" => $idTransaksi,
										"kdSablon" => $kdSablon);

			$result = $this->Sablon_Model->selectDataHasilSablon($data);
			echo $result;
		}else{
			echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
			redirect("_main/main","refresh");
	...
	public function tambahBahanBaru(){
		$isLogin = $this->isLogin();
    if($isLogin){
			$kdHasilSablon = $this->input->post("kdHasilSablon");
			$kdGdBahan = $this->input->post("kdGdBahan");
			$jumlahBahan = $this->input->post("jumlahBahan");
			$sisaBahan = $this->input->post("sisaBahan");

			if($kdHasilSablon=="" || $kdGdBahan=="" || $jumlahBahan=="" || $sisaBahan==""){
				echo "Data Kosong";
			}else{
				$data = array("kd_hasil_sablon" 		=> $kdHasilSablon,
											"kd_gd_bahan" 				=> $kdGdBahan,
											"jumlah_pengambilan" 	=> $jumlahBahan,
											"sisa_pengambilan" 		=> $sisaBahan);

				$result = $this->Sablon_Model->tambahBarangBaru($data);
				echo $result;
			}
		}else{
			echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
	...
	public function ubahHasilSablon(){
		$isLogin = $this->isLogin();
    if($isLogin){
			$kdHasil 									= $this->input->post("kdHasil");
			$hasilLembar 							= $this->input->post("hasilLembar");
			$hasilBerat 							= $this->input->post("hasilBerat");
			$jenisBarang 							= $this->input->post("jenisBarang");
			$tglPengerjaan 						= $this->input->post("tglPengerjaan");
			$keterangan 							= $this->input->post("keterangan");
			$jumlahBeratPengambilan 	= $this->input->post("jumlahBeratPengambilan");
			$jumlahLembarPengambilan 	= $this->input->post("jumlahLembarPengambilan");
			$jumlahBeratPengembalian 	= $this->input->post("jumlahBeratPengembalian");
			$jumlahLembarPengembalian = $this->input->post("jumlahLembarPengembalian");

			if(empty($kdHasil) || empty($hasilLembar) || empty($hasilBerat) ||
				 empty($jenisBarang) || empty($tglPengerjaan) || empty($keterangan)){
				$result = "Data Kosong";
			}else{
				$data = array("kd_hasil_sablon" 		=> $kdHasil,
											"hasil_lembar" 				=> $hasilLembar,
											"hasil_berat" 				=> $hasilBerat,
											"jns_brg" 						=> $jenisBarang,
											"tanggal" 						=> $tglPengerjaan,
											"keterangan" 					=> $keterangan,
											"berat_pengambilan" 	=> $jumlahBeratPengambilan,
											"lembar_pengambilan" 	=> $jumlahLembarPengambilan,
											"berat_kembalian" 		=> $jumlahBeratPengembalian,
											"lembar_kembalian" 		=> $jumlahLembarPengembalian);
				$result = $this->Sablon_Model->ubahHasilSablon($data);
			}
			echo $result;
		}else{
			echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
	...
	public function getDetailPemakaianBahan(){
		$isLogin = $this->isLogin();
		if($isLogin){
			$idTransaksi = $this->input->post("idTransaksi");
			$result = $this->Sablon_Model->getDataPenggunaanBahanSablonById($idTransaksi);
			echo $result;
		}else{
			echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
			redirect("_main/main","refresh");
	...
	public function ubahPemakaianBahan(){
		$isLogin = $this->isLogin();
		if($isLogin){
			$idTransaksi = $this->input->post("idTransaksi");
			$kdGdBahan = $this->input->post("kdGdBahan");
			$jumlahBahan = $this->input->post("jumlahBahan");
			$sisaBahan = $this->input->post("sisaBahan");
			$data = array("id_penggunaan_sablon" 	=> $idTransaksi,
										"kd_gd_bahan" 					=> $kdGdBahan,
										"jumlah_pengambilan" 		=> $jumlahBahan,
										"sisa_pengambilan" 			=> $sisaBahan);
			$result = $this->Sablon_Model->ubahPemakaianBahan($data);
			echo $result;
		}else{
			echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
			redirect("_main/main","refresh");
	...
	public function deletePenggunaanBahan(){
		$isLogin = $this->isLogin();
		if($isLogin){
			$idTransaksi = $this->input->post("idTransaksi");
			$result = $this->Sablon_Model->deletePenggunaanBahan($idTransaksi);
			echo $result;
		}else{
			echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
			redirect("_main/main","refresh");
