...
<!--===============================================On Load Function (Start) ===============================================-->
      <script type="text/javascript">
        $(function () {
          ...
          orderMarketingDeadlineCounter("SKV");
          ...
          if($("#tableListDataOrderMarketing_SKV").length > 0){
            datatableDataOrderMarketing("SKV");
          }
...
//================================ MODAL METHOD (START) ============================//
        ...
        function modalEditPengirimanParsial(param){
          $.ajax({
            type : "POST",
            url : "<?= base_url('_pengiriman/main/getDetailPengirimanParsialItem'); ?>",
            dataType : "JSON",
            data : {
              id : param
            },
            success : function(response){
              $("#tgl_kirim").val(response[0].tanggal_kirim);
              $("#jml_kirim").val(response[0].jumlah_kirim);
              $("#btnKirimParsial").attr("onclick","editKirimBarangParsial('"+param+"')")
...
//================================ EDIT METHOD (START) ============================//
        ...
        function editKirimBarangParsial(param){
          var tgl = $("#tgl_kirim").val();
          var jml = $("#jml_kirim").val().replace(/,/gi,"");
          var id = param;
          var idDP = $("#idDp").val();

          $.ajax({
            type : "POST",
            url : "<?= base_url('_pengiriman/main/editKirimBarangParsial') ?>",
            dataType : "TEXT",
            data : {
              id : id,
              tgl : tgl,
...
//================================ REMOVE METHOD (START) ============================//
        ...
        function hapusPengirimanParsial(param, param2){
          if(confirm("Apakah anda yakin ingin menghapus data tersebut?")){
            $.ajax({
              type : "POST",
              url : "<?= base_url('_pengiriman/main/deletePengirimanParsial'); ?>",
              dataType : "TEXT",
              data : {
                id : param,
                idDP : param2
...
//================================ UNSPECIFIED METHOD (START) ============================//
        ...
        function orderMarketingDeadlineCounter(param){
          if (param=="DK") {
            ...
            var objId = "#MI_OrderKanvasDanOnline";
            var text = "Order Kanvas Dan Online";
          }
          $.ajax({
            type : "POST",
...
//================================ DATATABLE METHOD (START) ============================//
              ...
              if(AvData["sts_pesanan"] == "PACKING"){
                // if (status == "2"){
                  $("td:eq(11)", AvRow).html("<button class='btn btn-md btn-flat btn-primary' style='float:left; width:130px; margin-right:5px;' onclick=modalKirimParsial('"+AvData["id_dp"]+"')><i class='fa fa-send'></i> Kirim</button>"+
                                             "<button class='btn btn-md btn-flat btn-success' style='float:left; width:130px; margin-right:5px;' onclick=closePesanan('"+AvData["id_dp"]+"','"+AvData["no_order"]+"')><i class='fa fa-check'></i> Close</button>"+
                                             "<input type='checkbox' value='"+AvData["id_dp"]+"#"+AvData["no_order"]+"' class='minimal' name='pilihanItem' style='float:left;'>");
                // }else if (status == "1"){
                ...
                //                            "<input type='checkbox' value='"+AvData["id_dp"]+"#"+AvData["no_order"]+"' class='minimal' name='pilihanItem' style='float:left;'>");
                // }
              }else if(AvData["sts_pesanan"] == "PROGRESS" && parseFloat(AvData["jumlah_kirim"]) > 0){
                // if (status == "2"){
                //   $("td:eq(11)", AvRow).html("<button class='btn btn-md btn-flat btn-primary' style='float:left; width:130px; margin-right:5px;' onclick=modalKirimParsial('"+AvData["id_dp"]+"')><i class='fa fa-send'></i> Kirim</button>");
                // }else if (status == "1"){
                  $("td:eq(11)", AvRow).html("<button class='btn btn-md btn-flat btn-warning' onclick=modalKirimParsial('"+AvData["id_dp"]+"')><i class='fa fa-send'></i> Kirim Setengah</button>"+
        ...
        function modalKirimParsial(id)
        {
          ...
          $("#btnKirimParsial").attr("onclick","kirimBarangParsial()")
                               .removeClass("btn-warning").addClass("btn-primary")
                               .html("<i class='fa fa-send'></i> Kirim");
        }
              ...
              $.each(response, function(Index, Value){
                $("#tableListOrder tbody:last-child").append(
                  "<tr>"+
                    ...
                    "<td>"+parseFloat(Value.jumlah).toLocaleString()+" "+Value.satuan+"</td>"+
                    "<td>"+parseFloat(Value.jumlah_terkirim).toLocaleString()+" "+Value.satuan+"</td>"+
                    "<td>"+parseFloat(Value.jumlah-Value.jumlah_terkirim).toLocaleString()+" "+Value.satuan+"</td>"+
                  "</tr>"
                );
              });
              ...
              $.each(response, function(Index, Value){
                $("#tableListDetailPengirimanParsial tbody:last-child").append(
                  "<tr>"+
                    ...
                    "<td>"+parseFloat(Value.jumlah_kirim).toLocaleString()+"</td>"+
                    "<td><button class='btn btn-flat btn-primary btn-sm' style='width:80px' onclick=modalEditPengirimanParsial('"+Value.id_pengiriman+"')>Edit</button>"+
                    "<button class='btn btn-flat btn-danger btn-sm' style='width:80px' onclick=hapusPengirimanParsial('"+Value.id_pengiriman+"','"+Value.id_dp+"')>Hapus</button></td>"+
                  "</tr>"
                );
              });
              $("#idDp").val(id)
