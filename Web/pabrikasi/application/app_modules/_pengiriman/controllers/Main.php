...
class Main extends MX_Controller{
    ...
    if($isLogin){
      ...
      if($status==1){
        ...
        $listMenu["childMenu"] = array(
          ...
          array("Content" => "Order Kanvas Dan Online", "Link"=>"_pengiriman/main/order_kanvas_dan_online", "Parent"=>"Order", "Id"=>"MI_OrderKanvasDanOnline"),
  ...
  public function order_kanvas_dan_online(){
    $isLogin = $this->isLogin();
    if($isLogin){
      $data["Data"] = array("Title"=>"Order Sales Kanvas Dan Online");
      $data["Link"] = array("Segment1" =>$this->uri->rsegment(1),"Segment2"=>$this->uri->rsegment(2));
      $this->load->view("header");
      $this->load->view("sidebar",$this->checkStatus());
      $this->load->view("frm_order_kanvas_dan_online",$data);
      $this->load->view("footer");
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
      redirect("_main/main","refresh");
  ...
  public function getDetailPengirimanParsialItem(){
    $isLogin = $this->isLogin();
    if($isLogin){
      $id_dp = $this->input->post("id");
      $result = $this->Pengiriman_Model->selectDetailPengirimanParsialLK($id_dp);
      echo $result;
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
      redirect("_main/main","refresh");
  ...
  public function editKirimBarangParsial(){
    $isLogin = $this->isLogin();
    if($isLogin){
      $id = $this->input->post("id");
      ...
      if($tgl == "" || $jml == ""){
        ...
        $data = array("id_pengiriman" => $id,
                      ...
                      "jumlah_kirim" => $jml,
                      "id_dp" => $idDP);
        $result = $this->Pengiriman_Model->updateKirimBarangParsial($data);
        echo $result;
      }
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
  ...
  public function deletePengirimanParsial(){
    $isLogin = $this->isLogin();
    if($isLogin){
      $id = $this->input->post("id");
      $idDP = $this->input->post("idDP");
      $data = array("id_pengiriman" => $id,
                    "id_dp" => $idDP);
      $result = $this->Pengiriman_Model->deletePengirimanParsial($data);
      echo $result;
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
      redirect("_main/main","refresh");
