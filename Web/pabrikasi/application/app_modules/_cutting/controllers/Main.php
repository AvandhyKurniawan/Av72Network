...
class Main extends MX_Controller{
    ...
    if($isLogin){
      ...
      if($status==1){
        $listMenu["parentMenu"] = array(
          ...
          array("Content"=>"Laporan Bon Hasil Global","Link"=>"_cutting/main/laporan_bon_hasil_global","Name"=>"Laporan_Bon_Hasil_Global","Status"=>"Single","Icon"=>"fa fa-book","Id"=>"M_Laporan_Bon_Hasil_Global"),
          array("Content"=>"Laporan Bon Apal Global","Link"=>"_cutting/main/laporan_bon_apal_global","Name"=>"Laporan_Bon_Apal_Global","Status"=>"Single","Icon"=>"fa fa-book","Id"=>"M_Laporan_Bon_Apal_Global"),
          array("Content"=>"Laporan Apal Per Merek","Link"=>"_cutting/main/laporan_apal_per_merek","Name"=>"Laporan_Apal_Per_Merek","Status"=>"Single","Icon"=>"fa fa-book","Id"=>"M_Laporan_Apal_Per_Merek")
        );
        return $listMenu;
      }else if($status==2) {
        $listMenu["parentMenu"] = array();
  ...
  public function laporan_bon_hasil_global(){
    $isLogin = $this->isLogin();
    if($isLogin){
      $data["Data"] = array("Title"=>"Laporan Bon Hasil Global");
      $data["Link"] = array("Segment1" =>$this->uri->rsegment(1),"Segment2"=>$this->uri->rsegment(2));
      $this->load->view("header");
      $this->load->view("sidebar",$this->checkStatus());
      $this->load->view("frm_laporan_hasil_cutting_global",$data);
      $this->load->view("footer");
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
      redirect("_main/main","refresh");
  ...
  public function laporan_bon_apal_global(){
    $isLogin = $this->isLogin();
    if($isLogin){
      $data["Data"] = array("Title"=>"Laporan Bon Apal Global");
      $data["Link"] = array("Segment1" =>$this->uri->rsegment(1),"Segment2"=>$this->uri->rsegment(2));
      $this->load->view("header");
      $this->load->view("sidebar",$this->checkStatus());
      $this->load->view("frm_laporan_bon_apal_global",$data);
      $this->load->view("footer");
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
      redirect("_main/main","refresh");
  ...
  public function laporan_apal_per_merek(){
    $isLogin = $this->isLogin();
    if($isLogin){
      $data["Data"] = array("Title"=>"Laporan Apal Per Merek");
      $data["Link"] = array("Segment1" =>$this->uri->rsegment(1),"Segment2"=>$this->uri->rsegment(2));
      $this->load->view("header");
      $this->load->view("sidebar",$this->checkStatus());
      $this->load->view("frm_laporan_apal_per_merek",$data);
      $this->load->view("footer");
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
      redirect("_main/main","refresh");
    ...
    if($isLogin){
      ...
      if(empty($Key)){
        $data = array("JnsPermintaan" => $param,
                      "panjangPlastik" => str_replace("_"," ",$param2),
                      "merek" => str_replace("_"," ",$param3),
                      "warnaPlastik" => str_replace("_"," ",$param4));
  ...
  public function getComboBoxValueGudangRollAll(){
    $isLogin = $this->isLogin();
    if($isLogin){
      $Key = $this->input->get("q");
      $result = $this->Cutting_Model->selectComboBoxValueGudangRollAll($Key);
      echo $result;
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
      redirect("_main/main","refresh");
  ...
  public function getDataInputHasilTumpuk(){
    $isLogin = $this->isLogin();
    if($isLogin){
      $panjangPlastik = $this->input->post("panjangPlastik");
      $warnaPlastik = $this->input->post("warnaPlastik");
      ...
      $data = array("panjangPlastik" => $panjangPlastik,
                    ...
                    "kdGdRoll" => $kdGdRoll,
                    "tglRencana" => $tglRencana,
                    "kdPotong" => $kodeRencana);
      $result = $this->Cutting_Model->selectDataRencanaForInputHasilTumpuk($data);
      echo $result;
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
      redirect("_main/main","refresh");
      ...
      if( $idTransaksi == ""                   || $idUser == ""                        || $tglRencana == ""                    ||
          ...
          if(strpos(strtoupper($ketBarang),"TUMPUK") !== FALSE || strpos(strtoupper($ketMerek),"TUMPUK") !== FALSE){
            ...
            $data["TGR"] = array(array("kd_gd_roll" => $data["TSHP"][0]["kd_gd_roll"],
                                       "id_user" => $idUser,
                                       "jns_permintaan" => (strpos($data["TSHP"][0]["kd_gd_roll"],"RLC") !== FALSE ? "CETAK" : "POLOS"),
                                 ...
                                 array("kd_gd_roll" => $kdGdRollTumpuk,
                                       "id_user" => $idUser,
                                       "jns_permintaan" => (strpos($kdGdRollTumpuk,"RLC") !== FALSE ? "CETAK" : "POLOS"),
                                       "tgl_transaksi" => $tglRencana,
                                       "bagian" => "POTONG",
                                       "keterangan_transaksi" => "KELUAR KE POTONG",
                                       "status_history" => "KELUAR",
  ...
  public function getLaporanHasilPotongGlobalPeriode(){
    $isLogin = $this->isLogin();
    if($isLogin){
      $tglAwal = $this->input->post("tglAwal");
      $tglAkhir = $this->input->post("tglAkhir");
      $data = array("tglAwal" => $tglAwal,
                    "tglAkhir" => $tglAkhir);
      $result = $this->Cutting_Model->selectLaporanHasilPotongGlobalPeriode($data);
      echo $result;
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
      redirect("_main/main","refresh");
  ...
  public function getLaporanBonApalGlobalPeriode(){
    $isLogin = $this->isLogin();
    if($isLogin){
      $tglAwal = $this->input->post("tglAwal");
      $tglAkhir = $this->input->post("tglAkhir");
      $data = array("tglAwal" => $tglAwal,
                    "tglAkhir" => $tglAkhir);
      $result = $this->Cutting_Model->selectLaporanBonApalGlobalPeriode($data);
      echo $result;
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
      redirect("_main/main","refresh");
  ...
  public function getLaporanApalPerMerek(){
    $isLogin = $this->isLogin();
    if($isLogin){
      $tglAwal = $this->input->post("tglAwal");
      $tglAkhir = $this->input->post("tglAkhir");
      ...
      $data = array("tglAwal" => $tglAwal,
                    "tglAkhir" => $tglAkhir,
                    "kdGdRoll" => $kdGdRoll);
      $result = $this->Cutting_Model->selectLaporanApalPerMerek($data);
      echo $result;
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
      redirect("_main/main","refresh");
  ...
  public function deleteSisaPengambilanPotongTemp(){
    $isLogin = $this->isLogin();
    if($isLogin){
      $idTBPR = $this->input->post("idTBPR");
      if(empty($idTBPR)){
        echo "Data Kosong";
      }else{
        $result = $this->Cutting_Model->updateDeleteSisaPengambilanPotongTemp($idTBPR);
        echo $result;
      }
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
  ...
  public function saveCopyRencana(){
    $isLogin = $this->isLogin();
    if($isLogin){
      $kdPotongSrc  = $this->input->post("kdPotongSrc");
      $kdPotongDst  = $this->input->post("kdPotongDst");
      $tglRencana   = $this->input->post("tglRencana");
      $jumlahMesin  = $this->input->post("jumlahMesin");
      $noMesin      = $this->input->post("noMesin");

      if(empty($kdPotongSrc) || empty($kdPotongDst) ||
         empty($tglRencana) || empty($jumlahMesin) || empty($noMesin)){
        echo "Data Kosong";
      }else{
        $arrData = array("kdPotongSrc"  => $kdPotongSrc,
                         "kdPotongDst"  => $kdPotongDst,
                         "tglRencana"   => $tglRencana,
                         "jumlahMesin"  => $jumlahMesin,
                         "noMesin"      => $noMesin);
        $result = $this->Cutting_Model->insertCopyRencana($arrData);
        echo $result;
      }
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
  ...
  public function updateDeleteHasilCutting(){
    $isLogin = $this->isLogin();
    if($isLogin){
      $kdHasilPotong = $this->input->post("kdHasilPotong");
      if(empty($kdHasilPotong)){
        echo "Data Kosong";
      }else{
        $result = $this->Cutting_Model->updateDeleteHasilCutting($kdHasilPotong);
        if($result){
          echo "Berhasil";
        }else{
          echo "Gagal";
  ...
  public function updateSisaPengambilanPotongTemp(){
    $isLogin = $this->isLogin();
    if($isLogin){
      $id            = $this->input->post("id");
      ...
      if($berat=="" || $bobin=="" || $payung=="" || $payungKuning=="" ||
         empty($shift) || empty($keterangan) || empty($tglSisa) || empty($tglPotong)
      ){
        echo "Data Kosong";
      }else{
        ...
        if(empty($kode)){
          ...
          if($keterangan == "POTONG BESOK"){
            $data["TGR_IN"] = array("berat"           => $berat,
                                    "bobin"           => $bobin,
                                    "payung"          => $payung,
                                    "payung_kuning"   => $payungKuning,
                                    "shift"           => $shift,
                                    "tgl_transaksi"   => $tglSisa);

            $data["TGR_OUT"] = array("berat"           => $berat,
                                     "bobin"           => $bobin,
                                     "payung"          => $payung,
                                     "payung_kuning"   => $payungKuning,
                                     "shift"           => $shift,
          ...
          $kdGdRoll = $arrKode[1];
          $data = array("id"   => $id,
                        "TBPR" => array("kd_gd_roll"     => $kdGdRoll,
                                        "kd_potong"      => $kdPotong,
                                        "sisa"           => $berat,
                                        "bobin"          => $bobin,
                                        "payung"         => $payung,
                        ...
                        "TDPP" => array("kd_gd_roll"     => $kdGdRoll,
                                        "kd_potong"      => $kdPotong,
                                        "ukuran"         => $panjang,
                                        "panjang"        => $panjang,
                                        "merek"          => $merek,
        ...
        $result = $this->Cutting_Model->updateSisaPengambilanPotongTemp($data);
        echo $result;
      }
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
