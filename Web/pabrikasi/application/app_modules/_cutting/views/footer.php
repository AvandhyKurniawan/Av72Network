...
<!-- FastClick -->
		...
		<script type="text/javascript">
			...
			function modalBuatRencanaKerja(param, param2="", param3=""){
				...
				$("#lblInspector").hide();
				$("#btnTambahRencanaPotong").removeAttr("disabled");
				...
				$.ajax({
					...
					success : function(response){
						...
						var panjangPlastik = arrUkuranPlastik[0].replace(/ /gi,"_");
						...
						if(response[0].kd_gd_hasil.substring(0,4) == "HSLK"){
							var merek1 = "Kantong";
						}else{
							var merek1 = merek;
						}
		          ...
		          allowClear:true,
		          ajax:{
		            url : "<?php echo base_url(); ?>_cutting/main/getComboBoxValueGudangRoll/"+jnsPermintaan+"/"+panjangPlastik+"/"+merek1+"/"+warnaPlastik,
		            dataType : "JSON",
		            delay : 0,
		            processResults : function(data){
		              return{
						...
						$("#cmbKdGdRoll").on("select2:select",function(){
							var dataText = $(this).select2("data")[0]["text"];
							var arrDataText = dataText.split(" | ");
							var arrMerek = ["KLIP","KP","MP","ZIPPIN","PON","KANTONG","CB","KLIP IN",
					                    "KLIP PON","KLIPIN","KLIPPON","KLIPON","KLIP KLOP","KLIPKLOP","EXPORT"];
							var merek1 = response[0].merek.toUpperCase();
							var ukuran1 = response[0].ukuran.toUpperCase();
							var warnaPlastik1 = response[0].warna_plastik;
							var arrUkuran1 = ukuran1.split("X");
							var panjang1 = 0;
							if(merek1.indexOf("PON") != -1 || ukuran1.indexOf("PON") != -1){
								var arrPanjangPlastik1 = arrUkuran[0].split("+");
								if(arrPanjangPlastik1.length > 2){
									for (var i = 0; i < arrPanjangPlastik1.length; i++) {
										if(parseFloat(arrPanjangPlastik1[i]) != 1){
											panjang1 += parseFloat(arrPanjangPlastik1[i]);
										}
									}
									panjang1 = panjang1 + "+1";
								}
								...
								var arrPanjangPlastik1 = arrUkuran[0].split("+");
								if(parseFloat(arrPanjangPlastik1[1]) != 1){
									for (var i = 0; i < arrPanjangPlastik1.length; i++) {
										panjang1 += parseFloat(arrPanjangPlastik1[i]);
									}
								}else{
									panjang1 = arrUkuran1[0];
								}
							}
							if($.inArray(arrDataText[2], arrMerek) != -1){
								if(arrDataText[1].toLowerCase() == panjang1.toLowerCase() &&
									 arrDataText[2].toLowerCase() == merek1.toLowerCase() &&
								   arrDataText[3].toLowerCase() == warnaPlastik1.toLowerCase()){
									$("#btnTambahRencanaPotong").removeAttr("disabled");
									$("#lblInspector").hide();
								}else{
									$("#btnTambahRencanaPotong").attr("disabled","disabled");
									$("#lblInspector").show();
								}
							}else{
								if(arrDataText[1].toLowerCase() == panjang1.toLowerCase() &&
			...
			function modalBuatRencanaKerjaSusulan(){
				...
				$("#cmbKdHasil").select2({
					...
					ajax:{
						...
						processResults : function(data){
							return{
								results : $.map(data, function(item){
									if(item.keterangan.toUpperCase().indexOf("TUMPUK") != -1){
										var keterangan = " | "+item.keterangan;
									}else{
										var keterangan = "";
									}
									return{
										text:item.kd_gd_hasil+" | "+item.ukuran+" | "+item.merek+" | "+item.warna_plastik+" | "+item.jns_brg+" | "+item.jns_permintaan+keterangan,
										id:item.kd_gd_hasil
									}
								})
							};
				...
				$("#cmbKdHasil").on("select2:select",function(){
					...
					var panjangPlastik = arrUkuranPlastik[0].replace(/ /gi,"_");
					if(arrDataText.length >= 7){
						$("#txtKeterangan").val(arrDataText[6].toUpperCase());
					}
					...
					// }
					if(arrDataText[0].substring(0,4) == "HSLK"){
						var merek1 = 'Kantong';
					}else{
						var merek1 = merek;
					}
					$("#cmbKdBahan").select2({
						...
						allowClear:true,
						ajax:{
							url : "<?php echo base_url(); ?>_cutting/main/getComboBoxValueGudangRoll/"+arrDataText[5].replace(/\//g,"-")+"/"+panjangPlastik+"/"+merek1+"/"+warnaPlastik,
							dataType : "JSON",
							delay : 0,
							processResults : function(data){
								return{
			...
			function modalSisaPengambilan(param){
							...
							return{
								results : $.map(data, function(item){
									if(item.ket_barang.toUpperCase().indexOf("TUMPUK") != -1 ||
										 item.ket_merek.toUpperCase().indexOf("TUMPUK") != -1){
										var keterangan = "tumpuk";
										var panjang = item.ukuran;
									}else{
										var keterangan = "";
										var panjang = item.panjang;
									}
									return{
										text:item.customer+" | "+panjang+" | "+keterangan+" | "+item.merek+" | "+item.warna_plastik+" | "+item.tgl_rencana+" | "+item.jns_permintaan,
				...
				$("#cmbUkuran"+param).on("select2:select", function(){
		          ...
		          allowClear:true,
		          ajax:{
		            url : "<?php echo base_url(); ?>_cutting/main/getComboBoxValueGudangRoll/"+jnsPermintaan+"/"+arrPanjangSecondary[0].replace(/ /gi, "_")+"/"+arrDataText[3].replace(/ /gi, "_")+"/"+arrDataText[4].replace(/ /gi, "_"),
		            dataType : "JSON",
		            delay : 0,
		            processResults : function(data){
		              return{
			...
			function modalInputHasil(param1, param2, param3, param4){
				...
				$.ajax({
					...
					success : function(response){
						...
						for (var i = 0; i < response.DetailRencana.length; i++) {
							if(response.DetailRencana[i].ket_barang.toUpperCase().indexOf("TUMPUK") != -1 || response.DetailRencana[i].ket_merek.toUpperCase().indexOf("TUMPUK") != -1){
								var ketBarang_Check = response.DetailRencana[i].ket_barang.toUpperCase();
								var ketMerek_Check = response.DetailRencana[i].ket_merek.toUpperCase();
								var index_Check = i;
								break;
							}else{
								var ketBarang_Check = "";
						...
						if(ketBarang_Check.indexOf("TUMPUK") != -1 || ketMerek_Check.indexOf("TUMPUK") != -1){
							...
							var arrUkuran = response.DetailRencana[index_Check].ukuran.replace(/ /g,"").split("/");
							...
							$("#cmbUkuranTumpuk").select2({
								...
								allowClear:true,
								ajax:{
									url : "<?php echo base_url(); ?>_cutting/main/getComboBoxValueGudangRoll/"+jnsPermintaan+"/"+arrUkuranPlastikSecondary[0].replace(/ /gi,"_")+"/"+merek.replace(/ /gi,"_")+"/"+param2.replace(/ /gi,"_"),
									dataType : "JSON",
									delay : 0,
									processResults : function(data){
										return{
							...
							$("#cmbUkuranTumpuk").on("select2:select", function(){
								...
								var kodeRencana = $("input[name='txtKdCutting']").val();
								$.ajax({
									type : "POST",
									url : "<?php echo base_url('_cutting/main/getDataInputHasilTumpuk'); ?>",
									dataType : "JSON",
									data : {
							...
							$("#apalLainnya").css("display","table-row");
						}else{
							$("#tumpuk").css("display","none");
							$("#apalLainnya").css("display","none");
							$("#cmbUkuranTumpuk").on("select2:unselect", function(){
								$("#txtBahanTumpuk").val(0);
								$("#txtBobinTumpuk").val(0);
								$("#txtPayungTumpuk").val(0);
				...
				$("#cmbRollPipa").on("change",function(){
					if(this.value == "PAYUNG"){
						...
						if(param1.toUpperCase().indexOf("PON") != -1 || ketBarang.indexOf("PON") != -1 || ketMerek.indexOf("PON") != -1 || merek=="PON"){
							var arrPanjangPlastik = param1.replace(/ |pon|_/gi, "").replace(/,/g,".").split("+");
						...
						if(TPanjangPlastik < 5){
							$("#txtRumusRollPayung").val(5000);
						}else if(TPanjangPlastik >= 5 && TPanjangPlastik <= 40){
				...
				$.ajax({
					...
					success : function(response){
						...
						$.each(response.ListBon, function(AvIndex, AvValue){
							if(AvValue.kd_gd_roll != AvValue.kd_gd_roll_tumpuk){
								...
								}else if(AvValue.warna_plastik == "pink"){
									Pink += parseFloat(AvValue.apal);
								}
							}
						});

						$.each(response.ListTotalSubBon, function(AvIndex, AvValue){
							if(AvValue.jenis == "STRIP"){
								if(param2=="CETAK"){
									// $("#liStrip").text("Strip = 0");
								}else{
									strip += parseFloat(AvValue.jumlah);
									// $("#liStrip").text("Strip = " + parseFloat(AvValue.jumlah).toLocaleString());
								}
							}else{
								if(AvValue.sub_jenis == "PUTIH POLOS"){
									Putih = parseFloat(AvValue.jumlah);
								}
								// $("#liMerah").text("Merah = 0");
								// $("#liHijau").text("Hijau = 0");
								// $("#liKuning").text("Kuning = 0");
						...
						$("#liMerah").text("Merah = " + parseFloat(Merah).toLocaleString());
						$("#liHijau").text("Hijau = " + parseFloat(Hijau).toLocaleString());
						$("#liKuning").text("Kuning = " + parseFloat(Kuning).toLocaleString());
						$("#liBiru").text("Biru = " + parseFloat(Biru).toLocaleString());
						$("#liOrange").text("Orange = " + parseFloat(Orange).toLocaleString());
						$("#liCoklat").text("Coklat = " + parseFloat(Coklat).toLocaleString());
						$("#liHitam").text("Hitam = " + parseFloat(Hitam).toLocaleString());
						$("#liUngu").text("Ungu = " + parseFloat(Ungu).toLocaleString());
						$("#liSilver").text("Silver = " + parseFloat(Silver).toLocaleString());
						$("#liPink").text("Pink = " + parseFloat(Pink).toLocaleString());
						$("#liPutihSusu").text("Putih Susu = " + parseFloat(Putih_Susu).toLocaleString());
						$("#liPutih").text("Putih = " + parseFloat(Putih).toLocaleString());
						$("#liStrip").text("Strip = " + parseFloat(strip).toLocaleString());
			...
			function modalEditHasilCutting(param, param1, param3, param4){
				...
				$("#cmbRollPipa").on("change",function(){
					if(this.value == "PAYUNG"){
						...
						var ketMerek = $("#ketMerek").val().toUpperCase();
						var merek = $("#txtMerek").val().toUpperCase();
						if(param3.toUpperCase() == "PON" || ketBarang.indexOf("PON") != -1 || ketMerek.indexOf("PON") != -1){
							var arrPanjangPlastik = param1.replace(/ |pon|_/gi, "").replace(/,/g,".").split("+");
							switch (arrPanjangPlastik.length) {
								case 2 : if(parseFloat(arrPanjangPlastik[0].replace(/in/gi,'')) >= 14 && parseFloat(arrPanjangPlastik[0].replace(/in/gi,'')) < 26.5){
													 if(arrPanjangPlastik[0].indexOf("in") != -1){
				...
				$.ajax({
					...
					success : function(response){
						$.each(response.TransaksiHasilPotong, function(AvIndex, AvValue){
							$("#txtTglRencana").val(AvValue.tgl_rencana);
							...
							$("#txtBahan").val(parseFloat(
								(response.PengambilanPotong[0].jumlahBerat != null ? response.PengambilanPotong[0].jumlahBerat : 0)
							).toLocaleString());
							$("#txtBobin").val(parseFloat(
								(response.PengambilanPotong[0].jumlahBobin != null ? response.PengambilanPotong[0].jumlahBobin : 0)
							).toLocaleString());
							$("#txtPayung").val(parseFloat(
								(response.PengambilanPotong[0].jumlahPayung != null ? response.PengambilanPotong[0].jumlahPayung : 0)
							).toLocaleString());
							$("#txtPayungKuning").val(parseFloat(
								(response.PengambilanPotong[0].jumlahPayungKuning != null ? response.PengambilanPotong[0].jumlahPayungKuning : 0)
							).toLocaleString());

							$("#txtBeratSisaSemalam").val(parseFloat(
								(response.SisaPengambilanPotongSemalam[0].jumlahSisa != null ? response.SisaPengambilanPotongSemalam[0].jumlahSisa : 0)
							).toLocaleString());
							$("#txtBobinSisaSemalam").val(parseFloat(
								(response.SisaPengambilanPotongSemalam[0].jumlahBobin != null ? response.SisaPengambilanPotongSemalam[0].jumlahBobin : 0)
							).toLocaleString());
							$("#txtPayungSisaSemalam").val(parseFloat(
								(response.SisaPengambilanPotongSemalam[0].jumlahPayung != null ? response.SisaPengambilanPotongSemalam[0].jumlahPayung : 0)
							).toLocaleString());
							$("#txtPayungKuningSisaSemalam").val(parseFloat(
								(response.SisaPengambilanPotongSemalam[0].jumlahPayungKuning != null ? response.SisaPengambilanPotongSemalam[0].jumlahPayungKuning : 0)
							).toLocaleString());

							$("#txtSisa").val(parseFloat(
								(response.SisaPengambilanPotongHariIni[0].jumlahSisa != null ? response.SisaPengambilanPotongHariIni[0].jumlahSisa : 0)
							).toLocaleString());
							$("#txtSisaBobin").val(parseFloat(
								(response.SisaPengambilanPotongHariIni[0].jumlahBobin != null ? response.SisaPengambilanPotongHariIni[0].jumlahBobin : 0)
							).toLocaleString());
							$("#txtSisaPayung").val(parseFloat(
								(response.SisaPengambilanPotongHariIni[0].jumlahPayung != null ? response.SisaPengambilanPotongHariIni[0].jumlahPayung : 0)
							).toLocaleString());
							$("#txtSisaPayungKuning").val(parseFloat(
								(response.SisaPengambilanPotongHariIni[0].jumlahPayungKuning != null ? response.SisaPengambilanPotongHariIni[0].jumlahPayungKuning : 0)
							).toLocaleString());

							$("#txtBeratPengambilanTumpuk").val(parseFloat(
								(AvValue.berat_pengambilan_gudang_tumpuk != null ? AvValue.berat_pengambilan_gudang_tumpuk : 0)
							).toLocaleString());
							$("#txtBobinPengambilanTumpuk").val(parseFloat(
								(AvValue.bobin_pengambilan_gudang_tumpuk != null ? AvValue.bobin_pengambilan_gudang_tumpuk : 0)
							).toLocaleString());
							$("#txtPayungPengambilanTumpuk").val(parseFloat(
								(AvValue.payung_pengambilan_gudang_tumpuk != null ? AvValue.payung_pengambilan_gudang_tumpuk : 0)
							).toLocaleString());
							$("#txtPayungKuningPengambilanTumpuk").val(parseFloat(
								(AvValue.payung_kuning_pengambilan_gudang_tumpuk != null ? AvValue.payung_kuning_pengambilan_gudang_tumpuk : 0)
							).toLocaleString());

							$("#txtBeratSisaSemalamTumpuk").val(parseFloat(
								(response.SisaPengambilanPotongSemalamTumpuk[0].jumlahSisaTumpuk != null ? response.SisaPengambilanPotongSemalamTumpuk[0].jumlahSisaTumpuk : 0)
							).toLocaleString());
							$("#txtBobinSisaSemalamTumpuk").val(parseFloat(
								(response.SisaPengambilanPotongSemalamTumpuk[0].jumlahBobinTumpuk != null ? response.SisaPengambilanPotongSemalamTumpuk[0].jumlahBobinTumpuk : 0)
							).toLocaleString());
							$("#txtPayungSisaSemalamTumpuk").val(parseFloat(
								(response.SisaPengambilanPotongSemalamTumpuk[0].jumlahPayungTumpuk != null ? response.SisaPengambilanPotongSemalamTumpuk[0].jumlahPayungTumpuk : 0)
							).toLocaleString());
							$("#txtPayungKuningSisaSemalamTumpuk").val(parseFloat(
								(response.SisaPengambilanPotongSemalamTumpuk[0].jumlahPayungKuningTumpuk != null ? response.SisaPengambilanPotongSemalamTumpuk[0].jumlahPayungKuningTumpuk : 0)
							).toLocaleString());

							$("#txtSisaTumpuk").val(parseFloat(
								(response.SisaPengambilanPotongHariIniTumpuk[0].jumlahSisaTumpuk != null ? response.SisaPengambilanPotongHariIniTumpuk[0].jumlahSisaTumpuk : 0)
							).toLocaleString());
							$("#txtSisaBobinTumpuk").val(parseFloat(
								(response.SisaPengambilanPotongHariIniTumpuk[0].jumlahBobinTumpuk != null ? response.SisaPengambilanPotongHariIniTumpuk[0].jumlahBobinTumpuk : 0)
							).toLocaleString());
							$("#txtSisaPayungTumpuk").val(parseFloat(
								(response.SisaPengambilanPotongHariIniTumpuk[0].jumlahPayungTumpuk != null ? response.SisaPengambilanPotongHariIniTumpuk[0].jumlahPayungTumpuk : 0)
							).toLocaleString());
							$("#txtSisaPayungKuningTumpuk").val(parseFloat(
								(response.SisaPengambilanPotongHariIniTumpuk[0].jumlahPayungKuningTumpuk != null ? response.SisaPengambilanPotongHariIniTumpuk[0].jumlahPayungKuningTumpuk : 0)
							).toLocaleString());

							$("#txtBahanTumpuk").val(parseFloat(
								(response.PengambilanPotongTumpuk[0].jumlahBeratTumpuk != null ? response.PengambilanPotongTumpuk[0].jumlahBeratTumpuk : 0)
							).toLocaleString());
							$("#txtBobinTumpuk").val(parseFloat(
								(response.PengambilanPotongTumpuk[0].jumlahBobinTumpuk != null ? response.PengambilanPotongTumpuk[0].jumlahBobinTumpuk : 0)
							).toLocaleString());
							$("#txtPayungTumpuk").val(parseFloat(
								(response.PengambilanPotongTumpuk[0].jumlahPayungTumpuk != null ? response.PengambilanPotongTumpuk[0].jumlahPayungTumpuk : 0)
							).toLocaleString());
							$("#txtPayungKuningTumpuk").val(parseFloat(
								(response.PengambilanPotongTumpuk[0].jumlahPayungKuningTumpuk != null ? response.PengambilanPotongTumpuk[0].jumlahPayungKuningTumpuk : 0)
							).toLocaleString());
			...
			function modalEditHasilCuttingTemp(param){
				...
				$.ajax({
					...
					success : function(response){
						...
						if(response.DetailRencana[0].ket_barang.toUpperCase().indexOf("TUMPUK") != -1 || response.DetailRencana[0].ket_merek.toUpperCase().indexOf("TUMPUK") != -1){
							if($("#txtPermintaan").val() == "CETAK/POLOS"){
								var arrJnsPermintaan = $("#txtPermintaan").val().split("/");
								var jnsPermintaan = arrJnsPermintaan[1];
							}else{
							...
							$("#cmbUkuranTumpuk").select2({
								...
								allowClear:true,
								ajax:{
									url : "<?php echo base_url(); ?>_cutting/main/getComboBoxValueGudangRoll/"+jnsPermintaan+"/"+arrUkuranPlastikSecondary[0].replace(/ /gi,"_")+"/"+merek.replace(/ /gi,"_")+"/"+warnaPlastik.replace(/ /gi,"_"),
									dataType : "JSON",
									delay : 0,
									processResults : function(data){
										return{
			...
			function modalEditRencana(param){
				...
				$("#cmbKdHasil_Edit").on("select2:select",function(){
					...
					$("#cmbKdBahan_Edit").select2({
						...
						allowClear:true,
						ajax:{
							url : "<?php echo base_url(); ?>_cutting/main/getComboBoxValueGudangRoll/"+arrDataText[5].replace(/\//g,"-")+"/"+arrUkuran[0].replace(/ /gi,"_")+"/"+arrDataText[2].replace(/ /gi,"_")+"/"+arrDataText[3].replace(/ /gi,"_"),
							dataType : "JSON",
							delay : 0,
							processResults : function(data){
								return{
			...
			function modalCariDataApal(){
				$("#cmbMerek").select2({
					placeholder : "Pilih Roll",
					dropdownParent: $("#modalCariDataApal"),
					width : "100%",
					cache:false,
					allowClear:true,
					ajax:{
						url : "<?php echo base_url(); ?>_cutting/main/getComboBoxValueGudangRollAll",
						dataType : "JSON",
						delay : 0,
						processResults : function(data){
							return{
				...
				$("#modalCariDataApal").modal({
					backdrop : "static"
				});
			}

			function modalCopyRencana(param){
				$("#modalCopyRencana input[type='text']").val("");
				$("#modalCopyRencana .date").datepicker("setDate",null);

				$.get("<?= base_url('_cutting/main/getGeneratePotongCode') ?>","",function(response){
					$("#txtKodeRencanaCopyDST").val(response.Code);
					$("#txtKodeRencanaCopySRC").val(param);
				},"JSON");

				$("#modalCopyRencana").modal({
					backdrop : "static"
				});
			}
			//============================================MODAL METHOD (Finish)============================================//
			...
			function saveTambahSisaPengambilanPotong(param){
				...
				if(ketBarang1.toUpperCase().indexOf("TUMPUK") == -1){
					...
					var kdCutting1 = arrKdCutting1[0];
					var kdGdRoll1 = $("#txtKdGdRoll"+param).val();
					var arrJenisPermintaan = $("#txtPermintaan"+param).val().split("/");
					var jnsPermintaan1 = arrJenisPermintaan[0];
					var tglAwal1 = $("#txtTglAwal"+param).val();
					var tglAkhir1 = $("#txtTglAkhir"+param).val();
					var berat1 = $("#txtJumlahSisa"+param).val().replace(/,/g,"");
					var bobin1 = $("#txtJumlahBobin"+param).val().replace(/,/g,"");
						...
						success : function(response){
							if(jQuery.trim(response) === "Berhasil"){
								...
								setTimeout(function(){
									...
									$("select").val("");
									$(".date").datepicker("setDate",null);
									$("#cmbUkuran"+param).empty().trigger("change");
								},2000);
				...
				if(ketBarang1.indexOf("TUMPUK") != -1 || ketMerek1.indexOf("TUMPUK") != -1){
					...
					if(tglRencana1 == "" 						 || merek1 == "" 									|| jnsPermintaan1 == "" 					|| warnaPlastik1 == "" 									||
						 ...
						 plusMinus1 == "" 						 || kdGdRollTumpuk1 == "" 				|| arrLembar1.length <= 0 				|| arrBerat1.length <= 0 				 				||
	 					 arrKdCutting1.length <= 0 		 || arrKdGdHasil1.length <= 0 		|| arrKdGdRoll1.length <= 0				|| kdGdRollTumpuk1 == null
					  ){
			...
			function saveCopyRencana(){
				var kdPotongSrc = $("#txtKodeRencanaCopySRC").val();
				var kdPotongDst = $("#txtKodeRencanaCopyDST").val();
				var tglRencana = $("#txtTglRencanaCopy").val();
				var jumlahMesin = $("#txtJumlahMesinCopy").val().replace(/,/gi,"");
				var noMesin = $("#txtNomorMesinCopy").val();
				if(kdPotongDst==""||tglRencana==""||jumlahMesin==""||noMesin==""){
					$("#modal-notif").addClass("modal-warning");
					$("#modalNotifContent").text("Semua Kolom Berwarna Kuning Tidak Boleh Kosong!");
					$("#modal-notif").modal("show");
					setTimeout(function(){
					...
					$.ajax({
						type : "POST",
						url : "<?= base_url('_cutting/main/saveCopyRencana'); ?>",
						dataType : "TEXT",
						data : {
							kdPotongSrc : kdPotongSrc,
							kdPotongDst : kdPotongDst,
						...
						success : function(response){
							if($.trim(response) === "Berhasil"){
								$("#modal-notif").addClass("modal-info");
								$("#modalNotifContent").text("Rencana Berhasil Di Salin");
								$("#modal-notif").modal("show");
								setTimeout(function(){
									$("#modal-notif").modal("hide");
									$("#modal-notif").removeClass("modal-info");
									...
									$("#modalCopyRencana input[type='text']").val("");
									$("#modalCopyRencana .date").datepicker("setDate",null);
									$("#modalCopyRencana").modal("hide");
									$(".active a[aria-controls='tableDataRencanaMandor']").click();
								},2000);
							}else if($.trim(response) === "Gagal"){
								$("#modal-notif").addClass("modal-danger");
								$("#modalNotifContent").text("Rencana Gagal Di Salin");
								$("#modal-notif").modal("show");
								setTimeout(function(){
									$("#modal-notif").modal("hide");
									$("#modal-notif").removeClass("modal-danger");
			...
			function editSisaPengambilanTemp(param, param2){
				var kode 					= $("#cmbUkuran"+param2).val();
				var panjang 			= $("#txtPanjangPlastik"+param2).val();
				var merek 				= $("#txtMerek"+param2).val();
				var warnaPlastik 	= $("#txtWarnaPlastik"+param2).val();
				var jnsPermintaan = $("#txtPermintaan"+param2).val();
				var sisa 					= $("#txtJumlahSisa"+param2).val().replace(/,/gi, "");
				var payung 				= $("#txtJumlahPayung"+param2).val().replace(/,/gi, "");
				var payungKuning 	= $("#txtJumlahPayungKuning"+param2).val().replace(/,/gi, "");
				var bobin 				= $("#txtJumlahBobin"+param2).val().replace(/,/gi, "");
				var shift 				= $("#cmbShift"+param2).val();
				var keterangan 		= $("#cmbKeterangan"+param2).val();
				var tglAwal 			= $("#txtTglAwal"+param2).val();
				var tglAkhir 			= $("#txtTglAkhir"+param2).val();

				if(sisa=="" || payung=="" || payungKuning=="" || bobin=="" ||
					 shift=="" || keterangan=="" || tglAwal=="" || tglAkhir==""
				){
					$("#modal-notif").addClass("modal-warning");
					$("#modalNotifContent").text("Semua Kolom Berwarna Kuning Tidak Boleh Kosong!");
					$("#modal-notif").modal("show");
					...
					$.ajax({
						type : "POST",
						url : "<?php echo base_url('_cutting/main/updateSisaPengambilanPotongTemp'); ?>",
						dataType : "TEXT",
						data : {
							id 							: param,
							kode 						: kode,
						...
						success : function(response){
							if($.trim(response) === "Berhasil"){
								...
								setTimeout(function(){
									...
									$("#modalNotifContent").text("");
									$("#modalEditRencana").modal("hide");
									resetFormSisaPengambilan(param2);
									tableListSisaPengembalian(param2);
									$("#btnTambahSisaPengambilan"+param2).attr("onclick","saveTambahSisaPengambilanPotong('"+param2+"')").html("<i class='fa fa-plus'></i> Tambah");
								},2000);
							}else if($.trim(response) === "Gagal"){
								$("#modal-notif").addClass("modal-danger");
								$("#modalNotifContent").text("Data Berhasil Di Ubah");
			...
			function deleteSisaPengambilanPotongTemp(param, param2){
				if(confirm("Apakah Anda Yakin Ingin Menghapus Data Ini?")){
					$.ajax({
						type : "POST",
						url : "<?= base_url('_cutting/main/deleteSisaPengambilanPotongTemp'); ?>",
						dataType : "TEXT",
						data : {
							idTBPR : param
						},
			...
			function deleteHasilCutting(param){
				if(confirm("Apakah Anda Yakin Akan Menghapus Data Ini?")){
					$.ajax({
						type : "POST",
						url : "<?= base_url('_cutting/main/updateDeleteHasilCutting'); ?>",
						dataType : "TEXT",
						data : {
							kdHasilPotong : param
						},
						success : function(response){
							if($.trim(response) === "Berhasil"){
							...
							}else if($.trim(response) === "Berhasil"){
								$("#modal-notif").addClass("modal-danger");
								$("#modalNotifContent").text("Data Gagal Di Hapus");
								$("#modal-notif").modal("show");
								setTimeout(function(){
								...
								$("#modal-notif").addClass("modal-warning");
								$("#modalNotifContent").text("Maaf Parameter Tidak Boleh Kosong!, Silahkan Hubungi IT");
								$("#modal-notif").modal("show");
								setTimeout(function(){
									$("#modal-notif").modal("hide");
									$("#modal-notif").removeClass("modal-warning");
					...
					$("#cmbUkuran"+param).on("select2:select", function(){
						...
						if(arrDataText[2].toUpperCase().indexOf("TUMPUK") == -1){
							...
							$("#cmbUkuranTumpuk"+param).select2({
								...
								allowClear:true,
								ajax:{
									url : "<?php echo base_url(); ?>_cutting/main/getComboBoxValueGudangRoll/"+arrJenisPermintaanTumpuk[1].replace(/\//gi,"-")+"/"+arrPanjangSecondary[0].replace(/ /gi,"_")+"/"+arrDataText[3].replace(/ /gi,"_")+"/"+arrDataText[4].replace(/ /gi,"_"),
									dataType : "JSON",
									delay : 0,
									processResults : function(data){
										return{
			...
			function searchLaporanHasilPotongGlobal(){
				var tglAwal = $("#txtTanggalAwal").val();
				var tglAkhir = $("#txtTanggalAkhir").val();
				if(tglAwal=="" || tglAkhir==""){
					$("#modal-notif").addClass("modal-warning");
					...
					$.ajax({
						type : "POST",
						url : "<?php echo base_url('_cutting/main/getLaporanHasilPotongGlobalPeriode'); ?>",
						dataType : "JSON",
						data : {
							tglAwal : tglAwal,
							tglAkhir : tglAkhir
						...
						success : function(response){
							$("#tableLaporanHasilCuttingGlobal > tbody > tr").empty();
							var totalJumlahLembarPolos = 0;
							var totaljumlahBeratPolos = 0;
							var	totaljumlahLembarCetak = 0;
							var	totaljumlahBeratCetak = 0;
							$.each(response,function(AvIndex, AvValue){
								totalJumlahLembarPolos += parseFloat(AvValue.jumlahLembarPolos);
								totaljumlahBeratPolos += parseFloat(AvValue.jumlahBeratPolos);
								totaljumlahLembarCetak += parseFloat(AvValue.jumlahLembarCetak);
								totaljumlahBeratCetak += parseFloat(AvValue.jumlahBeratCetak);
								$("#tableLaporanHasilCuttingGlobal > tbody:last-child").append(
									"<tr>"+
										"<td>"+ ++AvIndex +"</td>"+
										"<td>"+ AvValue.tgl_rencana +"</td>"+
										"<td>"+parseFloat(AvValue.jumlahLembarPolos).toLocaleString()+"</td>"+
										"<td>"+parseFloat(AvValue.jumlahBeratPolos).toLocaleString()+"</td>"+
										"<td>"+parseFloat(AvValue.jumlahLembarCetak).toLocaleString()+"</td>"+
										"<td>"+parseFloat(AvValue.jumlahBeratCetak).toLocaleString()+"</td>"+
									"</tr>"
								);
							});
							$("#tableLaporanHasilCuttingGlobal > tbody:last-child").append(
								"<tr style='background-color:rgba(0, 115, 255, 0.8); color: #fff;'>"+
									"<td></td>"+
									"<td><label>Total</label></td>"+
									"<td>"+parseFloat(totalJumlahLembarPolos).toLocaleString()+"</td>"+
									"<td>"+parseFloat(totaljumlahBeratPolos).toLocaleString()+"</td>"+
									"<td>"+parseFloat(totaljumlahLembarCetak).toLocaleString()+"</td>"+
									"<td>"+parseFloat(totaljumlahBeratCetak).toLocaleString()+"</td>"+
								"</tr>"
							);
							$("#modalCariLaporanHasilCuttingGlobal").modal("hide");
						},
						error : function(response){
							$("#modal-notif").addClass("modal-danger");
							$("#modalNotifContent").text("Server Fault Error Code ( "+response.status+" ) "+response.statusText);
			...
			function searchLaporanBonApalGlobal(){
				var tglAwal = $("#txtTanggalAwal").val();
				var tglAkhir = $("#txtTanggalAkhir").val();
				if(tglAwal=="" || tglAkhir==""){
					$("#modal-notif").addClass("modal-warning");
					...
					$.ajax({
						type : "POST",
						url : "<?php echo base_url('_cutting/main/getLaporanBonApalGlobalPeriode'); ?>",
						dataType : "JSON",
						data : {
							tglAwal : tglAwal,
							tglAkhir : tglAkhir
						...
						success : function(response){
							$("#tableLaporanBonApalGlobal > tbody > tr").empty();
							var totaljumlahApal = 0;
							$.each(response,function(AvIndex, AvValue){
								totaljumlahApal += parseFloat(AvValue.jumlahApal);
								$("#tableLaporanBonApalGlobal > tbody:last-child").append(
									"<tr>"+
										"<td>"+ ++AvIndex +"</td>"+
										"<td>"+ AvValue.tgl_rencana +"</td>"+
										"<td>"+parseFloat(AvValue.jumlahApal).toLocaleString()+"</td>"+
									"</tr>"
								);
							});
							$("#tableLaporanBonApalGlobal > tbody:last-child").append(
								"<tr style='background-color:rgba(0, 115, 255, 0.8); color: #fff;'>"+
									"<td></td>"+
									"<td><label>Total</label></td>"+
									"<td>"+parseFloat(totaljumlahApal).toLocaleString()+"</td>"+
								"</tr>"
							);
							$("#modalCariLaporanBonApalGlobal").modal("hide");
						},
						error : function(response){
							$("#modal-notif").addClass("modal-danger");
							$("#modalNotifContent").text("Server Fault Error Code ( "+response.status+" ) "+response.statusText);
			...
			function cariLaporanApalPerMerek(){
				var tglAwal = $("#txtTglAwal").val();
				var tglAkhir = $("#txtTglAkhir").val();
				var kdGdRoll = $("#cmbMerek").val();

				if(tglAwal=="" || tglAkhir=="" ||kdGdRoll==""){
					$("#modal-notif").addClass("modal-warning");
					$("#modalNotifContent").text("Semua Kolom Berwarna Kuning Tidak Boleh Kosong!");
					$("#modal-notif").modal("show");
					setTimeout(function(){
					...
					$.ajax({
						type : "POST",
						url : "<?= base_url('_cutting/main/getLaporanApalPerMerek'); ?>",
						dataType : "JSON",
						data : {
							tglAwal : tglAwal,
							tglAkhir : tglAkhir,
						...
						success : function(response){
							var totalHasilLembar = 0;
							var totalBeratKotor = 0;
							var totalBeratBersih = 0;
							var totalBeratApal = 0;

							$("#tableLaporanApalPerMerek > tbody").empty();
							// $("#tableLaporanApalPerMerek > tfoot").empty();
							$.each(response, function(AvIndex, AvData){
								$("#tableLaporanApalPerMerek > tbody:last-child").append(
									"<tr>"+
										"<td>"+ ++AvIndex +"</td>"+
										"<td>"+AvData.nama_customer+"</td>"+
										"<td>"+AvData.nama_barang+"</td>"+
										"<td>"+AvData.tgl_rencana+"</td>"+
										"<td>"+parseFloat(AvData.hasil_lembar).toLocaleString()+"</td>"+
										"<td>"+parseFloat(AvData.hasil_berat_kotor).toLocaleString()+"</td>"+
										"<td>"+parseFloat(AvData.hasil_berat_bersih).toLocaleString()+"</td>"+
										"<td>"+parseFloat(AvData.jumlah_apal_global).toLocaleString()+"</td>"+
										"<td>"+parseFloat(AvData.persentase).toLocaleString()+"%</td>"+
									"</tr>"
								);
							});
							for (var i = 0; i < response.length; i++) {
								totalHasilLembar += parseFloat(response[i].hasil_lembar);
								totalBeratKotor += parseFloat(response[i].hasil_berat_kotor);
								totalBeratBersih += parseFloat(response[i].hasil_berat_bersih);
								totalBeratApal += parseFloat(response[i].jumlah_apal_global);
							}

							$("#thTotalHasilLembar").text(totalHasilLembar.toLocaleString());
							$("#thTotalBeratKotor").text(totalBeratKotor.toLocaleString());
							$("#thTotalBeratBersih").text(totalBeratBersih.toLocaleString());
							$("#thTotalBeratApal").text(totalBeratApal.toLocaleString());

							$("#modalCariDataApal").modal("hide");
						},
						error : function(response){
							$("#modal-notif").addClass("modal-danger");
							$("#modalNotifContent").text("Server Fault Error Code ( "+response.status+" ) "+response.statusText);
          ...
          "columns":[
						...
						{"data" : "permintaan_mesin","name":"RP.permintaan_mesin"},
						{"data" : "tebal","name":"RP.tebal"},
						{"data" : "berat","name":"RP.berat"},
						{"data" : "jumlah_permintaan","name":"RP.berat"},
						{"data" : "sisa","name":"RP.jumlah_permintaan"},
						{"data" : "strip","name":"RP.sisa"},
						{"data" : "ket_mandor","name":"RP.ket_mandor"},
						{"data" : "no_mesin","name":"RP.no_mesin"},
						{"data" : "kd_ppic","name":"RP.kd_ppic"}
          ],
          ...
          "fnRowCallback":function(nRow,aData,iDisplayIndex,iDisplayIndexFull){
						...
						if(aData["inspector"] == "TRUE"){
							if(aData["sts_pengerjaan"] == "FINISH"){
								$("td:eq(14)",nRow).html("<button class='btn btn-md btn-flat btn-primary' onclick=modalBuatRencanaKerja('"+aData["kd_ppic"]+"','"+param1+"','"+param2+"')>Buat Kembali Rencana</button>");
							}else{
							...
							$("td",nRow).css("background-color","rgba(255, 112, 0, 0.7)");
							$("td",nRow).css("color","#FFF");
							$("td:eq(14)",nRow).html("<label style='color : rgb(255, 255, 255)'>Rencana Tidak Bisa Dibuat (Kode Tidak Sama), Silahkan Hubungi PPIC Untuk Revisi Rencana</label>");
						}
          ...
          "columns":[
						...
						{"data" : "merek","name":"CAST(RC.tgl_rencana AS CHAR)"},
						...
						{"data" : "kd_potong","name":"RC.sts_pengerjaan"}
          ],
          "fnServerData": function (sSource, aoData, fnCallback){
          ...
          "fnRowCallback":function(nRow,aData,iDisplayIndex,iDisplayIndexFull){
						...
						if(aData["sts_pengerjaan"] == "FINISH"){
							if(aData["kd_ppic"] == null){
								$("td:eq(13)",nRow).html("<button class='btn btn-md btn-flat btn-warning' onclick=modalCopyRencana('"+aData["kd_potong"]+"')>Copy Rencana</button>");
							}else{
								$("td:eq(13)",nRow).html("");
							}
						}else{
							if(aData["kd_ppic"] == null){
								$("td:eq(13)",nRow).html("<a href='#' onclick=editStatusPrint('"+aData["kd_gd_roll"]+"','"+aData["tgl_rencana"]+"','"+aData["kd_potong"]+"','TRUE') class='btn btn-md btn-flat "+btn+"'>Print</a>"+
																				 ...
																				 "<button class='btn btn-md btn-flat btn-primary' onclick=modalEditGantiMesinMandor('"+aData["kd_potong"]+"','"+aData["kd_ppic"]+"')>Ganti Mesin</button>"+
																			 	 "<button class='btn btn-md btn-flat btn-warning' onclick=modalCopyRencana('"+aData["kd_potong"]+"')>Copy Rencana</button>");
							}else{
								$("td:eq(13)",nRow).html("<a href='#' onclick=editStatusPrint('"+aData["kd_gd_roll"]+"','"+aData["tgl_rencana"]+"','"+aData["kd_potong"]+"','TRUE') class='btn btn-md btn-flat "+btn+"'>Print</a>"+
																				 "<button class='btn btn-md btn-flat btn-warning' onclick=modalEditMerek('"+aData["kd_potong"]+"','"+aData["kd_ppic"]+"')>Edit Merek</button>"+
																			 	 "<button class='btn btn-md btn-flat btn-primary' onclick=modalEditTanggal('"+aData["kd_potong"]+"')>Edit Tanggal</button>"+
						...
						if(aData["satuan_ppic"] == "BAL"){
							$("td:eq(9)",nRow).text(parseFloat(aData["stok_permintaan"]).toLocaleString()+" BAL");
							$("td:eq(10)",nRow).text(parseFloat(aData["jml_permintaan"]).toLocaleString()+" KG");
						}else{
							$("td:eq(9)",nRow).text(parseFloat(aData["stok_permintaan"]).toLocaleString()+" "+aData["satuan"]);
							$("td:eq(10)",nRow).text(parseFloat(aData["jml_permintaan"]).toLocaleString()+" "+aData["satuan"]);
						}
          }
				});
			}
							...
							$("#tableBeratPengambilanOperator"+param+" > tbody:last-child").append(
								"<tr>"+
									...
									"<td>"+
										...
										"<button class='btn btn-md btn-flat btn-danger' onclick=deleteSisaPengambilanPotongTemp('"+AvValue.id+"','"+AvValue.jns_permintaan+"')>Hapus</button>"+
									"</td>"+
								"</tr>"
							);
						});
          ...
          "fnRowCallback":function(nRow,aData,iDisplayIndex,iDisplayIndexFull){
						$("td:eq(0)",nRow).text(++iDisplayIndex);
						$("td:eq(7)",nRow).text(parseFloat(aData["berat"]).toLocaleString());
						$("td:eq(9)",nRow).text(parseFloat(aData["jumlah_lembar"]).toLocaleString());
						$("td:eq(10)",nRow).text(parseFloat(aData["jumlah_berat"]).toLocaleString());
						$("td:eq(11)",nRow).text(parseFloat(aData["jumlah_apal_global"]).toLocaleString());
						$("td:eq(12)",nRow).text(parseFloat(aData["jumlah_roll_pipa"]).toLocaleString());
						$("td:eq(13)",nRow).text(parseFloat(aData["berat_sisa_hari_ini"]).toLocaleString());
						$("td:eq(14)",nRow).text(parseFloat(aData["plusminus"]).toLocaleString());
						$("td:eq(16)",nRow).html("<button class='btn btn-md btn-flat btn-warning' onclick=modalEditHasilCuttingTemp('"+aData["id_transaksi"]+"')>Ubah</button>"+
																		 "<button class='btn btn-md btn-flat btn-danger' onclick=deleteHasilCutting('"+aData["kd_hasil_potong"]+"')>Hapus</button>");
          }
				});
			}
          ...
          "fnRowCallback":function(nRow,aData,iDisplayIndex,iDisplayIndexFull){
          	$("td:eq(3)",nRow).html(aData["ukuran"]+" "+"<label class='text-blue'> ("+aData["kd_gd_hasil"]+")</label>");
						$("td:eq(6)",nRow).text(parseFloat(aData["hasil_lembar"]).toLocaleString());
						$("td:eq(7)",nRow).text(parseFloat(aData["hasil_berat_bersih"]).toLocaleString());
						$("td:eq(8)",nRow).text(parseFloat(aData["hasil_berat_kotor"]).toLocaleString());
						$("td:eq(9)",nRow).text(parseFloat(aData["jumlah_apal_global"]).toLocaleString());
						$("td:eq(10)",nRow).text(parseFloat(aData["jumlah_roll_pipa"]).toLocaleString());
						$("td:eq(11)",nRow).text(parseFloat(aData["jumlah_lembar"]).toLocaleString());
						$("td:eq(12)",nRow).text(parseFloat(aData["jumlah_berat"]).toLocaleString());
						$("td:eq(13)",nRow).text(parseFloat(aData["berat"]).toLocaleString());
						var arrUkuran = aData["ukuran"].split("x");
						$("td:eq(14)",nRow).html(parseFloat(aData["plusminus"]).toLocaleString());
          ...
          "fnRowCallback":function(nRow,aData,iDisplayIndex,iDisplayIndexFull){
						...
						$("td:eq(13)",nRow).html(aData["sts_pengerjaan"]+" <label class='text-red'>["+aData["prioritas"]+"]</label>")
          }
				});
			}
		...
		<script type="text/javascript">
			...
			function hitungRollPipa(param1="", param2=""){
				...
				if(jenisPipa == "PAYUNG"){
					...
					if(merek.toUpperCase().indexOf("PON") != -1 || ketBarang.indexOf("PON") != -1 || ketMerek.indexOf("PON") != -1 || panjangTemp.indexOf("PON") != -1){
						var arrPanjangPlastik = arrUkuran[0].replace(/ |pon|_/gi, "").replace(/,/g,".").split("+");
						switch (arrPanjangPlastik.length) {
							case 2 : if(parseFloat(arrPanjangPlastik[0].replace(/in/gi,'')) >= 14 && parseFloat(arrPanjangPlastik[0].replace(/in/gi,'')) < 26.5){
												 if(arrPanjangPlastik[0].indexOf("in") != -1){
													 var TPanjangPlastik = (parseFloat(arrPanjangPlastik[0].replace(/in/gi,'')) * 2.54) + 5;
					...
					if(parseFloat(TPanjangPlastik) < 5){
						var rumusRollPayung = 5000;
					}else if(parseFloat(TPanjangPlastik) >=5 && parseFloat(TPanjangPlastik) <=40){
			...
			function hitungPlusMinus(){
				...
				if(ketMerek.indexOf("TUMPUK") != -1 || ketBarang.indexOf("TUMPUK") != -1){
					...
					// var beratApal2 = (parseFloat($("#txtJumlahApalLainnya").val().replace(/,| /g,""))*1000);
			...
			function hitungHasilGlobalPotong(){
				...
				if(panjangPlastik >= 3 && panjangPlastik < 10){
					var multiplier = 1;
				}else if(panjangPlastik >= 10 && panjangPlastik < 13){
					var multiplier = 2;
				}else if(panjangPlastik >= 13 && panjangPlastik < 15){
					var multiplier = 2.5;
				}else if(panjangPlastik >= 15 && panjangPlastik < 17){
					var multiplier = 3;
				}else if(panjangPlastik >= 17 && panjangPlastik < 20){
					var multiplier = 3.5;
				}else if(panjangPlastik >= 20 && panjangPlastik < 25){
					var multiplier = 4;
				}else if(panjangPlastik >= 25 && panjangPlastik < 31){
					var multiplier = 5;
				}else if(panjangPlastik >= 31 && panjangPlastik < 37){
					var multiplier = 6;
				}else{
					var multiplier = 7;
				}
			...
			function hitungHasilGlobalPotongInputHasil(){
				...
				if(merek.toUpperCase().indexOf("PON") != -1 || ketBarang.indexOf("PON") != -1 || ketMerek.indexOf("PON") != -1){
					var arrPanjangPlastik = arrUkuranPlastik[0].replace(/ |pon|_/gi, "").replace(/,/g,".").split("+");
					switch (arrPanjangPlastik.length) {
						case 2 : if(parseFloat(arrPanjangPlastik[0].replace(/in/gi,'')) >= 14 && parseFloat(arrPanjangPlastik[0].replace(/in/gi,'')) < 26.5){
											 if(arrPanjangPlastik[0].indexOf("in") != -1){
