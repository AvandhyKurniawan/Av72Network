...
class Cutting_Model extends CI_Model{
  ...
  public function insertKirimBonHasilJadiGlobal($param){
    ...
    if($jnsBrg == "CAMPUR_STANDARD"){
      ...
      $where2 = "AND sts_barang IN('CAMPUR','STANDARD')";
    }else{
      $where = "AND GH.jns_brg='$jnsBrg'";
      $where2 = "AND sts_barang='$jnsBrg'";
    }
    ...
    if($checkLock[0]["jumlahLock"] > 0){
      ...
      $indicator = $this->db->insert_batch("transaksi_gudang_hasil",$param["DataBon"]);
      if($indicator != FALSE){
  ...
  public function insertCopyRencana($param){
    $this->db->trans_begin();
    $Q = "SELECT * FROM rencana_potong WHERE kd_potong = '$param[kdPotongSrc]'";
    $arrRencanaSrc = $this->db->query($Q)->result_array();
    $arrRencanaDst = array("kd_potong"        => $param["kdPotongDst"],
                           "kd_ppic"          => $arrRencanaSrc[0]["kd_ppic"],
                           "kd_gd_hasil"      => $arrRencanaSrc[0]["kd_gd_hasil"],
                           "kd_gd_roll"       => $arrRencanaSrc[0]["kd_gd_roll"],
                           "id_user"          => $arrRencanaSrc[0]["id_user"],
                           "tgl_rencana"      => $param["tglRencana"],
                           "customer"         => $arrRencanaSrc[0]["customer"],
                           "ukuran"           => $arrRencanaSrc[0]["ukuran"],
                           "warna_plastik"    => $arrRencanaSrc[0]["warna_plastik"],
                           "strip"            => $arrRencanaSrc[0]["strip"],
                           "tebal"            => $arrRencanaSrc[0]["tebal"],
                           "jns_permintaan"   => $arrRencanaSrc[0]["jns_permintaan"],
                           "jml_permintaan"   => $arrRencanaSrc[0]["stok_permintaan"],
                           "stok_permintaan"  => $arrRencanaSrc[0]["stok_permintaan"],
                           "satuan"           => $arrRencanaSrc[0]["satuan"],
                           "sts_pengerjaan"   => "PROGRESS",
                           "merek"            => $arrRencanaSrc[0]["merek"],
                           "no_mesin"         => $param["noMesin"],
                           "jml_mesin"        => $param["jumlahMesin"],
                           "shift"            => $arrRencanaSrc[0]["shift"],
                           "berat"            => $arrRencanaSrc[0]["berat"],
                           "ket_merek"        => $arrRencanaSrc[0]["ket_merek"],
                           "ket_barang"       => $arrRencanaSrc[0]["ket_barang"],
                           "gambar"           => $arrRencanaSrc[0]["gambar"]);
      $this->db->insert("rencana_potong", $arrRencanaDst);
    if($this->db->trans_status() === FALSE){
      $this->db->trans_rollback();
      return "Gagal";
    }else{
  ...
  public function selectRencanaPpicPotong($param){
    $this->datatables->select("RP.kd_ppic,
                               RP.tgl_rencana,
                               RP.nm_cust,
                               RP.jns_permintaan,
                               RP.ukuran,
                               ...
                               RP.prioritas,
                               RP.tebal,RP.satuan,
                               FORMAT(ROUND(RP.jumlah_permintaan),0) AS jumlah_permintaan,
                               FORMAT(ROUND(RP.sisa),0) AS sisa,
                               RP.strip,
                               RP.ket_mandor,
                               RP.keterangan,
                               RP.sts_pengerjaan,
                               ...
                               RP.warna_cetak,
                               RP.warna_cat,
                               IF(RP.ukuran = GH.ukuran AND
                                  RP.merek = GH.merek AND
                                  RP.warna_plastik = GH.warna_plastik, 'TRUE', 'FALSE')
                               AS inspector");
    $this->datatables->from("rencana_ppic RP");
    $this->datatables->join("gudang_hasil GH","RP.kd_gd_hasil = GH.kd_gd_hasil","LEFT");
    if(empty($param["tglAwal"])){
      $date = date("Y-m-d");
      $this->datatables->where("RP.deleted='FALSE' AND
                                RP.tgl_rencana = '$date' AND
                                GH.deleted = 'FALSE' AND
                                RP.bagian=","POTONG");
    }else{
      $this->datatables->where("RP.deleted='FALSE' AND
                                RP.tgl_rencana = '$param[tglAwal]' AND
                                GH.deleted = 'FALSE' AND
                                RP.bagian=","POTONG");
    }
    $this->db->order_by("RP.tgl_rencana DESC,RP.kd_ppic DESC");
  ...
  public function selectComboBoxValueGudangRoll($param){
    ...
    if(strpos(strtolower($param["panjangPlastik"]),"pon") !== FALSE ||
       strpos(strtolower($param["merek"]),"pon") !== FALSE ||
       strpos(strtolower($param["merek"]),"(pon)") !== FALSE ||
       strpos(strtolower($param["merek"]),"( pon )") !== FALSE
     ){
      $arrX = array("pon"," ");
      ...
      $arrUkuranTemp = explode("x",strtolower($UkuranTemp));
      $arrPanjangTemp = explode("+",strtolower($arrUkuranTemp[0]));
        for ($i=0; $i < count($arrPanjangTemp); $i++) {
          if($arrPanjangTemp[$i] != 1){
            $panjang += floatval($arrPanjangTemp[$i]);
          }else{
            $panjang .= "+".$arrPanjangTemp[$i];
          }
        }
    }

    if(strlen($UkuranTemp) == 1){
      $Av = " OR ukuran = '$UkuranTemp'";
    }else{
      $Av = " OR ukuran LIKE '%$UkuranTemp%' OR ukuran LIKE '%$panjang%'";
    }
    ...
    if(in_array(strtoupper($param["merek"]), $arrMerek)){
      ...
      $clause2="OR ((merek = '$param[merek]'
                AND warna_plastik = '$param[warnaPlastik]' AND deleted='FALSE') OR
                (merek LIKE '%$param[merek]%'
                AND warna_plastik = '$param[warnaPlastik]' AND deleted='FALSE'))";
    }
    ...
    if(strpos(strtolower($param["panjangPlastik"]),"pon") !== FALSE ||
      ...
      $arrData = $this->db->query("SELECT * FROM gudang_roll
                                   WHERE $clause merek NOT LIKE '%zipper%'
                                   AND ((ukuran LIKE '%$param[panjangPlastik]%' $Av) AND deleted='FALSE')
                                   $clause2
                                   AND deleted='FALSE'
                                   LIMIT 20")->result_array();
     // echo "SELECT * FROM gudang_roll
     //                              WHERE $clause merek NOT LIKE '%zipper%'
  ...
  public function selectComboBoxValueGudangRollAll($param){
    if(empty($param)){
      $Q = "SELECT kd_gd_roll,
                   CONCAT(kd_gd_roll,' | ',
                          ukuran,' | ',
                          ...
                          warna_plastik,' | ',
                          jns_permintaan,' | ',
                          jns_brg) AS nama_barang
            FROM gudang_roll
            WHERE deleted='FALSE' AND merek NOT LIKE '%zipper%'
            LIMIT 50";
    }else{
      if(strpos(strtolower($param),"|") !== FALSE){
        $arrKey = explode("|",strtolower($param));
        $Q = "SELECT kd_gd_roll,
                     CONCAT(kd_gd_roll,' | ',
                            ukuran,' | ',
                            merek,' | ',
              ...
              FROM gudang_roll
              WHERE (deleted='FALSE' AND merek NOT LIKE '%zipper%')
              AND (ukuran LIKE '%$arrKey[0]%' AND merek LIKE '%$arrKey[1]%' AND warna_plastik LIKE '%$arrKey[2]%')";
      }else{
        $Q = "SELECT kd_gd_roll,
                     CONCAT(kd_gd_roll,' | ',
                            ukuran,' | ',
              ...
              FROM gudang_roll
              WHERE (deleted='FALSE' AND merek NOT LIKE '%zipper%')
              AND (ukuran LIKE '%$param%' OR merek LIKE '%$param%' OR warna_plastik LIKE '%$param%')";
      }
    }
    $result = json_encode($this->db->query($Q)->result_array());
    return $result;
  }
  ...
  public function selectComboBoxValueGudangRollSearch($param){
    ...
    if(in_array(strtoupper($param["merek"]), $arrMerek)){
      if(strtoupper($param["merek"])=="PON" || strtoupper($param["merek"])=="KLIP" || strtoupper($param["merek"])=="KANTONG"){
        $clause3 = " AND (merek IN ('Klip','PON','Kantong')
                     AND warna_plastik = '$param[warnaPlastik]')";
      }else{
        $clause3 = " AND (merek = '$param[merek]'
                     AND warna_plastik = '$param[warnaPlastik]')";
      }
    }else{
      $clause3="";
  ...
  public function selectComboBoxValueGudangHasil(){
    $Q = "SELECT * FROM gudang_hasil
          WHERE deleted='FALSE'
          AND jns_brg NOT IN ('KANVAS','ONLINE','SABLON')
          AND merek NOT IN ('Kantong','kantong','Kantong (Tidak Dipakai)','kantong (Tidak Dipakai)')
          LIMIT 20";
  ...
  public function selectRencanaPotongPending(){
    $Q = "SELECT kd_potong, no_mesin, customer, merek, ukuran, warna_plastik,
                 tebal, stok_permintaan, jml_permintaan, strip
  ...
  public function selectRencanaMandorPotong($param){
    $this->datatables->select("RC.kd_gd_roll, RC.tgl_rencana, RC.no_mesin, RC.customer, RC.merek, RC.ukuran, RC.warna_plastik, RC.satuan, RC.kd_gd_roll,
                               ...
                               RP.foto_depan AS gambar_dari_ppic, RC.kd_potong, RP.kd_ppic, RC.gambar, RP.satuan AS satuan_ppic, RC.sts_pengerjaan");
    ...
    if(empty($param)){
      ...
      $this->datatables->where("RC.deleted='FALSE' AND RC.sts_pengerjaan IN ('PROGRESS','FINISH') AND RC.tgl_rencana < '$dateNow' AND RC.tgl_rencana > '$last2Weeks'");
    }else{
      $this->datatables->where("RC.deleted='FALSE' AND RC.sts_pengerjaan IN ('PROGRESS','FINISH') AND RC.tgl_rencana='$param'");
    }
    $this->db->order_by("RC.tgl_rencana DESC,
                         FIELD(RC.jns_permintaan, 'POLOS','CETAK', 'CETAK/POLOS','POLOS/CETAK','CETAK/CETAK','POLOS/POLOS'),
                         FIELD(RC.merek, 'Zippin','KP','MP','Klip','PON','CB','Kantong',RC.merek) ASC,
                         CAST(SUBSTRING_INDEX(RC.ukuran,'x',1) AS UNSIGNED) ASC,
                         SUBSTRING_INDEX(RC.ukuran,'x',-1) ASC");
    return $this->datatables->generate();
  }
  ...
  public function selectprintRencanaMandorPotong($param){
    $Q = "SELECT merek, tebal, strip, ukuran, no_mesin, berat, warna_plastik, customer
          ...
          FIELD(jns_permintaan, 'POLOS','CETAK', 'CETAK/POLOS','POLOS/CETAK','CETAK/CETAK','POLOS/POLOS'),
          FIELD(merek, 'Klip','PON','KP','MP','Zippin','CB','Kantong',merek) ASC,
          CAST(SUBSTRING_INDEX(ukuran,'x',1) AS UNSIGNED) ASC,
          SUBSTRING_INDEX(ukuran,'x',-1) ASC";
    $result = $this->db->query($Q)->result_array();
    return $result;
  }
  ...
  public function selectUkuranPengembalianPotong($param){
    ...
    if($param["Key"]==""){
      $Q = "SELECT kd_potong, kd_gd_roll, customer, SUBSTRING_INDEX(ukuran, 'x',1) AS panjang, jns_permintaan, ukuran,
                   ket_barang, merek, warna_plastik, DATE_FORMAT(tgl_rencana, '%d %M %Y') AS tgl_rencana,ket_merek
            FROM rencana_potong
            WHERE $clause
            AND tgl_rencana  BETWEEN '$last7Days' AND '$dateNow'
            AND deleted = 'FALSE'
            GROUP BY panjang, warna_plastik, merek
            ORDER BY DATE_FORMAT(tgl_rencana,'%Y-%m-%d') DESC, FIELD(merek,'Zippin','KP','MP','Klip','PON','Kantong','CB',merek)";
    }else{
      if(strpos($param["Key"],"|") !== FALSE){
        ...
        $Q = "SELECT kd_potong, kd_gd_roll, customer, SUBSTRING_INDEX(ukuran, 'x',1) AS panjang, jns_permintaan, ukuran,
              ...
              -- AND tgl_rencana BETWEEN '$last7Days' AND '$dateNow'
              AND deleted = 'FALSE'
              AND DATE_FORMAT(tgl_rencana, '%Y-%m-%d') LIKE '%$arrKey[0]%'
        ...
        $Q = "SELECT kd_potong, kd_gd_roll, customer, SUBSTRING_INDEX(ukuran, 'x',1) AS panjang, jns_permintaan, ukuran,
              ...
              OR DATE_FORMAT(tgl_rencana, '%Y-%m-%d') LIKE '%$param[Key]%')
              GROUP BY panjang, warna_plastik, merek
              ORDER BY DATE_FORMAT(tgl_rencana,'%Y-%m-%d') DESC, FIELD(merek,'Zippin','KP','MP','Klip','PON','Kantong','CB',merek)";
      }
    }
  ...
  public function selectSisaPengambilan($param){
    $Q = "SELECT TBPR.id,
          ...
          ORDER BY FIELD(GR.merek,'Zippin','KP','MP','Klip','PON','Kantong Zippin','Kantong KP','Kantong MP','Kantong Klip',GR.merek),
                   GR.ukuran ASC";
    $result = $this->db->query($Q)->result_array();
    return json_encode($result);
  }
  ...
  public function selectDataRencanaForInputHasil($param){
    ...
    $Q = "SELECT RC.*, GH.jns_brg, RP.ket_merek AS ket_merek_ppic FROM rencana_potong RC
          ...
          ORDER BY tgl_rencana DESC,
          FIELD(RC.jns_permintaan, 'POLOS','CETAK', 'CETAK/POLOS','POLOS/CETAK','CETAK/CETAK','POLOS/POLOS'),
          FIELD(RC.merek, 'Klip','PON','KP','MP','Zippin','CB','Kantong',RC.merek) ASC,
          CAST(SUBSTRING_INDEX(RC.ukuran,'x',1) AS UNSIGNED) ASC,
          SUBSTRING_INDEX(RC.ukuran,'x',-1) ASC";
    $detailRencana = $this->db->query($Q)->result_array();
    $kdPotong = $detailRencana[0]["kd_potong"];

    $QTumpuk = "SELECT kd_potong FROM rencana_potong
                WHERE tgl_rencana = '$param[tglRencana]'
                AND deleted='FALSE'
                AND (ket_merek LIKE '%tumpuk%' OR ket_barang LIKE '%tumpuk%')";
    $arrDataTumpuk = $this->db->query($QTumpuk)->result_array();
    $kdPotongTumpuk = $arrDataTumpuk[0]["kd_potong"];

    $Qtest = "SELECT A.kd_potong FROM transaksi_berat_pengambilan_roll A
              INNER JOIN rencana_potong B ON A.kd_potong = B.kd_potong
              WHERE A.kd_gd_roll = ''
              AND A.tgl_potong = ''
              AND A.keterangan IN ('POTONG BESOK')
              AND A.deleted = 'FALSE'
              AND B.deleted = 'FALSE'";

    if($detailRencana[0]["jns_permintaan"] == "CETAK" || $detailRencana[0]["jns_permintaan"] == "CETAK/POLOS"){
      if(strtoupper($detailRencana[0]["ket_merek"]) == "TUMPUK" ||
         strtoupper($detailRencana[0]["ket_barang"]) == "TUMPUK" ||
         strtoupper($detailRencana[0]["ket_merek_ppic"]) == "TUMPUK"){
           $QSumPengambilanPotong = "SELECT SUM(berat) AS jumlahBerat,
                                     ...
                                     AND kd_potong IN (SELECT A.kd_potong FROM transaksi_detail_pengambilan_potong A
                                                       INNER JOIN rencana_potong B ON A.kd_potong = B.kd_potong
                                                       WHERE A.kd_gd_roll = '$param[kdGdRoll]'
                                                       AND A.tgl_potong = '$param[tglRencana]'
                                                       AND A.status IN ('MANDOR POTONG (CETAK)')
                                                       AND (B.ket_barang LIKE '%tumpuk%' OR B.ket_merek LIKE '%tumpuk%')
                                                       AND A.deleted = 'FALSE'
                                                       AND B.deleted = 'FALSE')
           ...
           $QSumPengambilanPotong = "SELECT SUM(berat) AS jumlahBerat,
                                     ...
                                     WHERE kd_gd_roll = '$param[kdGdRoll]'
                                     AND tgl_potong = '$param[tglRencana]'
                                     AND kd_potong NOT IN (SELECT A.kd_potong FROM transaksi_detail_pengambilan_potong A
                                                           INNER JOIN rencana_potong B ON A.kd_potong = B.kd_potong
                                                           WHERE A.kd_gd_roll = '$param[kdGdRoll]'
                                                           AND A.tgl_potong = '$param[tglRencana]'
                                                           AND A.status IN ('MANDOR POTONG (CETAK)')
         ...
         strtoupper($detailRencana[0]["ket_merek_ppic"]) == "TUMPUK"){
           $QSumPengambilanPotong = "SELECT SUM(berat) AS jumlahBerat,
                                     ...
                                     AND kd_potong IN (SELECT A.kd_potong FROM transaksi_detail_pengambilan_potong A
                                                       ...
                                                       WHERE A.kd_gd_roll = '$param[kdGdRoll]'
                                                       AND A.tgl_potong = '$param[tglRencana]'
                                                       AND A.status IN ('MANDOR POTONG (EXTRUDER)')
                                                       AND (B.ket_barang LIKE '%tumpuk%' OR B.ket_merek LIKE '%tumpuk%')
                                                       AND A.deleted = 'FALSE'
                                                       AND B.deleted = 'FALSE')
    ...
    if(strtoupper($detailRencana[0]["ket_merek"]) == "TUMPUK" ||
       ...
       strtoupper($detailRencana[0]["ket_merek_ppic"]) == "TUMPUK"){
         $QSumSisaPengambilanPotongSemalam = "SELECT SUM(TBPR.sisa) AS jumlahSisa,
                                              ...
                                              AND TBPR.kd_potong IN (SELECT A.kd_potong FROM transaksi_berat_pengambilan_roll A
                                                                     INNER JOIN rencana_potong B ON A.kd_potong = B.kd_potong
                                                                     WHERE A.kd_gd_roll = '$param[kdGdRoll]'
                                                                     AND A.tgl_potong = '$param[tglRencana]'
                                                                     AND A.keterangan IN ('POTONG BESOK')
                                                                     AND (B.ket_merek LIKE '%tumpuk%' OR B.ket_barang LIKE '%tumpuk%')
                                                                     AND A.deleted = 'FALSE'
                                                                     AND B.deleted = 'FALSE')
      ...
      $QSumSisaPengambilanPotongSemalam = "SELECT SUM(TBPR.sisa) AS jumlahSisa,
                                           ...
                                           WHERE GR.kd_gd_roll = '$param[kdGdRoll]'
                                           AND TBPR.tgl_potong='$param[tglRencana]'
                                           AND TBPR.kd_potong NOT IN (SELECT A.kd_potong FROM transaksi_berat_pengambilan_roll A
                                                                      INNER JOIN rencana_potong B ON A.kd_potong = B.kd_potong
                                                                      WHERE A.kd_gd_roll = '$param[kdGdRoll]'
                                                                      AND A.tgl_potong = '$param[tglRencana]'
                                                                      AND A.keterangan IN ('POTONG BESOK')
       ...
       strtoupper($detailRencana[0]["ket_merek_ppic"]) == "TUMPUK"){
         $QSumSisaPengambilanPotongHariIni = "SELECT SUM(TBPR.sisa) AS jumlahSisa,
                                              ...
                                              AND TBPR.kd_potong IN (SELECT A.kd_potong FROM transaksi_berat_pengambilan_roll A
                                                                     INNER JOIN rencana_potong B ON A.kd_potong = B.kd_potong
                                                                     WHERE A.kd_gd_roll = '$param[kdGdRoll]'
                                                                     AND A.tgl_sisa = '$param[tglRencana]'
                                                                     AND A.keterangan IN ('POTONG BESOK','KEMBALI GUDANG')
                                                                     AND (B.ket_merek LIKE '%tumpuk%' OR B.ket_barang LIKE '%tumpuk%')
                                                                     AND A.deleted = 'FALSE'
                                                                     AND B.deleted = 'FALSE')
  ...
  public function selectDataRencanaForInputHasilTumpuk($param){
    $currentDate = date("Y-m-d");
    $Q = "SELECT RC.*, GH.jns_brg, RP.ket_merek AS ket_merek_ppic FROM rencana_potong RC
          INNER JOIN gudang_hasil GH ON RC.kd_gd_hasil = GH.kd_gd_hasil
          LEFT JOIN rencana_ppic RP ON RC.kd_ppic = RP.kd_ppic
          WHERE RC.kd_gd_roll='$param[kdGdRoll]'
          AND RC.tgl_rencana = '$param[tglRencana]'
          AND RC.kd_potong = '$param[kdPotong]'
          AND RC.deleted='FALSE'
          AND GH.deleted='FALSE'
          ORDER BY tgl_rencana DESC,
          FIELD(RC.jns_permintaan, 'POLOS','CETAK', 'CETAK/POLOS','POLOS/CETAK','CETAK/CETAK','POLOS/POLOS'),
    ...
    $QSumSisaPengambilanPotongSemalam = "SELECT SUM(TBPR.sisa) AS jumlahSisa,
                                         ...
                                         WHERE GR.kd_gd_roll = '$param[kdGdRoll]'
                                         AND TBPR.tgl_potong='$param[tglRencana]'
                                         AND TBPR.kd_potong = '$param[kdPotong]'
                                         AND TBPR.keterangan IN ('POTONG BESOK')
                                         AND GR.deleted = 'FALSE'
                                         AND TBPR.deleted = 'FALSE'";
    $QSumSisaPengambilanPotongHariIni = "SELECT SUM(TBPR.sisa) AS jumlahSisa,
  ...
  public function selectPengambilanPotong($param){
    ...
    $this->datatables->select("TDPP.id, TDPP.tgl_sisa, TDPP.tgl_potong, TDPP.ukuran, TDPP.kd_gd_roll ,
                               TDPP.warna_plastik, TDPP.merek, TDPP.berat, TDPP.payung, TDPP.payung_kuning,
                               TDPP.bobin, TDPP.status, TGR.keterangan_waktu");
  ...
  public function selectHasilJobPotongPending(){
    $this->datatables->select("THP.kd_hasil_potong, THP.tgl_rencana, RC.no_mesin,
                               ...
                               RC.warna_plastik, TSHP.jumlah_lembar, TSHP.jumlah_berat,
                               THP.jumlah_apal_global, THP.jumlah_roll_pipa, THP.plusminus,
                               TPHP.berat_sisa_hari_ini, THP.shift,
  ...
  public function selectHasilJobPotong($param){
    $this->datatables->select("RC.no_mesin,TSHP.kd_gd_hasil,THP.kd_hasil_potong,
                 ...
                 THP.hasil_lembar, THP.hasil_berat_bersih,
                 THP.hasil_berat_kotor, TSHP.id_transaksi,
                 RC.warna_plastik, TSHP.jumlah_lembar, TSHP.jumlah_berat,
                 THP.jumlah_apal_global, THP.jumlah_roll_pipa, THP.plusminus,
                 (TPHP.berat_pengambilan_bagian + TPHP.berat_pengambilan_gudang + TPHP.berat_pengambilan_bagian_tumpuk + TPHP.berat_pengambilan_gudang_tumpuk) AS berat");
    $this->datatables->from("transaksi_hasil_potong THP");
  ...
  public function selectListBonHasilJadi($param){
    if(strtoupper($param["merek"]) == "CETAK"){
      $where = "AND RC.merek NOT IN('Zippin','MP','KP','Klip','Kantong','CB','PON','Klip In','Klip Klop') AND RC.merek NOT LIKE '%kantong%'";
    }else{
  	  if($param['merek']=="Klip"){
  		    $where = "AND (RC.merek='$param[merek]' OR RC.merek IN('PON','POLOS','KLIPPOLOS','Export','Klip Double'))";
  	  }else{
          if($param["merek"] == "Kantong"){
            $where = "AND RC.merek LIKE '%$param[merek]%'";
          }else{
  ...
  public function selectListBonHasilJadiGlobal($param){
    ...
    $Q = "SELECT RC.merek,
          ...
          AND GH.deleted='FALSE'
          AND THP.tgl_rencana='$param[tglJadi]' $where
          ORDER BY RC.merek DESC, CAST(SUBSTRING_INDEX(LCASE(RC.ukuran),'x',1) AS UNSIGNED) ASC ";
    $result = $this->db->query($Q)->result_array();
    return json_encode($result);
  }
  ...
  public function selectJumlahPerSubApal($param){
    $Q = "SELECT TDHA.jumlah_apal AS jumlah, GA.sub_jenis, GA.jenis FROM transaksi_detail_history_apal TDHA
  ...
  public function selectDataForUpdateHasil($param){
    $Q = "SELECT THP.tgl_rencana,
                 ...
                 RC.kd_ppic,
                 RC.kd_potong,
                 RC.jns_permintaan AS jnsPermintaanRencana,
    ...
    $QTumpuk = "SELECT kd_potong FROM rencana_potong
                WHERE tgl_rencana = '".$result[0]["tgl_rencana"]."'
                AND deleted='FALSE'
                AND (ket_merek LIKE '%tumpuk%' OR ket_barang LIKE '%tumpuk%')";
    $arrDataTumpuk = $this->db->query($QTumpuk)->result_array();
    $kdPotongTumpuk = $arrDataTumpuk[0]["kd_potong"];

    if(strpos(strtoupper($result[0]["ket_merek"]),"TUMPUK") !== FALSE ||
       strpos(strtoupper($result[0]["ket_barang"]),"TUMPUK") !== FALSE){
      $clause = "AND (RP.ket_merek LIKE'%tumpuk%' OR RP.ket_barang LIKE '%tumpuk%') AND RP.jns_permintaan = '".$result[0]["jnsPermintaanRencana"]."'";
    }else{
      $clause = "AND (RP.ket_merek NOT LIKE '%tumpuk%' OR RP.ket_barang NOT LIKE '%tumpuk%')";
    }

    if($result[0]["jnsPermintaanRencana"] == "CETAK" || $result[0]["jnsPermintaanRencana"] == "CETAK/POLOS"){
      $QSumPengambilanPotong = "SELECT SUM(TDPP.berat) AS jumlahBerat,
                                       SUM(TDPP.bobin) AS jumlahBobin,
                                       SUM(TDPP.payung) AS jumlahPayung,
                                       SUM(TDPP.payung_kuning) AS jumlahPayungKuning
                                FROM transaksi_detail_pengambilan_potong TDPP
                                INNER JOIN rencana_potong RP ON TDPP.kd_potong = RP.kd_potong
                                WHERE TDPP.kd_gd_roll = '".$result[0]["kd_gd_roll"]."'
                                AND TDPP.tgl_potong = '".$result[0]["tgl_rencana"]."'
                                AND TDPP.status = 'MANDOR POTONG (CETAK)'
                                $clause
                                AND RP.deleted = 'FALSE'
                                AND TDPP.deleted = 'FALSE'";

      $QSumPengambilanPotongTumpuk = "SELECT SUM(TDPP.berat) AS jumlahBeratTumpuk,
                                             SUM(TDPP.bobin) AS jumlahBobinTumpuk,
                                             SUM(TDPP.payung) AS jumlahPayungTumpuk,
                                             SUM(TDPP.payung_kuning) AS jumlahPayungKuningTumpuk
                                      FROM transaksi_detail_pengambilan_potong TDPP
                                      INNER JOIN rencana_potong RP ON TDPP.kd_potong = RP.kd_potong
                                      WHERE TDPP.kd_gd_roll = '".$result[0]["kd_gd_roll_tumpuk"]."'
                                      AND TDPP.tgl_potong = '".$result[0]["tgl_rencana"]."'
                                      AND TDPP.status = 'MANDOR POTONG (CETAK)'
                                      $clause
                                      AND RP.deleted = 'FALSE'
      ...
      $QSumPengambilanPotong = "SELECT SUM(TDPP.berat) AS jumlahBerat,
                                ...
                                WHERE TDPP.kd_gd_roll = '".$result[0]["kd_gd_roll"]."'
                                AND TDPP.tgl_potong = '".$result[0]["tgl_rencana"]."'
                                AND TDPP.status = 'MANDOR POTONG (EXTRUDER)'
                                $clause
                                AND TDPP.deleted = 'FALSE'
                                AND RP.deleted='FALSE'";
      $QSumPengambilanPotongTumpuk = "SELECT SUM(TDPP.berat) AS jumlahBeratTumpuk,
    ...
    $QSumSisaPengambilanPotongSemalam = "SELECT SUM(TBPR.sisa) AS jumlahSisa,
                                         ...
                                         FROM transaksi_berat_pengambilan_roll TBPR
                                         INNER JOIN gudang_roll GR ON TBPR.kd_gd_roll = GR.kd_gd_roll
                                         INNER JOIN rencana_potong RP ON TBPR.kd_potong = RP.kd_potong
                                         WHERE GR.kd_gd_roll = '".$result[0]["kd_gd_roll"]."'
                                         AND TBPR.tgl_potong='".$result[0]["tgl_rencana"]."'
                                         AND TBPR.keterangan IN ('POTONG BESOK')
                                         AND GR.deleted = 'FALSE'
                                         $clause
                                         AND TBPR.deleted = 'FALSE'
     ...
     $QSumSisaPengambilanPotongSemalamTumpuk = "SELECT SUM(TBPR.sisa) AS jumlahSisaTumpuk,
                                                       SUM(TBPR.payung) AS jumlahPayungTumpuk,
                                                       SUM(TBPR.payung_kuning) AS jumlahPayungKuningTumpuk,
                                                       SUM(TBPR.bobin) AS jumlahBobinTumpuk
                                                FROM transaksi_berat_pengambilan_roll TBPR
                                                INNER JOIN gudang_roll GR ON TBPR.kd_gd_roll = GR.kd_gd_roll
                                                INNER JOIN rencana_potong RP ON TBPR.kd_potong = RP.kd_potong
                                                WHERE GR.kd_gd_roll = '".$result[0]["kd_gd_roll_tumpuk"]."'
                                                AND TBPR.tgl_potong='".$result[0]["tgl_rencana"]."'
                                                AND TBPR.keterangan IN ('POTONG BESOK')
                                                AND GR.deleted = 'FALSE'
                                                $clause
     ...
     $QSumSisaPengambilanPotongHariIni = "SELECT SUM(TBPR.sisa) AS jumlahSisa,
                                          ...
                                          INNER JOIN rencana_potong RP ON TBPR.kd_potong = RP.kd_potong
                                          WHERE GR.kd_gd_roll = '".$result[0]["kd_gd_roll"]."'
                                          AND TBPR.tgl_sisa='".$result[0]["tgl_rencana"]."'
                                          AND TBPR.keterangan IN ('POTONG BESOK','KEMBALI GUDANG')
                                          AND GR.deleted = 'FALSE'
                                          $clause
                                          AND TBPR.deleted = 'FALSE'
     ...
     $QSumSisaPengambilanPotongHariIniTumpuk = "SELECT SUM(TBPR.sisa) AS jumlahSisaTumpuk,
                                                       SUM(TBPR.payung) AS jumlahPayungTumpuk,
                                                       SUM(TBPR.payung_kuning) AS jumlahPayungKuningTumpuk,
                                                       SUM(TBPR.bobin) AS jumlahBobinTumpuk
                                                FROM transaksi_berat_pengambilan_roll TBPR
    ...
    $pengambilanPotongTumpuk = $this->db->query($QSumPengambilanPotongTumpuk)->result_array();
    $sisaPengambilanPotongSemalamTumpuk = $this->db->query($QSumSisaPengambilanPotongSemalamTumpuk)->result_array();
    $sisaPengambilanPotongHariIniTumpuk = $this->db->query($QSumSisaPengambilanPotongHariIniTumpuk)->result_array();

    $returnResult = array("TransaksiHasilPotong"                => $result,
                          "PengambilanPotong"                   => $pengambilanPotong,
                          "SisaPengambilanPotongSemalam"        => $sisaPengambilanPotongSemalam,
                          "SisaPengambilanPotongHariIni"        => $sisaPengambilanPotongHariIni,
                          "PengambilanPotongTumpuk"             => $pengambilanPotongTumpuk,
                          "SisaPengambilanPotongSemalamTumpuk"  => $sisaPengambilanPotongSemalamTumpuk,
                          "SisaPengambilanPotongHariIniTumpuk"  => $sisaPengambilanPotongHariIniTumpuk,);
  ...
  public function selectLaporanHasilPotongGlobalPeriode($param){
    $Q = "SELECT SUM(IF(GR.jns_permintaan = 'POLOS',TSHP.jumlah_lembar,0)) AS jumlahLembarPolos,
                 SUM(IF(GR.jns_permintaan = 'POLOS',TSHP.jumlah_berat,0)) AS jumlahBeratPolos,
                 SUM(IF(GR.jns_permintaan = 'CETAK',TSHP.jumlah_lembar,0)) AS jumlahLembarCetak,
                 SUM(IF(GR.jns_permintaan = 'CETAK',TSHP.jumlah_berat,0)) AS jumlahBeratCetak,
                 THP.tgl_rencana
          FROM transaksi_sub_hasil_potong TSHP
          INNER JOIN transaksi_hasil_potong THP ON TSHP.kd_hasil_potong = THP.kd_hasil_potong
          INNER JOIN gudang_roll GR ON TSHP.kd_gd_roll = GR.kd_gd_roll
          ...
          AND TSHP.deleted='FALSE'
          AND GR.deleted='FALSE'
          GROUP BY THP.tgl_rencana";

    $result = $this->db->query($Q)->result_array();
    return json_encode($result);
  }

  public function selectLaporanBonApalGlobalPeriode($param){
    $Q = "SELECT SUM(THP.jumlah_apal_global) AS jumlahApal,
                 THP.tgl_rencana
          FROM transaksi_hasil_potong THP
          WHERE THP.tgl_rencana BETWEEN '$param[tglAwal]' AND '$param[tglAkhir]'
          AND THP.deleted='FALSE'
  ...
  public function selectLaporanApalPerMerek($param){
    $Q = "SELECT GROUP_CONCAT(RP.customer SEPARATOR '<br>') AS nama_customer,
                 GROUP_CONCAT(GH.ukuran,' ',GH.merek SEPARATOR '<br>') nama_barang,
                 GROUP_CONCAT(TSHP.jumlah_lembar SEPARATOR '|') AS lembar_per_job,
                 GROUP_CONCAT(TSHP.jumlah_berat SEPARATOR '|') AS berat_per_job,
                 THP.tgl_rencana,
                 THP.hasil_lembar,
                 THP.hasil_berat_kotor,
                 THP.hasil_berat_bersih,
                 THP.jumlah_apal_global,
                 ((THP.jumlah_apal_global / THP.hasil_berat_bersih) * 100) AS persentase
         FROM transaksi_hasil_potong THP
         INNER JOIN transaksi_sub_hasil_potong TSHP ON TSHP.kd_hasil_potong = THP.kd_hasil_potong
         INNER JOIN rencana_potong RP ON TSHP.kd_potong = RP.kd_potong
         INNER JOIN gudang_hasil GH ON TSHP.kd_gd_hasil = GH.kd_gd_hasil
         WHERE THP.tgl_rencana BETWEEN '$param[tglAwal]' AND '$param[tglAkhir]'
  ...
  public function updateHasilCutting($param){
    ...
    $checkBarang = $this->db->query("SELECT GH.jns_brg
                                     FROM gudang_hasil GH
                                     INNER JOIN transaksi_sub_hasil_potong TSHP ON TSHP.kd_gd_hasil = GH.kd_gd_hasil
                                     WHERE TSHP.id_transaksi = '$idTransaksi'
                                     AND TSHP.deleted='FALSE'
                                     AND GH.deleted='FALSE'")->result_array();
    $jnsBarang = $checkBarang[0]["jns_brg"];
    ...
    if(floatval($param["THP"]["hasil_lembar"])        != floatval($arrDataLama[0]["hasil_lembar"]) ||
       floatval($param["THP"]["hasil_berat_bersih"])  != floatval($arrDataLama[0]["hasil_berat_bersih"]) ||
       floatval($param["THP"]["hasil_berat_kotor"])   != floatval($arrDataLama[0]["hasil_berat_kotor"]) ||
       floatval($param["THP"]["jumlah_apal_global"])  != floatval($arrDataLama[0]["jumlah_apal_global"]) ||
       floatval($param["TSHP"]["jumlah_lembar"])      != floatval($arrDataLama[0]["jumlah_lembar"]) ||
       floatval($param["TSHP"]["jumlah_berat"])       != floatval($arrDataLama[0]["jumlah_berat"])
    ){
      $QCheckLock = "SELECT COUNT(id_permintaan_jadi) AS counterLock FROM transaksi_gudang_hasil
                     ...
                     AND sts_barang = '$jnsBarang'
                     AND deleted='FALSE'";
      $arrCheckLock = $this->db->query($QCheckLock)->result_array();
      $totalLock = $arrCheckLock[0]["counterLock"];
    }else if(floatval($param["THP"]["jumlah_payung"])                             != floatval($arrData[0]["jumlah_payung"]) ||
             floatval($param["THP"]["jumlah_payung_kuning"])                      != floatval($arrData[0]["jumlah_payung_kuning"]) ||
             floatval($param["THP"]["jumlah_bobin"])                              != floatval($arrData[0]["jumlah_bobin"]) ||
             floatval($param["THP"]["jumlah_roll_pipa"])                          != floatval($arrData[0]["jumlah_roll_pipa"]) ||
             floatval($param["TPHP"]["berat_pengambilan_gudang"])                 != floatval($arrData[0]["berat_pengambilan_gudang"]) ||
             floatval($param["TPHP"]["bobin_pengambilan_gudang"])                 != floatval($arrData[0]["bobin_pengambilan_gudang"]) ||
             floatval($param["TPHP"]["payung_pengambilan_gudang"])                != floatval($arrData[0]["payung_pengambilan_gudang"]) ||
             floatval($param["TPHP"]["payung_kuning_pengambilan_gudang"])         != floatval($arrData[0]["payung_kuning_pengambilan_gudang"]) ||
             floatval($param["TPHP"]["berat_pengambilan_gudang_tumpuk"])          != floatval($arrData[0]["berat_pengambilan_gudang_tumpuk"]) ||
             floatval($param["TPHP"]["bobin_pengambilan_gudang_tumpuk"])          != floatval($arrData[0]["bobin_pengambilan_gudang_tumpuk"]) ||
             floatval($param["TPHP"]["payung_pengambilan_gudang_tumpuk"])         != floatval($arrData[0]["payung_pengambilan_gudang_tumpuk"]) ||
             floatval($param["TPHP"]["payung_kuning_pengambilan_gudang_tumpuk"])  != floatval($arrData[0]["payung_kuning_pengambilan_gudang_tumpuk"])
    ){
      if(strpos($arrDataLama[0]["jns_permintaan"], "/") !== FALSE){
        $QCheckLock = "SELECT COUNT(id) AS counterLock FROM transaksi_gudang_roll
                       WHERE status_lock='TRUE'
                       AND DATE_FORMAT(tgl_transaksi, '%Y-%m') = '$periodeLock'
                       AND jns_permintaan IN ('CETAK','POLOS')
                       AND deleted='FALSE'";
      }else{
        $jnsPermintaan1 = $arrDataLama[0]["jns_permintaan"];
        $QCheckLock = "SELECT COUNT(id) AS counterLock FROM transaksi_gudang_roll
                       WHERE status_lock='TRUE'
                       AND DATE_FORMAT(tgl_transaksi, '%Y-%m') = '$periodeLock'
                       AND jns_permintaan = '$jnsPermintaan1'
                       AND deleted='FALSE'";
      }
      $arrCheckLock = $this->db->query($QCheckLock)->result_array();
      $totalLock = $arrCheckLock[0]["counterLock"];
      ...
      if(strpos($arrDataLama[0]["jns_permintaan"], "/") !== FALSE){
        $QCheckLock1 = "SELECT COUNT(id) AS counterLock FROM transaksi_gudang_roll
                       WHERE status_lock='TRUE'
                       AND DATE_FORMAT(tgl_transaksi, '%Y-%m') = '$periodeLock'
                       AND jns_permintaan IN ('CETAK','POLOS')
                       AND deleted='FALSE'";
      ...
      $arrCheckLock1 = $this->db->query($QCheckLock1)->result_array();
      ...
      $QCheckLock = "SELECT COUNT(id_permintaan_jadi) AS counterLock FROM transaksi_gudang_hasil
                     ...
                     AND deleted='FALSE'";
      $arrCheckLock = $this->db->query($QCheckLock)->result_array();
      $totalLock = $arrCheckLock[0]["counterLock"] + $arrCheckLock1[0]["counterLock"];
    }
    ...
    if($totalLock > 0){
      ...
      if(count($arrDataTransaksiGudangRoll) > 0){
        ...
        if(strpos($arrDataLama[0]["jns_permintaan"], "/") !== FALSE){
          $arrJnsPermintaan = explode("/",$arrDataLama[0]["jns_permintaan"]);
          $jnsPermintaan = $arrJnsPermintaan[0];
        }else{
          $jnsPermintaan = $arrDataLama[0]["jns_permintaan"];
        }

        $this->db->set("kd_gd_roll",$arrDataLama[0]["kd_gd_roll"]);
        $this->db->set("id_user",$param["THP"]["id_user"]);
        $this->db->set("jns_permintaan",$jnsPermintaan);
        $this->db->set("tgl_transaksi",$arrDataLama[0]["tgl_rencana"]);
        $this->db->set("bagian","POTONG");
        $this->db->set("keterangan_transaksi","KELUAR KE POTONG");
        $this->db->set("status_history","KELUAR");
        $this->db->set("status_transaksi","PROGRESS");
        $this->db->set("keterangan_history","OPERATOR POTONG");
        $this->db->set("berat",$beratPengambilanBaru);
        $this->db->set("bobin",$bobinPengambilanBaru);
        $this->db->set("payung",$payungPengambilanBaru);
        $this->db->set("payung_kuning",$payungKuningPengambilanBaru);
        $this->db->set("shift","1");
        $this->db->insert("transaksi_gudang_roll");
      }
      ...
      if(!empty($arrDataLama[0]["kd_gd_roll_tumpuk"])){
        ...
        if(count($arrDataTransaksiGudangRollTumpuk) > 0){
          ...
          $beratPengambilanBaru = $param["TPHP"]["berat_pengambilan_gudang_tumpuk"];
          $bobinPengambilanBaru = $param["TPHP"]["bobin_pengambilan_gudang_tumpuk"];
          $payungPengambilanBaru = $param["TPHP"]["payung_pengambilan_gudang_tumpuk"];
          $payungKuningPengambilanBaru = $param["TPHP"]["payung_kuning_pengambilan_gudang_tumpuk"];

          if(strpos($arrDataLama[0]["jns_permintaan"], "/") !== FALSE){
            $arrJnsPermintaan = explode("/",$arrDataLama[0]["jns_permintaan"]);
            $jnsPermintaan = $arrJnsPermintaan[1];
          }else{
            $jnsPermintaan = $arrDataLama[0]["jns_permintaan"];
          }

          $this->db->set("kd_gd_roll",$arrDataLama[0]["kd_gd_roll_tumpuk"]);
          $this->db->set("id_user",$param["THP"]["id_user"]);
          $this->db->set("jns_permintaan",$jnsPermintaan);
          $this->db->set("tgl_transaksi",$arrDataLama[0]["tgl_rencana"]);
          $this->db->set("bagian","POTONG");
    ...
    public function updateHistorySisa($param){
      ...
      if($QCheckLockTGR[0]["counter"] > 0){
        ...
        $QTDPP = "SELECT TDPP.id,
              ...
              $this->db->where("tgl_transaksi IN('$tglPotong','$tglSisa')");
              $this->db->where("deleted","FALSE");
              $this->db->update("transaksi_gudang_roll");
    ...
    public function updateDeleteAndRestorePengambilanPotong($param){
      ...
      $periodeLock = date("Y-m",strtotime($arrDataTDPP[0]["tgl_sisa"]));
    ...
    public function updateDeleteSisaPengambilanPotongTemp($param){
      $this->db->trans_begin();
      $Q = "SELECT * FROM transaksi_berat_pengambilan_roll WHERE id='$param'";
      $arrDataTBPR = $this->db->query($Q)->result_array();
      foreach ($arrDataTBPR as $arrValue) {
        $kdGdRoll = $arrValue["kd_gd_roll"];
        $kdPotong = $arrValue["kd_potong"];
        $shift = $arrValue["shift"];
        $berat = $arrValue["sisa"];
        $bobin = $arrValue["bobin"];
        $payung = $arrValue["payung"];
        $payungKuning = $arrValue["payung_kuning"];
        $tglSisa = $arrValue["tgl_sisa"];
        $tglPotong = $arrValue["tgl_potong"];
        $keterangan = $arrValue["keterangan"];
        $statusTransaksi = $arrValue["sts_transaksi"];
        ...
        #========== Update Table Transaksi Detail Pengambilan Potong (Start) ==========#
        $this->db->set("deleted","TRUE");
        $this->db->where(array("kd_gd_roll"     => $kdGdRoll,
                               "kd_potong"      => $kdPotong,
                               "tgl_sisa"       => $tglSisa,
        ...
        $this->db->update("transaksi_detail_pengambilan_potong");
        #========== Update Table Transaksi Detail Pengambilan Potong (Finish) ==========#

        #========== Update Table Transaksi Gudang Roll (Start) ==========#
        $this->db->set("deleted","TRUE");
        $this->db->where("kd_gd_roll", $kdGdRoll);
        $this->db->where("tgl_transaksi IN ('$tglSisa','$tglPotong')");
        $this->db->where("berat", $berat);
        $this->db->where("bobin", $bobin);
        $this->db->where("payung", $payung);
        $this->db->where("payung_kuning", $payungKuning);
        $this->db->where("status_transaksi", $statusTransaksi);
        $this->db->update("transaksi_gudang_roll");
        #========== Update Table Transaksi Gudang Roll (Finish) ==========#

        #========== Update Table Transaksi Berat Pengambilan Roll (Start) ==========#
        $this->db->set("deleted","TRUE");
        $this->db->where("id", $param);
        $this->db->update("transaksi_berat_pengambilan_roll");
        #========== Update Table Transaksi Berat Pengambilan Roll (Finish) ==========#
      }
      if($this->db->trans_status() === FALSE){
        $this->db->trans_rollback();
        return "Gagal";
    ...
    public function updateSisaPengambilanPotongTemp($param){
      $this->db->trans_begin();
      $Q = "SELECT * FROM transaksi_berat_pengambilan_roll WHERE id='$param[id]'";
      $arrDataTBPR = $this->db->query($Q)->result_array();
      foreach ($arrDataTBPR as $arrValue) {
        $kdGdRoll = $arrValue["kd_gd_roll"];
        $kdPotong = $arrValue["kd_potong"];
        ...
        $this->db->where(array("kd_gd_roll"     => $kdGdRoll,
                               ...
                               "payung_kuning"  => $payungKuning,
                               "sts_transaksi"  => $statusTransaksi));
        $this->db->update("transaksi_detail_pengambilan_potong",$param["TDPP"]);
        #========== Update Table Transaksi Detail Pengambilan Potong (Finish) ==========#

        #========== Update Table Transaksi Gudang Roll (Start) ==========#
        if($keterangan == "POTONG BESOK"){
          $this->db->where("kd_gd_roll", $kdGdRoll);
          $this->db->where("tgl_transaksi",$tglSisa);
          $this->db->where("berat", $berat);
          $this->db->where("bobin", $bobin);
          $this->db->where("payung", $payung);
          $this->db->where("payung_kuning", $payungKuning);
          $this->db->where("status_transaksi", $statusTransaksi);
          $this->db->where("keterangan_history", $keteranganHistory);
          $this->db->update("transaksi_gudang_roll",$param["TGR_IN"]);

          $this->db->where("kd_gd_roll", $kdGdRoll);
          $this->db->where("tgl_transaksi",$tglPotong);
          $this->db->where("berat", $berat);
          $this->db->where("bobin", $bobin);
          $this->db->where("payung", $payung);
          $this->db->where("payung_kuning", $payungKuning);
          $this->db->where("status_transaksi", $statusTransaksi);
          $this->db->where("keterangan_history", $keteranganHistory);
          $this->db->update("transaksi_gudang_roll",$param["TGR_OUT"]);
        }else{
          $this->db->where("kd_gd_roll", $kdGdRoll);
          $this->db->where("tgl_transaksi",$tglSisa);
          $this->db->where("berat", $berat);
    ...
    public function updateDeleteHasilCutting($param){
      $this->db->trans_begin();
      $Q = "SELECT TSHP.*, RC.satuan, RC.kd_ppic
            FROM transaksi_sub_hasil_potong TSHP
            INNER JOIN rencana_potong RC ON TSHP.kd_potong = RC.kd_potong
            WHERE TSHP.kd_hasil_potong='$param'";
      $resultTSHP = $this->db->query($Q)->result_array();
      $kodeTumpuk = "";
      foreach ($resultTSHP as $arrTSHP) {
        if(!empty($arrTSHP["kd_gd_roll_tumpuk"])){
          $kodeTumpuk = $arrTSHP["kd_gd_roll_tumpuk"];
        }
        #========== UPDATE RENCANA POTONG (START) ==========#
        if($arrTSHP["satuan"] == "LEMBAR"){
          $this->db->set("jml_permintaan","jml_permintaan + ".$arrTSHP["jumlah_lembar"], FALSE);
        }else{
          $this->db->set("jml_permintaan","jml_permintaan + ".$arrTSHP["jumlah_berat"], FALSE);
        }
        $this->db->set("sts_pengerjaan","PROGRESS");
        $this->db->where("kd_potong","$arrTSHP[kd_potong]");
        $this->db->update("rencana_potong");
        #========== UPDATE RENCANA POTONG (FINISH) ==========#

        #========== UPDATE RENCANA PPIC (START) ==========#
        if(!empty($arrTSHP["kd_ppic"])){
          if($arrTSHP["satuan"] == "LEMBAR"){
            $this->db->set("sisa","sisa + ".$arrTSHP["jumlah_lembar"], FALSE);
          }else{
            $this->db->set("sisa","sisa + ".$arrTSHP["jumlah_berat"], FALSE);
          }
          $this->db->set("sts_pengerjaan","PROGRESS");
          $this->db->where("kd_ppic","$arrTSHP[kd_ppic]");
          $this->db->update("rencana_ppic");
        }
        #========== UPDATE RENCANA POTONG (FINISH) ==========#

        $QTHP = "SELECT THP.*, TPHP.*
                 FROM transaksi_hasil_potong THP
                 INNER JOIN transaksi_pengambilan_hasil_potong TPHP ON TPHP.kd_hasil_potong = THP.kd_hasil_potong
                 WHERE THP.kd_hasil_potong = '$param'
                 AND THP.deleted='FALSE'
                 AND TPHP.deleted='FALSE'";
        $resultTHP = $this->db->query($QTHP)->result_array();
        foreach ($resultTHP as $arrTHP) {
          if(!empty($kodeTumpuk)){
            $this->db->set("deleted","TRUE");
            $this->db->where("tgl_transaksi",$arrTHP["tgl_rencana"]);
            $this->db->where("bagian","POTONG");
            $this->db->where("keterangan_transaksi","KELUAR KE POTONG");
            $this->db->where("status_history","KELUAR");
            $this->db->where("status_transaksi",$arrTHP["status_transaksi"]);
            $this->db->where("keterangan_history","OPERATOR POTONG");
            $this->db->where("berat",round($arrTHP["berat_pengambilan_gudang"],2));
            $this->db->where("bobin",$arrTHP["bobin_pengambilan_gudang"]);
            $this->db->where("payung",$arrTHP["payung_pengambilan_gudang"]);
            $this->db->where("payung_kuning",$arrTHP["payung_kuning_pengambilan_gudang"]);
            $this->db->where("shift",$arrTHP["shift"]);
            $this->db->update("transaksi_gudang_roll");
            ...
            $this->db->where("status_transaksi",$arrTHP["status_transaksi"]);
            $this->db->where("keterangan_history","OPERATOR POTONG");
            $this->db->where("berat",round($arrTHP["berat_pengambilan_gudang_tumpuk"],2));
            $this->db->where("bobin",$arrTHP["bobin_pengambilan_gudang_tumpuk"]);
            $this->db->where("payung",$arrTHP["payung_pengambilan_gudang_tumpuk"]);
            $this->db->where("payung_kuning",$arrTHP["payung_kuning_pengambilan_gudang_tumpuk"]);
            $this->db->where("shift",$arrTHP["shift"]);
            $this->db->update("transaksi_gudang_roll");
          }else{
            $this->db->set("deleted","TRUE");
        ...
        $this->db->set("deleted","TRUE");
        $this->db->where("kd_hasil_potong",$param);
        $this->db->update("transaksi_hasil_potong");
  ...
  public function hitungBeratRoll($param){
    if(is_array($param)){
      ...
      if($jenisPipa == "PAYUNG"){
        ...
        if($panjangPlastik < 5){
          $rumusRoll = 5000;
        }else if($panjangPlastik >= 5 && $panjangPlastik <= 40){
          $rumusRoll = 6000;
        }else{
          $rumusRoll = 7000;
        }
