  ...
  <script type="text/javascript">
    $(function () {
      ...
      // countTrashPurceshing()
			...
			if($("#tableDataPermintaanPlastikHD").length > 0){
				datatableDataPermintaanPlastik();
			}
    });
  </script>
  <script type="text/javascript">
  ...
  <script type="text/javascript">
    function counterPermintaanBarang(){
      $.ajax({
		...
		function counterPermintaanPlastik(){
      $.ajax({
        url  : "<?php echo site_url('_purchasing/main/checkPermintaanPlastik'); ?>",
        dataType : "JSON",
        success:function(response){
					var jumlah = parseFloat(response.PermintaanPlastik);
         $("#Permintaan_Plastik").html('<i class="fa fa-book"></i>Permintaan Plastik HD <span class="pull-right-container">'+
				 '<span class="label bg-blue">'+jumlah+'</span>'+
				 '</span>');
        }
      });
  ...
  function datatableHistoryApal(tgl_awal,tgl_akhir,kd_apal){
	...
	function datatableDataPermintaan(){
		...
		$("#tableDataPermintaan").dataTable({
			...
			"fnRowCallback":function(nRow,aData,iDisplayIndex,iDisplayIndexFull){
				$("td:eq(0)",nRow).text(++iDisplayIndex);
				$("td:eq(6)",nRow).html("<button class='btn btn-md btn-flat btn-primary' onclick=modalLihatDetailPermintaan('"+aData["kd_permintaan_barang"]+"','"+aData["status_permintaan"]+"')>Lihat Permintaan</button>"+
	...
	function modalLihatDetailPermintaan(param, param2=''){
		$("#modalLihatPesanan").modal({
			backdrop:"static"
		});
		if(param2.toUpperCase() != "PENDING"){
			$("#btnSetujuiPermintaanBarang").attr("onclick","batalSetujuiPermintaanBarang('"+param+"')")
																			.html("<i class='fa fa-undo'></i> Batal Setujui");
		}else{
				...
				success : function(response){
					if($.trim(response) === "Berhasil"){
						...
						setTimeout(function(){
							...
							modalLihatDetailPermintaan(param,'PROGRESS');
						},2000);
						setTimeout(function(){
							$("#modal-notif").removeClass("modal-info");
							$("#modalNotifContent").text("");
	...
	function batalSetujuiPermintaanBarang(param){
		if(confirm("Apakah Anda Yakin Membatalkan Persetujuan Permintaan Ini?")){
			$.ajax({
				type : "POST",
				url : "<?php echo base_url('_purchasing/main/batalSetujuiPermintaanBarang') ?>",
				dataType : "TEXT",
				data : {
					idPermintaan : param
				},
				success : function(response){
					if($.trim(response) === "Berhasil"){
						...
						setTimeout(function(){
							$("#modal-notif").modal("hide");
							$(".active a").trigger("click");
							modalLihatDetailPermintaan(param,'PENDING');
						},2000);
						setTimeout(function(){
							$("#modal-notif").removeClass("modal-info");
							$("#modalNotifContent").text("");
	...
	function setujuiPermintaanPlastik(param){
		if(confirm("Apakah Anda Yakin Menyetujui Permintaan Ini?")){
			$.ajax({
				type : "POST",
				url : "<?php echo base_url('_purchasing/main/setujuiPermintaanPlastik') ?>",
				dataType : "TEXT",
				data : {
					idPermintaan : param
				},
	...
	function modalBatalkanPermintaanPlastik(param1, param2){
		$("#modalBatalPermintaan").modal({
			backdrop : "static"
		});
		$("#btnBatalPermintaan").attr("onclick","batalkanPermintaanPlastik('"+param1+"','"+param2+"')");
	}
	...
	function modalAktifkanPermintaanPlastik(param1, param2){
		$("#modalAktifkanPermintaan").modal({
			backdrop : "static"
		});
		$("#btnAktifkanPermintaan").attr("onclick","aktifkanPermintaanPlastik('"+param1+"','"+param2+"')");
	}
	...
	function modalUbahPermintaan(param1, param2){
		$.ajax({
			type : "POST",
			url : "<?= base_url('_purchasing/main/getDetailDataPembelianBarangItem'); ?>",
			dataType : "JSON",
			data : {
				idDpb : param1
			},
			success : function(response){
				$("#btnSimpanPembelian").attr("onclick","editDetailPermintaan('"+param1+"')");
				$("#txtKdPermintaan").val(response[0].kd_permintaan_barang);
				$("#txtTanggalBeli").val(response[0].tgl_permintaan);
				$("#txtNamaBahan").val(response[0].nm_barang);
				$("#txtJumlahPermintaan").val(parseFloat(response[0].jumlah_permintaan));
				$("#txtKeterangan").val(response[0].keterangan);

				$("#cmbGudangBahan").select2({
					placeholder : "Pilih Bahan ("+response[0].jenis+")",
					dropdownParent: $('#modalTambahPembelianBahanBaru'),
					width : "100%",
					cache:true,
					allowClear:true,
					ajax:{
						url : "<?php echo base_url(); ?>_purchasing/main/getComboBoxValueBahan/"+response[0].jenis.replace(/ /gi,"_"),
						dataType : "JSON",
						delay : 0,
						processResults : function(data){
							return{
				...
				$("#cmbGudangBahan").on("select2:select", function(){
					var text = $("#cmbGudangBahan").select2("data")[0]["text"];
					var arrText = text.split(" - ");
					$("#txtNamaBahan").val(arrText[1]);
				});
		...
		$("#modalTambahPembelianBahanBaru").modal({
			backdrop:"static"
		});
		$("#modalTambahPembelianBahanBaruTitle").text("Revisi Pembelian Bahan Baku");
		$("#modalTambahPembelianBahanBaruLegend").text("Formulir Revisi Permintaan Pembelian Bahan Baku");
	}

	function editDetailPermintaan(param){
		var kdPermintaan = $("#txtKdPermintaan").val();
		var kdGdBahan = $("#cmbGudangBahan").val();
		var tglBeli = $("#txtTanggalBeli").val();
		var namaBahan = $("#txtNamaBahan").val();
		var jumlahPermintaan = $("#txtJumlahPermintaan").val().replace(/,/gi, "");
		var keterangan = $("#txtKeterangan").val();

		if(namaBahan=="" ||  jumlahPermintaan=="" ||  keterangan==""){
			$("#modal-notif").addClass("modal-warning");
			...
			$.ajax({
				type : "POST",
				url : "<?= base_url('_purchasing/main/editDetailPermintaan'); ?>",
				dataType : "TEXT",
				data : {
					idDpb							: param,
					kdGdBahan					: kdGdBahan,
				...
				success : function(response){
					if(jQuery.trim(response) === "Berhasil"){
						...
						setTimeout(function(){
							...
							$("#modal-notif").removeClass("modal-info");
							$("#modalNotifContent").text("");
							$("#cmbGudangBahan").val("").change();
							$("#txtNamaBahan").val("");
							$("#txtJumlahPermintaan").val("");
							$("#txtKeterangan").val("");
							$("#modalTambahPembelianBahanBaru").modal("hide");
							$(".active a[aria-controls='tableDetailPermintaan']").click();
						},2000);
	...
	function batalkanPermintaanPlastik(param,param2){
		var keteranganPurchasing = $("#txtKeteranganPurchasing").val();
		if(keteranganPurchasing == ""){
			$("#modal-notif").modal("show")
			$("#modalNotifContent").html("<div style='text-align: center;'><b>Semua kolom harus diisi.</b></div>");
			...
			$.ajax({
				type : "POST",
				url : "<?php echo base_url('_purchasing/main/batalkanPermintaanPlastik') ?>",
				dataType : "TEXT",
				data : {
					idTransaksi : param,
					idPermintaan : param2,
	...
	function aktifkanPermintaanPlastik(param,param2){
		var statusPermintaan = $("#cmbStatusPermintaan").val();
		if(statusPermintaan == ""){
			$("#modal-notif").modal("show")
			$("#modalNotifContent").html("<div style='text-align: center;'><b>Semua kolom harus diisi.</b></div>");
			...
			$.ajax({
				type : "POST",
				url : "<?php echo base_url('_purchasing/main/aktifkanPermintaanPlastik') ?>",
				dataType : "TEXT",
				data : {
					idTransaksi : param,
					idPermintaan : param2,
	...
	function datatablesDetailPermintaan(param){
		...
		$("#tableDetailPermintaan").dataTable({
			...
			"fnRowCallback":function(nRow,aData,iDisplayIndex,iDisplayIndexFull){
				$("td:eq(0)",nRow).text(++iDisplayIndex);
				$("td:eq(3)",nRow).text(parseFloat(aData["jumlah_permintaan"]).toLocaleString());
				if(aData["status_permintaan"] == "PENDING"){
					$("td:eq(6)",nRow).html("<button class='btn btn-md btn-flat btn-warning' onclick=modalUbahPermintaan('"+aData["id_dpb"]+"','"	+aData["kd_permintaan_barang"]+"')><i class='fa fa-edit'></i> Ubah</button>"+
				...
				$.each(response,function(index,value){
					...
					$("#tableListPembelianBahanBakuTemp > tbody:last-child").append(
						"<tr>"+
							...
							"<td>"+parseFloat(value.jumlah_permintaan).toLocaleString()+"</td>"+
			...
			success : function(response){
				if(jQuery.trim(response) === "Berhasil"){
					...
					setTimeout(function(){
						...
						tableListPembelianBahanBakuTemp(param);
						modalTambahPembelianBahanBaku(param.replace(" ","_"));
					},2000);
				}else if(jQuery.trim(response) === "Gagal"){
					$("#modal-notif").addClass("modal-danger");
	...
	function datatableDataPermintaanPlastik(){
		$("#tableDataPermintaanPlastikHD").dataTable().fnDestroy();
		$("#tableDataPermintaanPlastikHD").dataTable({
			// "fixedHeader" : true,
			"autoWidth" : false,
			"ordering" : false,
			"bProcessing" : true,
			...
			"info" : false,
			"sPaginationType": "full_numbers",
			"sAjaxSource":"<?php echo base_url(); ?>_purchasing/main/getPermintaanPlastik",
			"columns":[
				{"data" : "kd_permintaan_plastik","name":"TPP.kd_permintaan_plastik"},
				{"data" : "kd_permintaan_plastik","name":"TPP.kd_permintaan_plastik"},
				{"data" : "tgl_permintaan","name":"TPP.tgl_permintaan"},
				{"data" : "status_permintaan","name":"TPP.status_permintaan"},
				{"data" : "username","name":"USR.username"},
				{"data" : "kd_permintaan_plastik","name":"TPP.kd_permintaan_plastik"}
			],
			"fnServerData": function (sSource, aoData, fnCallback){
			...
			"fnRowCallback":function(nRow,aData,iDisplayIndex,iDisplayIndexFull){
				$("td:eq(0)",nRow).text(++iDisplayIndex);
				$("td:eq(5)",nRow).html("<button class='btn btn-md btn-flat btn-primary' onclick=modalLihatDetailPermintaanPlastik('"+aData["kd_permintaan_plastik"]+"')>Lihat Permintaan</button>"+
																"<a href='<?php echo base_url('_purchasing/main/printBonPermintaanPlastik/') ?>"+aData["kd_permintaan_plastik"]+"' target='_blank' class='btn btn-md btn-flat btn-success'>Cetak Bon Permintaan</a>");//+
																//"<button class='btn btn-md btn-flat btn-danger' onclick=deleteAndRestorePermintaanBarang('"+aData["kd_permintaan_barang"]+"','TRUE')>Hapus</button>");
			}
		});
	}
	...
	function datatablesDetailPermintaanPlastikHd(param=""){
		...
		$("#tableDetailPermintaanPlastikHD").dataTable({
			...
			// "scrollY" : "720px",
			"sPaginationType": "full_numbers",
			"sAjaxSource":"<?php echo base_url(); ?>_purchasing/main/getDetailPermintaanPlastikHdBaru",
			"columns":[
			...
			"fnRowCallback":function(nRow,aData,iDisplayIndex,iDisplayIndexFull){
				$("td:eq(0)",nRow).text(++iDisplayIndex);
				$("td:eq(5)",nRow).text(parseFloat(aData["jumlah_berat_permintaan"]).toLocaleString());
				$("td:eq(6)",nRow).text(parseFloat(aData["jumlah_lembar_permintaan"]).toLocaleString());
				if(aData["status_permintaan"] == "PENDING"){
					$("td:eq(9)",nRow).html(//"<button class='btn btn-md btn-flat btn-default'><i class='fa fa-lock'></i> Permintaan Belum Disetujui Oleh Purchasing</button>"+
																	"<button class='btn btn-md btn-flat btn-danger' onclick=modalBatalkanPermintaanPlastik('"+aData["id_dpp"]+"','"+aData["kd_permintaan_plastik"]+"')>Batalkan Permintaan</button>");
				}else if(aData["status_permintaan"] == "CANCEL"){
					$("td:eq(9)",nRow).html(//"<button class='btn btn-md btn-flat btn-default'><i class='fa fa-lock'></i> Permintaan Belum Disetujui Oleh Purchasing</button>"+
																	"<button class='btn btn-md btn-flat btn-success' onclick=modalAktifkanPermintaanPlastik('"+aData["id_dpp"]+"','"+aData["kd_permintaan_plastik"]+"')>Aktifkan Permintaan</button>");
				}else{
					$("td:eq(9)",nRow).html(//"<button class='btn btn-md btn-flat btn-default'><i class='fa fa-lock'></i> Permintaan Belum Disetujui Oleh Purchasing</button>"+
																	"<button class='btn btn-md btn-flat btn-default')><i class='fa fa-lock'></i> Tidak Ada Aksi</button>");
				}
				...
				if(aData["status_permintaan"] == "CANCEL" && (aData["keterangan_purchasing"] != null && aData["keterangan_purchasing"] != "")){
					$("td:eq(8)",nRow).html("<label class='text-red'>["+aData["status_permintaan"]+"]</label><br>"+
																	 "<label class='text-blue'>["+aData["keterangan_purchasing"]+"]</label>");
				}else{
					$("td:eq(8)",nRow).text(aData["status_permintaan"]);
				}
			}
		});
	}

	function modalLihatDetailPermintaanPlastik(param){
		$("#modalLihatPesanan").modal({
			backdrop:"static"
		});
		$("#btnSetujuiPermintaanBarang").attr("onclick","setujuiPermintaanPlastik('"+param+"')");
		datatablesDetailPermintaanPlastikHd(param);
	}
  </script>
	</body>
</html>
