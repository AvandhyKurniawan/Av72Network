...
class Main extends MX_Controller{
    ...
    if($isLogin){
		...
		$listMenu["parentMenu"] = array(
		  ...
		  array("Content"=>"Spare Part","Link"=>"_purchasing/main/master_sparepart","Name"=>"Master_SparePart","Status"=>"Single","Icon"=>"fa fa-book","Id"=>""),
			...
			array("Content"=>"Permintaan Plastik HD","Link"=>"_purchasing/main/permintaan_plastik","Name"=>"Permintaan_Plastik","Status"=>"Single","Icon"=>"fa fa-book","Id"=>"Permintaan_Plastik"),
			...
			array("Content"=>"Bukti Penerimaan Plastik HD","Link"=>"_purchasing/main/bukti_penerimaan_plastik","Name"=>"Bukti_Penerimaan_Plastik","Status"=>"Single","Icon"=>"fa fa-book","Id"=>"M_Bukti_Penerimaan_Plastik")
		);
		return $listMenu;
    	}else if($status==2) {
    		$listMenu["parentMenu"] = array();
  ...
  function getTotalKeluarApal(){
	...
	public function batalSetujuiPermintaanBarang(){
		$isLogin = $this->isLogin();
    if($isLogin){
			$idPermintaan = $this->input->post("idPermintaan");
			$result = $this->Purchase_Model->updateBatalSetujuiPermintaanBarang($idPermintaan);
			echo $result;
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
      redirect("_main/main","refresh");
	...
	public function permintaan_plastik(){
    $isLogin = $this->isLogin();
    if($isLogin){
	...
	public function getPermintaanPlastik(){
    $isLogin = $this->isLogin();
    if($isLogin){
      $result = $this->Purchase_Model->selectPermintaanPlastik();
      echo $result;
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
      redirect("_main/main","refresh");
    ...
    $isLogin = $this->isLogin();
    if($isLogin){
      $data = array("DataPermintaan" => $this->Purchase_Model->selectPermintaanPlastik_Print($param),
                    "DataDetailPermintaan" => $this->Purchase_Model->selectDetailPermintaanPlastik_Print($param));
    ...
    if($isLogin){
      ...
      $data = array("idUser" => $idUser,
                    "bagian" => $bagian,
                    "idPermintaan" => $idPermintaan);
      $result = $this->Purchase_Model->selectDetailPermintaanPlastikHdBaru($data);
      echo $result;
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
      redirect("_main/main","refresh");
	...
	function checkPermintaanPlastik(){
		$result = $this->Purchase_Model->checkPermintaanPlastik();
		echo $result;
	}

	public function batalkanPermintaanPlastik(){
		$isLogin = $this->isLogin();
    if($isLogin){
			$idTransaksi = $this->input->post("idTransaksi");
			$idPermintaan = $this->input->post("idPermintaan");
			...
			if(empty($keteranganPurchasing)){
				...
				$data = array("idTransaksi" => $idTransaksi,
											"idPermintaan" => $idPermintaan,
											"keteranganPurchasing" => $keteranganPurchasing);
				$result = $this->Purchase_Model->updateBatalkanPermintaanPlastik($data);
				echo $result;
			}
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
	...
	public function aktifkanPermintaanPlastik(){
		$isLogin = $this->isLogin();
    if($isLogin){
			$idTransaksi = $this->input->post("idTransaksi");
			$idPermintaan = $this->input->post("idPermintaan");
			...
			if(empty($statusPermintaan)){
				...
				$data = array("idTransaksi" => $idTransaksi,
											"idPermintaan" => $idPermintaan,
											"statusPermintaan" => $statusPermintaan);
				$result = $this->Purchase_Model->updateAktifkanPermintaanPlastik($data);
				echo $result;
			}
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
	...
	public function setujuiPermintaanPlastik(){
		$isLogin = $this->isLogin();
    if($isLogin){
			$idPermintaan = $this->input->post("idPermintaan");
			$result = $this->Purchase_Model->updateSetujuiPermintaanPlastik($idPermintaan);
			echo $result;
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
      redirect("_main/main","refresh");
	...
	public function getDetailDataPembelianBarangItem(){
		$isLogin = $this->isLogin();
    if($isLogin){
			$idDpb = $this->input->post("idDpb");
			$result = $this->Purchase_Model->selectDetailDataPembelianBarangItem($idDpb);
			echo $result;
		}else{
			echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
			redirect("_main/main","refresh");
	...
	public function editDetailPermintaan(){
		$isLogin = $this->isLogin();
    if($isLogin){
			$idDpb = $this->input->post("idDpb");
			$kdGdBahan = $this->input->post("kdGdBahan");
			...
			if(empty($namaBahan) || empty($jumlahPermintaan) || empty($keterangan)){
				echo "Data Kosong";
			}else{
				if(empty($kdGdBahan)){
				...
				$result = $this->Purchase_Model->editDetailPermintaan($data);
				echo $result;
			}
		}else{
			echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
