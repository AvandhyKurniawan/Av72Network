...
class Purchase_Model extends CI_Model{
  ...
  public function updateAktifkanPermintaanPlastik($param){
    $this->db->trans_begin();
    ...
    $arrTotalItem = $this->db->query("SELECT COUNT(id_dpp) AS counter
                                      FROM transaksi_detail_permintaan_plastik_hd
                                      WHERE kd_permintaan_plastik='$param[idPermintaan]'
                                      AND deleted='FALSE'")->result_array();

    $arrTotalItemBatal = $this->db->query("SELECT COUNT(id_dpp) AS counter
                                           FROM transaksi_detail_permintaan_plastik_hd
                                           WHERE kd_permintaan_plastik='$param[idPermintaan]'
                                           AND deleted='FALSE'
                                           AND keterangan_purchasing IS NOT NULL
    ...
    if($arrTotalItemBatal[0]["counter"] < $arrTotalItem[0]["counter"]){
      $this->db->set("status_permintaan",$param["statusPermintaan"]);
      $this->db->where("kd_permintaan_plastik",$param["idPermintaan"]);
  ...
  public function updateBatalSetujuiPermintaanBarang($param){
    $this->db->trans_begin();
    $this->db->set("status_permintaan","PENDING");
    $this->db->set("status_approve","FALSE");
    $this->db->where("kd_permintaan_barang",$param);
    $this->db->where("deleted","FALSE");
    $this->db->update("transaksi_permintaan_barang");
  ...
  public function selectPermintaanPlastik(){
    $this->datatables->select("TPP.kd_permintaan_plastik, TPP.tgl_permintaan, TPP.status_permintaan, USR.username");
    $this->datatables->from("transaksi_permintaan_plastik_hd TPP");
    $this->datatables->join("users USR","TPP.id_user = USR.id_user","INNER");
    $this->datatables->join("transaksi_detail_permintaan_plastik_hd TDPP","TDPP.kd_permintaan_plastik = TPP.kd_permintaan_plastik","INNER");
    $this->datatables->join("gudang_hasil GH","TDPP.kd_gd_hasil = GH.kd_gd_hasil","INNER");
    $this->datatables->where("TPP.deleted = 'FALSE' AND TPP.status_permintaan NOT IN ('FINISH')");
    $this->db->order_by("TPP.tgl_permintaan DESC, FIELD(TPP.status_permintaan,'PENDING','PROGRESS')");
    $this->db->group_by("TPP.kd_permintaan_plastik");
    return $this->datatables->generate();
  }
  ...
  public function selectDetailPermintaanPlastikHdBaru($param){
    ...
    if(empty($param["idPermintaan"])){
      $this->datatables->where("TDPP.deleted='FALSE' AND
                                GH.deleted='FALSE'");
    }else{
      $this->datatables->where("TPP.kd_permintaan_plastik = '$param[idPermintaan]' AND
                                TDPP.deleted='FALSE' AND
                                GH.deleted='FALSE'");
    }
    return $this->datatables->generate();
  ...
  public function checkPermintaanPlastik(){
    $data = $this->db->query("SELECT COUNT(kd_permintaan_plastik) AS counter
                              FROM transaksi_permintaan_plastik_hd
                              WHERE status_permintaan='PENDING'
                              AND deleted='FALSE'")->result_array();
    $result = array("PermintaanPlastik" => $data[0]["counter"]);
    return json_encode($result);
  }

  public function updateBatalkanPermintaanPlastik($param){
    $this->db->trans_begin();
  ...
  public function updateSetujuiPermintaanPlastik($param){
    $this->db->trans_begin();
    $this->db->set("status_permintaan","PROGRESS");
    $this->db->set("status_approve","TRUE");
    $this->db->where("kd_permintaan_plastik",$param);
    $this->db->where("deleted","FALSE");
    $this->db->update("transaksi_permintaan_plastik_hd");
  ...
  public function selectDetailDataPembelianBarangItem($param){
    $Q = "SELECT TDPB.*, TPB.tgl_permintaan, GB.nm_barang, GB.jenis FROM transaksi_detail_permintaan_barang TDPB
          INNER JOIN gudang_bahan GB ON TDPB.kd_gd_bahan = GB.kd_gd_bahan
          INNER JOIN transaksi_permintaan_barang TPB ON TDPB.kd_permintaan_barang = TPB.kd_permintaan_barang
  ...
  public function editDetailPermintaan($param){
    $this->db->trans_begin();
