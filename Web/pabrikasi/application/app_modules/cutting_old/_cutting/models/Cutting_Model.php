  ...
  public function selectComboBoxValueGudangRoll($param){
    ...
    if(in_array(strtoupper($param["merek"]), $arrMerek)){
      ...
      $clause2="OR ((merek = '$param[merek]'
                AND warna_plastik = '$param[warnaPlastik]') OR
  ...
  public function selectListBonHasilJadi($param){
    if(strtoupper($param["merek"]) == "CETAK"){
      $where = "AND RC.merek NOT IN('Zippin','MP','KP','Klip','Kantong','CB','PON','Klip In','Klip Klop') AND RC.merek NOT LIKE '%kantong%'";
  	  ...
  	  if($param['merek']=="Klip"){
  		    $where = "AND (RC.merek='$param[merek]' OR RC.merek IN('PON','POLOS','KLIPPOLOS','Export'))";
  	  }else{
  ...
  public function selectDataForUpdateHasil($param){
    ...
    if($result[0]["jnsPermintaanRencana"] == "CETAK" || $result[0]["jnsPermintaanRencana"] == "CETAK/POLOS"){
      $QSumPengambilanPotong = "SELECT SUM(berat) AS jumlahBerat,
                                       ...
                                       SUM(payung_kuning) AS jumlahPayungKuning
                                FROM transaksi_detail_pengambilan_potong
                                WHERE kd_gd_roll = '".$result[0]["kd_gd_roll"]."'
                                AND tgl_potong = '".$result[0]["tgl_rencana"]."'
                                AND status = 'MANDOR POTONG (CETAK)'
                                AND deleted = 'FALSE'";

      $QSumPengambilanPotongTumpuk = "SELECT SUM(berat) AS jumlahBeratTumpuk,
                                             SUM(bobin) AS jumlahBobinTumpuk,
                                             SUM(payung) AS jumlahPayungTumpuk,
                                             SUM(payung_kuning) AS jumlahPayungKuningTumpuk
                                      FROM transaksi_detail_pengambilan_potong
                                      WHERE kd_gd_roll = '".$result[0]["kd_gd_roll_tumpuk"]."'
                                      AND tgl_potong = '".$result[0]["tgl_rencana"]."'
                                      AND status = 'MANDOR POTONG (CETAK)'
                                      AND deleted = 'FALSE'";
    }else{
