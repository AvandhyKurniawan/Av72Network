...
class Main extends MX_Controller{
      ...
      if($noOrder==""||$kdBarang==""||
         ...
         $satuan==""){
           ...
           if(substr($kdBarang,0,3) == "BHN"){
             $kode = "kd_gd_bahan";
           } elseif (substr($kdBarang,0,4) == "PACK") {
             $kode = "kd_pack";
           }
           else{
             $kode = "kd_gd_hasil";
  ...
  public function getDaftarBarangAnekaPlastik(){
    $isLogin = $this->isLogin();
    if($isLogin){
      $items = $this->Cabang_Model->getItem(['kd_produk'=>$this->input->get('kodeBarang')]);
  ...
  public function cetakFakturNew($param){
    $isLogin = $this->isLogin();
    if($isLogin){
      $arrDataPesanan = $this->Cabang_Model->selectFakturPesananMarketing($param);
      $arrDataPesananDetail = $this->Cabang_Model->selectFakturPesananDetailProduksiMarketing($param);
      $pesananDetail = $this->Cabang_Model->selectPesananDetails($param);
      $arrListItem = $this->Cabang_Model->selectAllListItemAneka($pesananDetail[0]['kd_pack']);
