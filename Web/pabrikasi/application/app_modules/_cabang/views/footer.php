				...
				$("#cmbUkuran").on("select2:select",function(){
					...
					$.ajax({
						...
						success : function(response){
							...
							if(typeof response[0].kd_produk !== 'undefined'){
								$("#txtMerek").val(response[0].nama_produk);
								$("#txtUkuran").val('-');
								$("#txtWarnaPlastik").val('-');
								$("#txtStrip").val('-');
								$("#txtCetak").val('-');
					...
					$.ajax({
						...
						success : function(response){
							if(jQuery.trim(response) === "Berhasil"){
								...
								$("#cetakFakturLoad").attr("src","<?php echo base_url(); ?>_cabang/main/cetakFakturNew/"+param);
								dataTablePantauOrder();
							}else if(jQuery.trim(response) === "Gagal"){
								$("#modal-notif").addClass("modal-warning");
...
//============================= Data Table Function (Start) =============================//
		  ...
		  function dataTableDetailOrder(param){
				...
				$("#tableDetailOrder").dataTable({
					...
					"fnRowCallback":function(nRow,aData,iDisplayIndex,iDisplayIndexFull){
						...
						if(aData["jns_brg"] == 'VARIASI'){
							$("td:eq(10)",nRow).html(
								"<button class='btn btn-md btn-flat btn-info' data-toggle='modal' data-target='#daftarAnekaItem' onclick=tabelDaftarBarangAnekaPlastikModal('"+aData["kd_pack"]+"')>Daftar Barang</button>"+
								"<button class='btn btn-md btn-flat btn-warning' onclick=modalModifyOrder('"+aData["id_dp"]+"','"+aData["no_order"]+"')>Edit</button>&nbsp;"+
		...
		function tabelDaftarBarangAnekaPlastikModal(kd){
          ...
          $.get("<?= base_url('_cabang/main/getDaftarBarangAnekaPlastik'); ?>",
