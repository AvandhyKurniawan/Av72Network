		...
		<script type="text/javascript">
			...
			function modalEditStatus(param, param2, param3){
				if(param=="" || param2==""){
					...
					$("#cmbStatusRencana").val(param2.toUpperCase());
          ...
          ajax:{
            ...
            processResults : function(data){
              return{
				...
				$("#cmbMerekGudangRoll").on("select2:select", function(){
					...
					$("#txtUkuranPlastik").val(arrDataText[1]);
				});
				...
				$.ajax({
					...
					success : function(response){
						$.each(response, function(AvIndex, AvValue) {
							...
							$("#txtUkuranPlastik").val(arrUkuran[0]);
							$("#txtLebarPlastik").val(arrUkuran[1]);
							//$("#txtWarna").val(AvValue.warna);
				...
				$.ajax({
					...
					success : function(response) {
						...
						$.ajax({
							...
							success : function(response){
								$("#cmbBijiWarna").empty();
								if(warnaPlastik1.toLowerCase() != "putih"){
			...
			function saveAddRencanaExtruderSusulan(){
				...
				var panjangPlastik = $("#txtUkuranPlastik").val();
				var lebarPlastik = $("#txtLebarPlastik").val();
				if(panjangPlastik=="" || lebarPlastik=="" || panjangPlastik==" " || lebarPlastik==" "){
					var ukuran1 = "";
				}else{
					var ukuran1 = panjangPlastik+"x"+lebarPlastik;
				}
			...
			function saveKirimDataBonRollPolosPerShift(param, param2){
				if(confirm("Apakah Anda Yakin Ingin Mengirim Hasil Ini?")){
					$.ajax({
						type : "POST",
						url : "<?php echo base_url(); ?>_extruder/main/saveKirimDataBonRollPolosPerShift",
						dataType : "TEXT",
						data : {tanggal : param,
										shift : param2},
						success : function(response){
			...
			function editRencanaExtruderSusulanPending(){
				...
				var panjang = $("#txtUkuranPlastik").val();
				var lebar = $("#txtLebarPlastik").val();
				if(panjang=="" || lebar=="" || panjang==" " || lebar==" "){
					var ukuran1 = "";
				}else{
					var ukuran1 = panjang+"x"+lebar;
				}
				var tebal1 = $("#txtTebal1").val();
				var jumlahPermintaan1 = $("#txtJumlahRencanaPembuatan	").val().replace(/,/g, "");
				if($("#cmbStrip").val() == "CUSTOM"){
						...
						success : function(response){
							if(jQuery.trim(response) === "Berhasil"){
								...
								setTimeout(function(){
									...
									$("#modal-notif").removeClass("modal-info");
									$("#modalNotifContent").text("");
									$(".active a[aria-controls='tableRencanaMandorExtruder_KP']").click();
									$(".active a[aria-controls='tableRencanaMandorExtruder_ZP']").click();
								},2000);
							}else if(jQuery.trim(response) === "Gagal"){
								$("#modal-notif").addClass("modal-danger");
			...
			function editSelesaiRencanaExtruder(param="", param2=""){
				if(confirm("Apakah Anda Yakin Akan Menyelesaikan Rencana Ini?")){
					if(param =="" || param2 ==""){
						$("#modal-notif").addClass("modal-warning");
						$("#modalNotifContent").text("Semua Kolom Berwarna Kuning Tidak Boleh Kosong!");
						$("#modal-notif").modal("show");
						...
						$.ajax({
							type : "POST",
							url : "<?php echo base_url(); ?>_extruder/main/editSelesaiRencanaExtruder",
							dataType : "TEXT",
							data : {
								kodeExtruder : param,
								kdPpic : param2
						 ...
						 $.ajax({
							 ...
							 success : function(response) {
							 	if(jQuery.trim(response) === "Berhasil"){
									...
									$("#modal-notif").addClass("modal-danger");
									$("#modalNotifContent").text("Periode Tersebut Sudah Di Kunci");
									$("#modal-notif").modal("show");
									setTimeout(function(){
										$("#modal-notif").modal("hide");
										$("#modal-notif").removeClass("modal-danger");
			...
			function resetFormMintaBahanBaku(){
				...
				$.ajax({
					...
					success : function(response){
						$("#txtBahan").empty();
						$.each(response,function(AvIndex, AvValue){
							$("#txtBahan").append("<option value='"+AvValue.kd_gd_bahan+"'>"+AvValue.nm_barang+" ["+AvValue.warna+"]"+"</option>")
						});
					},
					error : function(response){
						$("#modal-notif").addClass("modal-danger");
					...
					if(tglRencana1=="" || jnsBrg1=="" || shift1==""){
						...
						$.ajax({
							...
							success : function(response){
								...
								$("#btnKirimDataBonRollPolosPerShift").attr("onclick","saveKirimDataBonRollPolosPerShift('"+tglRencana1+"','"+shift1+"')");
								if(response[0].kirim_ke_ppic == "FALSE"){
			...
			function searchHistoryKembalianBijiWarna(){
				...
				function cariDataPemakaianBahanBakuGlobal(){
					var tglAwal = $("#txtTglAwal").val();
					var tglAkhir = $("#txtTglAkhir").val();
					if(tglAwal=="" || tglAkhir==""){
						$("#modal-notif").addClass("modal-warning");
						...
						tablePemakaianBahanBakuGlobal(data);
					}
				}

				function cariDataApalGlobal(){
					var tglAwal = $("#txtTglAwal").val();
					var tglAkhir = $("#txtTglAkhir").val();
					if(tglAwal=="" || tglAkhir==""){
						$("#modal-notif").addClass("modal-warning");
						...
						tableApalGlobal(data);
					}
				}

				function cariDataHasilGlobal(){
					var tglAwal = $("#txtTglAwal").val();
					var tglAkhir = $("#txtTglAkhir").val();
					if(tglAwal=="" || tglAkhir==""){
						$("#modal-notif").addClass("modal-warning");
          ...
          "columns":[
            ...
            {"data" : "tgl_rencana","name":"CAST(RP.tgl_rencana AS CHAR)","sWidth":"65px"},
          ...
          "fnRowCallback":function(nRow,aData,iDisplayIndex,iDisplayIndexFull){
	        ...
	        "columns":[
			     	{"data" : "tgl_rencana","name":"CAST(EX.tgl_rencana AS CHAR)","sWidth":"65px"},
          ...
          "fnRowCallback":function(nRow,aData,iDisplayIndex,iDisplayIndexFull){
						if(aData["prioritas"].toUpperCase() == "URGENT"){
							$("td:eq(11)",nRow).html(aData["sts_pengerjaan"] +" "+"<label class='text-red'>["+aData["prioritas"]+"]<label>");
						}else if(aData["prioritas"].toUpperCase() == "TUNDA"){
							$("td:eq(11)",nRow).html("PENDING "+"<label style='color:#FFF;'>["+aData["prioritas"]+"]<label>");
							$("td",nRow).css({"background-color":"rgba(215, 44, 44, 0.7)","color":"#FFF"});
						}else{
						...
						if(aData["sts_pengerjaan"] != "COMPLETE"){
							if(parseFloat(aData["jml_permintaan"].replace(/,/gi,"")) >= parseFloat(aData["stok_permintaan"].replace(/,/gi,""))){
								$("td:eq(14)",nRow).html("<button class='btn btn-md btn-flat btn-success' data-toggle='modal' data-target='#modalEditStatus' data-backdrop='static' onclick=modalEditStatus('"+aData["kd_extruder"]+"','"+aData["prioritas"]+"','"+aData["kd_ppic"]+"')>Edit Status</button><br>"+
																				 ...
																				 "<button class='btn btn-md btn-flat btn-warning' data-toggle='modal' data-target='#modalGantiMesin' data-backdrop='static' onclick=modalGantiMesin('"+aData["kd_extruder"]+"','"+aData["kd_mesin"]+"','"+aData["jns_mesin"]+"')>Ganti Mesin</button><br>"+
																			 	 ...
																			 	 "<button class='btn btn-md btn-flat btn-danger' onclick=deleteAndRestoreRencanaExtruderPending('"+aData["kd_extruder"]+"','TRUE')>Hapus Rencana</button><br>"+
																			 	 "<button class='btn btn-md btn-flat btn-primary' onclick=editSelesaiRencanaExtruder('"+aData["kd_extruder"]+"','"+aData["kd_ppic"]+"')>Selesai Rencana</button>");
							}else{
								$("td:eq(14)",nRow).html("<button class='btn btn-md btn-flat btn-success' data-toggle='modal' data-target='#modalEditStatus' data-backdrop='static' onclick=modalEditStatus('"+aData["kd_extruder"]+"','"+aData["prioritas"]+"','"+aData["kd_ppic"]+"')>Edit Status</button><br>"+
																				 // "<button class='btn btn-md btn-flat btn-info' data-toggle='modal' data-target='#modalInputHasil' data-backdrop='static' onclick=modalInputHasil('"+aData["kd_extruder"]+"')>Input Hasil</button>"+
																				 "<a href=<?php echo base_url('_extruder/main/input_hasil'); ?>/"+aData["kd_extruder"]+" target='_blank' class='btn btn-md btn-flat btn-info'>Input Hasil</a><br>"+
          ...
          "columns":[
            {"data" : "tgl_rencana","name":"CAST(RE.tgl_rencana AS CHAR)","sWidth":"70px"},
            ...
            {"data" : "strip","name":"RE.strip","sWidth":"50px"},
						...
						{"data" : "jumlah_permintaan","name":"RP.jumlah_permintaan","sWidth":"80px"},
          ...
          "fnRowCallback":function(nRow,aData,iDisplayIndex,iDisplayIndexFull){
						$("td:eq(10)",nRow).text(parseFloat(aData["stok_permintaan"]).toLocaleString());
						$("td:eq(11)",nRow).text(parseFloat(aData["jml_permintaan"]).toLocaleString());
						if(aData["jumlah_permintaan"] != null){
							$("td:eq(12)",nRow).text(parseFloat(aData["jumlah_permintaan"]).toLocaleString());
						}else{
							$("td:eq(12)",nRow).text("");
						}
          }
          ...
          "columns":[
            {"data" : "id","name":"TGB.id"},
            {"data" : "tgl_permintaan","name":"CAST(TGB.tgl_permintaan AS CHAR)"},
          ...
          "fnRowCallback":function(nRow,aData,iDisplayIndex,iDisplayIndexFull){
				...
				$.ajax({
					type : "POST",
					url : "<?= base_url('_extruder/main/getHistoryPermintaanBahanBaku'); ?>",
					dataType : "JSON",
					data : {
						tglAwal : param1,
						tglAkhir : param2
					...
					success : function(response){
						for (var i = 0; i < response.data.length; i++) {
							totalBerat += parseFloat(response.data[i].jumlah_permintaan);
						}
						$("#tdTotalBerat").text(totalBerat.toLocaleString());
					}
				});
			}
          ...
          "columns":[
						{"data" : "tgl_transaksi","name":"CAST(TGBE.tgl_transaksi AS CHAR)"},
						{"data" : "nm_barang","name":"GB.nm_barang"},
          ...
          "fnRowCallback":function(nRow,aData,iDisplayIndex,iDisplayIndexFull){
			...
			function tablePemakaianBahanBakuGlobal(param){
				$.ajax({
					type : "POST",
					url : "<?= base_url('_extruder/main/getDataJobGlobal'); ?>",
					dataType : "JSON",
					data : {
					...
					success : function(response){
						...
						var totalPemakaian = 0;
						for (var i = 0; i < response.length; i++) {
							totalPemakaian += parseFloat(response[i].sub_total);
						}
						$("#thTotal").text(totalPemakaian.toLocaleString());
						$.each(response, function(AvIndex, AvValue){
							var arrJumlah = AvValue.jumlah.split("|");
							var arrShift = AvValue.shift.split("|");
							for (var i = 0; i < arrJumlah.length; i++) {
								arrJumlah[i] = parseFloat(arrJumlah[i]).toLocaleString();
							}
							for (var i = 0; i < arrShift.length; i++) {
								arrShift[i] = parseFloat(arrShift[i]).toLocaleString();
							}
							$("#tableData > tbody:last-child").append(
								"<tr>"+
									"<td>"+ ++AvIndex +"</td>"+
									"<td>"+AvValue.tgl_rencana+"</td>"+
									"<td>"+arrShift.join('<br>')+"</td>"+
									"<td>"+arrJumlah.join('<br>')+"</td>"+
									"<td>"+parseFloat(AvValue.sub_total).toLocaleString()+"</td>"+
								"</tr>"
							);
						});
						$("#modalCariData input[type='text']").val("");
						$("#modalCariData .date").datepicker("setDate",null);
			...
			function tableApalGlobal(param){
				$.ajax({
					type : "POST",
					url : "<?= base_url('_extruder/main/getDataJobGlobal'); ?>",
					dataType : "JSON",
					...
					success : function(response){
						...
						var totalPemakaian = 0;
						for (var i = 0; i < response.length; i++) {
							totalPemakaian += parseFloat(response[i].sub_total_apal);
						}
						$("#thTotal").text(totalPemakaian.toLocaleString());
						$.each(response, function(AvIndex, AvValue){
							var arrJumlah = AvValue.jumlah_apal.split("|");
							var arrShift = AvValue.shift.split("|");
							for (var i = 0; i < arrJumlah.length; i++) {
								arrJumlah[i] = parseFloat(arrJumlah[i]).toLocaleString();
							}
							...
							$("#tableData > tbody:last-child").append(
								"<tr>"+
									...
									"<td>"+arrShift.join('<br>')+"</td>"+
									"<td>"+arrJumlah.join('<br>')+"</td>"+
									"<td>"+parseFloat(AvValue.sub_total_apal).toLocaleString()+"</td>"+
								"</tr>"
							);
						});
						$("#modalCariData input[type='text']").val("");
			...
			function tableHasilGlobal(param){
				$.ajax({
					type : "POST",
					url : "<?= base_url('_extruder/main/getDataHasilGlobal'); ?>",
					dataType : "JSON",
					data : {
						tglAwal : param.tglAwal,
						tglAkhir : param.tglAkhir
					...
					success : function(response){
						...
						var totalPemakaian = 0;
						for (var i = 0; i < response.length; i++) {
							totalPemakaian += parseFloat(response[i].total);
						}
						$("#thTotal").text(totalPemakaian.toLocaleString());
						$.each(response, function(AvIndex, AvValue){
							var arrJumlah = AvValue.group_total.split("|");
							var arrShift = AvValue.group_shift.split("|");
							for (var i = 0; i < arrJumlah.length; i++) {
								arrJumlah[i] = parseFloat(arrJumlah[i]).toLocaleString();
							}
							for (var i = 0; i < arrShift.length; i++) {
							...
							$("#tableData > tbody:last-child").append(
								"<tr>"+
									...
									"<td>"+arrShift.join('<br>')+"</td>"+
									"<td>"+arrJumlah.join('<br>')+"</td>"+
									"<td>"+parseFloat(AvValue.total).toLocaleString()+"</td>"+
								"</tr>"
							);
						});
						$("#modalCariData input[type='text']").val("");
