...
class Extruder_Model extends CI_Model{
  ...
  public function insertKirimDataBonRollPolosPerShift($param){
    $this->db->trans_begin();
    $tglTransaksi = date("Y-m",strtotime($param["Data"][0]["tgl_transaksi"]));
    $checkLock = $this->db->query("SELECT COUNT(status_lock) AS Jumlah
                                   FROM transaksi_gudang_roll
  ...
  public function selectComboBoxValueMesin(){
    $arrData = $this->db->query("SELECT * FROM mesin WHERE sts_mesin NOT IN('ZP')")->result_array();
    return json_encode($arrData);
  }
  ...
  public function selectBijiWarna(){
    $result = $this->db->query("SELECT GBE.id, FORMAT(GBE.jumlah_stok,2) AS jumlah_stok, GB.nm_barang, GB.warna
                                FROM gudang_buffer_extruder GBE
                                INNER JOIN gudang_bahan GB ON GBE.kd_gd_bahan = GB.kd_gd_bahan
                                WHERE GBE.deleted='FALSE'
  ...
  public function selectDataRencanaMandorSelesai($param){
    $this->datatables->select("RE.*, CONCAT(MSN.no_mesin, ' ',MSN.sts_mesin) AS no_mesin,
                               RP.keterangan, RP.ket_merek, RP.jumlah_permintaan");
  ...
  public function selectDataBonRollPolosPerShift($param){
    $result = $this->db->query("SELECT RE.tebal, THE.kd_gd_roll, THE.hasil_ukuran, THE.jumlah_selesai, THE.warna_plastik, THE.roll_lembar,THE.tgl_rencana,
                                       THE.roll_pipa, THE.jenis_roll, THE.jns_brg, THE.shift, THE.sts_pengiriman, THE.merek, THE.tgl_jadi
                                FROM transaksi_hasil_extruder THE
                                INNER JOIN rencana_extruder RE ON THE.kd_extruder = RE.kd_extruder
                                WHERE THE.tgl_rencana = '$param[tanggal]'
  ...
  public function selectDetailBonRollPolos($param){
    if($param["merek"] == "Klip"){
      $clause = "AND THE.merek IN ('$param[merek]','Export','PON','Klip Pon','Klipon','Klip Double') ";
    }else{
  ...
  public function selectComboBoxValueGudangApal(){
    $result = $this->db->query("SELECT kd_gd_apal, jenis, sub_jenis FROM gudang_apal WHERE deleted='FALSE' AND jenis != 'CETAK'")->result_array();
    return json_encode($result);
  }
  ...
  public function selectDataJobGlobal($param){
    $Q = "SELECT tgl_rencana,
                 GROUP_CONCAT(shift ORDER BY shift ASC SEPARATOR '|') AS shift,
                 GROUP_CONCAT(total ORDER BY shift ASC SEPARATOR '|') AS jumlah,
                 GROUP_CONCAT(jumlah_apal SEPARATOR '|') AS jumlah_apal,
                 SUM(total) AS sub_total,
                 SUM(jumlah_apal) AS sub_total_apal
          FROM transaksi_data_job_extruder
          WHERE tgl_rencana BETWEEN '$param[tglAwal]' AND '$param[tglAkhir]'
          AND deleted='FALSE'
  ...
  public function selectDataHasilGlobal($param){
    $Q = "SELECT tgl_rencana,
	               GROUP_CONCAT(total ORDER BY shift ASC SEPARATOR '|') AS group_total,
                 GROUP_CONCAT(shift ORDER BY shift ASC SEPARATOR '|') AS group_shift,
                 SUM(total) AS total
          FROM (SELECT tgl_rencana,
      		             SUM(jumlah_selesai) AS total,
      		             shift
                FROM transaksi_hasil_extruder
                WHERE tgl_rencana
                BETWEEN '$param[tglAwal]' AND '$param[tglAkhir]'
                GROUP BY tgl_rencana, shift) AS X
          GROUP BY tgl_rencana";
    $result = $this->db->query($Q)->result_array();
    return json_encode($result);
  }
  ...
  public function updateLaporanRencanaMandor($param){
    ...
    $periodeLock = date("Y-m", strtotime($param["THE"][0]["tgl_rencana"]));
    ...
    if($checkLock[0]["counterLock"] > 0){
      ...
      $dataHasilExtruder = $this->db->query("SELECT kd_extruder, tgl_rencana, jumlah_biji_warna,
        ...
        $this->db->set("tgl_rencana",$param["THE"][0]["tgl_rencana"]);
        $this->db->where("tgl_rencana",$dataHasilExtruder[0]["tgl_rencana"]);
  ...
  public function updateDataJobExtruder($param){
    $this->db->trans_begin();
  ...
  public function updateDeleteAndRestoreHasilExtruderTemp($param){
    ...
    if($param["deleted"] == "TRUE"){
      foreach ($arrDataGudangBufferExtruder as $key) {
        $this->db->set("jumlah_stok","jumlah_stok + ".$key["jumlah"],FALSE);
  ...
  public function updateStatusRencanaExtruder($param){
    $this->db->trans_begin();
    $this->db->query("UPDATE rencana_extruder SET prioritas = '$param[sts_pengerjaan]',id_user='$param[id_user]' WHERE kd_extruder='$param[kd_extruder]'");
    if(!empty($param["sts_pengerjaan"])){
      $this->db->query("UPDATE rencana_ppic SET prioritas = '$param[sts_pengerjaan]',id_user='$param[id_user]' WHERE kd_ppic='$param[kd_ppic]'");
    }
    if($this->db->trans_status() === FALSE){
      $this->db->trans_rollback();
      return FALSE;
  ...
  public function updateSelesaiRencanaExtruder($param){
    $this->db->trans_begin();
    $this->db->query("UPDATE rencana_extruder SET sts_pengerjaan = 'COMPLETE',id_user='$param[id_user]' WHERE kd_extruder='$param[kd_extruder]'");
