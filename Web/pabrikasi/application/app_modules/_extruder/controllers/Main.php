...
class Main extends MX_Controller{
    ...
    if($isLogin){
      ...
      if($status==1){
        $listMenu["parentMenu"] = array(
          ...
          array("Content"=>"Laporan Global","Link"=>"#","Name"=>"Laporan_Global","Status"=>"Parent","Icon"=>"fa fa-book","Id"=>"M_Laporan_Global"),
        ...
        $listMenu["childMenu"] = array(
          array("Content"=>"Pemakaian Bahan Baku Global","Link"=>"_extruder/main/pemakaian_bahan_baku_global","Name"=>"Pemakaian_Bahan_Baku_Global","Parent"=>"Laporan_Global","Icon"=>"fa fa-book","Id"=>"M_Pemakaian_Bahan_Baku_Global"),
          array("Content"=>"Apal Global","Link"=>"_extruder/main/apal_global","Name"=>"Apal_Global","Parent"=>"Laporan_Global","Icon"=>"fa fa-book","Id"=>"M_Apal_Global"),
          array("Content"=>"Hasil Extruder Global","Link"=>"_extruder/main/hasil_extruder_global","Name"=>"Hasil_Extruder_Global","Parent"=>"Laporan_Global","Icon"=>"fa fa-book","Id"=>"M_Hasil_Extruder_Global"),
        );
        return $listMenu;
      }else if($status==2) {
        $listMenu["parentMenu"] = array(
  ...
  public function pemakaian_bahan_baku_global(){
    $isLogin = $this->isLogin();
    if($isLogin){
      $data["Data"] = array("Title"=>"PEMAKAIAN BAHAN BAKU GLOBAL");
      $data["Link"] = array("Segment1" =>$this->uri->rsegment(1),"Segment2"=>$this->uri->rsegment(2));
      $this->load->view("header");
      $this->load->view("sidebar",$this->checkStatus());
      $this->load->view("frm_pemakaian_bahan_baku_global",$data);
      $this->load->view("footer");
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
      redirect("_main/main","refresh");
  ...
  public function apal_global(){
    $isLogin = $this->isLogin();
    if($isLogin){
      $data["Data"] = array("Title"=>"APAL GLOBAL");
      $data["Link"] = array("Segment1" =>$this->uri->rsegment(1),"Segment2"=>$this->uri->rsegment(2));
      $this->load->view("header");
      $this->load->view("sidebar",$this->checkStatus());
      $this->load->view("frm_apal_global",$data);
      $this->load->view("footer");
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
      redirect("_main/main","refresh");
  ...
  public function hasil_extruder_global(){
    $isLogin = $this->isLogin();
    if($isLogin){
      $data["Data"] = array("Title"=>"HASIL EXTRUDER GLOBAL");
      $data["Link"] = array("Segment1" =>$this->uri->rsegment(1),"Segment2"=>$this->uri->rsegment(2));
      $this->load->view("header");
      $this->load->view("sidebar",$this->checkStatus());
      $this->load->view("frm_hasil_global",$data);
      $this->load->view("footer");
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
      redirect("_main/main","refresh");
  ...
  public function editSelesaiRencanaExtruder(){
    $isLogin = $this->isLogin();
    if($isLogin){
      $kdExtruder = $this->input->post("kodeExtruder");
      $kdPpic = $this->input->post("kdPpic");
      ...
      if(empty($kdExtruder)){
        ...
        $data = array("kd_extruder" => $kdExtruder,
                      "id_user" => $idUser,
                      "kd_ppic" => $kdPpic);
        $result = $this->Extruder_Model->updateSelesaiRencanaExtruder($data);
        if($result){
          echo "Berhasil";
        }else{
          echo "Gagal";
  ...
  public function saveKirimDataBonRollPolosPerShift(){
    $isLogin = $this->isLogin();
    if($isLogin){
      $tanggal = $this->input->post("tanggal");
      $shift = $this->input->post("shift");
      ...
      $result = json_decode($this->Extruder_Model->selectDataBonRollPolosPerShift($param),TRUE);
      $dataContainer = array("tglRencana" => $tanggal,
                             "shift" => $shift,
                             "Data" => array());
      if(count($result) > 0){
        ...
        $result = $this->Extruder_Model->insertKirimDataBonRollPolosPerShift($dataContainer);
        echo $result;
      }else{
        echo "Data Kosong";
      }
    ...
    if($isLogin){
      ...
      if(empty($kdExtruder)      || empty($kdGdRoll)      || empty($idUser)          ||
         ...
         $data = array("id_hasil_extruder" => $idHasilExtruder,
          ...
          $Result = $this->Extruder_Model->updateLaporanRencanaMandor($dataHasil);
          echo $Result;
       }
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
  ...
  public function getDataJobGlobal(){
    $isLogin = $this->isLogin();
    if($isLogin){
      $tglAwal = $this->input->post("tglAwal");
      $tglAkhir = $this->input->post("tglAkhir");
      $data = array("tglAwal" => $tglAwal,
                    "tglAkhir" => $tglAkhir);
      $result = $this->Extruder_Model->selectDataJobGlobal($data);
      echo $result;
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
      redirect("_main/main","refresh");
  ...
  public function getDataHasilGlobal(){
    $isLogin = $this->isLogin();
    if($isLogin){
      $tglAwal = $this->input->post("tglAwal");
      $tglAkhir = $this->input->post("tglAkhir");
      $data = array("tglAwal" => $tglAwal,
                    "tglAkhir" => $tglAkhir);
      $result = $this->Extruder_Model->selectDataHasilGlobal($data);
      echo $result;
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
      redirect("_main/main","refresh");
