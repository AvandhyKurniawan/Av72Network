...
class Main extends MX_Controller{
    ...
    if($isLogin){
      ...
      if($status==1){
        $listMenu["parentMenu"] = array(
          ...
          array("Content"=>"Permintaan","Link"=>"#","Name"=>"Permintaan","Status"=>"Parent","Icon"=>"fa fa-book","Id"=>"M_Permintaan"),
          ...
          array("Content"=>"Cek Kartu Stok Cat","Link"=>"_gudanghasil/main/cek_kartu_stok_cat","Name"=>"Cek_Kartu_Stok_Cat","Status"=>"Single","Icon"=>"fa fa-book","Id"=>"M_Cek_Kartu_Stok_Cat"),
        );
        ...
        $listMenu["childMenu"] = array(
          ...
          array("Content"=>"Cat Murni","Link"=>"_gudanghasil/main/permintaan_cat_murni","Parent"=>"Permintaan","Id"=>""),
          array("Content"=>"Plastik HD","Link"=>"_gudanghasil/main/permintaan_plastik_hd","Parent"=>"Permintaan","Id"=>""),
        );
        return $listMenu;
      }else if($status==2) {
        $listMenu["parentMenu"] = array();
  ...
  public function permintaan_plastik_hd(){
    $isLogin = $this->isLogin();
    if($isLogin){
      $data["Data"] = array("Title"=>"Permintaan Plastik HD");
      $data["Link"] = array("Segment1" =>$this->uri->rsegment(1),"Segment2"=>$this->uri->rsegment(2));
      $this->load->view("header");
      $this->load->view("sidebar",$this->checkStatus());
      $this->load->view("frm_permintaan_plastik_hd",$data);
      $this->load->view("footer");
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
      redirect("_main/main","refresh");
  ...
  public function cek_kartu_stok_cat(){
    $isLogin = $this->isLogin();
    if($isLogin){
      if($this->session->userdata("fabricationGroup") == "administrator" ||
         $this->session->userdata("fabricationGroup") == "it_department"){
           $data["Data"] = array("Title"=>"Cek Kartu Stok Cat Murni");
           $data["Link"] = array("Segment1" =>$this->uri->rsegment(1),"Segment2"=>$this->uri->rsegment(2));
           $this->load->view("header");
           $this->load->view("sidebar",$this->checkStatus());
           $this->load->view("frm_cek_kartu_cat_murni",$data);
           $this->load->view("footer");
      }else{
        echo "<script>alert('anda tidak memiliki hak akses!');</script>";
        redirect("_main/main","refresh");
          ...
          if($result){
            ...
            if(!empty($kdGdHasil2)){
              $data_2 = array("tglAwal"=>$tglAwal,
                              "tglAkhir"=>$tglAkhir,
                              "kdGdHasil"=>$kdGdHasil2);
              $arrSaldo2 = json_decode($this->Gudang_Hasil_Model->selectSaldoAwalBulanGudangHasil($data_2),TRUE);
              $dataForUpdateMaster2 = array("stok_berat"=>str_replace(",","",$arrSaldo2["saldoAkhirBerat"]),
                                           "stok_lembar"=>str_replace(",","",$arrSaldo2["saldoAkhirLembar"]),
                                           "kd_gd_hasil"=>$kdGdHasil2);
              $result_2 = $this->Gudang_Hasil_Model->updateGudangHasil($dataForUpdateMaster2);
            }
  ...
  public function deleteAndRestoreTransaksiGudangHasil(){
    ...
    if($isLogin){
      ...
      $data = array("tglAwal" => $tglAwal,
                    ...
                    "kdGdHasil" => $kdGdHasil,
                    "deleted" => $deleted);
      $result = $this->Gudang_Hasil_Model->updateDeleteTransaksiPengambilan($data);
      echo $result;
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
      redirect("_main/main","refresh");
  ...
  public function addPengambilanKanvas(){
    ...
    if($isLogin){
      ...
      if(empty($tanggal) || empty($idUser) || $jumlahLembar == "" || $jumlahBerat ==""){
        ...
        $result = $this->Gudang_Hasil_Model->insertTransaksiPengambilanKanvas($data);
        echo $result;
      }
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
  ...
  public function getListPengambilanKanvasTemp(){
    ...
    if($isLogin){
      ...
      $result = $this->Gudang_Hasil_Model->selectListPengambilanKanvasTemp($jnsGudang);
      echo $result;
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
      redirect("_main/main","refresh");
  ...
  public function deleteAndRestorePengambilanKanvasTemp(){
    $isLogin = $this->isLogin();
    if($isLogin){
      $idTransaksi = $this->input->post("idPengambilanKanvas");
      $deleted = $this->input->post("deleted");
      $data = array("id_pengambilan_kanvas" => $idTransaksi,
                    "deleted" => $deleted);
      $result = $this->Gudang_Hasil_Model->updateTransaksiPengambilanKanvas($data);
      if($result){
        echo "Berhasil";
      }else{
        echo "Gagal";
  ...
  public function savePengambilanKanvas(){
    ...
    if($isLogin){
      ...
      $arrData = json_decode($this->Gudang_Hasil_Model->selectListPengambilanKanvasTemp($jnsGudang),TRUE);
      $jumlahDataPengambilan = count($arrData);
      $jumlahDataSelesai = 0;
      if(empty($arrData)){
        echo "Item Kosong";
        ...
        foreach ($arrData as $data) {
          ...
          $result = $this->Gudang_Hasil_Model->insertTransaksiGudangHasil_PengambilanKanvas($newArrData);
          if($result){
            $jumlahDataSelesai++;
          }
        }
  ...
  public function approveKembalianHDSablon()
  {
    ...
    if($isLogin){
      $stsBarang = $this->input->post("stsBarang");
      $result = $this->Gudang_Hasil_Model->approveKembalianHDSablon($stsBarang);
      echo $result;
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
      redirect("_main/main","refresh");
  ...
  public function getCekKartuStok(){
    ...
    if($isLogin){
      ...
      if($stsBarang != "SABLON_POLOS"){
        ...
        $result = $this->Gudang_Hasil_Model->selectCekKartuStokSablonPolos($data);;
      }
      echo $result;
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
  ...
  public function getCekKartuStokSort(){
    ...
    if($isLogin){
      ...
      if($stsBarang != "SABLON_POLOS"){
        ...
        $result = $this->Gudang_Hasil_Model->selectCekKartuStokSortSablonPolos($data);
      }
      echo $result;
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
  ...
  public function deleteAndRestoreDetailPermintaanBarang(){
    ...
    if($isLogin){
      ...
      $data = array("idTransaksi" => $idTransaksi,
                    ...
                    "deleted" => $deleted);
      $result = $this->Gudang_Hasil_Model->updateDeleteAndRestoreDetailPermintaanBarang($data);
      echo $result;
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
      redirect("_main/main","refresh");
  ...
  public function updateCheckList(){
    ...
    if($isLogin){
      ...
      $data = array("kd_gd_hasil" => $kdGdHasil,
                    ...
                    "id_user" => $idUser,
                    "periode" => $periode);
      $result = $this->Gudang_Hasil_Model->updateCheckList($data);
      echo $result;
    }else{
         echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
         redirect("_main/main","refresh");
  ...
  public function getCekKartuStokCat(){
    $isLogin = $this->isLogin();
    if($isLogin){
      $tglAwal = $this->input->post("tglAwal");
      $tglAkhir = $this->input->post("tglAkhir");
      ...
      $data = array("tglAwal" => $tglAwal,
                    ...
                    "havingStokAwal" => $havingStokAwal,
                    "havingStokAkhir" => $havingStokAkhir);
      $result = $this->Gudang_Hasil_Model->selectCekKartuStokCat($data);
      echo $result;
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
      redirect("_main/main","refresh");
  ...
  public function updateCheckListBahan(){
    $isLogin = $this->isLogin();
    if($isLogin){
      $kdGdBahan = $this->input->post("kdGdBahan");
      $ceklis = $this->input->post("ceklis");
      $idCeklisGb = $this->input->post("idCeklisGb");
      $idUser = $this->session->userdata("fabricationIdUser");
      $periode = $this->input->post("periode");
      $data = array("kd_gd_bahan" => $kdGdBahan,
                    "ceklis" => $ceklis,
                    "id_ceklis_gb" => $idCeklisGb,
                    "id_user" => $idUser,
                    "periode" => $periode);
      $result = $this->Gudang_Hasil_Model->updateCheckListBahan($data);
      echo $result;
    }else{
         echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
         redirect("_main/main","refresh");
  ...
  public function getCountOrderDeadline(){
    $isLogin = $this->isLogin();
    if($isLogin){
      $result = $this->Gudang_Hasil_Model->selectCountOrderDeadline();
      echo $result;
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
      redirect("_main/main","refresh");
  ...
  public function getOrderDeadline(){
    $isLogin = $this->isLogin();
    if($isLogin){
      $result = $this->Gudang_Hasil_Model->selectOrderDeadline();
      echo $result;
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
      redirect("_main/main","refresh");
  ...
  public function getLihatPesanan(){
    ...
    if($isLogin){
      ...
      echo json_encode($this->Gudang_Hasil_Model->selectLihatPesanan($no_order));
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
      redirect("_main/main","refresh");
    }
  ...
  public function getLihatPesananDetail(){
    ...
    if($isLogin){
      ...
      echo $this->Gudang_Hasil_Model->selectLihatPesananDetails($no_order);
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
      redirect("_main/main","refresh");
    }
  ...
  public function getDataUntukPermintaanPlastikHD(){
    $isLogin = $this->isLogin();
    if($isLogin){
      $result["DataPlastik"] = json_decode($this->Gudang_Hasil_Model->selectComboBoxValueGudangHasilHd(), TRUE);
      $result["KodePRHD"] = $this->Gudang_Hasil_Model->generateKodePermintaanHd();
      echo json_encode($result);
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
      redirect("_main/main","refresh");
  ...
  public function saveAddPermintaanHd(){
    $isLogin = $this->isLogin();
    if($isLogin){
      ...
      $ukuran = $this->input->post("ukuran");
      $merek = $this->input->post("merek");
      $warna_plastik = $this->input->post("warna_plastik");
      $tebal = $this->input->post("tebal");
      $specialChar = array('(',')');
      $idUser = $this->session->userdata("fabricationIdUser");
      $bagian = $this->session->userdata("fabricationGroup");
      $data = array("id"                       => $kdGdHasil,
                    "qty"                      => $jumlahBerat,
                    "price"                    => $jumlahLembar,
                    "name"                     => str_replace($specialChar,'',$merek),
                    "namaBarang"               => str_replace($specialChar,'',$merek),
                    "kd_permintaan_plastik"    => $kodePermintaan,
                    "kd_gd_hasil"              => $kdGdHasil,
                    "id_user"                  => $idUser,
                    "bagian"                   => $bagian,
                    "tgl_permintaan"           => $tanggal,
                    "jumlah_berat_permintaan"  => $jumlahBerat,
                    "jumlah_lembar_permintaan" => $jumlahLembar,
                    "ukuran"                   => $ukuran,
                    "merek"                    => $merek,
                    "warna_plastik"            => $warna_plastik,
                    "tebal"                    => $tebal,
                    "keterangan"               => $keterangan,
                    "prefix"                   => "PEMBELIAN PLASTIK HD IDUSER=".$idUser);
      $result = $this->cart->insert($data);
      if($result){
        echo "Berhasil";
      }else{
  ...
  public function getPermintaanPlastikHdTemp(){
    $isLogin = $this->isLogin();
    if($isLogin){
      $idUser = $this->session->userdata("fabricationIdUser");
      $data=array();
      foreach ($this->cart->contents() as $key) {
        if($key["prefix"] == "PEMBELIAN PLASTIK HD IDUSER=".$idUser){
          array_push($data,$key);
        }
      }
      echo json_encode($data);
  ...
  public function savePermintaanHd(){
    $isLogin = $this->isLogin();
    if($isLogin){
      $idUser = $this->session->userdata("fabricationIdUser");
      $data=array();
      ...
      if(count($data) == 0){
        ...
        for($i=0; $i<count($data); $i++){
          $result = $this->Gudang_Hasil_Model->insertPermintaanPlastikHd($data[$i]);
          $this->cart->remove($data[$i]["rowid"]);
          if($result){
            $counter++;
          }
  ...
  public function getDetailPermintaanPlastikHdBaru(){
    $isLogin = $this->isLogin();
    if($isLogin){
      $idPermintaan = $this->input->post("idPermintaan");
      $idUser = $this->session->userdata("fabricationIdUser");
      ...
      $data = array("idUser" => $idUser,
                    "bagian" => $bagian,
                    "idPermintaan" => $idPermintaan);
      $result = $this->Gudang_Hasil_Model->selectDetailPermintaanPlastikHdBaru($data);
      echo $result;
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
      redirect("_main/main","refresh");
  ...
  public function printBonPermintaanPlastik($param){
    $isLogin = $this->isLogin();
    if($isLogin){
      $data = array("DataPermintaan" => $this->Gudang_Hasil_Model->selectPermintaanPlastik_Print($param),
                    "DataDetailPermintaan" => $this->Gudang_Hasil_Model->selectDetailPermintaanPlastik_Print($param));
      $this->load->view("frm_print_bon_permintaan_plastik",$data);
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
      redirect("_main/main","refresh");
    }
  ...
  public function deleteAndRestoreDetailPermintaanPlastik(){
    $isLogin = $this->isLogin();
    if($isLogin){
      $idTransaksi = $this->input->post("idTransaksi");
      $kdPermintaan = $this->input->post("kdPermintaan");
      ...
      $data = array("idTransaksi" => $idTransaksi,
                    "kdPermintaan" => $kdPermintaan,
                    "deleted" => $deleted);
      $result = $this->Gudang_Hasil_Model->updateDeleteAndRestoreDetailPermintaanPlastik($data);
      echo $result;
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
      redirect("_main/main","refresh");
  ...
  public function terimaPlastikFull(){
    $isLogin = $this->isLogin();
    if($isLogin){
      $idDPP = $this->input->post("idDpp");
      $idPermintaan = $this->input->post("idPermintaan");
      $idUser = $this->session->userdata("fabricationIdUser");
      $namaSupplier = $this->input->post("namaSupplier");
      $tglTerima = $this->input->post("tglTerima");
      $arrData = $this->Gudang_Hasil_Model->selectDataPermintaanUntukInputTransaksiGudangHasil($idDPP);
      $Data = array("TGH" => array("kd_gd_hasil"        => $arrData[0]["kd_gd_hasil"],
                                   "id_user"            => $idUser,
                                   "customer"           => $namaSupplier,
                                   "ukuran"             => $arrData[0]["ukuran"],
                                   "warna"              => $arrData[0]["warna_plastik"],
                                   "merek"              => $arrData[0]["merek"],
                                   "jns_permintaan"     => $arrData[0]["jns_permintaan"],
                                   "sts_barang"         => $arrData[0]["jns_brg"],
                                   "jumlah_berat"       => $arrData[0]["jumlah_berat_permintaan"],
                                   "jumlah_lembar"      => $arrData[0]["jumlah_lembar_permintaan"],
                                   "tgl_transaksi"      => $tglTerima,
                                   "bagian"             => str_replace("_"," ",$arrData[0]["bagian"]),
                                   "status_transaksi"   => "FINISH",
                                   "status_history"     => "MASUK",
                                   "keterangan_history" => "PEMBELIAN BARANG",
                                   "keterangan_history" => "PEMBELIAN BARANG (HD)"
                                  ),
                    "TDPP" => array("id_dpp"                => $idDPP,
                                    "kd_permintaan_plastik" => $idPermintaan
                                   ),
                    "TBPP" => array("id_user"       => $idUser,
                                    "id_dpp"        => $idDPP,
                                    "supplier"      => $namaSupplier,
                                    "tgl_terima"    => $tglTerima,
                                    "jumlah_berat_terima" => $arrData[0]["jumlah_berat_permintaan"],
                                    "jumlah_lembar_terima" => $arrData[0]["jumlah_lembar_permintaan"],
                                   )
                    );
      $result = $this->Gudang_Hasil_Model->updateTerimaPlastikFull($Data);
      echo $result;
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
      redirect("_main/main","refresh");
  ...
  public function terimaPlastikSetengah(){
    $isLogin = $this->isLogin();
    if($isLogin){
      $idDPP = $this->input->post("idDpp");
      $idPermintaan = $this->input->post("idPermintaan");
      ...
      $namaSupplier = $this->input->post("namaSupplier");
      $tglTerima = $this->input->post("tglTerima");
      $jumlahBeratTerima = $this->input->post("jumlahBeratTerima");
      $jumlahLembarTerima = $this->input->post("jumlahLembarTerima");
      $arrData = $this->Gudang_Hasil_Model->selectDataPermintaanUntukInputTransaksiGudangHasil($idDPP);
      $Data = array("TGH" => array("kd_gd_hasil"        => $arrData[0]["kd_gd_hasil"],
                                   "id_user"            => $idUser,
                                   "customer"           => $namaSupplier,
                                   ...
                                   "jns_permintaan"     => $arrData[0]["jns_permintaan"],
                                   "sts_barang"         => $arrData[0]["jns_brg"],
                                   "jumlah_berat"       => $jumlahBeratTerima,
                                   "jumlah_lembar"      => $jumlahLembarTerima,
                                   "tgl_transaksi"      => $tglTerima,
                                   "bagian"             => str_replace("_"," ",$arrData[0]["bagian"]),
                                   "status_transaksi"   => "FINISH",
                                   "status_history"     => "MASUK",
                    ...
                    "TBPP" => array("id_user"       => $idUser,
                                    ...
                                    "supplier"      => $namaSupplier,
                                    "tgl_terima"    => $tglTerima,
                                    "jumlah_berat_terima" => $jumlahBeratTerima,
                                    "jumlah_lembar_terima" => $jumlahLembarTerima,
                                   )
                    );
      $result = $this->Gudang_Hasil_Model->updateTerimaPlastikSetengah($Data);
      echo $result;
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
      redirect("_main/main","refresh");
  ...
  public function selesaiPermintaanPlastik(){
    $isLogin = $this->isLogin();
    if($isLogin){
      $idDpp = $this->input->post("idDpp");
      $idPermintaan = $this->input->post("idPermintaan");
      $data = array("id_dpp"        => $idDpp,
                    "kd_permintaan" => $idPermintaan);
      $result = $this->Gudang_Hasil_Model->updateSelesaiPermintaanPlastik($data);
      if($result){
        echo "Berhasil";
      }else{
        echo "Gagal";
  ...
  public function getCounterKembalianSablon(){
    $isLogin = $this->isLogin();
    if($isLogin){
      $result = $this->Gudang_Hasil_Model->selectCounterKembalianSablon();
      echo $result;
    }else{
         echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
         redirect("_main/main","refresh");
  ...
  public function getCounterKembalianSablonCampur(){
    $isLogin = $this->isLogin();
    if($isLogin){
      $result = $this->Gudang_Hasil_Model->selectCounterKembalianSablonCampur();
      echo $result;
    }else{
         echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
         redirect("_main/main","refresh");
  ...
  public function getListKembalianSablon(){
    $isLogin = $this->isLogin();
    if($isLogin){
      $stsBarang = $this->input->post("stsBarang");
      $result = $this->Gudang_Hasil_Model->selectListKembalianSablon($stsBarang);
      echo $result;
    }else{
         echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
         redirect("_main/main","refresh");
    ...
    if($isLogin){
      ...
      if(empty($idTransaksi) || $jumlah=="" || empty($tanggal)){
        ...
        if($result){
          ...
          $arrSaldo = json_decode($this->Gudang_Hasil_Model->selectSaldoAwalBulanGudangBahan($data2),TRUE); #Menghitung Saldo Barang Tujuan
          $arrSaldo2 = json_decode($this->Gudang_Hasil_Model->selectSaldoAwalBulanGudangBahan($data2_1),TRUE); #Menghitung Saldo Barang Asal
          ...
          $result2 = $this->Gudang_Hasil_Model->updateGudangBahan($data3);
          $result2_1 = $this->Gudang_Hasil_Model->updateGudangBahan($data3_1);
  ...
  public function perbaruiStokMaster(){
    ...
    if($isLogin){
      ...
      $result = $this->Gudang_Hasil_Model->updateStokMasterBatch($arrData);
      if($result){
        echo "Berhasil";
      }else{
        echo "Gagal";
