...
class Gudang_Hasil_Model extends CI_Model{
  ...
  public function generateKodePermintaanHd(){
    $idUser = $this->session->userdata("fabricationIdUser");
    $idUserLen = strlen($idUser);
    $maxCode = $this->db->query("SELECT MAX(RIGHT(kd_permintaan_plastik,3)) AS kode FROM transaksi_permintaan_plastik_hd
                                 WHERE SUBSTRING(kd_permintaan_plastik,5,4) = DATE_FORMAT(NOW(),'%y%m')
                                 AND SUBSTRING(kd_permintaan_plastik,9,$idUserLen)");
    foreach ($maxCode->result() as $arrMaxCode) {
      if($arrMaxCode->kode == NULL){
        $tempCode = "00";
      }
      $tempCode = "00".(intval($arrMaxCode->kode)+1);
      $fixCode = "PRHD".date("ym").$idUser.substr($tempCode,(strlen($tempCode)-3));
    }
    return $fixCode;
  }
    ...
    if($param["jns_permintaan"]=="CETAK" && $param["jns_brg"]=="SABLON"){
      $this->db->query("INSERT INTO transaksi_gudang_hasil (kd_gd_hasil,kd_gd_hasil_secondary,id_user,ukuran,jumlah_berat,jumlah_lembar,warna,customer,tgl_transaksi,merek,
                        ...
                        VALUES ('$param[kd_gd_hasil]','$param[kd_gd_hasil]','$param[id_user]','$param[ukuran]','$param[stok_berat]','$param[stok_lembar]','$param[warna]','GUDANG HASIL','$param[tgl_buat]','$param[merek]','$param[jns_permintaan]',
                                '$param[jns_brg]','TRUE','MASUK','DATA AWAL','$param[keterangan]','FALSE','FINISH')");
    }else{
      $this->db->query("INSERT INTO transaksi_gudang_hasil (kd_gd_hasil,id_user,ukuran,jumlah_berat,jumlah_lembar,warna,customer,tgl_transaksi,merek,
                        ...
                        VALUES ('$param[kd_gd_hasil]','$param[id_user]','$param[ukuran]','$param[stok_berat]','$param[stok_lembar]','$param[warna_plastik]','GUDANG HASIL','$param[tgl_buat]','$param[merek]','$param[jns_permintaan]',
                                '$param[jns_brg]','TRUE','MASUK','FINISH','DATA AWAL','$param[keterangan]','FALSE')");
    }
  ...
  public function insertAddPengirimanGudang($param){
    ...
    $checkLock = $this->db->query("SELECT COUNT(status_lock) AS count_lock FROM transaksi_gudang_hasil
                                   ...
                                   AND kd_gd_hasil='$param[kd_gd_hasil]'")->result_array();
  ...
  public function insertTransaksiGudangHasil_Pengiriman($param){
    ...
    $checkLock = $this->db->query("SELECT COUNT(status_lock) AS count_lock FROM transaksi_gudang_hasil
                                   ...
                                   AND status_lock='TRUE'
                                   AND sts_barang = '$param[sts_barang]'")->result_array();
    if($checkLock[0]["count_lock"] > 0){
      $this->db->trans_rollback();
      return FALSE;
    }else{
      $Q = "SELECT * FROM pesanan WHERE no_order='$param[no_order]' AND deleted='FALSE'";
      $arrData = $this->db->query($Q)->result_array();
      $namaPemesan = strtoupper($arrData[0]["nm_pemesan"]);
      ...
      if($namaPemesan == "GANEFO"     || $namaPemesan == "SURABAYA"   ||
         $namaPemesan == "MEDAN"      || $namaPemesan == "BREBES"     ||
         $namaPemesan == "SEMARANG"   || $namaPemesan == "BANDUNG"
      ){
        $this->db->query("UPDATE pesanan_detail
                          SET sts_pesanan = 'FINISH', sts_kirim = 'TERKIRIM', tgl_kirim=NOW()
      ...
      if($counterProgress[0]["counter"] <= 0){
        if($namaPemesan == "GANEFO"     || $namaPemesan == "SURABAYA"   ||
          ...
          $this->db->query("UPDATE pesanan SET sts_pesanan='FINISH', tgl_kirim=NOW() WHERE no_order='$param[no_order]'");
        }else{
  ...
  public function insertTransaksiGudangBahan_Pengiriman($param){
    ...
    $checkLock = $this->db->query("SELECT COUNT(status_lock) AS count_lock FROM transaksi_gudang_bahan
                                   ...
                                   AND status_lock='TRUE'
                                   AND kd_gd_bahan = '$param[kd_gd_bahan]'")->result_array();
    if($checkLock[0]["count_lock"] > 0){
      $this->db->trans_rollback();
      return FALSE;
    }else{
  ...
  public function insertTransaksiPengambilanSablon($param){
    $this->db->trans_begin();
    $kode = substr($param["kd_gd_hasil"],0,4);
    ...
    $checkLock2 = $this->db->query("SELECT COUNT(status_lock) AS count_lock FROM transaksi_gudang_hasil
                                    ...
                                    AND status_lock='TRUE'
                                    AND LEFT(kd_gd_hasil,4) = '$kode'")->result_array();
  ...
  public function insertTransaksiPengambilanKanvas($param){
    $this->db->trans_begin();
    $prefix = substr($param["kd_gd_hasil"], 0, 4);
    if($prefix == "HSLS"){
      $jnsGudang = "STANDARD";
    }else if($prefix == "HSLC"){
      $jnsGudang = "CAMPUR";
    ...
    $periode = date("Y-m", strtotime($param["tgl_pengambilan"]));
    $checkLock = $this->db->query("SELECT COUNT(TPK.status_lock) AS count_lock FROM transaksi_pengambilan_kanvas TPK
                                   INNER JOIN gudang_hasil GH ON TPK.kd_gd_hasil = GH.kd_gd_hasil
                                   WHERE DATE_FORMAT(TPK.tgl_pengambilan,'%Y-%m')='$periode'
                                   AND TPK.status_lock='TRUE'
                                   AND GH.jns_brg='$jnsGudang'
                                   AND TPK.deleted='FALSE'")->result_array();
    $checkLock2 = $this->db->query("SELECT COUNT(status_lock) AS count_lock FROM transaksi_gudang_hasil
                                    WHERE DATE_FORMAT(tgl_transaksi,'%Y-%m')='$periode'
                                    AND status_lock='TRUE'
                                    AND sts_barang = 'KANVAS'
                                    AND deleted = 'FALSE'
    ...
    if($checkLock[0]["count_lock"] > 0 || $checkLock2[0]["count_lock"] > 0){
      ...
      $this->db->insert("transaksi_pengambilan_kanvas",$param);
      if($this->db->trans_status() === FALSE){
        $this->db->trans_rollback();
        return "Gagal";
      }else{
  ...
  public function insertTransaksiGudangHasil_PengambilanKanvas($param){
    $this->db->trans_begin();
    #=======Insert Data Ke Tabel Transaksi Gudang Hasil Dari Tabel Transaksi Pengambilan Sablon (Start)=======#
    $this->db->insert("transaksi_gudang_hasil",array_diff_key($param,array_flip(array("id_pengambilan_kanvas"))));
    $this->db->set("stok_lembar","stok_lembar - ".$param["jumlah_lembar"],FALSE);
    $this->db->set("stok_berat","stok_berat - ".$param["jumlah_berat"],FALSE);
    ...
    #=======Update Status Pengambilan Menjadi True Pada Tabel Transaksi Pengambilan Sablon Saat Data Telah Masuk Ke Tabel Transaksi Gudang Hasil (Start)=======#
    $this->db->query("UPDATE transaksi_pengambilan_kanvas
                      SET status_pengambilan='TRUE'
                      WHERE id_pengambilan_kanvas = '$param[id_pengambilan_kanvas]'");
    #=======Update Status Pengambilan Menjadi True Pada Tabel Transaksi Pengambilan Sablon Saat Data Telah Masuk Ke Tabel Transaksi Gudang Hasil (Finish)=======#
  ...
  public function insertPermintaanPlastikHd($param){
    if(array_key_exists("rowid",$param)){
      unset($param["rowid"]);
    }
    $this->db->trans_begin();
    $checkTransaksi = $this->db->query("SELECT kd_permintaan_plastik
                                        FROM transaksi_permintaan_plastik_hd
                                        WHERE kd_permintaan_plastik='$param[kd_permintaan_plastik]'")->num_rows();
    if($checkTransaksi <= 0){
      $this->db->set("kd_permintaan_plastik",$param["kd_permintaan_plastik"]);
      $this->db->set("id_user",$param["id_user"]);
      $this->db->set("tgl_permintaan",$param["tgl_permintaan"]);
      $this->db->set("bagian",$param["bagian"]);
      $this->db->insert("transaksi_permintaan_plastik_hd");
    }

    $this->db->set("kd_permintaan_plastik",$param["kd_permintaan_plastik"]);
    $this->db->set("kd_gd_hasil",$param["kd_gd_hasil"]);
    $this->db->set("jumlah_berat_permintaan",$param["jumlah_berat_permintaan"]);
    $this->db->set("jumlah_lembar_permintaan",$param["jumlah_lembar_permintaan"]);
    $this->db->set("keterangan",$param["keterangan"]);
    $this->db->insert("transaksi_detail_permintaan_plastik_hd");
    ...
    if(empty($param["jns_permintaan"])){
      ...
      if(strpos($param["key"],"|") !== FALSE){
        ...
        $Q = "SELECT * FROM gudang_hasil
              WHERE ((jns_brg = '$param[jenis]'
              AND jns_permintaan = '$param[jns_permintaan]')
              OR (merek IN ('Klip','kartunama','kartu nama','Kantong Klip Aneka Warna','Kantong Klip Aneka Macam') AND jns_brg = '$param[jenis]'))
              AND deleted = 'FALSE'
              AND ukuran LIKE '%$arrKey[0]%'
              AND merek LIKE '%$arrKey[1]%'
              AND warna_plastik LIKE '%$arrKey[2]%'
  ...
  public function selectComboBoxValueGudangHasilHd(){
    $this->db->select("kd_gd_hasil, ukuran, merek, warna_plastik, jns_brg, sts_brg, jns_permintaan, tebal");
  ...
  public function selectListPengirimanBaruDatatable($key){
    $this->datatables->select("IF(C.nm_perusahaan_update IS NULL, C.nm_perusahaan,
                               C.nm_perusahaan_update) AS nm_perusahaan,
                               P.no_order,P.tgl_pesan,P.nm_pemesan,P.kd_order,P.no_po,P.note,
                               GH.ukuran,GH.warna_plastik,PD.satuan, GH.jns_brg,
                               PD.merek,PD.dll,PD.jumlah, PD.jumlah_kirim,PD.keterangan,
                               PD.sts_pesanan, PD.sts_kirim,
                               (PD.jumlah - PD.jumlah_kirim) AS sisa, PD.sm, PD.id_dp, PD.warna_cetak");
    $this->datatables->from("pesanan P");
    if (empty($key)) {
      $this->datatables->where("P.sts_pesanan = 'PROGRESS' AND PD.kd_gd_hasil IS NOT NULL AND PD.sts_pesanan = 'PROGRESS' AND PD.sts_kirim != 'TERKIRIM'
  ...
  public function selectListPesananTerkirim($key){
    ...
    if (empty($key)) {
      ...
      $this->datatables->where(" $search P.deleted = 'FALSE' AND
                                 PD.deleted = 'FALSE' AND
                                 PD.tgl_kirim_gudang IS NOT NULL AND
                                 PD.jumlah_kirim > 0");
    }
  ...
  public function selectListPesananBelumTerkirim(){
    ...
    $this->datatables->from("pesanan P");
    $this->datatables->where("P.sts_pesanan = 'PROGRESS' AND
                              PD.sts_pesanan = 'PROGRESS' AND
                              PD.sts_kirim = 'BELUM TERKIRIM' AND
                              P.deleted = 'FALSE' AND
                              PD.deleted = 'FALSE' AND
                              PD.tgl_kirim_gudang IS NOT NULL AND
                              (PD.jumlah - PD.jumlah_kirim) > 0");
    $this->datatables->join("pesanan_detail PD","PD.no_order = P.no_order","INNER");
    $this->datatables->join("gudang_hasil GH","PD.kd_gd_hasil = GH.kd_gd_hasil","LEFT");
    $this->datatables->join("gudang_bahan GB","PD.kd_gd_bahan = GB.kd_gd_bahan","LEFT");
    $this->db->order_by("P.tgl_pesan","DESC");
  ...
  public function selectListPengirimanBaruTemp(){
    $arrData = $this->db->query("SELECT TDP.*,
                                 ...
                                 AND PD.deleted='FALSE'")->result_array();
    return json_encode($arrData);
  }
  ...
  public function selectListGudangHasilDatatable($param){
    $this->datatables->select("kd_gd_hasil,kd_accurate,ukuran,
                               FORMAT(stok_berat,2) AS stok_berat,
                               FORMAT(stok_lembar,0) AS stok_lembar,
                               warna_plastik,merek,jns_permintaan,jns_brg,tebal");
  ...
  public function selectTrashListTransaksiGudangHasil(){
    $this->datatables->select("id_permintaan_jadi,kd_gd_hasil,tgl_transaksi,
                               CONCAT(merek,' ',ukuran,' ', warna,' ',jns_permintaan, ' (',sts_barang, ')') AS merek,
                               jumlah_berat,jumlah_lembar,keterangan_history,status_history");
  ...
  public function selectListHistoryGudangHasilMasukDatatable($param){
    ...
    $this->datatables->select("id_permintaan_jadi,tgl_transaksi,
                               FORMAT(jumlah_berat,2) AS jumlah_berat,
                               FORMAT(jumlah_lembar,2) AS jumlah_lembar,
  ...
  public function selectListHistoryGudangHasilBufferMasukDatatable($param){
    ...
    $this->datatables->where("(status_history = 'KELUAR' OR status_history = 'MASUK') AND tgl_transaksi BETWEEN '$tglAwalPlus1' AND
                              '$param[tglAkhir]' AND
                              deleted='FALSE' AND
                              keterangan_history NOT IN ('PENGAMBILAN SABLON (SABLON)','KOREKSI BARANG (MINUS)') AND
                              status_transaksi='FINISH' AND
                              (kd_gd_hasil_secondary='$param[kdGdHasil]' OR kd_gd_hasil='$param[kdGdHasil]')");
    $this->db->order_by('tgl_transaksi','DESC');
    return $this->datatables->generate();
  }
  ...
  public function selectListHistoryGudangHasilKeluarDatatable($param){
    $tglAwalPlus1 = date("Y-m-d",strtotime("+1 days",strtotime($param["tglAwal"])));
    if(strpos($param['kdGdHasil'],"HSSB") !== FALSE){
      $clause = " keterangan_history NOT IN('PENGAMBILAN SABLON (SABLON)') AND ";
    }else{
      $clause = "";
    }
    $this->datatables->select("id_permintaan_jadi,tgl_transaksi,
                               FORMAT(jumlah_berat,2) AS jumlah_berat,
                               FORMAT(jumlah_lembar,2) AS jumlah_lembar,
                               keterangan_history,status_history,status_lock,
                               customer,keterangan_barang, keterangan");
    $this->datatables->from("transaksi_gudang_hasil");
  ...
  public function selectListHistoryGudangBufferKeluarDatatable($param){
    ...
    $this->datatables->where("status_history = 'KELUAR' AND tgl_transaksi BETWEEN '$tglAwalPlus1' AND
                              ...
                              deleted='FALSE' AND
                              status_transaksi='FINISH' AND
                              keterangan_history IN ('PENGAMBILAN SABLON (SABLON)', 'KOREKSI BARANG (MINUS)') AND
                              (kd_gd_hasil_secondary='$param[kdGdHasil]' OR kd_gd_hasil='$param[kdGdHasil]')");
    $this->db->order_by('tgl_transaksi','DESC');
    return $this->datatables->generate();
  }
  ...
  public function selectSaldoAwalBulanGudangHasil($param){
    $tglAwalPlus1 = date("Y-m-d",strtotime("+1 days",strtotime($param["tglAwal"])));
    if(strpos($param["kdGdHasil"],"HSSB") !== FALSE){
      $clause = " AND keterangan_history NOT IN('PENGAMBILAN SABLON (SABLON)')";
    }else{
      $clause = "";
    }
    $saldoAwal = $this->db->query("SELECT (
                                           SUM(IF(status_history='MASUK' AND tgl_transaksi <= '$param[tglAwal]',jumlah_berat,0))-
                                           SUM(IF(status_history='KELUAR' AND tgl_transaksi <= '$param[tglAwal]' $clause,jumlah_berat,0))
                                          )
                                AS saldo_awal_berat,
                                          (
                                           SUM(IF(status_history='MASUK' AND tgl_transaksi <= '$param[tglAwal]',jumlah_lembar,0))-
                                           SUM(IF(status_history='KELUAR' AND tgl_transaksi <= '$param[tglAwal]' $clause,jumlah_lembar,0))
                                          )
    ...
    $totalPerPeriode = $this->db->query("SELECT SUM(IF(status_history='MASUK',jumlah_berat,0)) AS total_masuk_berat_per_periode,
                                                SUM(IF(status_history='KELUAR' $clause,jumlah_berat,0)) AS total_keluar_berat_per_periode,
                                                SUM(IF(status_history='MASUK',jumlah_lembar,0)) AS total_masuk_lembar_per_periode,
                                                SUM(IF(status_history='KELUAR' $clause,jumlah_lembar,0)) AS total_keluar_lembar_per_periode
                                         FROM transaksi_gudang_hasil
                                         WHERE kd_gd_hasil = '$param[kdGdHasil]'
  ...
  public function selectSaldoAwalBulanGudangHasilBuffer($param){
    ...
    $saldoAwal = $this->db->query("SELECT (
                                           ...
                                           SUM(IF(kd_gd_hasil = '$param[kdGdHasil]' AND
                                                  keterangan_history='DATA AWAl', jumlah_berat, 0)) +
                                           (
                                             SUM(IF(kd_gd_hasil = '$param[kdGdHasil]' AND
                                                  keterangan_history IN ('KOREKSI BARANG (PLUS)','PENGEMBALIAN BARANG') AND
                                                  tgl_transaksi <= '$param[tglAwal]', jumlah_berat, 0)) -
                                             SUM(IF(kd_gd_hasil = '$param[kdGdHasil]' AND
                                                  keterangan_history='KOREKSI BARANG (MINUS)' AND
                                                  tgl_transaksi <= '$param[tglAwal]', jumlah_berat, 0))
                                           )
                                          )
                                AS saldo_awal_berat,
                                          (
                                            ...
                                            SUM(IF(kd_gd_hasil = '$param[kdGdHasil]' AND
                                                   keterangan_history='DATA AWAl', jumlah_lembar, 0))+

                                            (
                                              SUM(IF(kd_gd_hasil = '$param[kdGdHasil]' AND
                                                   keterangan_history IN ('KOREKSI BARANG (PLUS)','PENGEMBALIAN BARANG') AND
                                                  tgl_transaksi <= '$param[tglAwal]', jumlah_lembar, 0)) -
                                              SUM(IF(kd_gd_hasil = '$param[kdGdHasil]' AND
                                                   keterangan_history='KOREKSI BARANG (MINUS)' AND
                                                   tgl_transaksi <= '$param[tglAwal]', jumlah_lembar, 0))
                                            )
    ...
    $totalPerPeriode = $this->db->query("SELECT SUM(IF((keterangan_history != 'PENGAMBILAN SABLON (SABLON)'
                                                       AND kd_gd_hasil_secondary='$param[kdGdHasil]') OR
                                                       (keterangan_history IN ('KOREKSI BARANG (PLUS)','PENGEMBALIAN BARANG') AND status_history='MASUK'
                                                       AND kd_gd_hasil='$param[kdGdHasil]'),jumlah_berat,0)) AS total_masuk_berat_per_periode,

                                                SUM(IF(status_history='KELUAR' AND keterangan_history IN ('PENGEMBALIAN BARANG','PENGAMBILAN SABLON (SABLON)','KOREKSI BARANG (MINUS)'),jumlah_berat,0)) AS total_keluar_berat_per_periode,

                                                SUM(IF((keterangan_history != 'PENGAMBILAN SABLON (SABLON)'
                                                        AND kd_gd_hasil_secondary='$param[kdGdHasil]') OR
                                                        (keterangan_history IN ('KOREKSI BARANG (PLUS)','PENGEMBALIAN BARANG') AND status_history='KELUAR'
                                                        AND kd_gd_hasil='$param[kdGdHasil]'),jumlah_lembar,0)) AS total_masuk_lembar_per_periode,

                                                SUM(IF(status_history='KELUAR' AND keterangan_history IN ('PENGEMBALIAN BARANG','PENGAMBILAN SABLON (SABLON)','KOREKSI BARANG (MINUS)'),jumlah_lembar,0)) AS total_keluar_lembar_per_periode
                                         FROM transaksi_gudang_hasil
                                         WHERE (kd_gd_hasil_secondary = '$param[kdGdHasil]' OR kd_gd_hasil = '$param[kdGdHasil]')
                                         AND tgl_transaksi BETWEEN '$tglAwalPlus1' AND '$param[tglAkhir]'
                                         AND deleted='FALSE'
  ...
  public function selectSaldoAwalBulanGudangHasilKanvas($param){
    $tglAwalPlus1 = date("Y-m-d",strtotime("+1 days",strtotime($param["tglAwal"])));
    $saldoAwal = $this->db->query("SELECT (
                                           SUM(IF((keterangan_history != 'PENGAMBILAN SABLON (SABLON)'
                                                  AND keterangan_history NOT LIKE '%PENGEMBALIAN BARANG%')
                                           ...
                                           SUM(IF((keterangan_history LIKE '%PENGEMBALIAN BARANG%'
                                                  ...
                                                  AND kd_gd_hasil_secondary = '$param[kdGdHasil]'
                                                  AND tgl_transaksi <= '$param[tglAwal]',jumlah_berat,0))-
                                           SUM(IF((keterangan_history IN ('PENGEMBALIAN BARANG','PENGIRIMAN BARANG','UNTUK ANEKA MACAM'))
                                                  AND kd_gd_hasil = '$param[kdGdHasil]'
                                                  AND tgl_transaksi <= '$param[tglAwal]',jumlah_berat,0))
                                          )
                                AS saldo_awal_berat,
                                          (
    ...
    $totalPerPeriode = $this->db->query("SELECT SUM(IF(keterangan_history NOT IN ('PENGEMBALIAN BARANG','PENGIRIMAN BARANG','UNTUK ANEKA MACAM')
                                                       AND kd_gd_hasil_secondary='$param[kdGdHasil]',jumlah_berat,0)) AS total_masuk_berat_per_periode,

                                                SUM(IF(status_history='KELUAR' AND
                                                       keterangan_history IN ('PENGEMBALIAN BARANG','PENGIRIMAN BARANG','UNTUK ANEKA MACAM') AND
                                                       kd_gd_hasil = '$param[kdGdHasil]',jumlah_berat,0)) AS total_keluar_berat_per_periode,

                                                SUM(IF(keterangan_history NOT IN ('PENGEMBALIAN BARANG','PENGIRIMAN BARANG','UNTUK ANEKA MACAM')
                                                    AND kd_gd_hasil_secondary='$param[kdGdHasil]',jumlah_lembar,0)) AS total_masuk_lembar_per_periode,

                                                SUM(IF(status_history='KELUAR' AND
                                                       keterangan_history IN ('PENGEMBALIAN BARANG','PENGIRIMAN BARANG','UNTUK ANEKA MACAM') AND
                                                       kd_gd_hasil = '$param[kdGdHasil]',jumlah_lembar,0)) AS total_keluar_lembar_per_periode
                                         FROM transaksi_gudang_hasil
                                         WHERE (kd_gd_hasil_secondary = '$param[kdGdHasil]' OR kd_gd_hasil = '$param[kdGdHasil]')
                                         AND tgl_transaksi BETWEEN '$tglAwalPlus1' AND '$param[tglAkhir]'
                                         AND deleted='FALSE'
    ...
    $stokMaster = $this->db->query("SELECT stok_berat, stok_lembar FROM gudang_hasil WHERE kd_gd_hasil = '$param[kdGdHasil]'")->result_array();
    $result = array("saldoAwalBerat"=>number_format($saldoAwal[0]["saldo_awal_berat"],2,".",","),
                    "saldoAwalLembar"=>number_format($saldoAwal[0]["saldo_awal_lembar"],2,".",","),
                    "saldoAkhirBerat"=>number_format($saldoAwal[0]["saldo_awal_berat"]+$totalPerPeriode[0]["total_masuk_berat_per_periode"]-$totalPerPeriode[0]["total_keluar_berat_per_periode"],2,".",","),
                    "saldoAkhirLembar"=>number_format($saldoAwal[0]["saldo_awal_lembar"]+$totalPerPeriode[0]["total_masuk_lembar_per_periode"]-$totalPerPeriode[0]["total_keluar_lembar_per_periode"],2,".",","),
                    ...
                    "saldoMasukLembar"=>number_format($totalPerPeriode[0]["total_masuk_lembar_per_periode"],2,".",","),
                    "saldoKeluarLembar"=>number_format($totalPerPeriode[0]["total_keluar_lembar_per_periode"],2,".",","),
                    "saldoBerat"=>number_format($stokMaster[0]["stok_berat"],2,".",","),
                    "saldoLembar"=>number_format($stokMaster[0]["stok_lembar"],2,".",","),);
    return json_encode($result);
  }
  ...
  public function selectListPengambilanKanvasTemp($param){
    if($param == "ONLINE"){
      $prefix = "HSOL";
    }else{
      $prefix = "HSKV";
    }
    $result = $this->db->query("SELECT TPK.*,GH.ukuran,GH.warna_plastik,GH.merek,GH.jns_permintaan,GH.jns_brg
                                FROM transaksi_pengambilan_kanvas TPK
                                INNER JOIN gudang_hasil GH ON TPK.kd_gd_hasil = GH.kd_gd_hasil
                                WHERE TPK.status_pengambilan = 'FALSE'
                                AND SUBSTRING(TPK.kd_gd_hasil_secondary,1,4) = '$prefix'
                                AND TPK.deleted='FALSE'
                                AND GH.deleted='FALSE'")->result_array();
    return json_encode($result);
  }
  ...
  public function countKirimanBarangKeGudangBuffer(){
    $data = $this->db->query("SELECT id_permintaan_jadi FROM transaksi_gudang_hasil
                              WHERE (kd_gd_hasil_secondary IS NOT NULL AND
                              customer='GUDANG HASIL' AND
                              bagian = 'SABLON' AND
                              jns_permintaan = 'POLOS' AND
  ...
  public function selectDataStokMaster($kd_gd_hasil){
    $data = $this->db->query("SELECT FORMAT(stok_berat,2) AS stok_berat, FORMAT(stok_lembar,2) AS stok_lembar
                              FROM gudang_hasil
                              WHERE kd_gd_hasil= '$kd_gd_hasil'
                              AND deleted = 'FALSE'")->result_array();
  ...
  public function selectCountAlert($param){
    if(strpos($param['bagian'],"|")===FALSE){
      $arrData = $this->db->query("SELECT COUNT(TGB.id) AS Jumlah
                                   FROM transaksi_gudang_bahan TGB
                                   INNER JOIN gudang_bahan GB ON TGB.kd_gd_bahan = GB.kd_gd_bahan
  ...
  public function selectCekKartuStok($param){
    if($param["stsBarang"] == "SABLON_HASIL"){
      $clause = "GH.jns_brg = 'SABLON' AND TGH.jns_permintaan='CETAK' ";
    }else{
      $clause = "GH.jns_brg = '$param[stsBarang]' ";
    }
    $tglAwalPlus1 = date("Y-m-d",strtotime("+1 days",strtotime($param["tglAwal"])));
    $periode = date("Y-m", strtotime($param["tglAkhir"]));
    $saldoAwal = $this->db->query("SELECT (
                                ...
                                GROUP BY GH.kd_gd_hasil")->result_array();

    $totalPerPeriode = $this->db->query("SELECT SUM(IF(TGH.status_history='MASUK',TGH.jumlah_berat,0)) AS total_masuk_berat_per_periode,
                                                SUM(IF(TGH.status_history='KELUAR',TGH.jumlah_berat,0)) AS total_keluar_berat_per_periode,
                                                SUM(IF(TGH.status_history='MASUK',TGH.jumlah_lembar,0)) AS total_masuk_lembar_per_periode,
                                                SUM(IF(TGH.status_history='KELUAR',TGH.jumlah_lembar,0)) AS total_keluar_lembar_per_periode,
                                                GH.kd_gd_hasil
                                         FROM transaksi_gudang_hasil TGH
                                         INNER JOIN gudang_hasil GH ON TGH.kd_gd_hasil = GH.kd_gd_hasil
                                         WHERE $clause
                                         AND TGH.tgl_transaksi BETWEEN '$tglAwalPlus1' AND '$param[tglAkhir]'
                                         AND TGH.deleted='FALSE'
                                         AND GH.deleted='FALSE'
                                         AND TGH.status_transaksi = 'FINISH'
                                         AND TGH.keterangan_history !='DATA AWAL'
                                         GROUP BY TGH.kd_gd_hasil")->result_array();
    $dataMaster = $this->db->query("SELECT kd_gd_hasil, CONCAT(ukuran,' ',merek,' ',warna_plastik) AS nm_barang,
                                           stok_berat, stok_lembar, ceklist
                                    FROM gudang_hasil
                                    WHERE deleted = 'FALSE'
                                    AND jns_brg = '$param[stsBarang]'
                                    ORDER BY merek ASC, CAST(SUBSTRING_INDEX(ukuran,'+',1) AS UNSIGNED) ASC")->result_array();

    $dataCheklist = $this->db->query("SELECT * FROM ceklis_gudang_hasil WHERE periode = '$periode'")->result_array();

    $result = array("saldoAwal" => $saldoAwal,
                    "saldoPerPeriode" => $totalPerPeriode,
                    "dataMaster" => $dataMaster,
                    "dataCheklist" => $dataCheklist);
    return json_encode($result);
  }
  ...
  public function selectCekKartuStokSort($param){
    ...
    $tglAwalPlus1 = date("Y-m-d",strtotime("+1 days",strtotime($param["tglAwal"])));
    $periode = date("Y-m",strtotime($param["tglAkhir"]));
    $saldo = $this->db->query("SELECT GH.stok_berat, GH.stok_lembar, GH.kd_gd_hasil,
                                      CONCAT(GH.ukuran,' ',GH.merek,' ',GH.warna_plastik) AS nm_barang,
                                      (
                                       SUM(IF(TGH.status_history='MASUK' AND TGH.tgl_transaksi <= '$param[tglAwal]',TGH.jumlah_berat,0))-
                                       SUM(IF(TGH.status_history='KELUAR' AND TGH.tgl_transaksi <= '$param[tglAwal]',TGH.jumlah_berat,0))
                                      ) AS saldoAwalBerat,
  ...
  public function selectCekKartuStokCat($param){
    $having = "";
    $flags = "";
    if(!empty($param["havingStokMaster"])){
      $flags .= "1";
    ...
    if(!empty($flags)){
      ...
      for ($i=0; $i <count($arrFlags) ; $i++) {
        switch ($arrFlags[$i]) {
          case '1': $having .= " GB.stok ".$param["havingStokMaster"];break;
          case '2': $having .= " saldoAwal ".$param["havingStokAwal"];break;
    ...
    $saldoAwal = $this->db->query("SELECT (
                                  ...
                                  (
                                    SUM(IF(TGB.status_history='MASUK' AND TGB.tgl_permintaan BETWEEN '$tglAwalPlus1' AND '$param[tglAkhir]', TGB.jumlah_permintaan, 0))
                                  ) AS beratMasukPerPeriode,

                                  (
                                    SUM(IF(TGB.status_history='KELUAR' AND TGB.tgl_permintaan BETWEEN '$tglAwalPlus1' AND '$param[tglAkhir]', TGB.jumlah_permintaan, 0))
                                  ) AS beratKeluarPerPeriode,
                                  ...
                                  CONCAT(GB.nm_barang, ' ', GB.warna) AS nm_barang, GB.kd_gd_bahan, GB.stok, GB.ceklist
                                  FROM transaksi_gudang_bahan TGB
                                  INNER JOIN gudang_bahan GB ON TGB.kd_gd_bahan = GB.kd_gd_bahan
                                  WHERE TGB.tgl_permintaan BETWEEN '2015-01-01' AND '$param[tglAkhir]'
                                  AND GB.jenis = '$param[jenis]'
                                  AND TGB.deleted='FALSE'
                                  AND TGB.status = 'FINISH'
                                  AND GB.deleted='FALSE'
                                  GROUP BY GB.kd_gd_bahan $having")->result_array();

    $dataCheklist = $this->db->query("SELECT * FROM ceklis_gudang_bahan WHERE periode = '$periode'")->result_array();
  ...
  public function selectCekKartuStokSablonPolos($param){
    $QBarangMaster = "SELECT CONCAT(ukuran,' ',merek,' ',warna_plastik) AS nm_barang,
                             stok_berat,
                             stok_lembar,
                             kd_gd_hasil,
                             ceklist
    ...
    $periode = date("Y-m",strtotime($param["tglAkhir"]));
    $dataCheklist = $this->db->query("SELECT * FROM ceklis_gudang_hasil WHERE periode = '$periode'")->result_array();
    $arrData = array("saldoAwal"       => array(),
                     "saldoPerPeriode" => array(),
                     "dataMaster"      => array(),
                     "dataCheklist" => $dataCheklist);
    $arrBarangMaster = $this->db->query($QBarangMaster)->result_array();
    foreach ($arrBarangMaster as $value) {
      $saldoAwal = $this->db->query("SELECT (
                                              (
                                                SUM(IF((keterangan_history != 'PENGAMBILAN SABLON (SABLON)'
                                                        AND keterangan_history NOT LIKE '%PENGEMBALIAN BARANG%')
                                                        AND kd_gd_hasil_secondary='$value[kd_gd_hasil]'
                                                        AND tgl_transaksi <= '$param[tglAwal]',jumlah_berat,0))-
                                                SUM(IF((keterangan_history LIKE '%PENGEMBALIAN BARANG%'
                                                        OR keterangan_history = 'PENGAMBILAN SABLON (SABLON)')
                                                        AND kd_gd_hasil_secondary = '$value[kd_gd_hasil]'
                                                ...
                                                SUM(IF(kd_gd_hasil = '$value[kd_gd_hasil]' AND
                                                       keterangan_history='DATA AWAl', jumlah_berat, 0)) +
                                              (
                                                SUM(IF(kd_gd_hasil = '$value[kd_gd_hasil]' AND
                                                       keterangan_history='KOREKSI BARANG (PLUS)' AND
                                                       tgl_transaksi <= '$param[tglAwal]', jumlah_berat, 0)) -
                                                SUM(IF(kd_gd_hasil = '$value[kd_gd_hasil]' AND
                                                       keterangan_history='KOREKSI BARANG (MINUS)' AND
                                                       tgl_transaksi <= '$param[tglAwal]', jumlah_berat, 0))
                                            ...
                                            IF(kd_gd_hasil NOT LIKE '%HSLP%',kd_gd_hasil_secondary, kd_gd_hasil) AS kd_gd_hasil
                                     FROM transaksi_gudang_hasil
                                     WHERE (kd_gd_hasil_secondary = '$value[kd_gd_hasil]' OR kd_gd_hasil = '$value[kd_gd_hasil]')
                                     AND tgl_transaksi BETWEEN '2015-01-01' AND '$param[tglAkhir]'
                                     AND deleted='FALSE'
                                     AND status_transaksi = 'FINISH'")->result_array();

      $totalPerPeriode = $this->db->query("SELECT SUM(IF((keterangan_history != 'PENGAMBILAN SABLON (SABLON)'
                                                          AND kd_gd_hasil_secondary='$value[kd_gd_hasil]') OR
                                                         (keterangan_history IN ('KOREKSI BARANG (PLUS)')
                                                          AND kd_gd_hasil='$value[kd_gd_hasil]'),jumlah_berat,0)) AS total_masuk_berat_per_periode,
                                                  ...
                                                  SUM(IF((keterangan_history != 'PENGAMBILAN SABLON (SABLON)'
                                                          AND kd_gd_hasil_secondary='$value[kd_gd_hasil]') OR
                                                         (keterangan_history IN ('KOREKSI BARANG (PLUS)')
                                                          AND kd_gd_hasil='$value[kd_gd_hasil]'),jumlah_lembar,0)) AS total_masuk_lembar_per_periode,
                                           ...
                                           AND status_transaksi = 'FINISH'
                                           AND keterangan_history !='DATA AWAL'")->result_array();
        array_push($arrData["saldoAwal"], array("saldo_awal_berat" => $saldoAwal[0]["saldo_awal_berat"],
                                                "saldo_awal_lembar" => $saldoAwal[0]["saldo_awal_lembar"],
                                                "kd_gd_hasil" => $saldoAwal[0]["kd_gd_hasil"]));

        array_push($arrData["saldoPerPeriode"], array("total_masuk_berat_per_periode" => $totalPerPeriode[0]["total_masuk_berat_per_periode"],
                                                      "total_masuk_lembar_per_periode" => $totalPerPeriode[0]["total_masuk_lembar_per_periode"],
                                                      "total_keluar_berat_per_periode" => $totalPerPeriode[0]["total_keluar_berat_per_periode"],
                                                      "total_keluar_lembar_per_periode" => $totalPerPeriode[0]["total_keluar_lembar_per_periode"],
                                                      "kd_gd_hasil" => $totalPerPeriode[0]["kd_gd_hasil"]));

        array_push($arrData["dataMaster"], array("nm_barang" => $value["nm_barang"],
                                                 "stok_berat" => $value["stok_berat"],
                                                 "stok_lembar" => $value["stok_lembar"],
                                                 "kd_gd_hasil" => $value["kd_gd_hasil"],
                                                 "ceklist" => $value["ceklist"]));

    }
    return json_encode($arrData);
  }

  public function selectCekKartuStokSortSablonPolos($param){
    $QBarangMaster = "SELECT CONCAT(ukuran,' ',merek,' ',warna_plastik) AS nm_barang,
                             stok_berat,
                             stok_lembar,
                             kd_gd_hasil
    ...
    foreach ($arrBarangMaster as $value) {
      ...
      $totalPerPeriode = $this->db->query("SELECT SUM(IF((keterangan_history != 'PENGAMBILAN SABLON (SABLON)'
        ...
        $saldoAwalBerat = ($saldoAwal[0]["saldo_awal_berat"] == null ? 0 : $saldoAwal[0]["saldo_awal_berat"]);
        $saldoAwalLembar = ($saldoAwal[0]["saldo_awal_lembar"] == null ? 0 : $saldoAwal[0]["saldo_awal_lembar"]);
        $totalBeratMasukPerPeriode = ($totalPerPeriode[0]["total_masuk_berat_per_periode"] == null ? 0 : $totalPerPeriode[0]["total_masuk_berat_per_periode"]);
        $totalLembarMasukPerPeriode = ($totalPerPeriode[0]["total_masuk_lembar_per_periode"] == null ? 0 : $totalPerPeriode[0]["total_masuk_lembar_per_periode"]);
        $totalBeratKeluarPerPeriode = ($totalPerPeriode[0]["total_keluar_berat_per_periode"] == null ? 0 : $totalPerPeriode[0]["total_keluar_berat_per_periode"]);
        $totalLembarKeluarPerPeriode = ($totalPerPeriode[0]["total_keluar_lembar_per_periode"] == null ? 0 : $totalPerPeriode[0]["total_keluar_lembar_per_periode"]);
        $saldoAkhirBerat = $saldoAwalBerat + $totalBeratMasukPerPeriode - $totalBeratKeluarPerPeriode;
        $saldoAkhirLembar = $saldoAwalLembar + $totalLembarMasukPerPeriode - $totalLembarKeluarPerPeriode;
        $saldoMasterBerat = $value["stok_berat"];

        if(!empty($param["havingStokAkhir"])){
          if($param["havingStokAkhir"] == "> 0"){
            if($saldoAkhirBerat > 0){
              array_push($arrData["saldo"], array("saldoAwalBerat" => $saldoAwalBerat,
                                                  "saldoAwalLembar" => $saldoAwalLembar,
                                                  "totalBeratMasukPerPeriode" => $totalBeratMasukPerPeriode,
                                                  "totalLembarMasukPerPeriode" => $totalLembarMasukPerPeriode,
                                                  "totalBeratKeluarPerPeriode" => $totalBeratKeluarPerPeriode,
                                                  "totalLembarKeluarPerPeriode" => $totalLembarKeluarPerPeriode,
                                                  "nm_barang" => $value["nm_barang"],
                                                  "stok_berat" => $value["stok_berat"],
                                                  "stok_lembar" => $value["stok_lembar"],
                                                  "kd_gd_hasil" => $value["kd_gd_hasil"],
                                                  "saldoAkhirBerat" => $saldoAkhirBerat,
                                                  "saldoAkhirLembar" => $saldoAkhirLembar));
            }
          }else if($param["havingStokAkhir"] == "< 0"){
            if($saldoAkhirBerat < 0){
              array_push($arrData["saldo"], array("saldoAwalBerat" => $saldoAwalBerat,
                                                  "saldoAwalLembar" => $saldoAwalLembar,
                                                  "totalBeratMasukPerPeriode" => $totalBeratMasukPerPeriode,
        ...
        }else if(!empty($param["havingStokAwal"])){
          if($param["havingStokAwal"] == "> 0"){
            if($saldoAwalBerat > 0){
              array_push($arrData["saldo"], array("saldoAwalBerat" => $saldoAwalBerat,
                                                  "saldoAwalLembar" => $saldoAwalLembar,
                                                  "totalBeratMasukPerPeriode" => $totalBeratMasukPerPeriode,
          ...
          }else if($param["havingStokAwal"] == "< 0"){
            if($saldoAwalBerat < 0){
              array_push($arrData["saldo"], array("saldoAwalBerat" => $saldoAwalBerat,
                                                  "saldoAwalLembar" => $saldoAwalLembar,
                                                  "totalBeratMasukPerPeriode" => $totalBeratMasukPerPeriode,
        ...
        }else if(!empty($param["havingStokMaster"])){
          if($param["havingStokMaster"] == "> 0"){
            if($saldoMasterBerat > 0){
              array_push($arrData["saldo"], array("saldoAwalBerat" => $saldoAwalBerat,
                                                  "saldoAwalLembar" => $saldoAwalLembar,
                                                  "totalBeratMasukPerPeriode" => $totalBeratMasukPerPeriode,
                                                  "totalLembarMasukPerPeriode" => $totalLembarMasukPerPeriode,
          ...
          }else if($param["havingStokMaster"] == "< 0"){
            if($saldoMasterBerat < 0){
              array_push($arrData["saldo"], array("saldoAwalBerat" => $saldoAwalBerat,
                                                  "saldoAwalLembar" => $saldoAwalLembar,
                                                  "totalBeratMasukPerPeriode" => $totalBeratMasukPerPeriode,
        ...
        }else if(!empty($param["havingStokAwal"]) && !empty($param["havingStokAkhir"])){

        }else if(!empty($param["havingStokAwal"]) && !empty($param["havingStokMaster"])) {

        }else if(!empty($param["havingStokAkhir"]) && !empty($param["havingStokMaster"])) {
  ...
  public function selectDetailPermintaanPlastikHdBaru($param){
    $this->datatables->select("GH.ukuran, GH.warna_plastik, GH.tebal, GH.merek,
                               TDPP.jumlah_berat_permintaan, TDPP.jumlah_lembar_permintaan,
                               (TDPP.jumlah_berat_permintaan - TDPP.jumlah_berat_terima) AS sisaBeratPermintaan,
                               (TDPP.jumlah_lembar_permintaan - TDPP.jumlah_lembar_terima) AS sisaLembarPermintaan,
                               TDPP.keterangan, TDPP.status_permintaan, TDPP.id_dpp, TPP.tgl_permintaan,
                               TDPP.kd_permintaan_plastik, TDPP.keterangan_purchasing");
    $this->datatables->from("transaksi_detail_permintaan_plastik_hd TDPP");
    $this->datatables->join("gudang_hasil GH","TDPP.kd_gd_hasil = GH.kd_gd_hasil","INNER");
    $this->datatables->join("transaksi_permintaan_plastik_hd TPP","TDPP.kd_permintaan_plastik = TPP.kd_permintaan_plastik","INNER");
    if(empty($param["idPermintaan"])){
      $this->datatables->where("TPP.id_user = '$param[idUser]' AND
                                TPP.bagian = '$param[bagian]' AND
                                TDPP.deleted='FALSE' AND
                                TDPP.status_permintaan NOT IN('FINISH') AND
                                GH.deleted='FALSE'");
    }else{
      $this->datatables->where("TPP.id_user = '$param[idUser]' AND
                                TPP.bagian = '$param[bagian]' AND
                                TPP.kd_permintaan_plastik = '$param[idPermintaan]' AND
                                TDPP.deleted='FALSE' AND
                                TDPP.status_permintaan NOT IN('FINISH') AND
                                GH.deleted='FALSE'");
    }
  ...
  public function selectPermintaanPlastik_Print($param){
    $Q = "SELECT TPP.kd_permintaan_plastik, TPP.tgl_permintaan, TPP.status_permintaan, USR.username, TPP.bagian
          FROM transaksi_permintaan_plastik_hd TPP
          INNER JOIN users USR ON TPP.id_user = USR.id_user
          WHERE TPP.kd_permintaan_plastik = '$param'";
    $result = $this->db->query($Q)->result_array();
    return $result;
  }

  public function selectDetailPermintaanPlastik_Print($param){
    $Q = "SELECT GH.merek, GH.warna_plastik, GH.ukuran, GH.tebal,
                 TDPP.jumlah_berat_permintaan, TDPP.jumlah_lembar_permintaan,
                 TDPP.keterangan, TDPP.status_permintaan, TDPP.id_dpp,
                 TDPP.kd_permintaan_plastik, TPP.tgl_permintaan
          FROM transaksi_detail_permintaan_plastik_hd TDPP
          INNER JOIN gudang_hasil GH ON TDPP.kd_gd_hasil = GH.kd_gd_hasil
          INNER JOIN transaksi_permintaan_plastik_hd TPP ON TDPP.kd_permintaan_plastik = TPP.kd_permintaan_plastik
          WHERE TPP.kd_permintaan_plastik = '$param' AND
                TDPP.deleted='FALSE' AND
                GH.deleted='FALSE'";
    $result = $this->db->query($Q)->result_array();
    return $result;
  ...
  public function selectDataPermintaanUntukInputTransaksiGudangHasil($param){
    $Q = "SELECT TDPP.kd_gd_hasil,
                 (TDPP.jumlah_berat_permintaan - TDPP.jumlah_berat_terima) AS jumlah_berat_permintaan,
                 (TDPP.jumlah_lembar_permintaan - TDPP.jumlah_lembar_terima) AS jumlah_lembar_permintaan,
                 TPP.bagian, GH.merek, GH.ukuran, GH.warna_plastik, GH.jns_permintaan, GH.jns_brg
          FROM transaksi_detail_permintaan_plastik_hd TDPP
          INNER JOIN gudang_hasil GH ON TDPP.kd_gd_hasil = GH.kd_gd_hasil
          INNER JOIN transaksi_permintaan_plastik_hd TPP ON TDPP.kd_permintaan_plastik = TPP.kd_permintaan_plastik
          WHERE TDPP.id_dpp = '$param'
          AND TPP.deleted='FALSE'
          AND TDPP.deleted='FALSE'";
    $result = $this->db->query($Q)->result_array();
    return $result;
  }

  public function selectCounterKembalianSablon(){
    $Q = "SELECT COUNT(id_permintaan_jadi) AS Counter
          FROM transaksi_gudang_hasil
          WHERE jns_permintaan = 'POLOS'
          AND sts_barang = 'SABLON'
          AND status_transaksi = 'PROGRESS'
          AND keterangan_barang = 'PENGEMBALIAN SABLON'";
    $result = $this->db->query($Q)->result_array();
    $returnResult = array("Counter" => $result[0]["Counter"]);
    return json_encode($returnResult);
  }

  public function selectCounterKembalianSablonCampur(){
    $Q = "SELECT COUNT(id_permintaan_jadi) AS Counter
          FROM transaksi_gudang_hasil
          WHERE jns_permintaan = 'POLOS'
          AND sts_barang = 'CAMPUR'
          AND status_transaksi = 'PROGRESS'
          AND keterangan_barang = 'PENGEMBALIAN SABLON'";
    $result = $this->db->query($Q)->result_array();
    $returnResult = array("Counter" => $result[0]["Counter"]);
  ...
  public function selectListKembalianSablon($param){
    $this->datatables->select("id_permintaan_jadi, kd_gd_hasil, ukuran, jumlah_berat,
                               jumlah_lembar, warna, merek, tgl_transaksi");
    $this->datatables->from("transaksi_gudang_hasil");
    $this->datatables->where("deleted='FALSE' AND
                              jns_permintaan='POLOS' AND
                              status_transaksi = 'PROGRESS' AND
                              keterangan_barang = 'PENGEMBALIAN SABLON' AND
                              sts_approve = 'FALSE' AND
                              sts_barang = '$param'");
    $this->db->order_by("tgl_transaksi DESC");
    return $this->datatables->generate();
  ...
  public function updateTransaksiPengambilanKanvas($param){
    $this->db->trans_begin();
    $this->db->where("id_pengambilan_kanvas",$param["id_pengambilan_kanvas"]);
    $this->db->update("transaksi_pengambilan_kanvas",$param);
    if($this->db->trans_status() === FALSE){
      $this->db->trans_rollback();
      return FALSE;
    }else{
  ...
  public function approveKembalianHDSablon($param){
    $this->db->trans_begin();
    $hasil = $this->db->query("SELECT kd_gd_hasil, jumlah_lembar, jumlah_berat
                               FROM transaksi_gudang_hasil
                               WHERE deleted='FALSE'
                               AND jns_permintaan='POLOS'
                               AND status_transaksi = 'PROGRESS'
                               AND sts_approve = 'FALSE'
                               AND keterangan_barang = 'PENGEMBALIAN SABLON'
                               AND sts_barang = '$param'")->result_array();
    ...
    $this->db->query("UPDATE transaksi_gudang_hasil SET sts_approve = 'TRUE', status_transaksi = 'FINISH'
                      WHERE deleted='FALSE'
                      AND jns_permintaan='POLOS'
                      AND status_transaksi = 'PROGRESS'
                      AND sts_approve = 'FALSE'
  ...
  public function updateDataOrderTerkirim($data){
    ...
    $pesanan = $this->db->query("SELECT a.jumlah_terkirim, a.id_detail_pengiriman,
                                        b.no_order, b.id_dp, b.jumlah, b.jumlah_kirim, a.kd_gd_hasil, a.kd_gd_bahan,
                                        TGH.jumlah_berat, TGH.jumlah_lembar, TGB.jumlah_permintaan
                                 FROM transaksi_detail_pengiriman a
                                 JOIN pesanan_detail b ON a.id_dp = b.id_dp
                                 LEFT JOIN transaksi_gudang_hasil TGH ON TGH.id_detail_pengiriman = a.id_detail_pengiriman
                                 LEFT JOIN transaksi_gudang_bahan TGB ON TGB.id_detail_pengiriman = a.id_detail_pengiriman
    ...
    if(!empty($pesanan[0]["kd_gd_hasil"])){
      $this->db->set("jumlah_berat",$data['jumlah_berat']);
      $this->db->set("jumlah_lembar",$data['jumlah_lembar']);
      $this->db->where("id_detail_pengiriman",$pesanan[0]["id_detail_pengiriman"]);
      $this->db->update("transaksi_gudang_hasil");

      $this->db->set("stok_berat","stok_berat + ".($pesanan[0]["jumlah_berat"] - $data['jumlah_berat']), FALSE);
      $this->db->set("stok_lembar","stok_lembar + ".($pesanan[0]["jumlah_lembar"] - $data['jumlah_lembar']), FALSE);
      $this->db->where("kd_gd_hasil", $pesanan[0]["kd_gd_hasil"]);
      $this->db->update("gudang_hasil");
    }
  ...
  public function deleteDataOrderTerkirim($id){
    ...
    if(!empty($pesanan[0]["kd_gd_hasil"])){
      ...
      $this->db->set("stok_berat","stok_berat + ".$pesanan[0]["jumlah_berat"], FALSE);
      $this->db->set("stok_lembar","stok_lembar + ".$pesanan[0]["jumlah_lembar"], FALSE);
      $this->db->where("kd_gd_hasil",$pesanan[0]["kd_gd_hasil"]);
      $this->db->update("gudang_hasil");
    }

    if(!empty($pesanan[0]["kd_gd_bahan"])){
      $this->db->set("deleted","TRUE");
      $this->db->where("id_detail_pengiriman",$pesanan[0]["id_detail_pengiriman"], FALSE);
      $this->db->update("transaksi_gudang_bahan");

      $this->db->set("stok","stok + ".$pesanan[0]["jumlah_permintaan"], FALSE);
      $this->db->where("kd_gd_bahan",$pesanan[0]["kd_gd_bahan"]);
  ...
  public function updateStatus($id,$status){
    ...
    if ($status=="Open") {
      ...
      $arrNoOrder = $this->db->query("SELECT no_order FROM pesanan_detail WHERE id_dp = '$id'")->result_array();
      $counterJumlahItem = $this->db->query("SELECT COUNT(id_dp) AS counter
                                             FROM pesanan_detail
                                             WHERE deleted='FALSE'
                                             AND no_order='".$arrNoOrder[0]["no_order"]."'")->result_array();

      $counterJumlahItemPacking = $this->db->query("SELECT COUNT(id_dp) AS counter
                                                    FROM pesanan_detail
                                                    WHERE sts_pesanan IN ('PACKING','FINISH')
                                                    AND deleted = 'FALSE'
                                                    AND no_order='".$arrNoOrder[0]["no_order"]."'")->result_array();

      if($counterJumlahItemPacking[0]["counter"] >= $counterJumlahItem[0]["counter"]){
        $this->db->set("sts_pesanan","PACKING");
        $this->db->where("no_order",$arrNoOrder[0]["no_order"]);
        $this->db->update("pesanan");
      }
  ...
  public function updateDeleteAndRestoreDetailPermintaanPlastik($param){
    $this->db->trans_begin();
    $this->db->set("deleted",$param["deleted"]);
    $this->db->where("id_dpp",$param["idTransaksi"]);
    $this->db->update("transaksi_detail_permintaan_plastik_hd");

    $Q = "SELECT COUNT(id_dpp) AS counter
          FROM transaksi_detail_permintaan_plastik_hd
          WHERE kd_permintaan_plastik = '$param[kdPermintaan]'
          AND deleted='TRUE'";

    $Q2 = "SELECT COUNT(id_dpp) AS counter
           FROM transaksi_detail_permintaan_plastik_hd
           WHERE kd_permintaan_plastik = '$param[kdPermintaan]'";
    $arrCounter1 = $this->db->query($Q)->result_array();
    $arrCounter2 = $this->db->query($Q2)->result_array();
    if($arrCounter1[0]["counter"] >= $arrCounter2[0]["counter"]){
      $this->db->set("deleted","TRUE");
      $this->db->where("kd_permintaan_plastik",$param["kdPermintaan"]);
      $this->db->update("transaksi_permintaan_plastik_hd");
    }else{
      $this->db->set("deleted","FALSE");
      $this->db->where("kd_permintaan_plastik",$param["kdPermintaan"]);
      $this->db->update("transaksi_permintaan_plastik_hd");
  ...
  public function updateTerimaPlastikFull($param){
    $this->db->trans_begin();
    $arrJnsBarang = $this->db->query("SELECT jns_brg
                                      FROM gudang_hasil
                                      WHERE kd_gd_hasil = '".$param["TGH"]["kd_gd_hasil"]."'")->result_array();
    $QCheckLock = "SELECT COUNT(TGH.id_permintaan_jadi) AS counter
                   FROM transaksi_gudang_hasil TGH
                   INNER JOIN gudang_hasil GH ON TGH.kd_gd_hasil = GH.kd_gd_hasil
                   WHERE GH.jns_brg='".$arrJnsBarang[0]["jns_brg"]."'
                   AND MONTH(TGH.tgl_transaksi) = '".date("m",strtotime($param["TGH"]["tgl_transaksi"]))."'
                   AND YEAR(TGH.tgl_transaksi) = '".date("Y",strtotime($param["TGH"]["tgl_transaksi"]))."'
    ...
    if($arrCheckLock[0]["counter"] > 0){
      ...
      $this->db->insert("transaksi_gudang_hasil",$param["TGH"]);
      $this->db->insert("transaksi_bukti_penerimaan_plastik_hd",$param["TBPP"]);

      $this->db->set("stok_berat","stok_berat + ".$param["TGH"]["jumlah_berat"], FALSE);
      $this->db->set("stok_lembar","stok_lembar + ".$param["TGH"]["jumlah_lembar"], FALSE);
      $this->db->where("kd_gd_hasil",$param["TGH"]["kd_gd_hasil"]);
      $this->db->update("gudang_hasil");

      $arrDetailPermintaan = $this->db->query("SELECT (jumlah_berat_permintaan - jumlah_berat_terima) AS jumlah_berat_permintaan,
                                                      (jumlah_lembar_permintaan - jumlah_lembar_terima) AS jumlah_lembar_permintaan
                                               FROM transaksi_detail_permintaan_plastik_hd
                                               WHERE id_dpp = '".$param["TDPP"]["id_dpp"]."'")->result_array();
      $this->db->set("status_permintaan","FINISH");
      $this->db->set("jumlah_berat_terima","jumlah_berat_terima + ".$arrDetailPermintaan[0]["jumlah_berat_permintaan"], FALSE);
      $this->db->set("jumlah_lembar_terima","jumlah_lembar_terima + ".$arrDetailPermintaan[0]["jumlah_lembar_permintaan"], FALSE);
      $this->db->where("id_dpp",$param["TDPP"]["id_dpp"]);
      $this->db->update("transaksi_detail_permintaan_plastik_hd");

      $QCheckTotalItemTDPB = "SELECT COUNT(id_dpp) AS counter
                              FROM transaksi_detail_permintaan_plastik_hd
                              WHERE kd_permintaan_plastik = '".$param["TDPP"]["kd_permintaan_plastik"]."'
                              AND deleted='FALSE'
                              AND status_permintaan != 'CANCEL'";

      $QCheckTotalItemFinishTDPB = "SELECT COUNT(id_dpp) AS counter
                                    FROM transaksi_detail_permintaan_plastik_hd
                                    WHERE kd_permintaan_plastik = '".$param["TDPP"]["kd_permintaan_plastik"]."'
                                    AND deleted='FALSE'
                                    AND status_permintaan = 'FINISH'";
      ...
      if($arrTotalItemFinishTDPB[0]["counter"] >= $arrTotalItemTDPB[0]["counter"]){
        $this->db->set("status_permintaan","FINISH");
        $this->db->where("kd_permintaan_plastik",$param["TDPP"]["kd_permintaan_plastik"]);
        $this->db->update("transaksi_permintaan_plastik_hd");
      }
  ...
  public function updateTerimaPlastikSetengah($param){
    $this->db->trans_begin();
    $arrJnsBarang = $this->db->query("SELECT jns_brg
                                      FROM gudang_hasil
                                      WHERE kd_gd_hasil = '".$param["TGH"]["kd_gd_hasil"]."'")->result_array();
    ...
    if($arrCheckLock[0]["counter"] > 0){
      ...
      $this->db->set("jumlah_berat_terima","jumlah_berat_terima + ".$param["TBPP"]["jumlah_berat_terima"], FALSE);
      $this->db->set("jumlah_lembar_terima","jumlah_lembar_terima + ".$param["TBPP"]["jumlah_lembar_terima"], FALSE);
      $this->db->where("id_dpp",$param["TBPP"]["id_dpp"]);
      $this->db->update("transaksi_detail_permintaan_plastik_hd");

      $QCheckJumlahPermintaan = "SELECT IF(jumlah_berat_terima >= jumlah_berat_permintaan AND
                                           jumlah_lembar_terima >= jumlah_lembar_permintaan,'TRUE','FALSE') AS indikator
                                 FROM transaksi_detail_permintaan_plastik_hd
                                 WHERE id_dpp = '".$param["TDPP"]["id_dpp"]."'";
      $arrCheckJumlahPermintaan = $this->db->query($QCheckJumlahPermintaan)->result_array();
  ...
  public function updateSelesaiPermintaanPlastik($param){
    $this->db->trans_begin();
    $this->db->set("status_permintaan","FINISH");
    $this->db->where("id_dpp",$param["id_dpp"]);
    $this->db->update("transaksi_detail_permintaan_plastik_hd");

    $QCheckTotalItemTDPB = "SELECT COUNT(id_dpp) AS counter
                            FROM transaksi_detail_permintaan_plastik_hd
                            WHERE kd_permintaan_plastik = '$param[kd_permintaan]'
                            AND deleted='FALSE'
                            AND status_permintaan != 'CANCEL'";
    ...
    if($arrTotalItemFinishTDPB[0]["counter"] >= $arrTotalItemTDPB[0]["counter"]){
      $this->db->set("status_permintaan","FINISH");
      $this->db->where("kd_permintaan_plastik",$param["kd_permintaan"]);
      $this->db->update("transaksi_permintaan_plastik_hd");
    }
  ...
  public function updateStokMasterBatch($param){
    $this->db->trans_begin();
    for ($i=0; $i < count($param); $i++) {
      $this->db->where("kd_gd_hasil",$param[$i]["kd_gd_hasil"]);
      $this->db->update("gudang_hasil",$param[$i]);
    }
    if($this->db->trans_status() === FALSE){
      $this->db->trans_rollback();
      return FALSE;
  ...
  public function updateCheckList($param){
    $this->db->trans_begin();
    if(empty($param["id_ceklis_gh"])){
      $this->db->insert("ceklis_gudang_hasil",$param);
    }else{
      $this->db->set("ceklis",$param["ceklis"]);
      $this->db->set("id_user",$param["id_user"]);
      $this->db->where("id_ceklis_gh", $param["id_ceklis_gh"]);
      $this->db->update("ceklis_gudang_hasil");
    }
    if($this->db->trans_status() === FALSE){
      $this->db->trans_rollback();
      return "Gagal";
  ...
  public function updateCheckListBahan($param){
    $this->db->trans_begin();
    if(empty($param["id_ceklis_gb"])){
      $this->db->insert("ceklis_gudang_bahan",$param);
    }else{
      $this->db->set("ceklis",$param["ceklis"]);
      $this->db->set("id_user",$param["id_user"]);
      $this->db->where("id_ceklis_gb", $param["id_ceklis_gb"]);
      $this->db->update("ceklis_gudang_bahan");
    }
    if($this->db->trans_status() === FALSE){
      $this->db->trans_rollback();
      return "Gagal";
  ...
  public function updateDeleteTransaksiPengambilan($param){
    $this->db->trans_begin();

    $this->db->set("deleted",$param["deleted"]);
    $this->db->where("id_permintaan_jadi", $param["idTransaksi"]);
    $this->db->update("transaksi_gudang_hasil");

    $Q = "SELECT * FROM transaksi_gudang_hasil WHERE id_permintaan_jadi='$param[idTransaksi]'";
    $arrQ = $this->db->query($Q)->result_array();
    foreach ($arrQ as $arrValue) {
      if(substr($arrValue["kd_gd_hasil_secondary"], 0, 4) == "HSLP"){
        $parameter = array("tglAwal" => $param["tglAwal"],
                           "tglAkhir" => $param["tglAkhir"],
                           "kdGdHasil" => $arrValue["kd_gd_hasil_secondary"]);

        $arrSaldoAkhir = json_decode($this->selectSaldoAwalBulanGudangHasilBuffer($parameter),TRUE);

        $arrData = array("stok_berat" => str_replace(",","",$arrSaldoAkhir["saldoAkhirBerat"]),
                         "stok_lembar" => str_replace(",","",$arrSaldoAkhir["saldoAkhirLembar"]),
                         "kd_gd_hasil" => $arrValue["kd_gd_hasil_secondary"]);
      }else{
        $parameter = array("tglAwal" => $param["tglAwal"],
                           "tglAkhir" => $param["tglAkhir"],
                           "kdGdHasil" => $arrValue["kd_gd_hasil_secondary"]);

        $arrSaldoAkhir = json_decode($this->selectSaldoAwalBulanGudangHasilKanvas($parameter),TRUE);
    ...
    $parameter2 = array("tglAwal" => $param["tglAwal"],
                        "tglAkhir" => $param["tglAkhir"],
                        "kdGdHasil" => $param["kdGdHasil"]);

    $arrSaldo = json_decode($this->Gudang_Hasil_Model->selectSaldoAwalBulanGudangHasil($parameter2),TRUE);

    $arrData2 = array("stok_berat" => str_replace(",","",$arrSaldo["saldoAkhirBerat"]),
                     "stok_lembar" => str_replace(",","",$arrSaldo["saldoAkhirLembar"]),
                     "kd_gd_hasil" => $param["kdGdHasil"]);

    $this->db->where("kd_gd_hasil",$arrData["kd_gd_hasil"]);
    $this->db->update("gudang_hasil",$arrData);

    $this->db->where("kd_gd_hasil",$arrData2["kd_gd_hasil"]);
    $this->db->update("gudang_hasil",$arrData2);
