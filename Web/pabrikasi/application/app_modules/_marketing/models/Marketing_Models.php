...
#===========================================GENERATE CODE METHOD START===========================================
  public function getMaxCode(){
    $maxCode = $this->db->query("SELECT MAX(SUBSTRING(kd_cust,7,6)) AS kode FROM customer WHERE SUBSTRING(kd_cust,5,2) = DATE_FORMAT(NOW(),'%y')");
  ...
  public function getMaxCodeBarang(){
    $maxCode = $this->db->query("SELECT MAX(RIGHT(kd_produk,4)) AS kode FROM produk_aneka_macam");
    foreach ($maxCode->result() as $arrMaxCode) {
      if($arrMaxCode->kode == NULL){
        $tempCode = "0000";
      }
      $tempCode = "0000".(intval($arrMaxCode->kode)+1);
      $fixCode = "PACK".date("ymd").substr($tempCode,(strlen($tempCode)-4));
    }
    return $fixCode;
  }
...
#===========================================SELECT METHOD START===========================================
  ...
  public function selectAllListItemAneka($param){
    if(!empty($param)){
      $this->db->select('gudang_hasil.*,produk_aneka_macam_item.jumlah, produk_aneka_macam_item.merek as merek_aneka, produk_aneka_macam_item.dll, produk_aneka_macam_item.strip,
                        produk_aneka_macam_item.warna_cetak, produk_aneka_macam_item.omset_lembar, produk_aneka_macam_item.omset_kg ');
  ...
  public function selectTotalOmset($kd_pack){
    if(!empty($kd_pack)){
      $this->db->select("SUM(omset_kg) as total_omset_kg, SUM(omset_lembar) as total_omset_lembar");
      $result = $this->db->get_where("produk_aneka_macam_item", ['kd_produk'=>$kd_pack])->result_array();
      return $result;
    } else {
      return false;
    }
  ...
  public function selectFakturPesananDetail($param){
    $arrFakturPesananDetail = $this->db->query("SELECT FORMAT(PD.jumlah,1) AS jumlah,PD.satuan, AM.nama_produk as namaProduk,
                                                       GH.ukuran,PD.merek,PD.harga,
                                                       IF(GH.merek IS NULL, GB.nm_barang, GH.merek) AS merekProduk,
  ...
  public function selectFakturPesananDetailProduksi($param){
    $arrFakturPesananDetail = $this->db->query("SELECT FORMAT(PD.jumlah,1) AS jumlah,PD.satuan,
                                                       ...
                                                       GH.warna_plastik,PD.warna_cetak,PD.sm,PD.dll, AM.nama_produk as namaProduk
                                                FROM pesanan_detail PD
                                                LEFT JOIN produk_aneka_macam AM on PD.kd_pack = AM.kd_produk
                                                LEFT JOIN gudang_hasil GH ON PD.kd_gd_hasil = GH.kd_gd_hasil
                                                LEFT JOIN gudang_bahan GB ON PD.kd_gd_bahan = GB.kd_gd_bahan
  ...
  public function selectStatistikCustomerGlobalJson(){
    $this->datatables->select("C.kd_cust,IF(C.nm_perusahaan_update IS NULL,C.nm_perusahaan,C.nm_perusahaan_update) AS nm_perusahaan,
                               SUM(PD.omset_kg) AS KG,
                               SUM(PD.omset_lembar) AS LEMBAR,
  ...
  public function selectHistoryOrderJsonLama($param){
    ...
    $this->datatables->select('DATE_FORMAT(P.tgl_pesan,"%d %M %Y") AS tgl_pesan,DATE_FORMAT(P.tgl_estimasi,"%d %M %Y") AS tgl_estimasi,
                               ...
                               PD.dll, PD.kd_harga, P.no_order, PD.omset, PD.omset_per_lembar');
    // $this->datatables->add_column("action","<a href='#' class='btn btn-xs btn-info print-out' title='Print Out' data-toggle='modal' data-target='#print-out'><i class='fa fa-print'></i></a>","C.nm_perusahaan");
  ...
  public function selectCustomerListJson(){
    $this->datatables->select('C.kd_cust,
                               ...
                               IF(TIMESTAMPDIFF(YEAR,MAX(P.tgl_pesan),NOW()) >= 1 OR
                                  TIMESTAMPDIFF(YEAR,MAX(P.tgl_pesan),NOW()) IS NULL,"TIDAK_AKTIF","AKTIF") AS status_customer');
  ...
  public function selectLihatPesananDetails($param){
    $this->datatables->select("DP.id_dp, DP.jumlah, DP.satuan,
                               DP.mata_uang,DP.satuan,(DP.jumlah - DP.jumlah_kirim) AS sisa,
                               FORMAT(DP.harga,0) AS harga, IF(DP.kd_pack IS NULL, DP.merek, AM.nama_produk) as merek, DP.mata_uang,
                               GH.ukuran,GH.warna_plastik,
                               DP.warna_cetak, DP.sm, DP.dll, DP.kd_hrg,DP.sts_pesanan,DP.sts_kirim,
                               IF(DP.kd_pack IS NULL, DP.omset_kg, (DP.jumlah * PAMI.omset_kg)) AS omset_kg,
                               IF(DP.kd_pack IS NULL, DP.omset_lembar, (DP.jumlah * PAMI.omset_lembar)) AS omset_lembar");
    $this->datatables->from("pesanan_detail DP");
    $this->datatables->where("(DP.kd_gd_hasil = GH.kd_gd_hasil OR
                              DP.kd_gd_bahan = DP.kd_gd_bahan OR
                              DP.kd_pack = DP.kd_pack) AND
                              DP.no_order = '$param'");
    $this->datatables->join("produk_aneka_macam AM", "DP.kd_pack = AM.kd_produk", "LEFT");
    $this->datatables->join("produk_aneka_macam_item PAMI", "PAMI.kd_produk = AM.kd_produk", "LEFT");
  ...
  public function selectDetailStatistikCustomerJson($param){
    $this->datatables->select("PD.no_order,PD.kd_gd_hasil,PD.kd_gd_bahan,PD.omset_kg,PD.omset_lembar,P.tgl_pesan,
  ...
  public function selectPesananTrash(){
    $this->datatables->select("P.no_order,
                               IF(C.nm_perusahaan_update IS NULL, C.nm_perusahaan, C.nm_perusahaan_update) AS nm_perusahaan,
                               P.nm_pemesan,P.diperbarui");
    $this->datatables->from("pesanan P");
    $this->datatables->join("customer C","P.kd_cust = C.kd_cust","INNER");
    $this->datatables->where("P.deleted='TRUE'");
    $this->db->order_by("P.diperbarui DESC");
    return $this->datatables->generate();
  }
  ...
  public function selectPencarian(){
    $this->datatables->select('PD.no_order,C.kd_cust,GH.kd_gd_hasil,C.nm_perusahaan,
                               ...
                               P.tgl_pesan,P.sts_pesanan, P.deleted');
    //$this->datatables->add_column("action","<a href='#' class='btn btn-xs btn-info print-out' title='Print Out' data-toggle='modal' data-target='#print-out'><i class='fa fa-print'></i></a>","C.nm_perusahaan");
    $this->datatables->from('pesanan_detail PD');
    $this->datatables->join('pesanan P','PD.no_order = P.no_order','INNER');
    $this->datatables->join('customer C','P.kd_cust = C.kd_cust','INNER');
  ...
  public function selectOrderListJson(){
    ...
    if($this->session->userdata("fabricationStatus") == 1){
      $this->datatables->add_column("action","<a href='#' style='float:left;'  class='btn btn-xs btn-info print-out' title='Print Out'><i class='fa fa-print'></i></a>
  ...
  public function selectClosedOrderListJson(){
    ...
    $this->datatables->add_column("action","<a href='#' class='btn btn-xs btn-info print-out' title='Print Out'><i class='fa fa-print'></i></a>
  ...
  public function selectCountOrderDeadline(){
    ...
    $Q2 = "SELECT (
                    SUM(IF(DATE_FORMAT(tgl_lahir_owner,'%m-%d') = DATE_FORMAT(NOW(),'%m-%d'), 1, 0)) +
                    SUM(IF(DATE_FORMAT(tgl_lahir_purchasing,'%m-%d') = DATE_FORMAT(NOW(),'%m-%d'), 1, 0))
                  ) AS counterBirthday FROM customer
           WHERE deleted='FALSE'";
    $result = $this->db->query($Q)->result_array();
    $result2 = $this->db->query($Q2)->result_array();
    $dataForReturned = array("Counter" => $result[0]["counter"],
                             "CounterBirthday" => $result2[0]["counterBirthday"]);
  ...
  public function getDataOrderPerHari($jenis,$tgl)
  {
    if(empty($tgl)){
      ...
      if ($jenis=="LK") {
        ...
        $this->datatables->where("P.jns_order ='SKV' AND P.kd_order LIKE '%SKV%' AND P.deleted='FALSE' and DATE(P.tgl_pesan)=CURDATE() and P.approve_cabang = 'FALSE'");
      }else if($jenis=="LN"){
        $this->datatables->where("(P.jns_order ='LN' OR P.jns_order='EXPORT') AND P.deleted='FALSE' and DATE(P.tgl_pesan)=CURDATE() and P.approve_cabang = 'FALSE'");
      }
      return $this->datatables->generate();
    }else{
      $this->datatables->select("P.no_order, P.kd_order, P.nm_pemesan,
      ...
      if ($jenis=="LK") {
        ...
        $this->datatables->where("P.jns_order IN ('DK','SKV') AND P.sales IN ('Ibnu','Ian') and P.deleted='FALSE' and P.tgl_pesan='$tgl' and P.approve_cabang = 'FALSE'");
      }else if($jenis=="LN"){
        $this->datatables->where("(P.jns_order ='LN' OR P.jns_order='EXPORT') AND P.deleted='FALSE' and P.tgl_pesan='$tgl' and P.approve_cabang = 'FALSE'");
      }
      return $this->datatables->generate();
    }
  ...
  public function getTotalOrderPerHari($jenis,$tgl)
  {
    if (empty($tgl)) {
      if ($jenis=="CBG") {
        $data = $this->db->query("SELECT FORMAT(SUM(b.jumlah),2) AS jumlah ,
                                         b.satuan
                                  FROM `pesanan` a
                                  INNER JOIN pesanan_detail b ON a.no_order = b.no_order
                                  WHERE DATE(a.tgl_pesan)=CURDATE()
                                  AND a.deleted = 'FALSE'
                                  AND a.approve_cabang = 'TRUE'
                                  AND b.deleted = 'FALSE'
                                  GROUP BY b.satuan")->result_array();
        return json_encode($data);
      }else{
        if($jenis == "LN"){
          $clause = "AND a.jns_order IN ('LN','EXPORT')";
        }else{
          $clause = "AND a.jns_order='$jenis'";
        }
        $data = $this->db->query("SELECT SUM(IF(b.kd_pack IS NOT NULL,c.omset_lembar * b.jumlah,b.omset_lembar)) AS jumlahLembar ,
                                         SUM(IF(b.kd_pack IS NOT NULL,(c.omset_kg * b.jumlah),b.omset_kg)) AS jumlahBerat,
                                         SUM(IF(b.satuan = 'BAL', b.jumlah, 0)) AS jumlahBal
                                  FROM pesanan a
                                  INNER JOIN pesanan_detail b ON a.no_order = b.no_order
                                  LEFT JOIN produk_aneka_macam_item c ON b.kd_pack = c.kd_produk
                                  WHERE DATE(a.tgl_pesan)=CURDATE()
                                  $clause
                                  AND a.deleted = 'FALSE'
                                  AND b.deleted = 'FALSE'
                                  AND a.approve_cabang = 'FALSE'")->result_array();
        return json_encode($data);
      }
    }else{
      if ($jenis=="CBG") {
  ...
  public function getDataChartOrder($tgl_awal,$tgl_akhir,$jenis)
  {
    if ($jenis=="CBG") {
      $data = $this->db->query("SELECT SUM(IF(b.satuan = 'Bal',b.jumlah,0)) AS Bal ,
                                ...
                                AND a.nm_pemesan IN ('MEDAN','GANEFO','JATINEGARA','SURABAYA','BREBES','SEMARANG')
                                AND a.deleted = 'FALSE'
                                AND b.deleted = 'FALSE'
      ...
      $data = $this->db->query("SELECT SUM(IF(b.satuan = 'Bal',b.jumlah,0)) AS Bal ,
                                       SUM(IF(b.kd_pack IS NOT NULL,c.omset_lembar * b.jumlah,b.omset_lembar)) AS Kg ,
                                       SUM(IF(b.kd_pack IS NOT NULL,(c.omset_kg * b.jumlah),b.omset_kg)) AS Lembar,
                                       b.satuan, a.tgl_pesan
                                FROM `pesanan` a
                                JOIN pesanan_detail b ON a.no_order = b.no_order
                                LEFT JOIN produk_aneka_macam_item c ON b.kd_pack = c.kd_produk
                                ...
                                BETWEEN '$tgl_awal' AND '$tgl_akhir'
                                $clause
                                AND a.nm_pemesan NOT IN ('MEDAN','GANEFO','JATINEGARA','SURABAYA','BREBES','SEMARANG')
                                AND a.deleted = 'FALSE'
                                AND b.deleted='FALSE'
  ...
  public function selectDataRincianStatistik($param){
    if($param["jenis"] == "CBG"){
      $this->datatables->select("C.nm_perusahaan, P.tgl_pesan, P.tgl_estimasi, P.sts_pesanan,
                                 SUM(IF(PD.satuan = 'Kg',PD.jumlah,0)) AS omsetKg ,
                                 SUM(IF(PD.satuan = 'Lembar',PD.jumlah,0)) AS omsetLembar ,
                                 SUM(IF(PD.satuan='BAL',PD.jumlah, 0)) AS jumlahBal");
    }else{
      $this->datatables->select("C.nm_perusahaan, P.tgl_pesan, P.tgl_estimasi, P.sts_pesanan,
                                 SUM(IF(PD.kd_pack IS NOT NULL,PAMI.omset_lembar * PD.jumlah,PD.omset_lembar)) AS omsetKg,
                                 SUM(IF(PD.kd_pack IS NOT NULL,(PAMI.omset_kg * PD.jumlah),PD.omset_kg)) AS omsetLembar,
                                 SUM(IF(PD.satuan='BAL',PD.jumlah, 0)) AS jumlahBal");
    }
    ...
    $this->datatables->join("customer C","P.kd_cust = C.kd_cust","INNER");
    $this->datatables->join("pesanan_detail PD","PD.no_order = P.no_order","INNER");
    $this->datatables->join("produk_aneka_macam_item PAMI","PD.kd_pack = PAMI.kd_produk","LEFT");
    if($param["jenis"] == "CBG"){
      $clause = "P.nm_pemesan IN ('MEDAN','GANEFO','JATINEGARA','SURABAYA','BREBES','SEMARANG')";
    }else{
      $clause = "P.nm_pemesan NOT IN ('MEDAN','GANEFO','JATINEGARA','SURABAYA','BREBES','SEMARANG') AND
                 P.jns_order='$param[jenis]'";
    }
    $this->datatables->where("P.deleted='FALSE' AND
                              PD.deleted='FALSE' AND
                              C.deleted='FALSE' AND
                              P.tgl_pesan BETWEEN '$param[tglAwal]' AND '$param[tglAkhir]' AND
                              $clause");
    $this->datatables->group_by("P.kd_cust");

    if(empty($param["jnsQty"]) || empty($param["sortir"])){
      $this->db->order_by("omsetKg","DESC");
    }else{
      if($param["sortir"] == "ASC"){
        switch ($param["jnsQty"]) {
          case 'KG' : $this->db->order_by("omsetKg","ASC");
                      break;
          case 'LEMBAR' : $this->db->order_by("omsetLembar","ASC");
                      break;
          case 'BAL' : $this->db->order_by("jumlahBal","ASC");
                      break;

          default : $this->db->order_by("omsetKg","DESC");
                    break;
        }
      }else{
        switch ($param["jnsQty"]) {
          case 'KG' : $this->db->order_by("omsetKg","DESC");
                      break;
          case 'LEMBAR' : $this->db->order_by("omsetLembar","DESC");
                      break;
          case 'BAL' : $this->db->order_by("jumlahBal","DESC");
                      break;
  ...
  public function selectKodeSales($param){
    $Q = "SELECT COUNT(*) AS counter FROM data_sales WHERE kode_sales = '$param' AND deleted='FALSE'";
    $result = $this->db->query($Q)->result_array();
    return json_encode($result);
  }
  ...
  public function selectDataSales(){
    ...
    $this->datatables->from("data_sales");
    $this->datatables->where("deleted","FALSE");
    $this->db->order_by("id_sales","DESC");
    return $this->datatables->generate();
  }

  public function selectDetailSales($id){
    $this->db->select("*");
    $this->db->where("id_sales",$id);
    $this->db->where("deleted","FALSE");
    return json_encode($this->db->get("data_sales")->result_array());
  }

  public function selectKodeJenisBarang($param){
    $Q = "SELECT COUNT(*) AS counter FROM jenis_barang WHERE kode_jenis = '$param' AND deleted='FALSE'";
    $result = $this->db->query($Q)->result_array();
    return json_encode($result);
  }

  public function selectDataJenisBarang(){
    $this->datatables->select("*");
  ...
  public function selectDetailJenisBarang($param){
    $this->db->select("*");
    $this->db->where("id_jenis",$param);
    $this->db->where("deleted","FALSE");
    return json_encode($this->db->get("jenis_barang")->result_array());
  }

  public function selectDataJenisBarangComboBox($param){
    if(empty($param)){
      $Q = "SELECT * FROM jenis_barang WHERE deleted='FALSE'";
    }else{
      if(strpos($param,"|") === FALSE){
        $Q = "SELECT * FROM jenis_barang
              WHERE (id_jenis LIKE '%$param%'
              OR kode_jenis LIKE '%$param%'
              OR nama_jenis LIKE '%$param%')
              AND deleted='FALSE'";
      }else{
        $arrKey = explode("|",$param);
        $Q = "SELECT * FROM jenis_barang
              WHERE kode_jenis = '$arrKey[0]'
              AND nama_jenis = '$arrKey[1]'
              AND deleted='FALSE'";
      }
    }
    $result = $this->db->query($Q)->result_array();
  ...
  public function selectDataSalesComboBox($param){
    if(empty($param)){
      $Q = "SELECT * FROM data_sales WHERE deleted='FALSE'";
    }else{
      $Q = "SELECT * FROM data_sales WHERE deleted='FALSE'
            AND (kode_sales LIKE '%$param%' OR
                 nama_lengkap LIKE '%$param%')";
    }
    $result = $this->db->query($Q)->result_array();
    return json_encode($result);
  }

  public function selectListDataOrderSalesPending($param){
    $this->datatables->select("TKS.id_transaksi, TKS.kode_order, TKS.tgl_order,
                               TKS.nama_pelanggan, TKS.jumlah_berat,
                               IF(PAM.kd_produk IS NULL, GH.ukuran, '') AS ukuran,
                               IF(PAM.kd_produk IS NULL, GH.merek, PAM.nama_produk) AS merek,
                               TKS.jumlah_harga, TKS.diskon_persen, TKS.hasil_diskon, TKS.potongan_harga,
                               TKS.lembar_8_persen, TKS.lembar_5_persen, TKS.lembar_2_persen,
                               TKS.lembar_1_persen, TKS.lembar_05_persen, TKS.jenis_harga");
    $this->datatables->from("transaksi_komisi_sales TKS");
    $this->datatables->join("gudang_hasil GH","TKS.kd_gd_hasil = GH.kd_gd_hasil","LEFT");
    $this->datatables->join("produk_aneka_macam PAM","TKS.kd_produk = PAM.kd_produk","LEFT");
    $this->datatables->where("id_sales",$param);
    $this->datatables->where("TKS.deleted='FALSE' AND (GH.deleted='FALSE' OR PAM.deleted='FALSE') AND TKS.status_transaksi='PENDING'");
    $this->db->order_by("TKS.id_transaksi","ASC");
    return $this->datatables->generate();
  }

  public function selectDetailTransaksiKomisiSales($param){
    $this->db->select("*");
    $this->db->from("transaksi_komisi_sales");
    $this->db->where("id_transaksi",$param);
    return json_encode($this->db->get()->result_array());
  }

  public function selectLaporanDataOrderSales($param){
    $this->datatables->select("TKS.id_transaksi, TKS.kode_order, TKS.tgl_order,
                               TKS.nama_pelanggan, TKS.jumlah_berat, GH.ukuran, GH.merek,
                               TKS.jumlah_harga, TKS.diskon_persen, TKS.hasil_diskon, TKS.potongan_harga,
                               TKS.lembar_8_persen, TKS.lembar_5_persen, TKS.lembar_2_persen,
                               TKS.lembar_1_persen, TKS.lembar_05_persen, TKS.jenis_harga");
    $this->datatables->from("transaksi_komisi_sales TKS");
    $this->datatables->join("gudang_hasil GH","TKS.kd_gd_hasil = GH.kd_gd_hasil","INNER");
    $this->datatables->where("id_sales = '$param[idSales]' AND
                              TKS.deleted='FALSE' AND
                              GH.deleted='FALSE' AND
                              TKS.status_transaksi!='PENDING' AND
                              TKS.tgl_order BETWEEN '$param[tglAwal]' AND '$param[tglAkhir]'");
    $this->db->order_by("TKS.id_transaksi","DESC");
    return $this->datatables->generate();
  }

  public function selectLaporanDataOrderSales_2($param){
    $this->db->select("TKS.id_transaksi, TKS.kode_order, TKS.tgl_order,
                       TKS.nama_pelanggan, TKS.jumlah_berat,
                       IF(PAM.kd_produk IS NULL, GH.ukuran, '') AS ukuran,
                       IF(PAM.kd_produk IS NULL, GH.merek, PAM.nama_produk) AS merek,
                       TKS.jumlah_harga, TKS.diskon_persen, TKS.hasil_diskon, TKS.potongan_harga,
                       TKS.lembar_8_persen, TKS.lembar_5_persen, TKS.lembar_2_persen,
                       TKS.lembar_1_persen, TKS.lembar_05_persen, TKS.jenis_harga");
    $this->db->from("transaksi_komisi_sales TKS");
    $this->db->join("gudang_hasil GH","TKS.kd_gd_hasil = GH.kd_gd_hasil","LEFT");
    $this->db->join("produk_aneka_macam PAM","TKS.kd_produk = PAM.kd_produk","LEFT");
    $this->db->where("id_sales = '$param[idSales]' AND
                              TKS.deleted='FALSE' AND
                              (GH.deleted='FALSE' OR PAM.deleted='FALSE') AND
                              TKS.status_transaksi!='PENDING' AND
                              TKS.tgl_order BETWEEN '$param[tglAwal]' AND '$param[tglAkhir]'");
    $this->db->order_by("TKS.id_transaksi","ASC");
  ...
  public function selectTotalLembarPerJenisBarang($param){
    $Q = "SELECT (SUM(TKS.lembar_8_persen) +
                  SUM(TKS.lembar_5_persen) +
                  SUM(TKS.lembar_2_persen) +
                  SUM(TKS.lembar_1_persen) +
                  SUM(TKS.lembar_05_persen)
                ) AS Jumlah, JB.nama_jenis
          FROM transaksi_komisi_sales TKS
          INNER JOIN jenis_barang JB ON TKS.id_jenis = JB.id_jenis
          WHERE TKS.id_sales = '$param[idSales]'
          AND TKS.tgl_order BETWEEN '$param[tglAwal]' AND '$param[tglAkhir]'
          AND TKS.deleted='FALSE'
          AND JB.deleted='FALSE'
          GROUP BY JB.id_jenis";

    $result = $this->db->query($Q)->result_array();
    return json_encode($result);
  }

  public function selectMarkingDataKomisiSales($param){
    $Q = "SELECT COUNT(id_transaksi) AS counter
          FROM transaksi_komisi_sales
          WHERE id_sales = '$param[idSales]'
          AND deleted='FALSE'
          AND status_transaksi='FINISH'
          AND tgl_order BETWEEN '$param[tglAwal]' AND '$param[tglAkhir]'";
    $result = $this->db->query($Q)->result_array();
    if(floatval($result[0]["counter"]) > 0){
      return "TRUE";
    }else{
      return "FALSE";
    }
  ...
  public function selectData($param){
    $this->db->select($param["COLUMN"]);
    $this->db->from($param["TABLE"]);
    if(!empty($param["JOIN"])){
      for ($i=0; $i < count($param["JOIN"]); $i++) {
        $this->db->join($param["JOIN"][$i][0],$param["JOIN"][$i][1],$param["JOIN"][$i][2]);
      }
    }
    if(array_key_exists("ID", $param) && !empty($param["ID"])){
      $this->db->where($param["ID"]["COLUMN"],$param["ID"]["VALUE"]);
    }
    if(array_key_exists("WHERE", $param) && !empty($param["WHERE"])){
      for ($i=0; $i < count($param["WHERE"]); $i++) {
        $this->db->where($param["WHERE"][$i]);
      }
    }
    if(array_key_exists("ORDERBY", $param) && !empty($param["ORDERBY"])){
      $this->db->order_by($param["ORDERBY"]);
    }
    if(array_key_exists("GROUPBY", $param) && !empty($param["GROUPBY"])){
      $this->db->group_by($param["GROUPBY"]);
    }
    if(array_key_exists("LIMIT", $param) && !empty($param["LIMIT"])){
  ...
  public function selectBirthdayListToday(){
    $Q = "SELECT nm_owner AS nm_lengkap, kd_cust, nm_perusahaan, DATE_FORMAT(tgl_lahir_owner, '%d %M %Y') AS tgl_lahir,
                 hp_owner AS no_hp
          FROM customer
          WHERE deleted='FALSE' AND DATE_FORMAT(tgl_lahir_owner, '%m-%d') = DATE_FORMAT(NOW(), '%m-%d')
          UNION
          SELECT nm_purchasing AS nm_lengkap, kd_cust, nm_perusahaan, DATE_FORMAT(tgl_lahir_purchasing, '%d %M %Y') AS tgl_lahir,
                 hp_purchasing AS no_hp
          FROM customer
          WHERE deleted='FALSE' AND DATE_FORMAT(tgl_lahir_purchasing, '%m-%d') = DATE_FORMAT(NOW(), '%m-%d')";
    $result = $this->db->query($Q)->result_array();
...
#===========================================INSERT METHOD START===========================================
  ...
  public function insertPesananFinal($param){
    ...
    if($this->db->trans_status()===FALSE){
      ...
      $this->db->trans_commit();
      $this->db->trans_begin();
      $pesananDetailTemp = $this->db->select("id_dp,no_order,kd_gd_hasil,kd_gd_bahan,kd_pack, merek,jumlah,satuan,harga,mata_uang,warna_cetak,dll,sm,kd_hrg,omset_lembar,omset_kg,potongan,diskon,cn")
  ...
  public function insertDataSales($param){
    $this->db->trans_begin();
    $this->db->insert("data_sales", $param);
    if($this->db->trans_status() === FALSE){
      $this->db->trans_rollback();
      return "Gagal";
    }else{
  ...
  public function insertDataJenisBarang($param){
    $this->db->trans_begin();
    $this->db->insert("jenis_barang",$param);
    if($this->db->trans_status() === FALSE){
      $this->db->trans_rollback();
      return "Gagal";
    }else{
  ...
  public function insertTransaksiKomisiSales($param){
    $this->db->trans_begin();
    $Q = "SELECT COUNT(id_transaksi) AS counter
          FROM transaksi_komisi_sales
          WHERE id_sales='$param[id_sales]'
          AND tgl_order = '$param[tgl_order]'
          AND deleted = 'FALSE'
          AND status_transaksi = 'FINISH'";
    $arrCheckQ = $this->db->query($Q)->result_array();

    if($arrCheckQ[0]["counter"] > 0){
      $this->db->trans_rollback();
      return "Lock";
    }else{
      $this->db->insert("transaksi_komisi_sales",$param);
    }
    if($this->db->trans_status() === FALSE){
      $this->db->trans_rollback();
      return "Gagal";
  ...
  public function insertData($param){
    ...
    if(count($param[$table]) > 1){
      foreach ($arrTable as $table) {
        $this->db->insert_batch($table, $param[$table]);
      }
    }else{
      foreach ($arrTable as $table) {
        $this->db->insert($table, $param[$table]);
      }
    }
    if($this->db->trans_status() === FALSE){
      $this->db->trans_rollback();
...
#===========================================UPDATE METHOD START===========================================
  ...
  public function updateDataSales($param){
    $this->db->trans_begin();
    $this->db->where("id_sales",$param["id_sales"]);
    $this->db->update("data_sales", $param);
    if($this->db->trans_status() === FALSE){
      $this->db->trans_rollback();
      return "Gagal";
    }else{
  ...
  public function updateDataJenisBarang($param){
    $this->db->trans_begin();
    $this->db->where("id_jenis",$param["id_jenis"]);
    $this->db->update("jenis_barang", $param);
    if($this->db->trans_status() === FALSE){
      $this->db->trans_rollback();
      return "Gagal";
    }else{
  ...
  public function updateTransaksiKomisiSales_Check($param){
    $this->db->trans_begin();
    $this->db->set("status_transaksi",$param["status_transaksi"]);
    $this->db->where("id_sales",$param["id_sales"]);
    $this->db->update("transaksi_komisi_sales");
    if($this->db->trans_status() === FALSE){
      $this->db->trans_rollback();
      return "Gagal";
    }else{
  ...
  public function updateTransaksiKomisiSales($param){
    $this->db->trans_begin();
    $this->db->where("id_transaksi",$param["id_transaksi"]);
    $this->db->update("transaksi_komisi_sales",$param);
    if($this->db->trans_status() === FALSE){
      $this->db->trans_rollback();
      return "Gagal";
    }else{
  ...
  public function updateApproveLaporanTransaksiKomisiSales($param){
    $this->db->trans_begin();
    $Q = "UPDATE transaksi_komisi_sales SET status_transaksi='$param[statusTransaksi]'
          WHERE id_sales = '$param[idSales]'
          AND tgl_order BETWEEN '$param[tglAwal]' AND '$param[tglAkhir]'
          AND deleted='FALSE'";
    $this->db->query($Q);
  ...
  public function selesaiTambahBarangAneka($param){
    $this->db->trans_begin();
    $this->db->update('produk_aneka_macam', ['status_produk'=>'FINISH'], ['kd_produk'=>$param]);
    $this->db->update('produk_aneka_macam_item', ['status_pembuatan'=>'FINISH'], ['kd_produk'=>$param]);
    if($this->db->trans_status() === FALSE){
      $this->db->trans_rollback();
      return false;
    } else {
  ...
  public function selesaiEditBarang($data, $where){
    $this->db->trans_begin();
    $this->db->update('produk_aneka_macam', $data, $where);
    if($this->db->trans_status() === FALSE){
  ...
  public function selesaiEditItem($data, $where){
    $this->db->trans_begin();
    $this->db->update('produk_aneka_macam_item', $data, $where);
    if($this->db->trans_status() === FALSE){
      $this->db->trans_rollbakc();
      return FALSE;
    } else {
...
#===========================================DELETE METHOD START===========================================
  ...
  public function deleteAnekaItem($param){
    $this->db->trans_begin();
    $this->db->where("id_detail_produk",$param);
    $this->db->update("produk_aneka_macam_item", ['deleted'=>'TRUE']);
    if($this->db->trans_status() === FALSE){
      $this->db->trans_rollback();
      return FALSE;
    }else{
  ...
  public function hapusBarangAneka($param){
    $this->db->trans_begin();
    $this->db->update('produk_aneka_macam', ['deleted'=>'TRUE'], ['kd_produk'=>$param]);
    $this->db->update('produk_aneka_macam_item', ['deleted'=>'TRUE'], ['kd_produk'=>$param]);
    if($this->db->trans_status() === FALSE){
      $this->db->trans_rollback();
      return FALSE;
    } else {
