...
class Main extends MX_Controller{
    ...
    if($isLogin){
      ...
      if($status==1){
        $listMenu["parentMenu"] = array(
          ...
          array("Content"=>"Komisi","Link"=>"#","Name"=>"Komisi","Status"=>"Parent","Icon"=>"fa fa-book"),
          array("Content"=>"Aneka Macam Plastik","Link"=>"_marketing/main/aneka_macam_plastik","Name"=>"AnekaMacam","Status"=>"Single","Icon"=>"fa fa-cubes"),
        );
        $listMenu["childMenu"] = array(
          ...
          array("Content"=>"Input Data Sales","Link"=>"_marketing/main/input_data_sales","Parent"=>"Komisi"),
          array("Content"=>"Input Jenis Barang","Link"=>"_marketing/main/input_jenis_barang","Parent"=>"Komisi"),
          array("Content"=>"Input Komisi Sales","Link"=>"_marketing/main/input_data_komisi","Parent"=>"Komisi"),
          array("Content"=>"Laporan Komisi Sales","Link"=>"_marketing/main/laporan_data_komisi","Parent"=>"Komisi"),
  ...
  public function input_data_komisi(){
    $isLogin = $this->isLogin();
    if($isLogin){
      $data["Data"] = array("Title"=>"Tambah Data Komisi Sales");
      $data["Link"] = array("Segment1" =>$this->uri->rsegment(1),"Segment2"=>$this->uri->rsegment(2));
      $this->load->view("header");
      $this->load->view("sidebar",$this->checkStatus());
      $this->load->view("frm_input_komisi",$data);
      $this->load->view("footer");
    }else{
         echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
         redirect("_main/main","refresh");
  ...
  public function input_data_sales(){
    $isLogin = $this->isLogin();
    if($isLogin){
      $data["Data"] = array("Title"=>"Tambah Data Sales");
      $data["Link"] = array("Segment1" =>$this->uri->rsegment(1),"Segment2"=>$this->uri->rsegment(2));
      $this->load->view("header");
      $this->load->view("sidebar",$this->checkStatus());
      $this->load->view("frm_input_sales",$data);
      $this->load->view("footer");
    }else{
         echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
         redirect("_main/main","refresh");
  ...
  public function input_jenis_barang(){
    $isLogin = $this->isLogin();
    if($isLogin){
      $data["Data"] = array("Title"=>"Tambah Jenis Barang");
      $data["Link"] = array("Segment1" =>$this->uri->rsegment(1),"Segment2"=>$this->uri->rsegment(2));
      $this->load->view("header");
      $this->load->view("sidebar",$this->checkStatus());
      $this->load->view("frm_input_jenis_barang",$data);
      $this->load->view("footer");
    }else{
         echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
         redirect("_main/main","refresh");
  ...
  public function laporan_data_komisi(){
    $isLogin = $this->isLogin();
    if($isLogin){
      $data["Data"] = array("Title"=>"Laporan Data Komisi");
      $data["Link"] = array("Segment1" =>$this->uri->rsegment(1),"Segment2"=>$this->uri->rsegment(2));
      $this->load->view("header");
      $this->load->view("sidebar",$this->checkStatus());
      $this->load->view("frm_laporan_data_komisi",$data);
      $this->load->view("footer");
    }else{
         echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
         redirect("_main/main","refresh");
  ...
  public function aneka_macam_plastik(){
    $isLogin = $this->isLogin();
    if($isLogin){
      $arrParam = array("COLUMN"  => "*",
                        "TABLE"   => "produk_aneka_macam",
                        "WHERE"   => array("status_produk = 'PROGRESS'"),
                        "ORDERBY" => "kd_produk ASC",
                        "LIMIT"   => 1);

      $data["Data"] = array("Title"=>"Data Barang Aneka Plastik");
      $data["Link"] = array("Segment1" =>$this->uri->rsegment(1),"Segment2"=>$this->uri->rsegment(2));
      $data["KDPRODUK"] = $this->Marketing_Models->getMaxCodeBarang();
      $data["TEMP"] = $this->Marketing_Models->selectData($arrParam);
      $this->load->view("header");
      $this->load->view("sidebar",$this->checkStatus());
      $this->load->view("frm_barang_aneka_plastik",$data);
      $this->load->view("footer");
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
      redirect("_main/main","refresh");
  ...
  public function saveCustomer(){
    ...
    if($isLogin){
      ...
      $jenkelOwner = $this->input->post('rb_jenkel_owner');
      ...
      $tglLahirOwner = $this->input->post('txt_tgl_lhr_owner');
      $tglLahirPurchasing = $this->input->post('txt_tgl_lhr_purchasing');
      ...
      if(empty($kd_cust)||
        ...
        $data = array("kd_cust" => $kd_cust,
                      ...
                      "note"=> $note,
                      "website"=> $web,
                      "tgl_lahir_owner" => $tglLahirOwner,
                      "tgl_lahir_purchasing" => $tglLahirPurchasing,
     ...
     if (empty($no_order) ||
        ...
        if(substr($kd_gudang,0,3)=="BHN"){
          $data = array("no_order"=>$no_order,
                        ...
                        "mata_uang"=>$mata_uang,
                        "cn"=>$cn);
        }elseif(substr($kd_gudang,0,4)=="PACK"){
          $data = array("no_order"=>$no_order,
                        "kd_cust"=>$kd_cust,
                        "kd_pack"=>$kd_gudang,
                        "merek"=>$merek,
    ...
    if($isLogin){
      ...
      foreach ($data as $arrData) {
        ...
        <tr>
          ...
          <td>$arrData[merek]</td>
          <td>$arrData[ukuran]$arrData[nama_produk]</td>
  ...
  public function getPesananDetails($param){
    ...
    if($isLogin){
      ...
      $no = 1;
      foreach ($data as $arrData) {
        if(($arrData["sts_pesanan"]=="PROGRESS" || $arrData["sts_pesanan"]=="PACKING" || $arrData["sts_pesanan"]=="FINISH")&& !empty($arrData["tgl_kirim_gudang"])){
          $disable = "disabled='disabled' title='Silahkan konfirmasi ke PPIC untuk membuka fitur ini'";
          $delete = "";
        }else{
          $disable = "";
          $delete = "deletePesananDetail($arrData[id_dp])";
        }
        $edit = "showModalEditPesananDetail($arrData[id_dp])";
        $url = base_url()."_marketing/main/modifyPesananDetailModal/".$arrData['id_dp'];
        echo "
        <tr>
          <td>$no</td>
          ...
          <td>$arrData[dll]</td>
          <td>
            <a href='#' class='btn btn-sm btn-warning btn-flat' data-toggle='modal' data-target='#edit-modal-pesanan-detail' onclick='$edit'><i class='fa fa-pencil-square-o'></i></a>
            <a href='#' class='btn btn-sm btn-danger btn-flat' onclick='$delete' $disable><i class='fa fa-trash'></i></a>
            </td>
        </tr>";
        $no++;
      }
  ...
  public function cetakFakturPesanan($param){
    ...
    if($isLogin){
      $arrDataPesanan = $this->Marketing_Models->selectFakturPesanan($param);
      $arrDataPesananDetail = $this->Marketing_Models->selectFakturPesananDetail($param);
      $pesananDetail = $this->Marketing_Models->selectPesananDetails($param);
      $arrListItem = $this->Marketing_Models->selectAllListItemAneka($pesananDetail[0]['kd_pack']);
      $result = array("arrDataPesanan" => $arrDataPesanan,
                      "arrDataPesananDetail" => $arrDataPesananDetail,
                      "arrDataListItem"=>$arrListItem,
                      "totalPesanan"=>$pesananDetail[0]['jumlah'],
                      "jumlahPesanan"=>$arrDataPesananDetail[0]['jumlah'],
                      "totalOmset"=>$this->Marketing_Models->selectTotalOmset($pesananDetail[0]['kd_pack'])
                    );
      $css = "assets/bootstrap/css/bootstrap.min.css";
      if($arrListItem){
        $this->load->view("frm_print_faktur_variasi", $result);
      } else {
        $this->load->view("frm_print_faktur",$result);
      }
      // $this->load->library('m_pdf');
      // $this->mpdf->mPDF("utf-8","B5-L",0,"",2,2,5,5,0,0,"P");
  ...
  // public function testFaktur($param){
  //   $isLogin = $this->isLogin();
  //   if($isLogin){
  //     $arrDataPesanan = $this->Marketing_Models->selectFakturPesanan($param);
  //     $arrDataPesananDetail = $this->Marketing_Models->selectFakturPesananDetail($param);
  ...
  //     $css = "assets/bootstrap/css/bootstrap.min.css";
  //     if($arrListItem){
  //       $this->load->view("frm_print_faktur_variasi_test", $result);
  //     } else {
  //       $this->load->view("frm_print_faktur_test",$result);
  //     }
  //   }else{
  //     echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
  //     redirect("_main/main","refresh");
  ...
  public function cetakFakturPesananTest($param){
    $isLogin = $this->isLogin();
    if($isLogin){
      $arrDataPesanan = $this->Marketing_Models->selectFakturPesanan($param);
      $arrDataPesananDetail = $this->Marketing_Models->selectFakturPesananDetail($param);
      ...
      $css = "assets/bootstrap/css/bootstrap.min.css";
      if($arrListItem){
        // $page = $this->load->view("frm_print_faktur_variasi", $result, true);
        $this->load->view("frm_print_faktur_variasi_test", $result);
      } else {
      ...
      // $this->load->library('m_pdf');
      // $this->mpdf->mPDF("utf-8","A4",0,"",2,2,5,5,0,0,"P");
      // // $this->mpdf->SetDisplayPreferences("/FullScreen/FitWindow");
      // $this->mpdf->showImageErrors = true;
      // $this->mpdf->WriteHTML(file_get_contents($css),1);
      // $this->mpdf->WriteHTML($page);
  ...
  public function cetakFakturPesananProduksi($param){
    ...
    if($isLogin){
      ...
      $css = "assets/bootstrap/css/bootstrap.min.css";
      if($arrListItem){
        $page = $this->load->view("frm_print_faktur_produksi_variasi", $result, true);
      } else {
  ...
  public function getDataRincian(){
    ...
    $data = array("tglAwal" => $tgl_awal,
                  ...
                  "jnsQty" => $jnsQty,
                  "sortir" => $sortir);
    $result = $this->Marketing_Models->selectDataRincianStatistik($data);
    echo $result;
  }
  ...
  public function cekKodeSales(){
    $isLogin = $this->isLogin();
    if($isLogin){
      $kodeSales = $this->input->post("kodeSales");
      $result = $this->Marketing_Models->selectKodeSales($kodeSales);
      echo $result;
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
      redirect("_main/main","refresh");
  ...
  public function saveDataSales(){
    ...
    if($isLogin){
      ...
      if(empty($kodeSales) || empty($namaLengkap) || empty($gender) || empty($noTelp)){
        echo "Data Kosong";
      }else{
        $data = array("kode_sales"   => $kodeSales,
                      "nama_lengkap" => $namaLengkap,
                      "gender"       => $gender,
                      "no_telp"      => $noTelp,
                      "alamat"       => $alamat);
        $result = $this->Marketing_Models->insertDataSales($data);
        echo $result;
      }
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
  ...
  public function getDataSales(){
    $isLogin = $this->isLogin();
    if($isLogin){
      $result = $this->Marketing_Models->selectDataSales();
      echo $result;
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
      redirect("_main/main","refresh");
  ...
  public function getDetailSales(){
    ...
    if($isLogin){
      ...
      $result = $this->Marketing_Models->selectDetailSales($idSales);
      echo $result;
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
      redirect("_main/main","refresh");
  ...
  public function deleteAndRestoreSales(){
    $isLogin = $this->isLogin();
    if($isLogin){
      $idSales = $this->input->post("idSales");
      $deleted = $this->input->post("deleted");
      $data = array("id_sales" => $idSales,
                    "deleted" => $deleted);
      $result = $this->Marketing_Models->updateDataSales($data);
      echo $result;
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
      redirect("_main/main","refresh");
  ...
  public function modifyDataSales(){
    $isLogin = $this->isLogin();
    if($isLogin){
      $idSales = $this->input->post("idSales");
      $kodeSales = $this->input->post("kodeSales");
      ...
      $noTelp = $this->input->post("noTelp");
      $alamat = $this->input->post("alamat");
      if(empty($kodeSales) || empty($namaLengkap) ||
         empty($gender) || empty($noTelp) || empty($idSales)){
        echo "Data Kosong";
      }else{
        $data = array("id_sales"     => $idSales,
  ...
  public function cekKodeJenisBarang(){
    $isLogin = $this->isLogin();
    if($isLogin){
      $kodeJenis = $this->input->post("kodeJenis");
      $result = $this->Marketing_Models->selectKodeJenisBarang($kodeJenis);
      echo $result;
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
      redirect("_main/main","refresh");
  ...
  public function saveDataJenisBarang(){
    ...
    if($isLogin){
      ...
      if(empty($kodeJenis) || empty($namaJenis) || empty($keterangan)){
        ...
        $data = array("kode_jenis"       => $kodeJenis,
                      "nama_jenis"       => $namaJenis,
                      "keterangan_jenis" => $keterangan);
        $result = $this->Marketing_Models->insertDataJenisBarang($data);
        echo $result;
      }
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
  ...
  public function getDataJenisBarang(){
    $isLogin = $this->isLogin();
    if($isLogin){
      $result = $this->Marketing_Models->selectDataJenisBarang();
      echo $result;
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
      redirect("_main/main","refresh");
  ...
  public function getDetailJenisBarang(){
    $isLogin = $this->isLogin();
    if($isLogin){
      $idJenis = $this->input->post("idJenis");
      $result = $this->Marketing_Models->selectDetailJenisBarang($idJenis);
      echo $result;
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
      redirect("_main/main","refresh");
  ...
  public function deleteAndRestoreJenisBarang(){
    $isLogin = $this->isLogin();
    if($isLogin){
      $idJenis = $this->input->post("idJenis");
      $deleted = $this->input->post("deleted");
      $data = array("id_jenis" => $idJenis,
                    "deleted" => $deleted);
      $result = $this->Marketing_Models->updateDataJenisBarang($data);
      echo $result;
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
      redirect("_main/main","refresh");
  ...
  public function modifyDataJenisBarang(){
    $isLogin = $this->isLogin();
    if($isLogin){
      $idJenis = $this->input->post("idJenis");
      $kodeJenis = $this->input->post("kodeJenis");
  ...
  public function getDataJenisBarangComboBox(){
    $isLogin = $this->isLogin();
    if($isLogin){
      $key = $this->input->get("q");
      $result = $this->Marketing_Models->selectDataJenisBarangComboBox($key);
      echo $result;
    }else{
         echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
         redirect("_main/main","refresh");
  ...
  public function getDataSalesComboBox(){
    $isLogin = $this->isLogin();
    if($isLogin){
      $key = $this->input->get("q");
      $result = $this->Marketing_Models->selectDataSalesComboBox($key);
      echo $result;
    }else{
        echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
        redirect("_main/main","refresh");
  ...
  public function saveTransaksiKomisiSales(){
    $isLogin = $this->isLogin();
    if($isLogin){
      $idSales = $this->input->post("idSales");
      $idJenis = $this->input->post("idJenis");
      ...
      $jumlahBal = $this->input->post("jumlahBal");
      $jenisHarga = $this->input->post("jenisHarga");
      $jumlahHarga = $this->input->post("jumlahHarga");
      $diskonPersen = $this->input->post("diskonPersen");
      $hasilDiskon = $this->input->post("hasilDiskon");
      $potonganHarga = $this->input->post("potonganHarga");
      $lembar8Persen = $this->input->post("lembar8Persen");
      $lembar5Persen = $this->input->post("lembar5Persen");
      $lembar2Persen = $this->input->post("lembar2Persen");
      $lembar1Persen = $this->input->post("lembar1Persen");
      $lembar05Persen = $this->input->post("lembar05Persen");

      if(empty($idSales) || empty($idJenis) || empty($idUser) ||
         empty($tglOrder) || empty($namaPelanggan) || empty($kodeOrder) ||
         empty($kdGdHasil)
       ){
         echo "Data Kosong";
       }else{
         if(strpos($kdGdHasil,"PACK") === false){
           $data = array("id_sales"           => $idSales,
                         "id_jenis"           => $idJenis,
                         ...
                         "hasil_diskon"       => $hasilDiskon,
                         "potongan_harga"     => $potonganHarga,
                         "lembar_8_persen"    => $lembar8Persen,
                         "lembar_5_persen"    => $lembar5Persen,
                         "lembar_2_persen"    => $lembar2Persen,
                         "lembar_1_persen"    => $lembar1Persen,
                         "lembar_05_persen"   => $lembar05Persen);
         }else{
           $data = array("id_sales"           => $idSales,
                         "id_jenis"           => $idJenis,
                         "kd_produk"          => $kdGdHasil,
          ...
          $result = $this->Marketing_Models->insertTransaksiKomisiSales($data);
          echo $result;
       }
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
  ...
  public function editTransaksiKomisiSales(){
    $isLogin = $this->isLogin();
    if($isLogin){
      ...
      if(empty($idSales) || empty($idJenis) || empty($idUser) ||
         empty($tglOrder) || empty($namaPelanggan) || empty($kodeOrder) ||
         empty($kdGdHasil) || empty($idTransaksi)
       ){
         echo "Data Kosong";
       }else{
         $data = array("id_transaksi"       => $idTransaksi,
                       ...
                       "lembar_1_persen"    => $lembar1Persen,
                       "lembar_05_persen"   => $lembar05Persen);
          $result = $this->Marketing_Models->updateTransaksiKomisiSales($data);
          echo $result;
       }
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
  ...
  public function getListDataOrderSalesPending(){
    $isLogin = $this->isLogin();
    if($isLogin){
      $idSales = $this->input->post("idSales");
      $result = $this->Marketing_Models->selectListDataOrderSalesPending($idSales);
      echo $result;
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
      redirect("_main/main","refresh");
  ...
  public function getDetailTransaksiKomisiSales(){
    $isLogin = $this->isLogin();
    if($isLogin){
      $idTransaksi = $this->input->post("idTransaksi");
      $result = $this->Marketing_Models->selectDetailTransaksiKomisiSales($idTransaksi);
      echo $result;
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
      redirect("_main/main","refresh");
  ...
  public function deleteAndRestoreTransaksiKomisiSales(){
    $isLogin = $this->isLogin();
    if($isLogin){
      $idUser = $this->session->userdata("fabricationIdUser");
      $idTransaksi = $this->input->post("idTransaksi");
  ...
  public function selesaiInputTransaksiKomisiSales(){
    $isLogin = $this->isLogin();
    if($isLogin){
      $idSales = $this->input->post("idSales");
      $statusTransaksi = $this->input->post("statusTransaksi");
      $data = array("status_transaksi" => $statusTransaksi,
                    "id_sales" => $idSales);
      $result = $this->Marketing_Models->updateTransaksiKomisiSales_Check($data);
      echo $result;
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
      redirect("_main/main","refresh");
  ...
  public function getLaporanDataOrderSales(){
    $isLogin = $this->isLogin();
    if($isLogin){
      $idSales = $this->input->post("idSales");
      $tglAwal = $this->input->post("tglAwal");
      $tglAkhir = $this->input->post("tglAkhir");
      $data = array("idSales" => $idSales,
                    "tglAwal" => $tglAwal,
                    "tglAkhir" => $tglAkhir);
      $result = $this->Marketing_Models->selectLaporanDataOrderSales($data);
      echo $result;
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
      redirect("_main/main","refresh");
  ...
  public function getLaporanDataOrderSales2(){
    $isLogin = $this->isLogin();
    if($isLogin){
      $idSales = $this->input->post("idSales");
      $tglAwal = $this->input->post("tglAwal");
      ...
      $data = array("idSales" => $idSales,
                    "tglAwal" => $tglAwal,
                    "tglAkhir" => $tglAkhir);
      $result["data"] = $this->Marketing_Models->selectLaporanDataOrderSales_2($data);
      $result["marking"] = $this->Marketing_Models->selectMarkingDataKomisiSales($data);
      $result["TotalPerJenis"] = json_decode($this->Marketing_Models->selectTotalLembarPerJenisBarang($data), TRUE);
  ...
  public function printLaporanDataOrderSales($param, $param2, $param3){
    $isLogin = $this->isLogin();
    if($isLogin){
      $data = array("idSales" => $param,
                    "tglAwal" => $param2,
                    "tglAkhir" => $param3);
      $result["ListOrder"] = json_decode($this->Marketing_Models->selectLaporanDataOrderSales($data), TRUE);
      $result["DetailSales"] = json_decode($this->Marketing_Models->selectDetailSales($param), TRUE);
      $result["TotalPerJenis"] = json_decode($this->Marketing_Models->selectTotalLembarPerJenisBarang($data), TRUE);
      $this->load->view("frm_print_laporan_data_komisi",$result);
    }else{
    	echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
    	redirect("_main/main","refresh");
    }
  ...
  public function exportLaporanDataOrderSales($param, $param2, $param3){
    $isLogin = $this->isLogin();
    if($isLogin){
      $data = array("idSales" => $param,
                    "tglAwal" => $param2,
      ...
      $result["DetailSales"] = json_decode($this->Marketing_Models->selectDetailSales($param), TRUE);
      $result["TotalPerJenis"] = json_decode($this->Marketing_Models->selectTotalLembarPerJenisBarang($data), TRUE);
      $namaFile = "Laporan Data Order Sales "
                   .$result["DetailSales"][0]["nama_lengkap"]
                   ."(".date("d-m-Y",strtotime($param2))
                   ." / ".date("d-m-Y", strtotime($param3)).")";
  ...
  public function approveLaporanTransaksiKomisiSales(){
    $isLogin = $this->isLogin();
  	if($isLogin){
      $idSales = $this->input->post("idSales");
      $tglAwal = $this->input->post("tglAwal");
      ...
      $result = $this->Marketing_Models->updateApproveLaporanTransaksiKomisiSales($data);
      echo $result;
  	}else{
  		echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
  		redirect("_main/main","refresh");
  ...
  public function perbaruiKodeBarang(){
    $isLogin = $this->isLogin();
    if($isLogin){
      $data["KodeBarang"] = $this->Marketing_Models->getMaxCodeBarang();
      echo json_encode($data);
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
      redirect("_main/main","refresh");
  ...
  public function simpanBarangBaru(){
    ...
    if($isLogin){
      ...
      if(empty($kodeBarang) || empty($namaBarang)){
        ...
        $data = array("produk_aneka_macam" => array("kd_produk"       => $kodeBarang,
                                                    "nama_produk"     => $namaBarang,
                                                    "keterangan"      => $keterangan,
                                                    "dibuat_oleh"     => $idUser,
                                                    "diperbarui_oleh" => $idUser)
                     );
        $result = $this->Marketing_Models->insertData($data);
        echo $result;
      }
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
  ...
  public function tambahItemAneka(){
    $isLogin = $this->isLogin();
    if($isLogin){
      $data = array(
        'kd_produk' => $this->input->post('kodeProduk'),
        'kd_gd_hasil' => $this->input->post('kdBarang'),
        'jumlah' => $this->input->post('jumlah'),
        'satuan' => $this->input->post('satuan'),
        'warna_cetak' => $this->input->post('warnaCetak'),
        'dll' => $this->input->post('dll'),
        'strip' => $this->input->post('strip'),
        'omset_lembar'=>$this->input->post('omsetLembar'),
        'omset_kg'=>$this->input->post('omsetKg'),
        'merek' => $this->input->post('merek'),
        'dibuat_oleh' => $this->session->userdata("fabricationIdUser"),
        'diperbarui_oleh' => $this->session->userdata("fabricationIdUser")
      );
      if(empty($data['kd_gd_hasil']) || empty($data['kd_produk'])|| empty($data['jumlah'])){
        echo json_encode(array("Code" => 400,
                               "Text" => "Semua Kolom Tidak Boleh Kosong!"));
      }else{
        $result = $this->Marketing_Models->insertData(['produk_aneka_macam_item'=>$data]);
        echo $result;
      }
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
  ...
  public function getDaftarBarangAnekaPlastik(){
    $isLogin = $this->isLogin();
    if($isLogin){
      $items = $this->Marketing_Models->getItem(['kd_produk'=>$this->input->get('kodeBarang')]);
  ...
  public function getAnekaItem(){
    $isLogin = $this->isLogin();
    if($isLogin){
      $items = $this->Marketing_Models->getItem(['id_detail_produk'=>$this->input->get('itemId')]);
      echo json_encode($items);
    } else {
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
      redirect("_main/main","refresh");
  ...
  public function hapusAnekaItem(){
    $isLogin = $this->isLogin();
    $itemId = $this->input->get('itemId');
    if($isLogin){
      if(empty($itemId)){
        echo json_encode(['Code'=>400, 'Text'=>'Tidak ada data yang dihapus!']);
      } else {
        if($this->Marketing_Models->deleteAnekaItem($itemId)){
          echo json_encode(['Code'=>200, 'Text'=>'Data berhasil dihapus!']);
        } else {
          echo json_encode(['Code'=>400, 'Text'=>'Gagal saat menghapus data!']);
        }
      }
    } else {
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
  ...
  public function getDaftarBarang(){
    if($this->isLogin()){
      ...
      $arrParam = array("COLUMN"  => "*",
                        "TABLE"   => "produk_aneka_macam",
                        "WHERE"   => array("status_produk = 'FINISH'", "deleted = 'FALSE'"),
                        "ORDERBY" => "kd_produk ASC");
      $data = $this->Marketing_Models->selectData($arrParam);
      echo json_encode($data);
    } else {
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
      redirect("_main/main","refresh");
  ...
  public function getSingleBarang(){
    if($this->isLogin()){
      ...
      $arrParam = array("COLUMN"  => "*",
                        "TABLE"   => "produk_aneka_macam",
                        "WHERE"   => array("status_produk = 'FINISH'", "deleted = 'FALSE'", "kd_produk = '$kd_produk'"),
                        "ORDERBY" => "kd_produk ASC");
      $data = $this->Marketing_Models->selectData($arrParam);
      echo json_encode($data);
    } else {
  ...
  public function selesaiBarangBaru(){
    $kd_produk = $this->input->post("kd_produk");
    $arrParam = array(
      "COLUMN"  => "*",
      "TABLE"   => "produk_aneka_macam_item",
      "WHERE"   => array("kd_produk = '$kd_produk'", "deleted = 'FALSE'")
    );
    $data = $this->Marketing_Models->selectData($arrParam);
    if(count($data) > 0) {
      if($this->Marketing_Models->selesaiTambahBarangAneka($kd_produk)){
        echo json_encode(['Code'=>200,'Text'=>'Berhasil proses penambahaan aneka barang']);
      } else {
        echo json_encode(['Code'=>400,'Text'=>'Gagal saat mengupdate data']);
      }
    } else {
      echo json_encode(['Code'=>400, 'Text'=>'Gagal anda belum menambahkan barang']);
    }
  }

  public function selesaiEditBarang(){
    $kd_produk = $this->input->post('kd_produk');
    ...
    if(!empty($kd_produk) && !empty($nama_barang)){
      if($this->Marketing_Models->selesaiEditBarang($data, ['kd_produk'=>$kd_produk])){
        echo json_encode(['Code'=>200, 'Text'=>'Berhasil mengubah barang']);
      } else {
        echo json_encode(['Code'=>400, 'Text'=>'Gagal mengubah barang']);
      }
    } else {
      echo json_encode(['Code'=>400, 'Text'=>'Data kosong!']);
    }
  }
  public function selesaiEditItem(){
    $id = $this->input->post("id");
    if(!empty($this->input->post('kdBarang'))){
      $data = array(
        'kd_gd_hasil' => $this->input->post('kdBarang'),
        'jumlah' => $this->input->post('jumlah'),
        'satuan' => $this->input->post('satuan'),
    ...
    if(!empty($data['kd_gd_hasil']) || !empty($data['jumlah']) || !empty($data['satuan'])){
      if($this->Marketing_Models->selesaiEditItem($data, ['id_detail_produk'=>$id])){
        echo json_encode(['Code'=>200, 'Text'=>'Berhasil mengubah data!']);
      } else {
        echo json_encode(['Code'=>400, 'Text'=>'Terjadi kesalahan saat mengubah data!']);
      }
    } else {
      echo json_encode(['Code'=>400, 'Text'=>'Data kosong']);
    }
  ...
  public function hapusBarang(){
    ...
    if($this->isLogin()){
      if(!empty($kd_produk)){
        if($this->Marketing_Models->hapusBarangAneka($kd_produk)){
          echo json_encode(['Code'=>200, 'Text'=>'Berhasil menghapus']);
        }
      } else {
        echo json_encode(['Code'=>400, 'Text'=>'Gagal, kode produk tidak ada!']);
      }
    } else {
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
      redirect("_main/main","refresh");
  ...
  public function getBirthdayListToday(){
    if($this->isLogin()){
      $result = $this->Marketing_Models->selectBirthdayListToday();
      echo $result;
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
      redirect("_main/main","refresh");
