...
class Cetak_Model extends CI_Model{
  ...
  public function insertKirimBonPengambilan($param){
    ...
    $jenis = strtoupper($param["Parameter"]["jenis"]);
    $periodeLock = date("Y-m", strtotime($tglAwal));
    $checkLock = $this->db->query("SELECT COUNT(TGB.id) AS counter
                                   FROM transaksi_gudang_bahan TGB
                                   INNER JOIN gudang_bahan GB ON TGB.kd_gd_bahan = GB.kd_gd_bahan
                                   WHERE DATE_FORMAT(TGB.tgl_permintaan, '%Y-%m') = '$periodeLock'
                                   AND GB.jenis='$jenis'
                                   AND TGB.status_lock='TRUE'")->result_array();
    if($checkLock[0]["counter"] > 0){
      return "Lock";
    }else{
  ...
  public function insertKirimBonPengembalian($param){
    ...
    $checkLock = $this->db->query("SELECT COUNT(id) AS counter
                                   ...
                                   WHERE DATE_FORMAT(tgl_permintaan, '%Y-%m') = '$periodeLock'
                                   AND status_lock='TRUE'")->result_array();
    if($checkLock[0]["counter"] > 0){
      return "Lock";
    }else{
  ...
  public function insertPenggunaanBahanCetak($param){
    $this->db->trans_begin();
    $this->db->insert("transaksi_penggunaan_bahan_cetak", $param);
    if($this->db->trans_status() === FALSE){
      $this->db->trans_rollback();
      return "Gagal";
    }else{
  ...
  public function selectComboBoxValueGudangRoll($param){
    ...
    if(strpos(strtolower($param["panjangPlastik"]),"pon") !== FALSE ||
      ...
      $arrPanjangTemp = explode("+",strtolower($UkuranTemp));
  ...
  public function selectJenisApal($param){
    if(empty($param)){
      $result = $this->db->query("SELECT kd_gd_apal, sub_jenis FROM gudang_apal WHERE jenis='CETAK' AND deleted='FALSE'")->result_array();
    }else{
      $result = $this->db->query("SELECT kd_gd_apal, sub_jenis FROM gudang_apal WHERE jenis='CETAK' AND deleted='FALSE' AND sub_jenis LIKE '%$param%'")->result_array();
    }
    return json_encode($result);
  }

  public function selectDetailRencanaForInputHasil($param){
  ...
  public function selectEditDetailJobCetak($param){
    $Q = "SELECT RC.kd_cetak,
                 ...
                 TDHC.jumlah_berat_pengambilan_extruder,
                 TDHC.jumlah_payung_pengambilan_extruder,
                 TDHC.jumlah_payung_kuning_pengambilan_extruder,
                 TDHC.jumlah_bobin_pengambilan_extruder,

                 TDHC.jumlah_berat_pengambilan,
                 TDHC.jumlah_payung_pengambilan,
                 TDHC.jumlah_payung_kuning_pengambilan,
                 TDHC.jumlah_bobin_pengambilan,

                 TDHC.jumlah_hasil_selesai,
                 TDHC.jumlah_hasil_payung,
                 TDHC.jumlah_hasil_payung_kuning,
                 TDHC.jumlah_hasil_bobin,

                 TDHC.kd_gd_apal,
                 TDHC.jumlah_apal,

                 TDHC.jumlah_payung_terbuang,
                 TDHC.jumlah_bobin_terbuang,
                 TDHC.jumlah_payung_kuning_terbuang,
                 TDHC.jumlah_sisa_pengambilan,
  ...
  public function selectBahan($param){
    if(empty($param["Keyword"])){
      $Q = "SELECT * FROM gudang_bahan
            WHERE jenis='$param[JenisBahan]'
            AND deleted='FALSE'";
    }else{
      $Q = "SELECT * FROM gudang_bahan
            WHERE jenis='$param[JenisBahan]'
            AND deleted='FALSE'
            AND (kd_gd_bahan LIKE '%$param[Keyword]%' OR
                 nm_barang LIKE '%$param[Keyword]%' OR
                 warna LIKE '%$param[Keyword]%')";
    }
    $result = $this->db->query($Q)->result_array();
    return json_encode($result);
  }
  ...
  public function updateDetailJobCetak($param){
    $this->db->trans_begin();
    $kdTHC = $param["THC"]["kd_hasil_cetak"];
    $periodeLock = date("Y-m", strtotime($param["THC"]["tgl_transaksi"]));
    $checkLock = $this->db->query("SELECT COUNT(id) AS counterLock
                                   FROM transaksi_gudang_roll
                                   WHERE jns_permintaan = 'CETAK'
                                   AND DATE_FORMAT(tgl_transaksi, '%Y-%m') = '$periodeLock'
                                   AND status_lock = 'TRUE'
                                   AND deleted='FALSE'")->result_array();
    if($checkLock[0]["counterLock"] > 0){
      $this->db->trans_rollback();
      return "Lock";
    }else{
      $Q = "SELECT TDHC.kd_gd_roll_cetak,
                   TDHC.kd_gd_roll_polos,
                   TDHC.kd_gd_apal,
                   TDHC.jumlah_berat_pengambilan_extruder,
                   TDHC.jumlah_bobin_pengambilan_extruder,
                   TDHC.jumlah_payung_pengambilan_extruder,
            ...
            FROM transaksi_detail_hasil_cetak TDHC
            INNER JOIN transaksi_hasil_cetak THC ON TDHC.kd_hasil_cetak = THC.kd_hasil_cetak
            WHERE THC.kd_hasil_cetak = '$kdTHC'";

      $arrData = $this->db->query($Q)->result_array();

      $Q2 = "SELECT id, status_transaksi
             FROM transaksi_gudang_roll
             WHERE kd_gd_roll = '".$arrData[0]["kd_gd_roll_cetak"]."'
             AND tgl_transaksi = '".$arrData[0]["tgl_transaksi"]."'
             AND bagian = 'CETAK'
             AND keterangan_transaksi = 'MASUK DARI CETAK'
             AND status_history = 'MASUK'
             AND keterangan_history = 'HASIL CETAK'
             AND berat = '".$arrData[0]["jumlah_hasil_selesai"]."'
             AND bobin = '".$arrData[0]["jumlah_hasil_bobin"]."'
             AND payung = '".$arrData[0]["jumlah_hasil_payung"]."'
             AND payung_kuning = '".$arrData[0]["jumlah_hasil_payung_kuning"]."'
             AND deleted='FALSE'";

      $Q3 = "SELECT id, status_transaksi
             FROM transaksi_gudang_roll
             WHERE kd_gd_roll = '".$arrData[0]["kd_gd_roll_polos"]."'
             AND tgl_transaksi = '".$arrData[0]["tgl_transaksi"]."'
             AND bagian = 'CETAK'
             AND keterangan_transaksi = 'KELUAR KE CETAK'
             AND status_history = 'KELUAR'
             AND keterangan_history = 'OPERATOR CETAK'
             AND berat = '".$arrData[0]["jumlah_berat_pengambilan"]."'
             AND bobin = '".$arrData[0]["jumlah_bobin_pengambilan"]."'
             AND payung = '".$arrData[0]["jumlah_payung_pengambilan"]."'
             AND payung_kuning = '".$arrData[0]["jumlah_payung_kuning_pengambilan"]."'
             AND deleted='FALSE'";

      $Q4 = "SELECT id, sts_transaksi
             FROM transaksi_detail_history_apal
             WHERE kd_gd_apal = '".$arrData[0]["kd_gd_apal"]."'
             AND nama = 'MANDOR CETAK'
             AND tgl_transaksi = '".$arrData[0]["tgl_transaksi"]."'
             AND bagian = 'CETAK'
             AND jumlah_apal = '".$arrData[0]["jumlah_apal"]."'
             AND keterangan_history = 'KIRIMAN APAL'
             AND status_history='MASUK'";

      $arrDataQ2 = $this->db->query($Q2)->result_array();
      $arrDataQ3 = $this->db->query($Q3)->result_array();
      $arrDataQ4 = $this->db->query($Q4)->result_array();

      $this->db->set("berat", $param["TDHC"]["jumlah_hasil_selesai"]);
      $this->db->set("bobin", $param["TDHC"]["jumlah_hasil_bobin"]);
      $this->db->set("payung", $param["TDHC"]["jumlah_hasil_payung"]);
      $this->db->set("payung_kuning", $param["TDHC"]["jumlah_hasil_payung_kuning"]);
      $this->db->where("id",$arrDataQ2[0]["id"]);
      $this->db->update("transaksi_gudang_roll");

      $this->db->set("berat", $param["TDHC"]["jumlah_berat_pengambilan"]);
      $this->db->set("bobin", $param["TDHC"]["jumlah_bobin_pengambilan"]);
      $this->db->set("payung", $param["TDHC"]["jumlah_payung_pengambilan"]);
      $this->db->set("payung_kuning", $param["TDHC"]["jumlah_payung_kuning_pengambilan"]);
      $this->db->where("id",$arrDataQ3[0]["id"]);
      $this->db->update("transaksi_gudang_roll");

      $this->db->set("jumlah_apal",$param["TDHC"]["jumlah_apal"]);
      $this->db->where("id",$arrDataQ4[0]["id"]);
      $this->db->update("transaksi_detail_history_apal");

      if($arrDataQ2[0]["status_transaksi"] == "FINISH"){
        $beratSelisih = $arrData[0]["jumlah_hasil_selesai"] - $param["TDHC"]["jumlah_hasil_selesai"];
        $bobinSelisih = $arrData[0]["jumlah_hasil_bobin"] - $param["TDHC"]["jumlah_hasil_bobin"];
        $payungSelisih = $arrData[0]["jumlah_hasil_payung"] - $param["TDHC"]["jumlah_hasil_payung"];
        $payungKuningSelisih = $arrData[0]["jumlah_hasil_payung_kuning"] - $param["TDHC"]["jumlah_hasil_payung_kuning"];

        $this->db->set("stok","stok + ".$beratSelisih,FALSE);
        ...
        $this->db->where("kd_gd_roll",$arrData[0]["kd_gd_roll_cetak"]);
        $this->db->update("gudang_roll");
      }

      if($arrDataQ3[0]["status_transaksi"] == "FINISH"){
        $beratSelisih = $arrData[0]["jumlah_berat_pengambilan"] - $param["TDHC"]["jumlah_berat_pengambilan"];
        $bobinSelisih = $arrData[0]["jumlah_bobin_pengambilan"] - $param["TDHC"]["jumlah_bobin_pengambilan"];
        $payungSelisih = $arrData[0]["jumlah_payung_pengambilan"] - $param["TDHC"]["jumlah_payung_pengambilan"];
        $payungKuningSelisih = $arrData[0]["jumlah_payung_kuning_pengambilan"] - $param["TDHC"]["jumlah_payung_kuning_pengambilan"];

        $this->db->set("stok","stok + ".$beratSelisih,FALSE);
        $this->db->set("bobin","bobin + ".$bobinSelisih,FALSE);
        $this->db->set("payung","payung + ".$payungSelisih,FALSE);
        $this->db->set("payung_kuning","payung_kuning + ".$payungKuningSelisih,FALSE);
        $this->db->where("kd_gd_roll",$arrData[0]["kd_gd_roll_polos"]);
        $this->db->update("gudang_roll");
      }

      if($arrDataQ4[0]["sts_transaksi"] == "FINISH"){
        $jumlahSelisih = $arrData[0]["jumlah_apal"] - $param["TDHC"]["jumlah_apal"];
        $this->db->set("stok","stok + ".$jumlahSelisih, FALSE);
        $this->db->where("kd_gd_apal",$arrDataQ3[0]["kd_gd_apal"]);
        $this->db->update("gudang_apal");
      }

      if($arrDataQ3[0]["kd_gd_apal"] != $param["TDHC"]["kd_gd_apal"]){
        $this->db->set("kd_gd_apal", $param["TDHC"]["kd_gd_apal"]);
        $this->db->where("id",$arrDataQ4[0]["id"]);
        $this->db->update("transaksi_detail_history_apal");

        $this->db->set("kd_gd_apal", $param["TDHC"]["kd_gd_apal"]);
        $this->db->where("id",$arrData[0]["id"]);
        $this->db->update("transaksi_detail_hasil_cetak");

        $this->db->set("stok","stok + ".$param["TDHC"]["jumlah_apal"], FALSE);
        $this->db->where("kd_gd_apal",$param["TDHC"]["kd_gd_apal"]);
        $this->db->update("gudang_apal");
  ...
  public function updatePengambilanCetak($param){
    ...
    $Q = "SELECT id, kd_hasil_cetak
          FROM transaksi_detail_hasil_cetak
          WHERE jumlah_berat_pengambilan_extruder='$berat'
          AND jumlah_bobin_pengambilan_extruder = '$bobin'
          AND jumlah_payung_pengambilan_extruder = '$payung'
          AND jumlah_payung_kuning_pengambilan_extruder = '$payungKuning'
          AND kd_gd_roll_polos = '$kd_gd_roll'
          AND deleted='FALSE'";

    $arrDataQ = $this->db->query($Q)->result_array();

    $this->db->set("jumlah_berat_pengambilan_extruder",$param["TDPC"]["berat"]);
    $this->db->set("jumlah_bobin_pengambilan_extruder",$param["TDPC"]["bobin"]);
    $this->db->set("jumlah_payung_pengambilan_extruder",$param["TDPC"]["payung"]);
    $this->db->set("jumlah_payung_kuning_pengambilan_extruder",$param["TDPC"]["payung_kuning"]);
    $this->db->where("id",$arrDataQ[0]["id"]);
    $this->db->update("transaksi_detail_hasil_cetak");
