...
class Main extends MX_Controller{
  ...
  public function getComboBoxValueGudangRoll($param,$param2, $param3){
    $isLogin = $this->isLogin();
    if($isLogin){
      ...
      if(empty($Key)){
        $data = array("JnsPermintaan" => $param,
                      ...
                      "warnaPlastik" => str_replace("%20"," ",$param3));
  ...
  public function getListApal(){
    $isLogin = $this->isLogin();
    if($isLogin){
      $param = $this->input->get("subApal");
      $result = $this->Cetak_Model->selectJenisApal($param);
      echo $result;
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
      redirect("_main/main","refresh");
  ...
  public function getDetailRencanaForInputHasil(){
    $isLogin = $this->isLogin();
    if($isLogin){
      $idTransaksi = $this->input->post("idTransaksi");
      $result = $this->Cetak_Model->selectDetailRencanaForInputHasil($idTransaksi);
      echo $result;
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
      redirect("_main/main","refresh");
  ...
  public function editDetailJobCetak(){
    ...
    if($isLogin){
      ...
      if($idTransaksi == "" || $jumlahBeratPengambilan == "" || $jumlahPayungPengambilan == "" ||
         ...
         $plusminus == "" || $tglTransaksi == "" || empty($idUser) || empty($kdGdApal)
       ){
         echo "Data Kosong";
       }else{
  ...
  public function getBahan($param){
    $isLogin = $this->isLogin();
    if($isLogin){
      $data = array("Keyword" => $this->input->get("q"),
                    "JenisBahan" => str_replace("%20"," ",$param));
      $result = $this->Cetak_Model->selectBahan($data);
      echo $result;
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
      redirect("_main/main","refresh");
  ...
  public function tambahBahanBaru(){
    $isLogin = $this->isLogin();
    if($isLogin){
      $kdHasilCetak = $this->input->post("kdHasilCetak");
      $kdGdBahan = $this->input->post("kdGdBahan");
      $idUser = $this->session->userdata("fabricationIdUser");
      $tglPengambilan = $this->input->post("tglPengambilan");
      $jumlahPengambilan = $this->input->post("jumlahPengambilan");
      $sisaPengambilan = $this->input->post("sisaPengambilan");

      if(empty($kdHasilCetak) || empty($kdGdBahan) ||
         $jumlahPengambilan=="" || $sisaPengambilan=="" ||
         empty($tglPengambilan)
      ){
        $result = "Data Kosong";
      }else{
        $data = array("kd_hasil_cetak"      => $kdHasilCetak,
                      "kd_gd_bahan"         => $kdGdBahan,
                      "id_user"             => $idUser,
                      "tgl_pengambilan"     => $tglPengambilan,
                      "jumlah_pengambilan"  => $jumlahPengambilan,
                      "sisa_pengambilan"    => $sisaPengambilan);

        $result = $this->Cetak_Model->insertPenggunaanBahanCetak($data);
      }
      echo $result;
    }else{
      echo "<script>alert('Session anda habis atau anda tidak memiliki hak akses!');</script>";
